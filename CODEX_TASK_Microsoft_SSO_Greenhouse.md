...
Implementar autenticación con Microsoft Entra ID (Azure AD) en el portal Greenhouse usando NextAuth.js. El login debe soportar dos flujos simultáneos: SSO con Microsoft (multi-tenant, para Efeonce y clientes) y credenciales email + password como fallback. La identidad Microsoft del usuario se vincula a su perfil en `greenhouse.clients`.
...
- **Auth library:** NextAuth.js (instalar `next-auth`)
- **Deploy:** Vercel (auto-deploy desde `main`, preview desde feature branches)
- **Data store de usuarios:** BigQuery tabla `efeonce-group.greenhouse.clients`
...
#### A2. Crear tabla `greenhouse.clients`
...
#### A3. Insertar registro de prueba para admin
...
#### B2. Crear el helper de BigQuery para auth
