...
- **El color de un texto pertenece a su FONDO, no a la clase que lo pinta (TASK-1835, 2026-09-06).** **NUNCA** compartir una clase, un módulo CSS o un `sx` de texto entre dos superficies con fondos opuestos (lienzo oscuro ↔ tarjeta clara, `immersive` ↔ `contained`, banda de marca ↔ paper): la regla escrita para una le impone su color a la otra. Costó texto a **1.53:1** en el consentimiento de Efeonce ID — una sola clase servía a la ficha sobre el azul y al bloque dentro de la tarjeta blanca. **NUNCA** arreglarlo subiendo especificidad: separar las clases, una por superficie.
- **Un `violations: 0` de axe NO es evidencia de contraste sobre fondo compuesto.** Cuando el fondo detrás del texto es un degradado, una imagen, un pseudo-elemento, translucidez o un `color-mix` sobre otra capa, axe no puede resolverlo y lo manda a `results.incomplete`; el gate del GVC corta antes y no emite ni finding ni artefacto (`scripts/frontend/lib/quality.ts:87`). Ese cero dice «no pude mirar». Leer el CSS tampoco sirve: subir buscando `background-color` salta el degradado y aterriza en el `body`. **SIEMPRE** medir sobre los píxeles de la captura; patrón de referencia `scripts/auth-server/verify-contrast.mjs`. Contrato: `docs/architecture/agent-invariants/UI_PLATFORM_AGENT_INVARIANTS.md` § Contraste sobre fondo compuesto.
