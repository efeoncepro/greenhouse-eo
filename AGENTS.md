...
**Cargar `docs/architecture/agent-invariants/COMMERCIAL_TENDERS_AGENT_INVARIANTS.md`** al tocar `src/lib/commercial/tenders/**`, `docs/architecture/tender-deck-composer-prototypes/**` o cualquier oferta/deck de licitacion. Contrato: `GREENHOUSE_TENDER_DECK_COMPOSER_V1.md` (deck) + `GREENHOUSE_TENDER_PROPOSAL_STUDIO_ARCHITECTURE_V1.md` **§0 = estado real** (leerlo antes que nada). Skills: `greenhouse-public-private-tenders` (proceso) + `deck-studio` (argumento y composición). Para `TimelineFull`, consultar el companion: el plan declara schedule y `barLabel`; nunca se edita geometría a mano.

**Estado real (no asumas de mas):** el composer YA emite el entregable — **UN PDF de N paginas** (cada lamina impresa por separado + merge `pdf-lib`) via `pnpm deck:compose <plan.json> [--out dir]` (outDir default `.captures/tender-deck`); las **25 plantillas** tienen `data-slot` + `*.slots.json` + `registry.json` y son **25/25 componibles** (TASK-1394); hay **15 resolvers** (iconos, ordinales, geometria); la **state machine (12 estados) es TS puro SIN tabla en DB**; y **NO existe** API/UI/migracion/capability/outbox — el **unico consumer hoy es el CLI**. TASK-1394 cerró el resolver compartido; la secuencia de productización continúa `TASK-1393 → TASK-1392 → TASK-1391`: 1393 resuelve `CompositionPlanInput` (intención, nunca template elegido por autor) en `ResolvedCompositionManifest` inmutable con catálogo/contratos/hashes/validadores/brand-font pack; 1392 agrega Proposal + evidencia runtime; 1391 sólo renderiza ese manifest. El worker sigue bloqueado por EPIC-027.
