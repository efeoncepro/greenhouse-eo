...
- **Hook obligatorio de diseno UI (skills + wireframe/flow/motion + GVC en loop) — ANY UI work:** para CUALQUIER trabajo de UI (componente nuevo, cambio visual, layout, estados, microinteracciones, mockup, copy visible), ANTES de escribir JSX nuevo invocar las skills de product design que apliquen (`greenhouse-ux`, `modern-ui`, `state-design`, `forms-ux`, `greenhouse-ux-writing`) y **crear o registrar el wireframe** en `docs/ui/wireframes/` cuando la UI sea nueva, visible, reutilizable, client/public/executive-facing, tenga estados/copy relevante, charts/reportes/PDF/email, o venga de un asset Product Design. Si la UI abre sidecar/drawer/modal/popover o conecta pantallas/rutas, crear `docs/ui/flows/` y declarar `Flow`. Si introduce motion/microinteracciones no triviales, crear `docs/ui/motion/` y declarar `Motion`. La task UI debe declarar los contratos aplicables; `pnpm task:lint --changed`, `pnpm ui:wireframe-check --task TASK-###`, `pnpm ui:flow-check --task TASK-###` y `pnpm ui:motion-check --task TASK-###` lo validan. Wireframe = contenido; flow = secuencia/routing/foco/recovery; motion = timing/feedback/reduced-motion. DESPUES verificar con GVC en loop. NUNCA pintar UI freehand ni declarar "listo" sin contratos requeridos + captura GVC mirada.
