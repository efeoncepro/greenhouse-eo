...
| Trade marketing, BTL, ejecución de canal (Channel & Commerce) | `commercial-expert`, `efeonce-business-model-operator` | `EFEONCE_CHANNEL_COMMERCE_LINE_DECISION_V1.md` (NUNCA staff augmentation de terreno ni producción física propia; back-to-back o no se firma; NUNCA prometer incremento de venta) + `docs/services/channel-commerce/README.md` |
| Product design / UI-UX como servicio (Product Design 360) | `creative-practice`, `commercial-expert`, `efeonce-business-model-operator` | `EFEONCE_PRODUCT_DESIGN_360_DECISION_V1.md` (`Proposed`; se venden lanes, NUNCA diseñadores por hora ni pantallas; NUNCA dos ofertas por la capacidad de diseño de una cuenta; la superficie de sitio público la vende Web Experience 360) + `docs/services/wave/product-design-360.md` |
...
| **Efeonce Marketing Studio** (`studio.efeonce.org`, repo hermano `efeonce-marketing-studio`) · EPIC-049 · tools `studio.*` | `efeonce-marketing-studio` (+ `efeonce-mcp-platform` para el gateway; `efeonce-campaign-planning` para planificar campañas con IA) | [`EFEONCE_MARKETING_STUDIO_ARCHITECTURE_V1.md`](docs/architecture/marketing-studio/EFEONCE_MARKETING_STUDIO_ARCHITECTURE_V1.md) + `EFEONCE_STUDIO_API_FIRST_DECISION_V1.md` + `operations/marketing-studio/MARKETING_STUDIO_RUNTIME_HANDOFF.md` (estado vivo). **Toda la documentación vive en greenhouse-eo, NUNCA en el repo de Studio. Toda capability nace en `packages/contracts/src/operations.ts` con tool o exclusión con razón. La autoridad de la persona sale del canje RFC 8693 en Greenhouse; NUNCA confiar en un `organizationId` enviado por el cliente (intersecta, nunca amplía). Studio ≠ Efeonce Creative Studio (Globe).** |
| **Efeonce MCP Platform / federación de providers** · `mcp.efeonce.org`, Streamable HTTP, OAuth, tools/resources/prompts, Cloud Run/ALB/TLS | `efeonce-mcp-platform` + skill dueña del provider | [`EFEONCE_MCP_AGENT_SKILL_ROUTER_V1.md`](docs/architecture/EFEONCE_MCP_AGENT_SKILL_ROUTER_V1.md) + [`EFEONCE_MCP_PLATFORM_GATEWAY_DECISION_V1.md`](docs/architecture/EFEONCE_MCP_PLATFORM_GATEWAY_DECISION_V1.md) + [`EFEONCE_MCP_PLATFORM_RUNBOOK_V1.md`](docs/operations/EFEONCE_MCP_PLATFORM_RUNBOOK_V1.md) + `TASK-1626`; para Globe cargar además `greenhouse-globe` y `TASK-1473`. El gateway es adapter neutral: nunca sustituye el contrato, datos ni policy del provider. El emisor nativo y gateway multi-issuer están desplegados; TASK-1832 certifica el canary externo y TASK-1841 conserva el piloto cliente. TASK-1844 completa la autoridad interna v2: una conexión, objetivos autorizados por llamada y nuevas organizaciones elegibles sin reconectar. Cargar [`EFEONCE_INTERNAL_NATIVE_AUTHORITY_DECISION_V1.md`](docs/architecture/EFEONCE_INTERNAL_NATIVE_AUTHORITY_DECISION_V1.md) y el [manual interno](docs/manual-de-uso/identity/usar-mcp-interno-multiorganizacion.md); cohort y capacidad inicial no se amplían por inferencia. El [binding externo](docs/documentation/identity/binding-identidad-externa-mcp.md) conserva su contrato separado. La entrada contextual por producto y la separación entre acceso first-party y consentimiento MCP delegado se rigen por [`EFEONCE_ID_RELYING_PARTY_ENTRY_AND_CONSENT_DECISION_V1.md`](docs/architecture/EFEONCE_ID_RELYING_PARTY_ENTRY_AND_CONSENT_DECISION_V1.md). Los tokens con `gv` nacen en el emisor. + [`GREENHOUSE_MCP_ARCHITECTURE_V1.md`](docs/architecture/GREENHOUSE_MCP_ARCHITECTURE_V1.md) §22 + [`agent-invariants/MCP_TOOL_SURFACE_INVARIANTS.md`](docs/architecture/agent-invariants/MCP_TOOL_SURFACE_INVARIANTS.md) — el inventario de tools del servidor interno es `src/mcp/greenhouse/tool-manifest.ts` (Greenhouse declara qué EXISTE; el gateway decide qué CRUZA) |
| **AXIS Design System** · tokens, contracts, registry, adapters, selección colaborativa y distribución privada | `axis-design-system` | `docs/architecture/EFEONCE_SHARED_PRODUCT_UI_PLATFORM_DECISION_V1.md` + `docs/operations/AXIS_PRIVATE_PACKAGE_CONSUMPTION_RUNBOOK_V1.md` + `docs/operations/AXIS_CONTINUITY_MAP_2026-07-29.md` |
| Notion sync/work management                         | `notion-platform`                                                | `GREENHOUSE_SOURCE_SYNC_PIPELINES_V1.md` + Notion architecture/runbook aplicable                                                                                                     |
| HubSpot bridge/CRM/services intake                  | `hubspot-greenhouse-bridge` o `hubspot-as-a-service`             | `GREENHOUSE_HUBSPOT_SERVICES_INTAKE_V1.md`                                                                                                                                           |
| HubSpot portfolio, Solutions Partner, public offer, agents or sectors | `hubspot-solutions-partner` + `hubspot-as-a-service` | `docs/services/hubspot-as-a-service/HUBSPOT_OFFER_ARCHITECTURE_V2.md` + `hubspot-solutions-partner/SOURCES.md` |
...
| DataForSEO (cliente `src/lib/ai/dataforseo*`, rank/keyword/backlink/site-audit, visibilidad AI/LLM Mentions) | `dataforseo-operator` (+ `seo-aeo` para el oficio)               | `GREENHOUSE_SEO_MODULE_ARCHITECTURE_V1.md` + references en `.claude/skills/dataforseo-operator/references/` (canónicas, compartidas con Codex) + `GREENHOUSE_DATAFORSEO_ETV_METHOD_VERSIONING_DECISION_V1.md` (**la fórmula ETV es identidad del hecho**: `buildEtvMethodologyRequest` es la ÚNICA forma de construir `use_improved_etv`; NUNCA inferir fórmula por fecha; los readers sirven UNA fórmula; improved sólo vía TASK-1806)                                        |
...
| Informes ejecutivos, auditorías y desempeño; HTML como insumo PDF | `report-studio` + skill de dominio | `docs/operations/EFEONCE_REPORT_BRAND_DELIVERY_STANDARD_V1.md`; fuentes, continuidad, narrativa, A4, gráficos y QA del archivo final |
| Publicidad/social con texto o selección colaborativa: posts, stories, reels, covers, banners, brochure, OOH, Bricolage/Poppins/Guttery, bounding box y cursores multiplayer | `efeonce-advertising-creative` + `axis-design-system` + skill de formato | [`ADVERTISING_CREATIVE_AGENT_EXECUTION_V1.md`](docs/operations/ADVERTISING_CREATIVE_AGENT_EXECUTION_V1.md): AXIS posee valores/contratos; el agente prueba tipografía, contraste y safe area, o resuelve intent semántico + adapter sin coordenadas decorativas. No autoriza publicación. |
...
| **Línea gráfica Efeonce «La órbita»** (órbita, lente, foco, voz con esfera, eslogan, **firma de piezas**, burbuja URL, logos oficiales) | `efeonce-brand-studio` + `axis-design-system` (+ `design-studio`) | [`EFEONCE_GRAPHIC_LINE_V1.md`](docs/operations/brand-graphic-line/EFEONCE_GRAPHIC_LINE_V1.md) + ADR `EFEONCE_GRAPHIC_LINE_ORBIT_DECISION_V1.md`. **Firma: logo Efeonce centrado; la burbuja URL sólo si el logo ya está en la imagen** (centrada, fusión, lecho oscuro). La órbita no sustituye la composición fotográfica. Valores y contrato `efeonce.graphic-line-orbit` en AXIS; archivos en `@efeoncepro/axis-brand-assets`, nunca copias a mano |
| **Imagen o fotografía de marca propia Efeonce** (nosotros, equipo, oficina, Nexa/mascotas, logo 3D en escena; cualquier pieza donde la foto represente a Efeonce) — **con cualquier motor generativo y cualquier agente** | `design-studio` → [`references/efeonce-photographic-language.md`](.codex/skills/design-studio/references/efeonce-photographic-language.md) (+ `efeonce-brand-studio` para vestuario/kits; + `efeonce-advertising-creative` si la pieza lleva texto) | **Comandos: `pnpm foto:doctor` (¿puede esta máquina generar? ejercita la cadena entera, sin costo), `pnpm foto:prompt <ficha.json> --batch <out.json>` (arma el prompt; el formato, el lecho y el límite de sujetos salen de UNA tabla) y `pnpm foto:validar <plate.png>` (valida las seis reservas, exit 1 si falla). NUNCA armar el prompt concatenando bloques a mano.** La ficha declara `identidad` (con **vista por ángulo**), `objetos` (diez kits de marca como **referencia de forma**: logo 3D, nave, Clawd, Codex, polo, hoodie, chaqueta softshell y bomber, gorra, lanyard, sprocket HubSpot — este último **uso interno hasta aprobación escrita**), **`palanca`** (UNA por pieza: `pov` · `manos` · `luz-motivada` · `larga-exposicion` · `oclusion`; combinar dos las diluye), `atmosfera` (`polvo`/`bruma`/`vapor`/`humo`, **exige un haz declarado**) y `suspendido` (**dosis 1 de cada 4**, el comando cuenta la tanda). **Editar conserva, generar reconstruye**: para un ángulo nuevo de una persona, edita su foto aprobada en vez de generar desde cero. **Marcadores verificables, no magnitudes**: «la oreja lejana no se ve», nunca «gira 45 grados». [`docs/operations/brand-photography/README.md`](docs/operations/brand-photography/README.md) — maestro, firma, colorimetría, catálogo de cámaras/ángulos, personas. **El estilo es portable y obligatorio; el motor es intercambiable**: usar `foto:prompt` para incorporar los bloques de [`EFEONCE_PHOTO_PROMPT_BLOCKS_AND_PIPELINE_V1.md`](docs/operations/brand-photography/EFEONCE_PHOTO_PROMPT_BLOCKS_AND_PIPELINE_V1.md) §3 y cumplir §0: plate **sin logo ni texto**, firma = SVG oficial **compuesto después** (20% del lado corto, ≥ 4,5:1 medido), formato nativo sin recortes, medir antes de aprobar, **nunca un scrim**. Toma que después llevará texto o selección → [`EFEONCE_PHOTO_PLATE_SPACE_RESERVATION_V1.md`](docs/operations/brand-photography/EFEONCE_PHOTO_PLATE_SPACE_RESERVATION_V1.md). **La capa gráfica sobre la foto NO está aprobada (2026-09-19)**: su canon es `efeonce-advertising-creative`, no los ejemplos de esa carpeta |
...
| Gemini Omni 1.1 en la CLI local de video (`pnpm ai:omni`) | `motion-design-studio` + `greenhouse-secret-hygiene`; `greenhouse-gcloud-auth-playwright` si requiere renovar ADC | `GREENHOUSE_GEMINI_OMNI_CLI_DECISION_V1.md` + `GREENHOUSE_AI_MEDIA_MODEL_SELECTION_GUIDE_V1.md` + `docs/manual-de-uso/ai-tooling/gemini-omni-1-1-cli.md`; esta CLI no es una ruta de Globe |
| Higgsfield: proveedor cloud (API `pnpm ai:fal --capability hf-*`, CLI `higgsfield`), MCP remoto y puentes MCP locales de Blender/Illustrator/Photoshop | `higgsfield-provider` (+ `design-studio`/`motion-design-studio` según el oficio) | ADR [`EFEONCE_HIGGSFIELD_PROVIDER_ADOPTION_DECISION_V1.md`](docs/architecture/creative-studio/EFEONCE_HIGGSFIELD_PROVIDER_ADOPTION_DECISION_V1.md) + [runbook](docs/operations/creative-studio/HIGGSFIELD_PROVIDER_RUNBOOK_V1.md) + [manual](docs/manual-de-uso/creative-studio/higgsfield-provider.md). Estado local verificado 2026-09-24 en la skill §«Estado local verificado»: puentes de Blender/Illustrator/Photoshop conectados (tools `bl_*`/`ai_*`/`ps_*` sólo en conversación nueva), After Effects sin app, CLI 1.1.26 con sesión y workspace fijado. Carril de operador out-of-band, nunca runtime; sin ruta Globe |
...
- Superficie MCP (`src/mcp/**`): `pnpm mcp:manifest:check` — el inventario de tools es un manifiesto con dos
  consumidores, uno en otro repo; el artefacto committeado no puede divergir del registro vivo.
...
## Assets de marca en piezas generadas

Antes de generar cualquier pieza donde aparezca un asset de marca Efeonce —ropa corporativa, lanyard,
...
1. **Arte plano → PRODUCIR vistas del kit · pieza aislada → CONSTRUIR · pieza en uso / producto
   terminado → USAR en una escena.** Darlos al revés hace que el modelo reinvente la marca.
2. **Lo sensible se COMPONE; el modelo sólo pone material y luz.** Un modelo no sostiene una marca:
   medido, cuatro pasadas sobre la misma pieza dieron cuatro logotipos distintos, y tres prendas
   dieron tres emblemas de los que ninguno era el de Efeonce.
