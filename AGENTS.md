...
- Contexto de negocio: leer `docs/context/00_INDEX.md` como mapa del context pack y cargar los archivos aplicables de `docs/context/` segun la tarea. Esta carpeta documenta a Greenhouse como plataforma/subproducto operativo de Efeonce Group dentro del modelo ASaaS; usarla para alinear producto, GTM, marca, experiencia cliente, metricas ICO y HubSpot Bow-tie.
- Greenhouse Operating Loop: todo trabajo formal debe respetar el ciclo `intake -> taxonomy -> plan -> execution -> verification -> closure -> handoff`; fuente canonica `docs/operations/GREENHOUSE_OPERATING_LOOP_V1.md`. Para cambios en tasks/epics/mini-tasks, usar `pnpm ops:lint --changed` como primera pasada.
...
- Autoria de tasks TASK-###: al crear o editar una task formal, recargar la skill task-planner vigente del agente (Codex: `.codex/skills/greenhouse-task-planner/SKILL.md`; Claude: `.claude/skills/greenhouse-task-planner/skill.md`) y no usar memoria vieja del formato. La task solo puede entregarse como lista si `pnpm task:lint --task TASK-###` reporta `template=1`, `errors=0`, `warnings=0`; si sale `legacy=1`, corregir el markdown antes de responder. Reglas duras: incluir markers `ZONE 0` a `ZONE 4`; usar solo enums vigentes (`Execution profile: standard|ui-ux|backend-data`, `UI impact: none|copy|layout|interaction|motion|primitive|flow`, `Backend impact: none|api|db|migration|command|reader|sync|cron|webhook|integration`); agregar `UI ready: no|yes|n/a` (`no` por defecto para UI; `yes` solo si mapping/GVC/decision log estan completos y lint pasa); agregar `## UI/UX Contract` y `Wireframe: docs/ui/wireframes/TASK-###-short-slug.md` existente si `UI impact != none`; agregar `Flow: docs/ui/flows/TASK-###-short-slug-flow.md` existente si `UI impact: flow` o la UI coordina sidecar/drawer/modal/popover/navegacion; agregar `Motion: docs/ui/motion/TASK-###-short-slug-motion.md` existente si `UI impact: motion` o hay motion/microinteracciones no triviales; agregar `## Backend/Data Contract` si `Backend impact != none`; sincronizar `docs/tasks/README.md` y `docs/tasks/TASK_ID_REGISTRY.md`; correr `pnpm task:lint --task TASK-###` y `pnpm ops:lint --changed`.
- Secuenciacion de tasks hibridas: cuando una capacidad combine backend/data y UI visible, preferir dividirla en tasks dependientes por `Execution profile`: primero `backend-data` para schema/API/reader/command/migration/sync/contrato de datos, luego `ui-ux` para ruta visible, layout, interaccion, copy y GVC. Una task vertical hibrida solo es aceptable si es pequena, reversible, sin migracion/schema riesgoso y declara `## Hybrid Execution Justification` + orden interno de ejecucion. Fuente canonica: `docs/tasks/TASK_PROCESS.md` (`Hybrid Execution Profile Discipline`).
- Goal preflight TASK-* para Codex: si el operador pide a Codex ejecutar/implementar/continuar una `TASK-###` y no entrego un `/goal` explicito en la conversacion actual, Codex debe proponer primero un `/goal` recomendado (objetivo de cierre, evidencia, limites, estado si falta rollout y si conviene `mantente en develop`/subagentes) y esperar confirmacion antes del hook. Si el operador ya dio `/goal`, continuar con `pnpm codex:task-hook`; si ordena ejecutar sin goal, documentar la excepcion en Audit/Plan/Handoff.
- Hook TASK-* para Codex solamente: si el pedido del operador menciona `TASK-###`, `[TASK-###]`, una ruta `docs/tasks/**/TASK-###-*.md` o usa alias slash-style como `/implement-task TASK-###`, `/implement-task ###`, `/task TASK-###` o `/task ###`, Codex debe ejecutar `pnpm codex:task-hook TASK-###` antes de implementar y aplicar el prompt que imprime. El hook tambien acepta ids numericos (`pnpm codex:task-hook 1033`). Si el operador dice `mantente en develop`, usar `pnpm codex:task-hook TASK-### --develop`; si pide subagentes/delegacion/trabajo paralelo, usar `pnpm codex:task-hook TASK-### --subagents` (combinable con `--develop`) para registrar autorizacion explicita y activar el contrato `SUBAGENT TOOLING` del prompt. Skill Codex local: `.codex/skills/greenhouse-task-execution-hook/SKILL.md`. No obliga automaticamente a Cursor, Claude u otros agentes; se documenta aqui para convivencia multi-agente. Aplica solo a tasks formales `TASK-*`; la excepcion de rama debe documentarse en Audit/Plan/Handoff. Codex no debe crear `git worktree`/folders clon por defecto; solo con pedido o aprobacion explicita del operador. Drift guard: si cambia el hook, el prompt, skills, comandos/gates o entrypoints TASK, ejecutar `pnpm codex:task-hook:check` y actualizar `docs/operations/CODEX_EXECUTION_PROMPT_V1.md` en el mismo cambio si aplica.
- Hook ISSUE-* para Codex solamente: si el pedido del operador menciona `ISSUE-###`, `[ISSUE-###]`, una ruta `docs/issues/**/ISSUE-###-*.md` o usa alias slash-style como `/fix-issue ISSUE-###`, `/fix-issue ###`, `/issue ISSUE-###` o `/issue ###`, Codex debe ejecutar `pnpm codex:issue-hook ISSUE-###` antes de escribir codigo y aplicar el prompt que imprime. El hook acepta ids numericos (`pnpm codex:issue-hook 045`) y bloquea issues resueltos salvo `--review-resolved` para lectura. Skill Codex local: `.codex/skills/greenhouse-issue-execution-hook/SKILL.md`; prompt canonico: `docs/operations/CODEX_ISSUE_EXECUTION_PROMPT_V1.md`. La primera decision obligatoria es `issue-only fix` vs `issue + TASK` vs `blocked`; no se debe meter una remediacion grande dentro de un issue localizado. Todo cierre debe identificar consumidores/contratos vecinos y documentar evidencia de no-regresion o riesgo residual. Si el operador dice `mantente en develop`, usar `pnpm codex:issue-hook ISSUE-### --develop`. Drift guard: si cambia el hook, prompt, skill, comandos/gates o entrypoints ISSUE, ejecutar `pnpm codex:issue-hook:check`.
...
- Sitio publico (producto): las decisiones de **producto/posicionamiento/GTM** del sitio publico y su **roadmap de ejecucion** viven en `docs/public-site/` (README indice + `PRODUCT_ROADMAP.md` + `decisions/` como **PDR**, genero hermano del ADR). Al conversar o trabajar sobre landings, posicionamiento del sitio o su orden de ejecucion, empezar por ahi. Un PDR fija producto; cuando obliga arquitectura, cita el ADR de `DECISIONS_INDEX.md` (no duplica). Ejecucion real del sitio via skill `efeonce-public-site-wordpress`; posicionamiento via `seo-aeo`/`commercial-expert`. Antes de operar Kinsta por SSH/WP-CLI, ejecutar `pnpm public-website:ssh-check` y cargar `docs/architecture/agent-invariants/PUBLIC_SITE_KINSTA_ACCESS_AGENT_INVARIANTS.md`; un Kinsta API token ausente no significa que SSH este caido. Para blogposts agentic end to end, cargar `docs/operations/public-site-content-factory/AGENTIC_BLOGPOST_END_TO_END_RUNBOOK_V1.md`: Content Factory termina en privado y `publish` exige autorización humana explícita, snapshot, rollback y QA live.
...
- Construccion durante EPIC-027: el desacople fisico avanza solo por tasks del epic; una feature aislada no crea deployables ni paquetes por anticipado. El producto no se pausa: trabajo nuevo declara su `Modular Placement Contract`, nace extraction-ready y respeta la frontera vigente en `GREENHOUSE_BUILD_UNIT_DECOMPOSITION_DECISION_V1.md` + `docs/operations/MODULAR_MIGRATION_NEW_WORK_OPERATING_MODEL_V1.md`.
...
- QA robusta cross-agent: antes de cerrar implementaciones no triviales, incidentes, rollouts, cambios UI/schema/integracion/tooling/skills o cualquier trabajo donde "tests verdes" no pruebe runtime real, invocar `greenhouse-qa-release-auditor` y usar `pnpm qa:gates --changed` como primera pasada mecanica. La skill clasifica riesgo, inyecta skills especializadas a demanda por namespace de agente (Codex y Claude pueden tener nombres/coberturas distintas: UI/GVC, finance, payroll, release, secrets, browser diagnostics, arquitectura, docs, etc.) y emite `PASS | CONDITIONAL PASS | BLOCK`; si falta evidencia runtime, el estado correcto es `code complete, rollout pendiente` u `operativamente bloqueado`. Codex conserva el script local `.codex/hooks/qa-release-stop-hook.mjs`, pero el `Stop` automático queda **desregistrado por defecto** en `.codex/hooks.json` para evitar prompts out-of-band; solo debe reactivarse de forma opt-in (`GREENHOUSE_QA_STOP_HOOK_MODE=enforce` o payload `enforce_qa_stop_hook=true`) cuando el operador lo pida.
...
- Radiografía AEO (muestra de trabajo viva): `think.efeoncepro.com/muestras/<slug>-<token>` es una **capacidad** con dos trabajos — **educación** de cliente/prospecto (enseña qué significa "aparecer en ChatGPT" sin una lámina de teoría; sirve **sin venta en curso**) y **habilitación de ventas** (enlace + lámina de deck + demo en vivo + prueba que el comité verifica solo). 🔴 **El runtime vive en el repo satélite `efeonce-think`, NO en `greenhouse-eo`**; el cliente es un **payload JSON**, cero código (`if (cliente === 'x')` en un componente rompe la frontera del motor). Gate propio: `pnpm build && pnpm verify:aeo-xray` (46 asserts) + `pnpm read:aeo-xray` **obligatorio antes de tocar el texto**. Canon: `docs/think/radiografia-aeo-architecture.md` (invariantes vigentes) + `radiografia-aeo-manual.md` + `docs/documentation/comercial/radiografia-aeo-muestra-de-trabajo.md` + `docs/manual-de-uso/comercial/usar-radiografia-aeo-en-venta.md`. Skills: `seo-aeo` (oficio/schema/citabilidad), `seo-aeo-practice` (uso comercial/sales enablement), `greenhouse-public-private-tenders` (consumer), `commercial-expert`, `astro` (+overlay).
...
- **Greenhouse Visual Capture (`GVC`, `pnpm fe:capture`)**: herramienta canonica Playwright + agent auth para grabar `.webm` + frames PNG marker-based + GIF opcional de cualquier ruta del portal, con `index.html`, readiness/assertions, failure taxonomy, quality findings, microinteraction evidence y multi-viewport opt-in. Reemplaza el patron ad-hoc de `_cap.mjs`. Scenario DSL declarativo bajo `scripts/frontend/scenarios/`. Output `.captures/<ISO>_<scenario>/` (gitignored). Triple gate para production. Comandos: `pnpm fe:capture <scenario> --env=staging [--gif] [--headed]` o `pnpm fe:capture --route=/path --env=staging --hold=3000`. Comandos relacionados: `pnpm fe:capture:review <scenario|capture-dir>` genera dossier para UI review, `pnpm fe:capture:diff <prev> <curr>` compara before/after, `pnpm fe:capture:health` audita salud local reciente y `pnpm fe:capture:gc [--apply]` purga >30d. Para pantallas largas usar scenario con `scroll selector`, `scrollTo`, `mark fullPage` o `mark clipSelector`; preferir `data-capture="<seccion>"` sobre offsets fragiles. Para flows críticos usar `readiness`, `assertions`, `interaction`, `viewports` y `baseline` cuando aplique. **GVC V1.5 (TASK-1018): los contract gates son ADITIVOS y opt-in, NO mockup-only** — GVC captura cualquier ruta (mockup, runtime, `--route`, staging/prod) igual que antes; los gates `quality.{layout,runtime,keyboard,performance,enterpriseRubric}` se activan por scenario sobre mockup **o runtime** (warning-first), y solo el `baseline` visual diff (`baseline.surfaceId` + home durable `scripts/frontend/baselines/` + `pnpm fe:capture:diff --promote`, degrada a `baseline_stale` si falta) es específico del flujo mockup aprobado→runtime. Arquitectura: `docs/architecture/GREENHOUSE_FRONTEND_CAPTURE_HELPER_V1.md` (Delta V1.5). Manual: `docs/manual-de-uso/plataforma/captura-visual-playwright.md`.
- **Hook obligatorio de diseno UI (skills + wireframe/flow/motion + readiness + GVC en loop) — ANY UI work:** para CUALQUIER trabajo de UI (componente nuevo, cambio visual, layout, estados, microinteracciones, mockup, copy visible), ANTES de escribir JSX nuevo invocar las skills de product design que apliquen (`greenhouse-ux`, `modern-ui`, `state-design`, `forms-ux`, `greenhouse-ux-writing`) y **crear o registrar el wireframe** en `docs/ui/wireframes/` cuando la UI sea nueva, visible, reutilizable, client/public/executive-facing, tenga estados/copy relevante, charts/reportes/PDF/email, o venga de un asset Product Design. Si la UI abre sidecar/drawer/modal/popover o conecta pantallas/rutas, crear `docs/ui/flows/` y declarar `Flow`. Si introduce motion/microinteracciones no triviales, crear `docs/ui/motion/` y declarar `Motion`. La task UI debe declarar `UI ready: no` hasta tener implementation mapping, GVC scenario plan y design decision log en task/wireframe; `UI ready: yes` solo pasa con `pnpm task:lint --task TASK-###` sin findings. `pnpm task:lint --changed`, `pnpm ui:wireframe-check --task TASK-###`, `pnpm ui:flow-check --task TASK-###`, `pnpm ui:motion-check --task TASK-###` y `pnpm ui:readiness-check --task TASK-###` validan los contratos. Wireframe = contenido; flow = secuencia/routing/foco/recovery; motion = timing/feedback/reduced-motion; readiness = permiso de implementar sin inventar. DESPUES verificar con GVC en loop. NUNCA pintar UI freehand ni declarar "listo" sin contratos requeridos + captura GVC mirada.
...
- **Figma Implementation Contract (canónico — al implementar CUALQUIER diseño, especialmente desde Figma):** Figma es **intención, no valores literales**. ANTES de escribir JSX, correr 2 gates:
  1. **Token mapping (siempre):** cada color → `theme.palette.*` / `theme.axis.*` / `var(--mui-palette-*)`; tipografía → variant/SoT (skill `typography-design`); spacing/radius → spacing scale `4n` / `theme.shape.customBorderRadius.*`; motion → tokens `motion/core/tokens.ts`. **NUNCA** transcribir HEX/px/fontFamily/ms crudos de Figma. El MCP Figma da los valores → **mapealos, no los pegues** (`get_variable_defs` + `get_code_connect_map`). Enforcement: lint `greenhouse/no-hardcoded-hex-color` (warn) + `no-hardcoded-fontfamily` (error) + `no-fontsize-inline-typography` (warn).
  2. **Primitive lookup en capas (en orden, ANTES de construir):** (a) ¿existe **primitive Greenhouse**? grep `src/components/greenhouse/primitives/index.ts` (~79 exports) + variants/kinds + `docs/architecture/ui-platform/PRIMITIVES.md` → **usar o expandir** (variant/kind, no fork paralelo); (b) ¿hay **wrapper Vuexy `Custom*`** o componente MUI base (Select/Autocomplete/List/TextField/Menu…)? → la primitive nueva **envuelve esa base** (hereda a11y/teclado/estados), NUNCA reinventar input/select/list/dropdown desde cero; (c) solo si no hay nada → desde cero.
  ...
  - **Nacimiento de una primitive nueva (dropdown/list/input/etc.)** = protocolo Primitive+Variants+Kinds COMPLETO: `src/components/greenhouse/primitives/` + export en barrel + resolver `kind→variant`; a11y/responsive/reduced-motion horneados; **cero hardcode** (solo tokens); **Lab interno** `/admin/design-system/<nombre>` (gate `administracion.design_system`, alcanzable por nav + route-reachability); **GVC** desktop+mobile; nodo AXIS Figma referenciado; contrato en `ui-platform/PRIMITIVES.md` (+ ADR si es platform-level). Patrón fuente: `GreenhouseButton`/`GreenhouseChip`/`GreenhouseActivityTimeline`/chart cards (2026-06-07).
  - **Reportar la decisión** (reuse / extend / new-primitive + por qué) ANTES de codear. One-off no-reusable puede vivir junto al consumer pero **igual tokenizado** (no va al registry). Skills que aplican: `greenhouse-product-ui-architect`, `greenhouse-portal-ui-implementer`, `greenhouse-vuexy-ui-expert`, `greenhouse-ui-orchestrator`, `greenhouse-mockup-builder`.
- **Patron canonico Composition Shell — BASE POR DEFECTO de toda interfaz nueva (TASK-1114/1117/1119; directiva del operador 2026-06-14):** toda superficie/pantalla/vista NUEVA de Greenhouse debe **partir del Composition Shell** como substrato de layout — declarar su composición (`single`/`leadPlusContext`/`split`/`focused`) y colocar el contenido en regiones singleton (`primary`/`aside`/`lead`/`dock`/`overlay`), en lugar de inventar grids/morph/reestructuración de layout ad-hoc. Es el **punto de partida arquitectónico, no una opción más**. Usar `CompositionShell` + `composition-shell-controller` desde `@/components/greenhouse/primitives`; opt-in a la fluidez con `fluidity='rich'` (stagger + morph interrumpible framer-motion `layout` + drawer temporal en compact `split`). **Única salvedad** (no la regla): one-offs genuinamente triviales sin composición de regiones (un form aislado, un `Dialog`, una página estática de texto) pueden quedar en `LayoutContent` plano — pero deben **considerar el substrato PRIMERO y justificar por qué no aplica**. NUNCA inventar un sistema de regiones/morph/coreografía de layout paralelo (es exactamente lo que el substrato absorbe). NUNCA hand-wirear el namespace reservado `gh-region-*` de view-transition-name (lint `greenhouse/no-ad-hoc-layout-morph`). Coherente con el ADR: el substrato es aditivo (no reescribe `LayoutContent` por fiat) y los siblings legítimos (`NexaMomentComposition`) coexisten — "base por defecto" = **primer recurso + dueño del modelo de regiones**, no prohibición de todo lo demás. Lab `/design-system/composition-shell`. ADR `docs/architecture/GREENHOUSE_COMPOSITION_SHELL_DECISION_V1.md` + companion `_UI_PLATFORM_V1.md`. **Default de fluidez = `rich`** (decisión del operador 2026-06-14): la coreografía rica (stagger de entrada) es el estándar; reduced-motion horneado; el primer paint no se retrasa. `baseline` es opt-out explícito.
- **Patron canonico Adaptive Card density + "The Seam" — todo elemento/card NUEVO nace adaptable + rich-ready (TASK-1115; directiva del operador 2026-06-14):** cuando un card vive en una región que puede condensar (o cualquier contenedor de ancho variable), debe **adaptarse a SU PROPIO ancho** (container query `inline-size`) con el contrato compartido `card-density` (`useContainerDensity` + `resolveCardDensity`/`isCardDensityAtLeast` desde `@/components/greenhouse/primitives`) — modos `full`/`condensed`/`peek`, **condensación honesta** (versión real más chica, NUNCA clip/overflow/`$0`; el dato clave nunca desaparece) + **morph fluido** del cambio de densidad (framer-motion `layout` + `cardDensityLayoutTransition`, reduced-motion horneado). **DIRECTIVA DEL OPERADOR: todo elemento/card NUEVO debe NACER con esta capacidad** (adaptable + rich-ready) — es el estándar de nacimiento, no opt-in; el `default full` legacy es solo para lo que existía antes. **El card NO hereda del shell** — el seam es la container query; componen sin cablearse. El patrón conjunto Composition Shell × Adaptive Card se llama **"The Seam" (La Costura)**. NUNCA crear un `adaptive-card` paralelo ni construir sobre `AdaptiveSidecarLayout`. Generaliza el density contract de tablas (TASK-743). Lab `/design-system/card-density` (sección The Seam). ADR `GREENHOUSE_COMPOSITION_SHELL_DECISION_V1.md` §Delta Adaptive Card / The Seam.
- **Contención de scroll horizontal — overflow de PÁGINA NUNCA por sr-only / Recharts / grid min-content (clase TASK-742 + ISSUE-015, recanonizado 2026-06-14):** un `scrollWidth > clientWidth` de página viene de un descendiente cuyo ancho intrínseco se propaga sin que ningún ancestro lo clipe. **3 fuentes + fix:** (1) **sr-only `visuallyHidden` MUI** (`position: absolute`, ej. tabla a11y de un chart) — "el aria label que ya pasó antes": su contenedor directo necesita `position: relative` (lo contiene; su `width:1px`+`overflow:hidden` lo clipa). (2) **Recharts `ResponsiveContainer`** con ancho stale: el `<Box>` padre directo lleva `minWidth: 0` + `overflowX: 'clip'` (clip = no clipa el tooltip vertical). (3) **CSS Grid `1fr`/`repeat(N,…)`** que crece al `min-content` del ítem: `minmax(0, 1fr)` + `'& > *': { minWidth: 0 }`, y apilar en xs (`{ xs: '1fr', sm: … }`) los layouts con piso de px (el `split` del Composition Shell no cabe a 390px). Red de seguridad **ISSUE-015**: `minWidth: 0` + `overflow-x: hidden|clip` en wrappers; `overflowX: 'clip'` en el root de una vista que nunca deba scrollear de página; contenido que SÍ debe scrollear horizontal → `overflow-x: auto` accesible (`role='region'`+`aria-label`+`tabIndex={0}`), nunca scroll de página. **⚠️ Verificación obligatoria:** medir `scrollWidth > clientWidth` con Playwright a desktop **Y mobile 390px** — GVC `fullPage` enmascara el scroll de página; el `scrollWidth` es la verdad. Los fixes van en la **primitive**, no parcheando cada view.
...
- **Patron canonico Floating Surface (TASK-1033):** para UI contextual **anclada y transitoria** (popovers, action menus, rich tooltips, evidence peeks, inline editors, validation bubbles, command previews) usar `GreenhouseFloatingSurface` + `floating-surface-controller` desde `@/components/greenhouse/primitives`. 6 variants oficiales con contrato a11y por variant (`richTooltip`/`actionMenu`/`evidencePeek`/`inlineEditor`/`validationBubble`/`commandPreview`) + resolver idempotente `kind→variant`. Los **views de producto NO importan `@floating-ui/react` directo** — la lint rule `greenhouse/no-direct-floating-ui-in-views` (modo `error`) lo bloquea; exentos solo las primitives (`src/components/greenhouse/primitives/**`) e infra Vuexy menu (`src/@menu/**`). Complementa Adaptive Sidecar; NO reemplaza lanes full-height ni `Dialog` de decisiones destructivas/legales/financieras. ADR: `docs/architecture/GREENHOUSE_FLOATING_SURFACE_DECISION_V1.md`; Delta en `docs/architecture/ui-platform/HISTORIAL.md` (2026-06-06).
- **Patron canonico Motion Primitive (TASK-1045):** para el tier cinemático / orquestado / scroll (hero intros, list mounts, section reveals, timelines, count-ups) usar `<Motion>` (declarativo) o `useGreenhouseGSAP` (escape hatch) desde `@/components/greenhouse/motion`. 4 variants (`entrance`/`stagger`/`scrollReveal`/`timeline`) + resolver `kind→variant`. Los **views NO importan `gsap`/`@gsap/react` directo** — lint rule `greenhouse/no-direct-gsap-in-views` (error) lo bloquea; exento solo `src/components/greenhouse/motion/**`. `prefers-reduced-motion` horneado en `useGreenhouseGSAP` (no-bypassable); degradación honesta; solo props de compositor. GSAP NO reemplaza CSS Tier 1 ni framer-motion (capa aditiva). Tokens SoT en `motion/core/tokens.ts` (portable, drift-guarded). Museo `/admin/design-system/motion`. ADR: `docs/architecture/GREENHOUSE_MOTION_PRIMITIVE_V1.md`.
- **Botones de Nexa — Nexa Mark + Shiny Button (navy) (convención de marca, 2026-06-20):** todo botón/CTA que **invoque o represente a Nexa** ("Pregúntale a Nexa", "Seguir con Nexa", etc.) usa la marca de Nexa, no un botón genérico. (1) **Nexa Mark obligatorio:** ícono = `GreenhouseNexaBrandMark` (desde `@/components/greenhouse/primitives`), NUNCA un ícono Tabler genérico ni solo texto; sobre fondo oscuro `kind='inlineMarkOnDark'`. (2) **Shiny Button para el navy de Nexa:** el midnight navy característico se expresa con `GreenhouseShinyBorder asButton variant='cta' palette='nexa'`, con el Mark + label como `children`; NO un `<Button>` MUI plano. Shape canónico: `<GreenhouseShinyBorder asButton variant='cta' palette='nexa' ariaLabel onClick>` + `<GreenhouseNexaBrandMark kind='inlineMarkOnDark' size='small' />` + label. Referencias: bridge CTA de `KnowledgeNexaCompositionLens`, CTA "Pregúntale a Nexa" de `NexaInsightDetailView` (TASK-1182). Copy en `src/lib/copy/nexa.ts` (`GH_NEXA`). NUNCA hardcodear el navy (sale del `palette='nexa'`). Detalle: `CLAUDE.md` §"Botones de Nexa".
...
- **Diagnostico local `Compiling...` / Turbopack:** si `localhost` queda compilando o `next-server` consume CPU alto, no resolver primero con `pnpm clean`. Secuencia canonica: `ps`/CPU → `curl -I` vs browser real → Playwright console/network filtrando `_next/static/chunks`, HMR y 404 → revisar `.next/dev/**/react-loadable-manifest.json` vs `.next/dev/static/chunks`. Si hay chunk huerfano, buscar fronteras `dynamic()`/imports nested en wrappers compartidos y fijar el owner canonico + guardrail. Caso fuente: `ISSUE-085`.
...
**Conectores MCP creativos / de diseño (solo sesiones Claude).** Aparte de los CLIs locales, en sesiones Claude hay conectores MCP autorizados vía `claude.ai` para trabajo creativo/de diseño. Son **session-scoped** (cargan al iniciar sesion; uno agregado a mitad de sesion no lo ve la sesion activa) y **solo Claude** (Codex/Cursor no los heredan; no existen en cron/headless). Disponibles: **Adobe for creativity** (verificado cuenta completa `auth` — antes de cualquier tool llamar `adobe_mandatory_init`; cubre edicion de imagen Photoshop/Lightroom, vectores Illustrator (`image_vectorize` raster→SVG, `document_render_vector` `.ai`→export, data merge), video/audio Premiere (`video_create_quick_cut`, `media_summarize`, `video_resize`, `media_enhance_speech`), documentos InDesign, y diseño de canvas fijo HTML→Express/PDF/PPTX + Adobe Stock + Adobe Fonts), **Figma**, **Higgsfield**, **Magnific**, **ElevenLabs**, **Metricool**, **Whimsical**, **WordPress.com**. **Reglas duras:** (1) **NUNCA** cablear un tool MCP creativo a un flujo runtime del producto — son produccion/edicion **out-of-band**; el asset se sube al portal por el uploader canonico y el entrypoint runtime de imagenes sigue siendo `src/lib/ai/image-generator.ts` (`pnpm ai:image`). (2) Casi toda la IA generativa Adobe **no esta disponible** en este entorno (excepcion: `image_generative_expand` outpaint); para generacion real usar los caminos canonicos (`pnpm ai:image`, Higgsfield, Magnific, Vertex). Detalle: `CLAUDE.md` → "MCP creativos / de diseño".
...
**⚠️ Prender/apagar un flag es MULTI-RUNTIME, no "en Vercel".** Un `*_ENABLED` lo lee **cada runtime que ejecuta el código gateado**, y hay **5 con env vars independientes**: Vercel (app Next.js) + 4 Cloud Run (`ops-worker`, `commercial-cost-worker`, `ico-batch-worker`, `hubspot-greenhouse-integration`). Prenderlo en uno NO lo prende en los otros. **NUNCA** prendas un flag sin mapear antes dónde se LEE (`grep -rn "<FLAG>" src/ services/ | grep -v __tests__`) y aplicarlo en **todos** los runtimes del mapeo. Heurística: si gatea algo **async** (email, projection reactiva, consumer del outbox, cron de Scheduler, materializer) vive en el **`ops-worker`, NO en Vercel**; si gatea una ruta/superficie visible, en Vercel; puede vivir en ambos. Cloud Run: **el SoT es `services/<worker>/deploy.sh`** — declarar el flag ahí (los `deploy.sh` usan `--set-env-vars` destructivo, que borra cualquier var agregada out-of-band) **y además** aplicarlo en vivo con `gcloud run services update <svc> --region <r> --project efeonce-group --update-env-vars <FLAG>=true` para efecto inmediato. Hacer sólo lo segundo = el flag desaparece en el próximo deploy, en silencio. Verificar la env en la **revisión activa** + ejercitar el flujo real (que la var exista ≠ que el consumer funcione), y **declarar el/los runtime(s) en la fila del ledger**. Runbook: `FEATURE_FLAG_STATE_LEDGER.md` → §`Cómo prender un env-var flag`. Caso fuente 2026-07-09: `GROWTH_EBOOK_EMAIL_DELIVERY_ENABLED` vive sólo en el `ops-worker` (gatea una projection reactiva); prenderlo en Vercel no habría hecho nada.

### 1. Antes de cambiar codigo

- Leer `project_context.md`.
- Leer `Handoff.md` para ver trabajo en curso, riesgos y proximos pasos.
- Leer `docs/context/00_INDEX.md` cuando el cambio toque producto, UX/copy, naming, metricas, HubSpot/Account 360, onboarding/cliente, GTM, marca o estrategia comercial. Seguir su tabla de carga selectiva: `05` para voz/copy, `06` para metricas/naming, `07` para ICO, `08` para prioridad comercial, `09` para marca Efeonce, `10` para experiencia cliente y `11` para HubSpot Bow-tie. Si un doc de contexto contradice arquitectura vigente, runtime real, `DESIGN.md` o una spec tecnica aprobada, prevalece el contrato tecnico verificado y se documenta el drift.
...
- **Design System catalog canónico: `/admin/design-system`** — superficie **INTERNA**, los clientes NUNCA la ven. Es la home navegable de AXIS/Design System y debe listar toda incorporación nueva (tokens, primitives, patrones, labs o governance) con ruta real, SoT/owner y links funcionales. La paleta AXIS vive como child route `/admin/design-system/colors` (TASK-1034) y renderiza los ramps AXIS live desde `theme.axis.*` / `src/@core/theme/axis-tokens.ts` (SoT 1:1 con AXIS Figma, fileKey `yyMksCoijfMaIoYplXKZaR` nodo `11205:5341`). Toda nueva página `/admin/design-system/<slug>` debe: agregarse al catálogo (`DesignSystemCatalogView`), declararse en `route-reachability-manifest.ts`, tener scenario GVC si es visual/repetible y documentarse en `ui-platform/*`/`project_context.md` cuando cambie el contrato. Gateada por viewCode `administracion.design_system` (routeGroup `internal`, solo roles internos — NUNCA `client_*`) + redirect defensivo si `tenantType==='client'`. Los HEX se resuelven desde `theme.palette.*` / `theme.axis.*`, NUNCA inline. El `AxisWordmark` es solo del design system (NUNCA en UI de producto/login/emails/PDFs/portal cliente).
...
- Si el cambio es UI, UX o seleccion de componentes, usar como criterio operativo los skills locales vigentes (`greenhouse-agent`, `greenhouse-portal-ui-implementer`, `greenhouse-ui-orchestrator` o `greenhouse-vuexy-ui-expert`), revisar `full-version` junto con la documentacion oficial de Vuexy antes de inventar componentes nuevos y leer `DESIGN.md` en raiz como contrato visual legible por agentes. La referencia de **plataforma UI** vive en `docs/architecture/ui-platform/` (empezar por `README.md` + mapa "donde vive X": STACK/PRIMITIVES/STATE/FORMS/TABLES/MOTION/I18N/PATTERNS/GOVERNANCE; historial cronologico en `HISTORIAL.md`). El viejo `GREENHOUSE_UI_PLATFORM_V1.md` quedo como router stub. Regla anti-monolito: cambio vigente edita el doc tematico, la cronologia va a `HISTORIAL.md`. ADR: `GREENHOUSE_UI_PLATFORM_RESTRUCTURE_DECISION_V1.md`.
...
- **AI image — gpt-image-2 raster + env por entorno + logo IA de organizacion (2026-06-09):** `gpt-image-*` es RASTER (PNG/WebP/JPEG), **no genera SVG**. `OPENAI_API_KEY_SECRET_REF=greenhouse-openai-api-key` debe existir en CADA entorno (local/Vercel/workers) o todo flujo OpenAI devuelve "not configured" (Runtime Rollout Completion Gate). La **generacion de logo de organizacion con IA** (TASK-999) vive en `src/lib/account-360/organization-logo-generation.ts` → `POST /api/organizations/[id]/brand-assets/logo/generate` (gate `organization.brand_asset` + fail-fast `is_operating_entity`); por decision explicita del operador el prompt **recrea el logo real** del cliente (excepcion al default de la skill). Detalle: CLAUDE.md → "AI Visual Asset Generator" + ADR `GREENHOUSE_ORGANIZATION_BRAND_ASSET_DECISION_V1.md` Delta 2026-06-09.
- **Vectores para UI vía Higgsfield CLI + Recraft V4.1 (desde 2026-06-09):** la CLI `higgsfield` (binario `~/.local/bin`, alias `hf`, cuenta `mkt@efeoncepro.com` Ultra, autenticada) + el MCP Higgsfield exponen **Recraft V4.1** (`recraft_v4_1`) con `--model_type vector` → **salida vectorial real**, el hueco que `gpt-image` (raster-only) deja abierto. Es la herramienta para **producir assets vectoriales de UI/marca** (iconos, logos, ilustraciones de design-system, empty states) con paleta controlada (`--colors`, p.ej. tonos AXIS) + `--background_color`/`--aspect_ratio`/`--resolution {1k,2k}`. Comando: `higgsfield generate create recraft_v4_1 --prompt "…" --model_type vector --aspect_ratio 1:1 --resolution 2k --wait`. **Caveats duros:** (1) es producción de assets **out-of-band** (se generan y se SUBEN al portal vía el uploader canonico), **NO** el path runtime — el entrypoint runtime canonico sigue siendo `src/lib/ai/image-generator.ts` (OpenAI/Imagen); NUNCA cablear Higgsfield a un flujo runtime del producto. (2) Usar las skills Higgsfield (`higgsfield-generate`/`-product-photoshoot`/`-soul-id`/`-marketplace-cards`) para el craft. (3) Verificar el formato del archivo entregado (SVG) en el primer uso real. (4) Aplica el contrato visual Greenhouse igual (tokens AXIS, no inventar hex) + revisar con skills de diseño antes de integrar. Detalle: CLAUDE.md → "AI Visual Asset Generator".
- **AI providers (texto/LLM) conviven en `src/lib/ai/` — NO crear clientes paralelos:** Gemini/Vertex es el path de texto canonico (`src/lib/ai/google-genai.ts` + `greenhouse-agent.ts`, modelos en `src/config/nexa-models.ts`); OpenAI cubre imagenes (`openai-image.ts`, secret `greenhouse-openai-api-key`). **Anthropic/Claude** esta disponible desde 2026-06-05 para drafting de documentos HR/legal (Workforce Contracting Studio, TASK-1019): secret canonico `greenhouse-anthropic-api-key` en GCP Secret Manager (`ANTHROPIC_API_KEY_SECRET_REF`), cliente canonico en `src/lib/ai/anthropic.ts` (lo crea TASK-1019 Slice 3, consumido por `src/lib/workforce/contracting/` detras del flag `WORKFORCE_CONTRACTING_AI_ENABLED=false`). Nunca hardcodear `sk-ant-*`; resolver server-side via `resolveSecretByRef`; no instanciar el SDK Anthropic dentro de un modulo de dominio.
- **Nexa Intelligence — documentación por capas + doc gate (TASK-1124):** la inteligencia conversacional de Nexa está documentada por capas (carpetas que crecen por dominio) en `docs/architecture/nexa-intelligence/` (índice `README.md`: `system-prompt/` · `behavior/` · `voice/` · `knowledge/` · `experience/` [Conversational Experience + Nexa Moments + Nexa Answers] · `governance/` · `technical/` [modelos LLM, RAG, técnicas, contratos]). SSOT del mapeo dominio↔código↔docs: `docs/architecture/nexa-intelligence/manifest.json`. Gate `pnpm nexa:doc-gate --changed` (en `ci.yml`): si tocás un dominio Nexa (prompt, voz, behavior/routing, tool/knowledge, evidencia, experience, flags, modelos LLM) y NO actualizás su doc de capa, falla; un archivo Nexa nuevo sin registrar en `manifest.json` (domains o codeAllowlist) también falla. **NUNCA** tocar Nexa sin actualizar su doc de capa en el mismo cambio. Funcional: `docs/documentation/plataforma/nexa-intelligence-capas.md`; manual: `docs/manual-de-uso/plataforma/nexa-intelligence-mantener.md`.
- **Nexa governed action runtime (TASK-1137):** Nexa puede ejecutar **acciones gobernadas** (rung `execute_requires_confirmation` del Action Maturity Ladder) detrás de `NEXA_ACTION_RUNTIME_ENABLED` (default OFF). Regla dura cross-agent: el **LLM NUNCA ejecuta un write** — solo PROPONE una `actionKey` **registrada** vía el tool `propose_action` (read-only); el humano confirma; el endpoint determinístico `POST /api/nexa/actions/[actionKey]/confirm` (capability `nexa.action.execute`) ejecuta el command bound vía `executeApiPlatformCommand` (TASK-655, `app_user`). El LLM NUNCA pasa un endpoint/URL/SQL ni llama el endpoint de confirmación; sin command canónico → gap honesto/deep-link. El binding `actionKey → command` vive en el registry (`src/lib/nexa/actions/registry.ts`); agregar una acción = code change humano, NUNCA inferido. Archivos nuevos bajo `src/lib/nexa/actions/**` se registran en el dominio `governed-actions` del `manifest.json`. Detalle canónico (registry, capability/grant, ledger `nexa_action_events`, signals, piloto) en `CLAUDE.md` §"Nexa governed action runtime invariants".
...
- **⚠️ Worker @core boundary (incidente ICO batch 2026-06-08, TASK-1048/1053)**: los Docker builds de los workers **EXCLUYEN `src/@core`** (capa de tema AXIS/Vuexy). **NUNCA importes `@core/theme/*` (ni `@menu`/`@layouts`/`@assets`) desde `src/lib/**` que un worker bundlee** (dominio ico-engine/sync/finance/payroll/projections/materializers + PDF/Excel generators consumidos por projections) → con `--packages=external` el import se externaliza → silent startup crash (`ERR_MODULE_NOT_FOUND`, container no escucha 8080, deploy failed). **DATA de design tokens runtime-agnóstica** (hexes puros para UI+worker+PDF) vive en `src/lib/design-tokens/*` (literales, cero deps); `@core/theme/*` la re-exporta para UI. **Concern de UI** (color de chart, tonos) vive en `src/components/**` (excluida de workers), no en dominio. Guard: esbuild de los 3 workers Node tiene `--alias:@core=./src/@core` (convierte el crash en loud-build-fail). Detalle completo + verificación local en CLAUDE.md → "Worker @core boundary".
- **⚠️ Worker runtime npm deps (hermano del @core boundary, ISSUE-090, 2026-06-09)**: el runtime stage del Dockerfile de los 3 workers Node hace `pnpm install --prod` (SOLO `dependencies`) + esbuild `--packages=external`. **TODO paquete npm importado por código worker-bundled (`src/lib/**` o `server.ts`) DEBE estar en `dependencies`, NUNCA solo en `devDependencies`** → si queda en devDeps, el runtime no lo instala → import externalizado no resuelve → silent startup crash (`ERR_MODULE_NOT_FOUND`, no escucha 8080, deploy failed). En Vercel "funciona" por casualidad (Next bundlea sin importar la sección); el worker exige `dependencies`. Causa: `organization-brand-assets-discovery.ts` (TASK-999, importado por `ops-worker/server.ts`) usa `pngjs` que estaba en devDeps. **Guard**: `pnpm worker:runtime-deps-gate` (`scripts/ci/worker-runtime-deps-gate.mjs`, en ci.yml) falla loud si un externalizado del bundle no está en deps. Fix: mover a `dependencies` (+ `pnpm install --lockfile-only`) o hacer el import lazy. `devDependencies` solo para lo que NO entra a ningún worker (tests/GVC/build: `pixelmatch`, `playwright`). Detalle en CLAUDE.md → "Worker runtime npm deps".
...
**Account 360 facet readers — anti silent-catch (TASK-1059, 2026-06-09):** los readers canónicos del 360 (`src/lib/account-360/facets/*`) **NUNCA** envuelven una sub-query de dato primario en `.catch(() => [])` — esconde drift de schema/scope como "sin datos" (causa de los tabs vacíos Equipo/Delivery/Economía). Usar `observeAndRethrow`/`observeAndDegrade` (`src/lib/account-360/facet-observability.ts`): primario re-lanza → `_meta.errors` + `captureWithDomain`; enriquecimiento degrada honesto. Conteo de tareas por `task-status-canonical` (no literales `status='completed'`); membresías as-of NULL-safe (`start_date IS NULL OR <= asOf`); ICO org-level desde BQ `ico_engine.metrics_by_organization` keyed por `spaces.client_id`. Validar SQL nuevo contra PG real. Contrato completo en CLAUDE.md → "Account 360 facet readers — anti silent-catch contract".
...
**Cargar `docs/architecture/agent-invariants/COMMERCIAL_TENDERS_AGENT_INVARIANTS.md`** al tocar `src/lib/commercial/tenders/**`, `src/lib/artifact-composer/**` o cualquier oferta/deck de licitacion. **El mapa operativo end-to-end (de "llegó un RFP" a "el PDF está en el asset store") vive en el companion de la skill: `.codex/skills/greenhouse-public-private-tenders/proposal-studio-runtime.md` — LEELO ANTES DE TOCAR NADA** (tiene las 6 recetas + las costuras por donde se extiende). Skills: `greenhouse-public-private-tenders` (proceso) + `deck-studio` (argumento y composición). Manual humano: `docs/manual-de-uso/proposal-studio/rfp-a-pdf-el-dia-a-dia.md`. Para `TimelineFull`, consultar el companion: el plan declara schedule y `barLabel`; nunca se edita geometría a mano.

**Estado real (2026-07-12 — el sistema COMPLETO existe y corrió de punta a punta con un caso real):** `TASK-1394 → 1393 → 1392 → 1391 → 1399` están **complete**. Lo que ANTES decía esta sección ("no existe API/migración/capability/outbox; el único consumer es el CLI; el worker sigue bloqueado por EPIC-027") **ya no es cierto** — no lo asumas:

- **Motor** (`src/lib/artifact-composer/**`, TASK-1393): domain-free, boundary por allowlist. El deck es UN catálogo (`catalogs/deck-axis/`: 25 plantillas tokenizadas, brand pack `axis` compilado, font pack hermético — el render **bloquea la red**). Un autor/agente declara INTENCIÓN (`contentType` + slots), **NUNCA** `template` (`TemplateAuthorityError`). Emite el `ResolvedCompositionManifest` (canónico + hasheado). CLI: `pnpm deck:compose <plan.json>`.
- **Dominio** (`src/lib/commercial/tenders/proposals/**`, TASK-1392): el aggregate **`Proposal` EXISTE en DB** (`greenhouse_commercial.proposal*`) con state machine persistida, gates humanos **en la DB** (triggers append-only), evidencia inmutable con `audience`, API `/api/commercial/proposals/**`, entitlement per-ORG `proposal_studio_v1` (**activo para Efeonce**), 6 eventos outbox y el intake agent.
- **Render** (`services/artifact-worker/`, TASK-1391): **PRIMER Cloud Run Job del ecosistema** (frontera por excepción documentada de EPIC-027 — **ya autorizada, no la re-litigues**). Cola `proposal_render_jobs` + dispatcher deadline+aging en el ops-worker + gates fail-closed al encolar + QA visual mecánica. Verificado en Cloud Run con el deck real de SKY (15 láminas / 25,2 s) y bench de 25 (32,3 s). Flag `ARTIFACT_RENDER_JOBS_ENABLED`: **ON en staging, OFF en prod** (sign-off + release control plane pendientes).
- **Nexa lo opera** (`src/lib/nexa/actions/proposal-studio.ts`, TASK-1399): 4 acciones gobernadas (`register_proposal`, `attach_proposal_rfp`, `record_proposal_evidence`, `request_proposal_render`) + tool read-only `proposal_status`. Flag `NEXA_PROPOSAL_ACTIONS_ENABLED`: **ON en staging** (verificado con el LLM real), OFF en prod. Nexa es **otro consumer del MISMO primitive**, por la MISMA puerta (`assertProposalStudioAccessForSubject`) — no un camino paralelo.
- **Gate visual + baseline** (`pnpm composer:visual-gate` a 0 píxeles). **ANTES de cualquier `--freeze`, leé el runbook `docs/operations/runbooks/composer-visual-gate.md`** (fuente única del proceso; cualquier agente lo carga al tocar el composer). Bug class `ISSUE-122`, dos reglas duras: **(a)** el `--freeze` es **SINGLE-OWNER, serializado y atómico** (freeze + commit juntos) — **NUNCA** congeles con el composer sucio por OTRO agente (`git status` con `M` ajenos en `src/lib/artifact-composer`/`scripts/frontend/baselines` = coordiná, no congeles; co-mingla su WIP → baseline corrupto). **(b)** las láminas con **fotos** (`TeamGalleryFull`/equipo) **driftean píxeles entre corridas/entornos** aunque no las toques; el `--selftest` de 2 corridas juntas NO lo atrapa. Si el gate flagea SOLO el área de foto de una lámina que no cambiaste, **es `ISSUE-122`, NO tu regresión — NO la rebaselines** (oculta el bug).

**Reglas duras que nacieron con Nexa y aplican a CUALQUIER consumer nuevo (UI, MCP, cron):** **NUNCA** escribas un segundo gate (el núcleo es `assertProposalStudioAccessForSubject`; la firma con `TenantContext` es su adapter). **NUNCA** metas un id de organización en el `inputSchema` de un agente (el scope se deriva del entitlement; el cliente se resuelve **por nombre**, fail-closed). **NUNCA** reimplementes los gates del render en un preview/afordance: se **ejercitan** (`assertProposalRenderAdmissible`) — una copia miente el día que se agregue un gate. **NUNCA** re-tipes el manifest sin `.passthrough()` (Zod borra su procedencia → otro hash → el mismo deck daría dos jobs).
...
**`audience` (TASK-1392) — la regla que impide filtrarle municion al comprador.** Cada `tender_asset` nace `internal` o `client_facing`; **solo lo `client_facing` y aprobado se empaqueta**. **NUNCA** promover `internal → client_facing` por default: el **squad blueprint lleva el loaded cost** y el diagnostico interno lleva la lente tecnica — entregarlos es darle a la contraparte tu estructura de costos y tu piso de negociacion. **NUNCA** subir un RFP/anexo/entregable fuera del asset store canonico (`src/lib/storage/greenhouse-assets.ts`): nada de bucket, uploader, URL publica ni carpeta paralela.

**El agente NUNCA escribe (TASK-1392, molde de toda fase agentic del dominio):** contexto read-only allowlisted → **propuesta tipada que cita sus inputs** → **confirmacion humana** → el **mismo command canonico** que usan API/CLI/Nexa/MCP. **Propuesta ≠ ejecucion.** Una evidencia client-facing debe resolver a registro/snapshot permitido con fuente, locator, método, fecha de corte, clasificación y hash: nunca es un string inventado por el modelo. **NUNCA** el LLM muta estado, adjunta un asset ni cruza un gate. **NUNCA** introducir LangChain/LangGraph/Agents SDK (se reusa `src/lib/ai/` + el patron tool-use de Nexa). **NUNCA** declarar algo "agentic" si solo hay un prompt: hace falta contexto tipado, tools sobre primitives, propuesta trazable, eval y confirmacion humana real.
