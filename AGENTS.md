...
- Sitio publico (producto): las decisiones de **producto/posicionamiento/GTM** del sitio publico y su **roadmap de ejecucion** viven en `docs/public-site/` (README indice + `PRODUCT_ROADMAP.md` + `decisions/` como **PDR**, genero hermano del ADR). Al conversar o trabajar sobre landings, posicionamiento del sitio o su orden de ejecucion, empezar por ahi. Un PDR fija producto; cuando obliga arquitectura, cita el ADR de `DECISIONS_INDEX.md` (no duplica). Ejecucion real del sitio via skill `efeonce-public-site-wordpress`; posicionamiento via `seo-aeo`/`commercial-expert`. Para blogposts agentic end to end, cargar `docs/operations/public-site-content-factory/AGENTIC_BLOGPOST_END_TO_END_RUNBOOK_V1.md`: Content Factory termina en privado y `publish` exige autorización humana explícita, snapshot, rollback y QA live.
