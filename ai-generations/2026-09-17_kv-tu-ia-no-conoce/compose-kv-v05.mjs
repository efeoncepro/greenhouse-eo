// KV paraguas v05 «Tu IA no conoce tu negocio» — plate nativo con muro navy (sin recortes ni repintado alrededor del
// sujeto), jerarquía tipográfica jerarquía tipográfica
...
const SPEC = {
  '4x5': { W: 1080, H: 1350, plate: 'feed/plate-4x5-wide-v05.png', wall: [250, 830], headTop: 371, top: 128, impactWidth: 0.47, desk: [1093, 1350], logoCenterY: 1176, logoW: 210, safe: 0 },
  '9x16': { W: 1080, H: 1920, plate: 'v05/plate-9x16-v05-b.png', wall: [122, 956], headTop: 525, top: 262, impactWidth: 0.47, desk: [1547, 1920], logoCenterY: 1640, logoW: 220, safe: 0.13 }
}[FORMAT]
const { W, H } = SPEC
const OUT = `kv-tu-ia-no-conoce-clawd-${FORMAT}-v05.png`
...
// Presentación de campaña: cursor y etiqueta del colaborador legibles a 390 px y en el color de marca de Clawd
// (rgb(215,119,87), tomado del binario oficial de Claude Code); la tinta la elige el adapter por contraste WCAG.
const PRESENTATION = { collaboratorScale: 1.9, localCursorScale: 1.25, participantColors: { claude: '#d77757' } }
const rendered = renderCollaborationSelection({ manifest, targetBounds: impactBox, canvas: { width: W, height: H }, measureLabel, presentation: PRESENTATION })
...
// ── Guardias ─────────────────────────────────────────────────────────────────────────────────────────────────────
const labelBottom = Math.max(...rendered.evidence.cursorEvidence.map(c => c.labelBounds?.bottom ?? 0))
const stackBottom = Math.max(chip.y + chip.h, labelBottom)
