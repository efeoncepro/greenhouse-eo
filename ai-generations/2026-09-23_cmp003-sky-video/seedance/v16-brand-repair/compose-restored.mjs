...
const source = D + '/topaz-full-4k.mp4';
...
if (v.width !== 2160 || v.height !== 3840 || v.r_frame_rate !== '24/1' || +meta.format.duration < 26) throw Error('Plate requires explicit conform review');
const bubbleDir = path.resolve(D, '../v14-finish/qa/bubble-alpha');
fs.mkdirSync(bubbleDir, { recursive: true });
const output = D + '/sky-v16-picture-4k.mp4';
if (fs.existsSync(output)) throw Error('Output exists; preserve existing review version');
const decoder = spawn('ffmpeg', ['-hide_banner', '-loglevel', 'error', '-i', source, '-i', path.resolve(D, '../v15-review/picture-lossless.mkv'), '-i', approved + '/titulos-y-firma-alpha.mov', '-filter_complex', '[0:v]trim=duration=26,setpts=PTS-STARTPTS,setparams=colorspace=bt709:color_primaries=bt709:color_trc=bt709:range=limited,scale=in_color_matrix=bt709:out_range=pc,format=rgb24[restored];[1:v]trim=start=26:end=30,setpts=PTS-STARTPTS,scale=2160:3840:flags=lanczos,format=rgb24[tail];[restored][tail]concat=n=2:v=1:a=0[base];[2:v]scale=2160:3840:flags=lanczos,format=rgba,split=2[ta][tb];[ta]trim=end=10.25,fade=t=out:st=9.60:d=0.65:alpha=1,setpts=PTS+15.75/TB[title];[tb]trim=start=10.25,setpts=PTS+15.75/TB[closing];[base][title]overlay=eof_action=pass:format=rgb[a];[a][closing]overlay=eof_action=pass:enable=gte(t\\,26):format=rgb[out]', '-map', '[out]', '-an', '-frames:v', '720', '-f', 'rawvideo', '-pix_fmt', 'rgba', 'pipe:1'], { stdio: ['ignore', 'pipe', 'inherit'] });
const encoder = spawn('ffmpeg', ['-hide_banner', '-loglevel', 'error', '-f', 'rawvideo', '-pixel_format', 'rgba', '-video_size', '2160x3840', '-framerate', '24', '-i', 'pipe:0', '-an', '-vf', 'scale=out_color_matrix=bt709:out_range=tv,format=yuv420p', '-c:v', 'libx264', '-crf', '15', '-preset', 'medium', '-pix_fmt', 'yuv420p', '-color_range', 'tv', '-colorspace', 'bt709', '-color_primaries', 'bt709', '-color_trc', 'bt709', '-bsf:v', 'h264_metadata=colour_primaries=1:transfer_characteristics=1:matrix_coefficients=1:video_full_range_flag=0', '-movflags', '+faststart+write_colr', output], { stdio: ['pipe', 'ignore', 'inherit'] });
const decoderDone = once(decoder, 'close'), encoderDone = once(encoder, 'close');
const frameSize = 2160 * 3840 * 4;
...
fs.writeFileSync(D + '/qa/composition.json', JSON.stringify({ sourceSha256: sha(source), outputSha256: sha(output), frames: index, titleOffset: 15.75, urlOffset: 26, urlWidth:640, urlMode: 'non-separable-luminosity', opacity: '1 on approved alpha containing original .72 and fade', audio: false, approvedSourceHashesVerified: true, samples, review: 'candidate; restore pilot accepted, full picture review required', closingCorrection: 'thanks exit softened 25.35-26; cloud-shaped blue reveal; logos and URL timing retained' }, null, 2));
