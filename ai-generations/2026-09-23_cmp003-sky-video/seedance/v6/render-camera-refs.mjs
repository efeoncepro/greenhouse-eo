...
const OUT = '/Users/jreye/Documents/greenhouse-eo/ai-generations/2026-09-23_cmp003-sky-video/seedance/v6/refs'
...
const depth=(html,transform,top=500)=>`<div style="position:absolute;left:0;top:${top}px;width:1080px;perspective:1100px"><div style="width:936px;margin:auto;transform:${transform};transform-style:preserve-3d;filter:drop-shadow(8px 14px 4px #062445aa)">${html}</div></div>`;
const boards={
 'camera-B-ui-side':depth(searchBox({text:Q}), 'rotateY(-48deg) rotateX(6deg) scale(1.3)',610),
 'camera-C-results-oblique':depth(searchBox({text:Q,cursor:false})+`<div style="margin-top:40px;display:grid;gap:26px">${RESULTS.map(r=>resultCard(r,1)).join('')}</div>`,'rotateY(32deg) rotateX(-12deg)',350),
 'camera-A-user-alone':at(userBubble(),480),
 'camera-C-agency-orbit':depth(PIECES['T10a-y-ahora-nos-eligio-como']+`<div style="margin-top:60px">${PIECES['T10b-su-agencia-seo-aeo']}</div>`,'rotateY(-22deg)',430)
};
