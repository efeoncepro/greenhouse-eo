...
for path in paths:
 segments,info=m.transcribe(path,language='es',word_timestamps=True,beam_size=5,condition_on_previous_text=False,vad_filter=True)
 data=[{'start':s.start,'end':s.end,'text':s.text,'words':[{'word':w.word,'start':w.start,'end':w.end} for w in s.words]} for s in segments]
 out=D/'qa'/('speech-vad-'+Path(path).stem+'.json');out.write_text(json.dumps(data,indent=2));print(out, data,flush=True)
