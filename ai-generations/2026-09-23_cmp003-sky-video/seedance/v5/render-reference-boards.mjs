...
const OUT = '/Users/jreye/Documents/greenhouse-eo/ai-generations/2026-09-23_cmp003-sky-video/seedance/v5/refs'
...
const at=(h,top,width=936)=>`<div style="position:absolute;top:${top}px;left:72px;width:${width}px">${h}</div>`;
const restPurple=chip(false).replace('background:#EEF1F6','background:#701C74').replace('color:#2F2B3D','color:#ffffff');
const boards={
 '01-search-empty':at(searchBox({text:''}),580),
 '02-search-question':at(searchBox({text:Q}),580),
 '03-results':at(searchBox({text:Q,cursor:false}),390)+at(`<div style="display:grid;gap:24px">${RESULTS.map(r=>resultCard(r)).join('')}</div>`,570),
 '04-user-turn':at(userBubble(),470),
 '05-answer-citation':at(userBubble(),400)+at(answer({chips:'one'}).replace(chip(false),restPurple),620),
 '06-chip-glow':`<div style="position:absolute;left:220px;top:920px;transform:scale(1.25)">${sparkles()}</div>`,
 '07-year':at(PIECES['T08-un-ano-creando-con-sky'],640),
 '08-2000':at(PIECES['T08-un-ano-creando-con-sky'],400)+at(PIECES['T09-mas-2000-piezas'],530),
 '09-agency':at(PIECES['T10a-y-ahora-nos-eligio-como'],370)+at(PIECES['T10b-su-agencia-seo-aeo'],530),
 '10-thanks':at(PIECES['T11-gracias-sky'],680)
};
const browser=await chromium.launch();const page=await browser.newPage({viewport:{width:1080,height:1920},deviceScaleFactor:1});
for(const [name,html] of Object.entries(boards)){
 await page.setContent(`<!doctype html><meta charset="utf-8"><style>${BASE_CSS}body{width:1080px;height:1920px;background:url(data:image/png;base64,${skyB64}) center/cover no-repeat;overflow:hidden} </style>${html}`);
 await page.evaluate(()=>document.fonts.ready);await page.screenshot({path:path.join(OUT,name+'.png')});
}
await browser.close();console.log(Object.keys(boards));
