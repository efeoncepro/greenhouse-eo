...
const outputPath = path.join(runRoot, 'masters/glitch-microphone-intro-keyref-hand-neutral-jpeg-probe.mp4')
const metadataPath = path.join(runRoot, 'masters/glitch-microphone-intro-keyref-hand-neutral-jpeg-probe.metadata.json')
...
const body = { contents: [{ role: 'user', parts: [{ inlineData: { mimeType: 'image/jpeg', data: (await fs.readFile(referencePath)).toString('base64') } }, { text: prompt }] }], generationConfig: { responseModalities: ['TEXT', 'VIDEO'] } }
...
return inline?.data && String(inline?.mimeType || inline?.mime_type || '').startsWith('video/') })
  ...
  if (!response.ok || !inline?.data) {
    ...
    throw new Error('Neutral one-image JPEG probe returned no video.')
  }
