...
const lw = Math.round(W * 0.2), lh = Math.round(lw * 196.68 / 837.07), left = Math.round((W - lw) / 2), top = Math.round(0.935 * H - lh / 2)
...
if (SEL) {
  ...
  const manifest = resolveCollaborationSelectionIntent({ targetId: 'obra', targetKind: 'object', variant: 'eight-handles', padding: 'standard', overlay: 'subtle',
  ...
  const r = renderCollaborationSelection({ manifest, targetBounds: s.box, canvas: { width: W, height: H }, measureLabel: (l, size) => shape(l, size).advance,
    presentation: { collaboratorScale: 1.8, localCursorScale: 1.2, participantColors: s.colors } })
