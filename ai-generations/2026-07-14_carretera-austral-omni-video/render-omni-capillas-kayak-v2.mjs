...
const firstFramePath = path.join(runRoot, 'keyframes/capillas-kayak-no-glasses-first-frame-16x9.png')
const promptPath = path.join(runRoot, 'prompts/omni-capillas-kayak-no-glasses.md')
const outputPath = path.join(runRoot, 'masters/carretera-austral-capillas-kayak-omni-v2-master.mp4')
const metadataPath = path.join(runRoot, 'masters/carretera-austral-capillas-kayak-omni-v2-master.metadata.json')
...
function parseMode() {
  ...
  throw new Error('Usage: node render-omni-capillas-kayak-v2.mjs [--plan|--execute]')
}
...
async function main() {
  ...
  const plan = {
    ...
    take: 'capillas-kayak-omni-v2-master',
    mode,
    ...
    model,
    endpoint,
    task: 'image_to_video continuity fix: no glasses / no eyewear',
    firstFramePath,
    outputPath,
    promptPath,
    constraints: [
