...
const portraitFramePath = path.join(runRoot, 'portrait-refs/capillas-kayak-native-reel-ref-x610.png')
const promptPath = path.join(runRoot, 'prompts/omni-capillas-kayak-reel-native-9x16-v2.md')
const outputPath = path.join(runRoot, 'masters/carretera-austral-capillas-kayak-reel-omni-native-9x16-v2-master.mp4')
const metadataPath = path.join(runRoot, 'masters/carretera-austral-capillas-kayak-reel-omni-native-9x16-v2-master.metadata.json')
...
function parseMode() {
  ...
  throw new Error('Usage: node render-omni-capillas-reel-native-9x16.mjs [--plan|--execute]')
}
...
async function main() {
  ...
  const plan = {
    ...
    take: 'capillas-kayak-reel-omni-native-9x16-v2-master',
    mode,
    ...
    model,
    endpoint,
    task: 'native portrait 9:16 image_to_video Reel',
    portraitFramePath,
    outputPath,
    promptPath,
    constraints: [
      'New native vertical Gemini Omni render, not a crop of the previous video.',
      'Grounded in guided kayak / watercraft visits to Capillas de Marmol from Puerto Rio Tranquilo.',
      'No glasses or sunglasses on either traveler at any moment.',
  ...
  const portraitFrame = await fs.readFile(portraitFramePath)
            ...
            parts: [
              {
                inlineData: {
                  mimeType: 'image/png',
                  data: portraitFrame.toString('base64')
                }
              },
              { text: prompt }
            ]
