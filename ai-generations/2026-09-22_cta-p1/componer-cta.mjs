...
return k.ink.right - k.ink.left }))
  ...
  if (s.cta) {
    const c=s.cta, cx=W*.07, cy=H*.34, padX=24, padY=18;
    const solid=c.variant==='solid', outline=c.variant==='outline';
    const ink=solid?C.inkOnLight:C.growthOnDark;
    const t=block({text:c.text,font:pop[700],size:42,tracking:0,leading:1.2,x:cx+padX,topY:cy+padY,maxWidth:W*.65,fill:ink});
    ...
    const descriptor=block({text:c.descriptor,font:pop[400],size:27,tracking:0,leading:1.2,x:cx+padX,topY:b.bottom+25,maxWidth:W*.7,fill:'#ffffff'});
    ...
    const cr=renderCollaborationSelection({manifest:cm,targetBounds:b,canvas:{width:W,height:H},measureLabel,presentation:{localCursorScale:1.3}});
    if(!cr.evidence.withinCanvas)throw Error('CTA selection outside canvas');
    body+=cr.underlay+labelToPaths(cr.overlay);
    fs.writeFileSync(`${PLAN_DIR}/out/${s.id}-cta-evidence.json`,JSON.stringify({intent:ci,manifest:cm,geometry:cr.evidence,textBounds:t.box,surface:b,variant:c.variant,solidTextContrast:solid?(lum(126,214,0)+.05)/(lum(0,40,77)+.05):null},null,2));
  }
