...
return k.ink.right - k.ink.left }))
  ...
  if (s.cta) {
    const c=s.cta, cx=W*c.x, cy=y+c.gapAfterNote, padX=c.paddingX, padY=c.paddingY;
    const solid=c.variant==='solid', outline=c.variant==='outline';
    const ink=solid?C.inkOnLight:C.growthOnDark;
    ...
    if(solid||outline)body+=`<rect x="${b.left}" y="${b.top}" width="${b.right-b.left}" height="${b.bottom-b.top}" rx="${c.radius}" fill="${solid?C.growthOnDark:'none'}" stroke="${C.growthOnDark}" stroke-width="2"/>`;
    body+=t.svg;
    checks.push({id:'cta',box:t.box,inkL:solid?lum(0,40,77):lum(126,214,0),...(solid?{skipContrast:true}:{})});
    const descriptor=block({text:c.descriptor,font:pop[400],size:c.descriptorSize,tracking:0,leading:1.2,x:cx,topY:b.bottom+c.descriptorGap,maxWidth:W*.7,fill:'#ffffff'});
    ...
    fs.writeFileSync(`${PLAN_DIR}/out/${s.id}-cta-evidence.json`,JSON.stringify({intent:ci,manifest:cm,geometry:cr.evidence,textBounds:t.box,descriptorBounds:descriptor.box,surface:b,variant:c.variant,solidTextContrast:solid?(lum(126,214,0)+.05)/(lum(0,40,77)+.05):null},null,2));
  }
