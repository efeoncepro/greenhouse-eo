...
const lw = Math.round(Math.min(W, H) * Number(process.env.LOGO ?? 0.20)), lh = Math.round(lw * 196.68 / 837.07), left = Math.round((W - lw) / 2), top = Math.round(Number(process.env.SIGNATURE_Y ?? 0.935) * H - lh / 2)
...
if(Math.max(white,navy)<4.5)throw Error('Signature contrast below4.5');
console.log(JSON.stringify({signatureBounds:{left,top,right:left+lw,bottom:top+lh},signatureContrast:Math.max(white,navy),variant:useWhite?'white':'navy'}));
