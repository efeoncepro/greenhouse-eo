# SEO+AEO · v06 · Finales por placement
...
Los titulares9:16 de v04 empezaban al6,5% y su firma estaba al93,5%: chocaban con la interfaz fullscreen. En v06 el texto empieza al 16,5%. Se editaron las cuatro fotos desde sus referencias4:5 para subir la escena y el lecho físico; la v05 dejó la firma al 63% y el operador la rechazó por quedar casi al centro. La v06 corrige únicamente la firma al 83,3%, siguiendo las piezas de Claude; ocupa aproximadamente 82–84,6% del alto. No hay scrim ni banda gráfica: el primer plano pertenece a la toma. La composición adapta escala y eje por escena, no traslada el bloque ciegamente. Los otros12 exports se conservan byte por byte.

El perfil conservador9:16 reserva16%arriba,35%abajo,8%izquierda y12%derecha. Protege texto, CTA, descriptor y cursor. La firma se evalúa por separado en su cierre editorial y queda fuera del guardrail inferior de Reels: puede solaparse con captions/controles; no se afirma seguridad universal. Los bordes de monitores/proyección pueden continuar fuera; la fuente seleccionada, la comparación de representaciones, Nexa, manos y gesto siguen reconocibles dentro. No hay captura live de Ads Manager: `out/qa-safe` es una máscara geométrica local y se identifica como tal.
...
- `componer-cta.mjs`: extensión del compositor canónico y rendererAXIS. `firma-placement.mjs`: adaptación mínima del compositor de firma oficial, con altura por plan y aborto si contraste<4,5. `firmar.mjs`: firma, previews y contactos. `validar.mjs`: contraste mínimo; `validar-safe-zones.mjs`: envolvente gráfica contra zona segura y firma contra su franja editorial, con riesgo de Reels explícito, máscaras de QA separadas.
...
```sh
node ai-generations/2026-09-22_aeo-final-safe-v06/componer-cta.mjs <piezas.json>
node ai-generations/2026-09-22_aeo-final-safe-v06/firmar.mjs <piezas.json>
node ai-generations/2026-09-22_aeo-final-safe-v06/validar.mjs <piezas.json>
node ai-generations/2026-09-22_aeo-final-safe-v06/validar-safe-zones.mjs <piezas.json>
```
