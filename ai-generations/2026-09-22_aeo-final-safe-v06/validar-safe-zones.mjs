...
for(const [index,p] of plan.entries()){
 ...
 const sa=p.signatureSafeArea??a;const signatureSafe={left:Math.ceil(sa.x0*W),top:Math.ceil(sa.y0*H),right:Math.floor(sa.x1*W),bottom:Math.floor(sa.y1*H)};const signatureInsideReelsGuard=contains(sign.signatureBounds,outputSafe);
 const pass=contains(b,safe)&&contains(sign.signatureBounds,signatureSafe)&&sign.signatureContrast>=4.5;
 results.push({id:p.id,profile:a.profile,graphicBoundsNative:b,safeNative:safe,signatureBoundsExport:sign.signatureBounds,safeExport:outputSafe,signatureContrast:sign.signatureContrast,signatureEditorialBounds:signatureSafe,signatureInsideReelsGuard,placementLimitation:p.placementLimitation??null,pass,preview:'local exclusion mask; not a live app screenshot'});
...
fs.writeFileSync(dir+'/out/safe-zones.json',JSON.stringify({method:'Bounds of all visible graphic pixels including cursor and descriptor plus official signature box evaluated against a separately declared editorial footer. Reels overlap is reported, never converted into a full platform PASS. Thresholds evaluated before rounding. Editorial profiles are explicit and are not official universal UI templates.',results},null,2)+'\n');
