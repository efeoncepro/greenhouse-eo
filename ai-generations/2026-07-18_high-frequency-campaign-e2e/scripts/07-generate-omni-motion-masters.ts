...
const main = async () => {
  const specs = JSON.parse(
    await readFile(path.join(runDir, 'prompts', 'omni-motion-masters.json'), 'utf8')
  ) as PromptSpec[]
  const results = []
  ...
  for (const spec of specs) {
    ...
    if (!result.ok || !result.output?.video?.url) {
      throw new Error(result.errorDetail ?? `Gemini Omni ${spec.id} failed with HTTP ${result.httpStatus}`)
    }

    const response = await fetch(result.output.video.url)
    if (!response.ok) throw new Error(`Gemini Omni ${spec.id} download failed with HTTP ${response.status}`)
    ...
    process.stdout.write(`Gemini Omni ${spec.id} clean master saved to ${spec.output}\n`)
  }

  await writeFile(
    path.join(runDir, 'manifests', '07-omni-motion-masters.json'),
    `${JSON.stringify({
      stage: 'motion-clean-masters',
