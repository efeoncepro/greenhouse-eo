// Campaign extension of scripts/foto/componer.mjs: Tres voces + acción.
// Reuses AXIS tokens, typography shaping and collaboration renderer.
// Usage: node componer-cta.mjs <piezas.json> [id...]
...
for (const s of SLIDES.filter(x => !only.length || only.includes(x.id))) {
 ...
 defs += hudEl.defs; body += hudEl.svg }
  ...
  const x = s.align === 'center' ? W * (s.centerX ?? .5) : M
  let y = (s.top ?? 0.05) * H
...
return k.ink.right - k.ink.left }))
  ...
  if (s.cta) {
    ...
    const surfaceColor=C[c.surfaceToken], ink=C[c.inkToken];
    const hexLum=h=>lum(...h.match(/[a-f\d]{2}/gi).map(x=>parseInt(x,16)));
    if(c.align==='center')cx=W*(s.centerX ?? .5)-(shape(c.text,pop[700],c.fontSize).advance+padX*2)/2;
    ...
    body+=t.svg;
    checks.push({id:'cta',box:t.box,inkL:hexLum(ink),...(solid?{skipContrast:true}:{})});
    const descriptor=block({text:c.descriptor,font:pop[400],size:c.descriptorSize,tracking:0,leading:1.2,x:c.align==='center'?W*(s.centerX ?? .5):cx,topY:b.bottom+c.descriptorGap,maxWidth:W*.7,fill:'#ffffff',align:c.align});
