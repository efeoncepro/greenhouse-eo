...
const plate = await sharp(path.join(DIR, 'plate-3d-v03.png')).resize(W, H, { kernel: 'lanczos3' }).toColourspace('srgb').png().toBuffer()
...
const out = path.join(DIR, 'post-17-v3.png')

await sharp(graded).composite([{ input: overlay }]).png({ compressionLevel: 9 }).toFile(out)
fs.writeFileSync(path.join(DIR, 'post-17-v3-overlay.svg'), svg)
...
fs.writeFileSync(path.join(DIR, 'qa-v3.json'), `${JSON.stringify(qa, null, 2)}\n`)
...
await sharp(out).resize(390, 488).png().toFile(path.join(DIR, 'post-17-v3-mobile-390.png'))
