...
const punchSize = (100 * W * 0.66) / (probe.ink.right - probe.ink.left)
const punch = shape('sí rediseñaríamos.', punchFont, punchSize, -0.03)
const lead = shape('Hay días que', popMedium, 52, -0.005)

// Historia 9:16: núcleo aprobado en y 285–1635; números terminan ≈1245 y el «7» ≈1315. Zona segura AXIS story: 13 % arriba/abajo.
const COPY_TOP = 1352
const GAP = 14
...
const SAFE_BLOCK = H * 0.13
const logo = { x: (W - LOGO_W) / 2, y: H - SAFE_BLOCK - 60 - LOGO_H }

if (logo.y + LOGO_H > H - SAFE_BLOCK) throw new Error('Logo fuera de la zona segura de historia')
if (copy.left < W * 0.1 || copy.right > W * 0.9) throw new Error('Copy fuera del margen lateral de historia')
...
const vignette = `<defs>
  <linearGradient id="floor" x1="0" y1="0" x2="0" y2="1">
    <stop offset="0.66" stop-color="#010e1d" stop-opacity="0"/>
    <stop offset="0.7" stop-color="#010e1d" stop-opacity="0.82"/>
    <stop offset="1" stop-color="#010e1d" stop-opacity="0.92"/>
  </linearGradient>
...
const plate = await sharp(path.join(DIR, 'plate-story.png')).resize(W, H, { kernel: 'lanczos3' }).toColourspace('srgb').png().toBuffer()
...
const out = path.join(DIR, 'post-17-story-9x16.png')

await sharp(graded).composite([{ input: overlay }]).png({ compressionLevel: 9 }).toFile(out)
fs.writeFileSync(path.join(DIR, 'post-17-story-9x16-overlay.svg'), svg)
...
fs.writeFileSync(path.join(DIR, 'qa-story.json'), `${JSON.stringify(qa, null, 2)}\n`)
...
await sharp(out).resize(390, 693).png().toFile(path.join(DIR, 'post-17-story-preview-390.png'))
