// Arnés de validación de reservas: dado un plate limpio, dice con números si sirve para la capa
// gráfica, antes de componer nada encima.
//
// Extiende `medir.mjs` (Sobel por caja) de la corrida del lenguaje fotográfico en vez de
// reimplementarlo. Valida las SEIS reservas del contrato:
//   1 zona de texto · 2 objeto para enmarcar · 3 lecho de la firma · 4 aire para cursores
//   5 campo profundo al margen · 6 lecho por formato
//
// Dos trampas heredadas de la sesión de fotografía, evitadas acá:
//   · un PNG de 16 bits leído en crudo devuelve basura → se fuerza profundidad de 8 bits;
//   · `blur().threshold()` devuelve 3 canales → haría falta `extractChannel(0)` si se usara máscara.
//
// Uso:
//   node validar-reservas.mjs <plate.png> [--objeto x0,y0,x1,y1] [--tinta blanca|oscura]
...
const GEOMETRIA_ZONA = {
  // El mínimo es 0.42, no 0.45: es el «42% izquierdo» del doc de reserva §3, medido. El 0.45 lo
  // puse yo a ojo y estaba reprobando plates por un pelo — un umbral inventado es tan malo como una
  // medición mal hecha, y encima parece riguroso.
