...
async function main() {
  ...
  const consumer = await upsertSisterPlatformConsumer({
    ...
    notes: 'TASK-1629 public OAuth client anchor. Its generated consumer credential is never used or printed.',
  ...
  if (existing) {
    existing = await updateSisterPlatformOAuthSessionRequirement({
      ...
      reason: 'TASK-1629 delegated agent funding policy enforced server-side'
    })
  }
