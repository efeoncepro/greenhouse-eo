...
export const scenario: CaptureScenario = {
  ...
  steps: [
    ...
    { kind: 'mark', label: 'pipeline-default', note: 'Kanban real con scroll interno y columnas canónicas.' },
    {
      kind: 'interaction',
      interaction: {
        name: 'pipeline-card-affordance',
        action: { kind: 'hover', selector: '[data-capture="hiring-application-card"]' },
        intent: 'La tarjeta confirma affordance clickeable sin esconder el menú de etapa.',
        frames: [
          { label: 'feedback', atMs: 140, clipSelector: '[data-capture="hiring-application-card"]' },
          { label: 'settled', atMs: 280, clipSelector: '[data-capture="hiring-application-card"]' },
        ],
        keyboardEquivalent: {
          action: { kind: 'focus', selector: '[data-capture="hiring-card-open"]' },
          expected: 'El foco del cuerpo de la tarjeta queda visible y no compite con el menú ⋮.',
        },
