...
/** Guarda la cuenta local de Wherex fuera del árbol versionado. */
...
if (!process.stdin.isTTY || !process.stdin.setRawMode) {
  console.error('[WHEREX_RADAR] El setup requiere una terminal interactiva para no exponer la clave.')
  process.exit(1)
}

try {
  await assertCredentialPathIsLocal()
  const email = await readVisible('[WHEREX_RADAR] Correo de Wherex: ')

  if (!email || !email.includes('@')) {
    throw new Error('debes indicar un correo válido')
  }

  const password = await readHidden('[WHEREX_RADAR] Clave de Wherex (no se mostrará): ')
  ...
  console.log('[WHEREX_RADAR] Credencial local guardada en .auth/ con permisos 0600.')
} catch (error) {
  console.error(`[WHEREX_RADAR] No se pudo guardar la credencial: ${safeMessage(error)}`)
  process.exitCode = 1
}
