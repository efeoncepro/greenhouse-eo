...
CREATE INDEX IF NOT EXISTS idx_period_closures_status
  ON greenhouse_cost_intelligence.period_closures (closure_status, period_year DESC, period_month DESC);
...
CREATE INDEX IF NOT EXISTS idx_period_closure_status_state
  ON greenhouse_serving.period_closure_status (closure_status, period_year DESC, period_month DESC);
...
CREATE INDEX IF NOT EXISTS idx_operational_pl_scope_period
  ON greenhouse_serving.operational_pl_snapshots (scope_type, scope_id, period_year DESC, period_month DESC);

CREATE INDEX IF NOT EXISTS idx_operational_pl_period
  ON greenhouse_serving.operational_pl_snapshots (period_year DESC, period_month DESC, period_closed);
...
VALUES (
  'setup-postgres-cost-intelligence-v1',
  'cost_intelligence',
  CURRENT_USER,
  'Cost Intelligence foundation: schema, period closure tables, operational P&L serving tables'
)
