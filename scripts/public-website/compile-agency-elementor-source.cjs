...
function compileFragment(html, module, prefix, repeats = []) {
  ...
  function field(type,label,value,extra={}) {
    ...
    fields.push({key,type,label,...extra}); defaults[key]=value; return token(key);
  }
  ...
  q('svg [id]').each((_,el)=>{const id=q(el).attr('id');template=template.replaceAll('id="'+id+'"','id="'+id+'-%%instance%%"').replaceAll('url(#'+id+')','url(#'+id+'-%%instance%%)').replaceAll('href="#'+id+'"','href="#'+id+'-%%instance%%"');});
...
function compileModule(key) {
  ...
  if(key==='trust') {const brand=root.find('span').filter((_,el)=>['Sky','Bresler','BEREL','S.Silva'].includes($(el).text().trim())); if(brand.length) repeatGroups.push({key:'brands',label:'Marcas',nodes:brand});}
  const repeats=[];
  for(const group of repeatGroups) {
    if(!group.nodes.length) continue;
    const rows=[], fields=new Map(), variants={};
    group.nodes.each((i,el)=> {
      const compiled=compileFragment($.html(el),key,group.key);
  ...
  const compiled=compileFragment($.html(root),key,'main');
...
if(key==='assets') {
  ...
  ast.walkRules(rule=>{let p=rule.parent;while(p){if(p.type==='atrule' && /keyframes/.test(p.name)) return;p=p.parent;}rule.selectors=rule.selectors.map(s=>{s=s.replace(/:root|\bbody\b/g,'.gh-agency-claude');return s.startsWith('.gh-agency-claude')?s:'.gh-agency-claude '+s;});});
