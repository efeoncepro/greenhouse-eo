...
function resolve(s,scope){return String(s||'').replace(/{{\s*([\s\S]*?)\s*}}/g,(_,expr)=>{const v=evaluate(expr,scope);return typeof v==='function'?'':String(v??'');});}
...
function render(node,scope){
 ...
 if(tag==='sc-for'){const list=evaluate(a.list.replace(/{{|}}/g,''),scope);return list.map((item,i)=>(node.children||[]).map(n=>render(n,{...scope,[a.as]:item,$index:i})).join('')).join('');}
...
function renderSource(selector,state={}){const prev=component.state;component.state={...prev,...state};const vals=component.renderVals();const html=src(selector).toArray().map(n=>render(n,vals)).join('');component.state=prev;return html;}
...
const rendered={};
for(const node of sections){const capture=node.attribs['data-capture'];rendered[capture.replace('hubspot-','')]=renderSource('[data-capture="'+capture+'"]');}
...
for(const [key,stateKey,count] of [['hubs','hub',14],['sectors','sector',4],['delivery','stg',5]]){
 ...
 for(let i=0;i<count;i++){
  ...
  const panel=p('.hsx-panel').first();panel.attr('data-hsx-panel',String(i));panel.attr('id','hsx-'+key+'-panel-'+i);panel.attr('tabindex','-1');
...
const formStates=[1,2,3].map(step=>{const old=component.state;component.state={...old,step,f:{...old.f,escenario:'Lo tenemos y no está rindiendo'}};const v=component.renderVals();component.state=old;return JSON.parse(JSON.stringify(v));});
...
for(const [key,html] of Object.entries(rendered)){
 ...
 q('a').addClass('-undash');
 q('img').each((_,el)=>{const url=q(el).attr('src');if(url.startsWith('public/'))q(el).attr('src','@hubspot/'+path.basename(url));else if(url.includes('simpleicons.org/hubspot'))q(el).attr('src','@hubspot/hubspot.svg');});
 ...
 if(key==='sectors')q('[data-hsx-panel] button').each((_,el)=>{const n=q(el).text().trim().match(/^0([1-6])/);if(n)q(el).attr('data-hsx-goto-hub',String([1,2,6,3,0,7][Number(n[1])-1]));});
 ...
 if(key==='conversion'){
  ...
  const formCard=q('[style]').filter((_,el)=>{const s=q(el).attr('style')||'';return s.includes('box-shadow')&&q(el).find('button').length>0;}).last();
 ...
 const c=compileFragment(q.html(),key,'main');
 ...
 if(key==='delivery'){
  ...
  grid.children('button').each((_,el)=>{const b=adjusted(el);b.removeAttr('style');b.children('span').eq(0).removeAttr('style').addClass('hsx-stage-dot');b.children('span').eq(1).removeAttr('style').addClass('hsx-stage-label');});
  c.template=adjusted.html();
 }
...
ast.walkRules(rule=>{let p=rule.parent;while(p){if(p.type==='atrule'&&/keyframes/.test(p.name))return;p=p.parent;}rule.selectors=rule.selectors.map(s=>{s=s.replace(/:root|\bbody\b/g,'.gh-hubspot-module');return s.startsWith('.gh-hubspot-module')?s:'.gh-hubspot-module '+s;});});
