<?php
/** Scoped TASK-1358 runtime release. Inputs: zip, sha256/previousSha256 manifest. */
...
if ( ( $manifest['contract'] ?? '' ) !== 'agency-elementor-release.v1' ) { WP_CLI::error( 'Invalid release contract.' ); }
$zip = new ZipArchive();
...
foreach ( $manifest['files'] as $entry ) {
	$path = $entry['path'];
	if ( strpos( $path, '..' ) !== false || ! preg_match( '#^(includes/(agency/(schemas|templates)/[a-z0-9_-]+\.(json|html)|widgets/class-eo-(?:agency-landing-(base|widgets)|logo-marquee-widget)\.php|class-eo-widgets-loader\.php)|assets/(css/(?:agency-(landing|elementor)|logo-marquee)\.css|js/agency-landing\.js|img/agency/[a-z-]+\.svg))$#', $path ) ) { WP_CLI::error( 'Path outside release scope.' ); }
...
$backup_path = sys_get_temp_dir() . '/eo-agency-before-' . gmdate( 'Ymd-His' ) . '.tar';
