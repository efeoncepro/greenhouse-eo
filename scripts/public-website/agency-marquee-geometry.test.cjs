...
setTimeout(()=>{
 try{
  ...
  const verify=()=>{const distance=parseFloat(track.style.getPropertyValue('--gh-marquee-distance'));assert.equal(distance,5*(itemWidth+14));assert.ok(track.children.length*(itemWidth+14)-14-distance>=viewportWidth);assert.equal(track.querySelectorAll('[data-marquee-clone][aria-hidden="true"]').length,track.children.length-5);assert.ok([...track.querySelectorAll('img')].every(img=>img.loading==='eager'));};
