...
function injectCmsLogos(tree,H,T){
 ...
 function walk(node){if(!node||typeof node==='string')return;node.children=node.children.flatMap(child=>{if(child?.tag==='p'&&child.children.join('')===anchor)return[cmsLogosTree(H,T),child];walk(child);return[child];});}walk(tree);return tree;
}
function apply(schema,html){
 ...
 const marker='<p style="margin:0;padding:12px 14px;';if(!html.includes('class="cm-cms-logos"')){if(!html.includes(marker))throw Error('CMS disclosure anchor drift');const group='<ul class="cm-cms-logos" aria-label="Ejemplos de CMS">'+brands.map(([id])=>`<li><span class="cm-cms-logo"><img src="%%cms_${id}_logo%%" alt="" width="96" height="56" loading="lazy" decoding="async"></span><span>%%cms_${id}_name%%</span></li>`).join('')+'</ul>';html=html.replace(marker,group+marker);}return html;
}
