...
const preview=process.argv.includes('--preview'),url='https://efeoncepro.com/servicios-contratar-hubspot/',root=path.resolve('../efeonce-public-site-runtime/wp-content/plugins/eo-elementor-widgets'),out='.captures/hubspot-product-marks-20260831';
...
try{if(preview){const d=new JSDOM(await(await fetch(url)).text()).window.document,local=new JSDOM(fs.readFileSync('tmp/hubspot-brand-extension/rendered.html','utf8')).window.document;
...
for(let i=0;i<14;i++){const tile=hubs.locator(`[data-hsx-select="${i}"]`),panel=hubs.locator(`[data-hsx-panel="${i}"]`);
...
if(i<8){const icon=panel.locator('img');
...
if(motion==='no-preference'&&[0,1,7,8].includes(i))await panel.screenshot({path:`${out}/${preview?'preview':'live'}-panel-${i}-${width}.png`,animations:'disabled'});states.push({width,motion,product:i,passed:true});}

if(motion==='no-preference')await hubs.locator('.hsx-band-grid').nth(2).screenshot({path:`${out}/${preview?'preview':'live'}-ai-${width}.png`});}
...
await nojs.goto(url);assert.equal(await nojs.locator('.hsx-panel-brand-icon:visible').count(),8);await nojs.close();}
