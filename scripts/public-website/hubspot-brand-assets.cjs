...
module.exports=function applyBrandAssets(key,c){
 if(key==='hubs'){
  ...
  products.forEach((name,i)=>{
   ...
   const b=q(`[data-hsx-select="${i}"]`);

b.find('.hsx-hub-brand-icon').remove();
  ...
  engines.forEach(e=>{const key='brand_'+e.key+'_logo';

if(!c.fields.some(f=>f.key===key))c.fields.push({key,type:'media',label:'Logo MCP · '+e.name});c.defaults[key]={url:'https://efeoncepro.com/wp-content/plugins/eo-elementor-widgets/assets/img/engines/engine-'+e.asset+'.png'};});

  for(const [index,icon] of Object.entries(semanticIcons)){const key='brand_'+icon.key+'_semantic_icon';

if(!c.fields.some(f=>f.key===key))c.fields.push({key,type:'media',label:'Icono semántico · '+icon.name});c.defaults[key]={url:'@hubspot/semantic-'+icon.icon+'.svg'};const b=q('[data-hsx-select="'+index+'"]');

b.find('.hsx-semantic-icon').remove();b.children().first().after(semanticImage(index));}

  const mcp=q('[data-hsx-select="12"]');

mcp.find('.hsx-mcp-brands').remove();mcp.children().first().after(engineGroup());
  c.template=q.html();
 }
 ...
 if(key==='proof-ledger'){
  ...
  if(!q('.hsx-case-heading').length){const intro=band.children().slice(0,3);

intro.wrapAll('<div class="hsx-case-heading"><div class="hsx-case-intro"></div></div>');q('.hsx-case-intro > p').last().css('margin','0');q('.hsx-case-heading').append('<img class="hsx-case-logo" src="%%f033_imagen_anam%%" alt="%%f034_alt_anam%%" width="280" height="76" loading="lazy" decoding="async">');}
...
if(!q('.hsx-licensing-logo').length)q('.hsx-band > .hsx-split > div').first().prepend('<img class="hsx-licensing-logo" src="%%brand_hubspot_logo%%" alt="%%brand_hubspot_alt%%" width="180" height="51" loading="lazy" decoding="async">');c.template=q.html();
 }
...
module.exports.panel=function(module,index,c){
 ...
 if(!panel.find('.hsx-panel-identity').length){
  const label=panel.find('.hsx-split > div').first().children('p').first().remove();

label.css('margin','0');
  const identity=q('<div class="hsx-panel-identity"></div>');
