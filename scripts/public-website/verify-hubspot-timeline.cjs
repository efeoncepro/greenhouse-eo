...
const out=path.resolve('.captures/hubspot-visual-fix-20260831');fs.mkdirSync(out,{recursive:true});
(async()=>{
 ...
 try{
  ...
  for(const motion of ['no-preference','reduce']){
   ...
   for(const width of [1414,768,390]){
    ...
    for(let i=0;i<5;i++){
     ...
     const bounds=await tabs.nth(i).boundingBox();await page.mouse.move(bounds.x+bounds.width/2,bounds.y+bounds.height+12);
  ...
  if(!preview){const nojs=await browser.newPage({javaScriptEnabled:false});await nojs.goto(url);assert.equal(await nojs.locator('[data-hsx-tabs="delivery"] [data-hsx-panel]:visible').count(),5);assert.equal(await nojs.locator('.hsx-partner-proof > div').count(),2);result.noJs=true;await nojs.close();}
  if(preview){const sourcePage=await browser.newPage({viewport:{width:1414,height:909},reducedMotion:'reduce'});await sourcePage.setContent(fs.readFileSync('tmp/hubspot-visual-fix/source-delivery.html','utf8'));await sourcePage.evaluate(()=>document.fonts.ready);await sourcePage.locator('section').screenshot({path:out+'/approved-timeline-1414.png',animations:'disabled'});await sourcePage.close();}
