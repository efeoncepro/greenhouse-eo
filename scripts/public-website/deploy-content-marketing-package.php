<?php
/** Scoped operator-approved Content Marketing runtime release. Inputs: zip, sha256/previousSha256 manifest. */
...
if ( ( $manifest['contract'] ?? '' ) !== 'content-marketing-elementor-release.v1' ) { WP_CLI::error( 'Invalid release contract.' ); }
$zip = new ZipArchive();
...
$backup_path = sys_get_temp_dir() . '/eo-content-marketing-before-' . gmdate( 'Ymd-His' ) . '.tar';
...
$registry = $files['includes/widgets/class-eo-content-marketing-widgets.php'] ?? null;
unset( $files['includes/widgets/class-eo-content-marketing-widgets.php'] );
if ( null !== $registry ) { $files['includes/widgets/class-eo-content-marketing-widgets.php'] = $registry; }
...
foreach ( $files as $path => $content ) {
	...
	$temp = $target . '.content-marketing-new';
