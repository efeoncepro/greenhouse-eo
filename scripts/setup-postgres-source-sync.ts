...
const main = async () => {
  ...
  const sqlPath = path.resolve(process.cwd(), 'scripts/setup-postgres-source-sync.sql')
  ...
  console.log(`Applied ${statements.length} PostgreSQL statements for Greenhouse source sync foundation.`)
}
