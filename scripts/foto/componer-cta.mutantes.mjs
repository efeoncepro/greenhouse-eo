...
export const CATALOGO = [
  ...
  { nombre: 'gate-t15-holgura', objetivo: 'gate', cambios: [["    if (pegados.length && bloquea(p, 'holgura',", "    if (false && bloquea(p, 'holgura',"]], pruebas: 'P10', espera: /rechaza voces pegadas ✗/ },
  { nombre: 'gate-t15-medio-trazo', objetivo: 'gate', cambios: [["    const medioTrazo = varianteDibujada === 'outline' ? Math.max(2, Math.ceil(L.canvas.width / (r.anchoPantalla ?? 390))) / 2 : 0", "    const medioTrazo = 0"]], pruebas: 'P10', espera: /el borde del contorno cuenta medio trazo ✗/ },
