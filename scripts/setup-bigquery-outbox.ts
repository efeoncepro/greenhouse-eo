...
const main = async () => {
  ...
  const sqlPath = path.resolve(process.cwd(), 'scripts/setup-bigquery-outbox.sql')
  ...
  console.log(`Applied ${statements.length} BigQuery statement(s) for Greenhouse outbox event stream.`)
}
