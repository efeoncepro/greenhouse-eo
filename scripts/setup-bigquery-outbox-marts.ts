...
const main = async () => {
  ...
  const sqlPath = path.resolve(process.cwd(), 'scripts/setup-bigquery-outbox-marts.sql')
  ...
  console.log(`Applied ${statements.length} BigQuery statement(s) for outbox analytical marts.`)
}
