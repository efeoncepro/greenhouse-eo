-- TASK-1349 — corrección compensatoria puntual (2026-09-03). Revierte el writeback de lifecycle aplicado por error
-- sobre Valentina Hoyos (member 'valentina-hoyos'): su salida employee (LWD 2026-04-30) está ejecutada, pero tiene
-- una relación contractor ACTIVA desde 2026-08-20 (reingreso). Restaura los valores exactos previos.
...
UPDATE greenhouse_core.members
 ...
 WHERE member_id = 'valentina-hoyos' AND active = FALSE;
...
SELECT 'offboarding-case-event-' || gen_random_uuid()::text, offboarding_case_id, 'offboarding_case.lifecycle_writeback_reverted', status, status, 'user-efeonce-admin-julio-reyes', 'admin',
       'TASK-1349 recovery 2026-09-03: writeback aplicado sobre un member con reingreso (relación contractor activa desde 2026-08-20). Revertido a valores previos exactos. Compensación por SQL porque updateMember falla por bug ajeno (identity_profile_source_links.link_id).',
