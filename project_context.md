...
- **Greenhouse microinteraction primitives V1 vigentes:** `GreenhouseAsyncActionButton` (`src/components/greenhouse/primitives/GreenhouseAsyncActionButton.tsx`) es la primitive para commands puntuales con estados `idle/loading/success/error`, aria-live, `aria-busy` en loading, reduced-motion y proteccion contra doble submit. `GreenhouseCommandFeedback` (`src/components/greenhouse/primitives/GreenhouseCommandFeedback.tsx`) cubre el resultado persistente post-accion con tonos `success/error/warning/info/retrying`, role status/alert, CTA opcional, timestamp y reference id. `GreenhouseStateTransition` (`src/components/greenhouse/primitives/GreenhouseStateTransition.tsx`) cubre cambios visibles de estado en rows/cards/panels con `fromLabel -> toLabel`, tonos `success/warning/error/info/neutral`, variants `surface/inline`, live region y reduced-motion. `GreenhouseInlineValidation` (`src/components/greenhouse/primitives/GreenhouseInlineValidation.tsx`) cubre validacion local/async con estados `idle/checking/valid/warning/error/blocked` y variants `field/section/summary/asyncCheck`. Usar en vez de botones locales con `submitting ? <CircularProgress /> : ...`, feedback local improvisado, cambios de estado invisibles o validaciones planas cuando el trabajo sea guardar/enviar/aprobar/generar/validar/reintentar/sincronizar. Lab interno: `/admin/design-system/microinteractions`. Nuevas variants deben iterarse en estas primitives + lab antes de crear componentes paralelos. No reemplazan procesos largos multi-step, timelines de auditoria completos, maker-checker ni confirmaciones destructivas/legales/financieras.
...
- **Floating UI / Greenhouse Floating Surface vigente (ADR aceptado, implementation pendiente):** Greenhouse adopta `@floating-ui/react` como engine canonico para superficies contextuales ancladas, pero expuesto via primitive Greenhouse futura, no imports ad-hoc en views de producto. Usos permitidos: rich tooltips, action menus, evidence peeks, inline editors, validation bubbles y command previews. No usarlo para sidecars full-height (`AdaptiveSidecar`), decisiones destructivas/legales/financieras (`Dialog`) ni workflows largos. ADR: `docs/architecture/GREENHOUSE_FLOATING_SURFACE_DECISION_V1.md`; task ejecutable: `docs/tasks/to-do/TASK-1033-greenhouse-floating-surface-primitive.md`.
