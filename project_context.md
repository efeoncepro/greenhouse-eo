...
- **Playwright CI smoke browser (2026-06-22):** los smokes GitHub usan el Chrome del runner (`PLAYWRIGHT_CHROMIUM_CHANNEL=chrome`), verifican `google-chrome --version` e instalan solo `pnpm exec playwright install ffmpeg` para conservar video on failure sin descargar Chrome for Testing. No volver al install completo de Playwright salvo que un spec necesite el browser empaquetado y se agregue cache/timeout explícito.
