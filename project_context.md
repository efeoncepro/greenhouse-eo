...
- **Tender Deck Composer — destinos editables aceptados, no implementados (2026-07-12):** PDF sigue
  ...
  y Adobe Express (después, Add-on nativo interactivo). El manifest de Greenhouse sigue siendo la
  fuente de verdad: ni PDF importado, HTML convertido ni documento externo editado reemplazan slots.
  Express REST beta es sólo POC de plantillas taggeadas, no renderer de Gantt variable ni producción.
  Canon: ADR en `GREENHOUSE_TENDER_DECK_COMPOSER_V1.md`; `TASK-1393` deja la costura, no entrega esos
  targets.
- **Tender Proposal Studio — foundation agentic y renderer planificados bajo EPIC-027 (2026-07-12):** el Deck Composer F1 determinista ya está shipped como CLI (`DeckPlan → selector → slot-fill → Chromium → PDF de N páginas`); la state machine existe sólo en TypeScript. No existe aún aggregate DB, API/UI, capabilities ni assets runtime. `TASK-1394` está **complete**: catálogo 25/25 y primitives genéricas `remove`, `resolver-only`, `itemSelector`/`fixedChildren` con geometría ChartSplit fail-closed; el contrato está espejado en `deck-studio` y `greenhouse-public-private-tenders` para Codex/Claude. Sigue `TASK-1393 → TASK-1392 → TASK-1391`: 1393 transforma `CompositionPlanInput` semántico (un autor/agente nunca elige template) en `ResolvedCompositionManifest` inmutable con catálogo, contratos/hashes, validadores semánticos y brand/font pack local; 1392 crea `Proposal`, assets/evidencia allowlisted y el contrato `contexto permitido → propuesta tipada/trazable → confirmación humana → command canónico`; 1391 sólo renderiza el manifest preflighted en `tender-worker` si `EPIC-027` autoriza la frontera. El LLM no muta estado, assets, evidencia ni gates. No crear una tabla paralela ni deployable anticipado. Carruseles/posts/stories no ensanchan Tender: la capa reutilizable `format_spec → composition_spec → artifact_manifest` pertenece a Creative Studio (EPIC-028); cualquier handoff Tender futuro será minimizado/versionado y no moverá RFPs, material interno ni storage credentials. Canon: `docs/architecture/GREENHOUSE_ARTIFACT_COMPOSER_PLATFORM_DECISION_V1.md`, `GREENHOUSE_TENDER_PROPOSAL_STUDIO_ARCHITECTURE_V1.md`, invariantes Commercial/Tenders y tasks `1394 → 1393 → 1392 → 1391`.
- **Efeonce Creative Studio — plataforma sister agentic aprobada como dirección (2026-07-11):** la capability de imagen/video/audio/3D no se implementa en Greenhouse. Nace como runtime hermano, primero interno y luego client-ready, con la **misma** capa command/reader para UI, MCP y agentes; `propose → reserve → approve → execute`, assets/lineage y credit ledger append-only. Cloud control plane + worker async se separan de Greenhouse; cliente, pagos/credit sales, uploads externos y publicación autónoma siguen gateados. Media Foundry, Creative Flow Studio y Creative Video Studio de Greenhouse quedan superseded como ubicación de runtime; TASK-996 es histórico/no ejecutable. Canon: `docs/architecture/EFEONCE_CREATIVE_STUDIO_AGENTIC_PLATFORM_{DECISION,ARCHITECTURE}_V1.md`; programa: `docs/epics/to-do/EPIC-028-efeonce-creative-studio-agentic-platform.md`.
...
- **TASK-1371 en progreso con runtime smoke verde (2026-07-09):** `docs/tasks/in-progress/TASK-1371-hiring-vacancy-publication-operator-command.md` implementa el operador backend-data `publishHiringVacancyFromBrief` (`dryRun|execute|publish`) sobre writers de Hiring, con campos publicos estructurados (`public_work_mode`, region/ubicacion, `public_area`, `public_skill_tags`, compensation band opcional), publish guards, idempotencia/audit via API Platform command ledger, CLI `pnpm hiring:publish-vacancy` y endpoint interno `POST /api/hiring/vacancy-publications`. Migraciones `20260709182000000` y `20260709183000000` aplicadas en Cloud SQL dev; `migrate:status` sin pendientes. Smoke `publish` reutilizo `EO-OPN-0009` por `publication_source_ref` (`outcome=duplicate`) sin crear demanda/opening nuevos; URL publica 200. Compatibilidad Careers: si legacy trae `public_location_mode=LATAM` sin `public_work_mode`, el view-model debe mostrar `Ubicacion=LATAM` y `Modalidad=Remoto`; chips visibles salen de `public_skill_tags` y solo degradan a reglas canonicas cortas, no fragments de requisitos. Revision UI/UX writing pre-release local verde: GVC `.captures/2026-07-09T11-11-01_task354-careers-runtime-audit` + Playwright copy probe (`ok=true`) cubren home/detail/apply desktop1440/mobile390; Playwright product audit `.captures/2026-07-09T10-49-careers-product-ui-audit` (`failed=[]`) cubre home/detail/apply/404 desktop1440/wide2048/mobile390. El `N` negro local es `nextjs-portal`, no Nexa; copy Careers fue refinado para sacar expresiones excluyentes/performance (`locos`, `Hollywood-level`) y mejorar errores/empty/success con recuperacion clara. Criterio de voz: en roles de marketing, mantener spanglish profesional cuando sea vocabulario real del oficio (`growth`, `performance`, `vendor management`, `brief`, etc.); no limpiar terminos de oficio por purismo. Produccion seguia en release `915be02a...` sin estos cambios, por lo que el bug live persiste hasta release/hotfix de codigo. Regla operativa: cero release, cero SQL manual y cero UI-only flow por vacante nueva.
...
- **Roadmap work item index reader (TASK-1152):** el backlog operativo Markdown (`docs/epics/**`, `docs/tasks/**`, `docs/mini-tasks/**`, `docs/issues/**`) tiene un reader read-only canónico en `src/lib/roadmap/work-item-index/` que lo proyecta como contrato versionado `roadmap-work-item-index.v1` vía `GET /api/roadmap/work-items` (gate `requireInternalTenantContext` + capability `roadmap.work_items.read`, módulo dedicado `roadmap`). El Markdown sigue siendo SSOT — el reader NUNCA mueve archivos, cambia lifecycle ni reescribe Markdown. El parser **espeja** la semántica de `task-lint`/`ops-artifact-lint` (no la duplica el binario; tests de paridad). Consumidores nuevos (UI cockpit TASK-1153, agentes) leen este contrato server-side, no parsean Markdown client-side. `docs/**` se bundlea en runtime Vercel vía `outputFileTracingIncludes` en `next.config.ts`.
- **Roadmap cockpit UI (TASK-1153):** la superficie `/roadmap` (item de menú top-level **Roadmap**, grupo Plataforma, fuera de Admin, solo interno vía viewCode `plataforma.roadmap`) es el cockpit humano del backlog. La página server consume `buildRoadmapCockpitData` (`src/lib/roadmap/cockpit/`, projection server-side sobre el reader de TASK-1152 — NO modifica 1152 salvo el export aditivo `getAllWorkItems` + el campo `rootCause`). Read-only: muestra KPIs + filtros + board de 7 lanes derivadas determinísticamente (`assignLane`) + inspector; **no edita Markdown**. La UI vive en `src/views/greenhouse/roadmap/` (CompositionShell split desktop / board + Drawer mobile). Toda superficie nueva que necesite "ver el backlog" consume este cockpit o el contrato `roadmap-work-item-index.v1`, no parsea Markdown client-side.
