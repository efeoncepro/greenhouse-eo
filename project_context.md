...
- **TASK-1371 en progreso/code-complete local pendiente de cierre (2026-07-09):** `docs/tasks/in-progress/TASK-1371-hiring-vacancy-publication-operator-command.md` implementa el operador backend-data `publishHiringVacancyFromBrief` (`dryRun|execute|publish`) sobre writers de Hiring, con campos publicos estructurados (`public_work_mode`, region/ubicacion, `public_area`, `public_skill_tags`, compensation band opcional), publish guards, idempotencia/audit via API Platform command ledger, CLI `pnpm hiring:publish-vacancy` y endpoint interno `POST /api/hiring/vacancy-publications`. Dry-run local Account Manager verde; execute/publish DB queda pendiente hasta levantar Cloud SQL Proxy/aplicar migracion en dev/staging. Regla operativa: cero release, cero SQL manual y cero UI-only flow por vacante nueva.
