...
- **HubSpot as a Service / ANAM tiene canon visible en Greenhouse (2026-07-16):** este repositorio es el SSOT
  ...
  clientes son candidatos de migración, ambos `NO-GO` hasta identidad/mapping/dry-run/aprobación; los demás son
...
- **Kortex / ANAM chat landing + HubSpot as a Service (actualizado 2026-07-16):** la landing `https://anam-2.hubspotpagebuilder.com/agente-anam` esta live desde HubSpot Developer Project `kortex-cms-react` (`projectId=103589049`, `deployedBuildId=22`) y documentada en `docs/architecture/kortex/hubspot-cms/anam-chat-landing.md`. UX vigente: canal oficial de atencion, CTA `Iniciar chat` e intents `Cotizar / Seguimiento del Servicio / Requerimientos de Calidad`; `anam_intent` + refresh del widget es el seam soportado y el prefill del composer no es confiable. La operación integral de RevOps, Customer Agent, QA y managed delivery se enruta por las skills espejo `.codex/.claude/skills/hubspot-as-a-service` y `docs/architecture/kortex/hubspot-as-a-service/README.md`; CMS/landing conserva ownership en Kortex. Modelo RevOps: Contact -> native Lead -> Company -> Deal -> Quote/line items -> Service, con Ticket como caso/SLA y Billing Event propuesto para el ledger. Inventario: 291 Leads, 506 line items, 10 Quotes sin adopción operativa, 1 Service, 18 Tickets y 0 Invoices; reconciliación canónica en `anam-revops-schema-reconciliation-2026-07-16.md` y catálogo dry-run en `anam-commercial-catalog-dry-run-2026-07-16.md`. Para la carga mensual de facturación se prefiere una superficie client-space ANAM autenticada, que reutiliza `greenhouse_core.assets`/scan y opera sobre un modelo reusable `client_billing_*` con `space_id` de sesión, aprobación humana y target dataset-bound al portal ANAM `19893546`; SharePoint queda como adaptador opcional. ANAM es cliente y dueño de source/CRM data; Greenhouse sólo opera el control plane y jamás proyecta filas a su CRM, Finance, Income o Account 360. Spec `client-billing-intake-data-model-spec-v1.md`; foundation no-write `TASK-1423`. Secuencia obligatoria ANAM: comercial primero (Company/Contact, Lead, Deal/line items, Service/renovación y paneles Data Quality/Growth/Retention/Loyalty), Operaciones después (Tickets, SLA, billing sync y paneles operativos). Estado vigente: `Down-sell` oculto porque es movimiento de Retención, no tipo de ingreso; `Variación vs. cotizado` reconciliada sin writes de records. Product read quedó consentido y verificado: instalación Kortex activa con 110 scopes, Product properties/search HTTP 200, 65 propiedades y 22 Products; el control plane aún debe corregir su generador de URL, que omitió el scope requerido hasta edición manual autorizada. Operacion: usar `hs project validate/upload --profile anam`, mantener default CLI en Kortex/Efeonce; publish por API requiere scopes `content` + `content.landing_pages.write`.
