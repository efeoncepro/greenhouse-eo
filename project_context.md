...
- **Growth Public Forms Engine (2026-06-24 arquitectura aceptada; runtime VIVO desde 2026-06-25):** Greenhouse debe controlar formularios públicos/lead magnets desde el dominio `growth`, con form definitions/versiones/render contracts/submissions/consent/destination attempts como SoT Greenhouse y HubSpot como destination adapter. V1 nace con `hsforms-v3-secure-submit` porque al 2026-06-24 no se validó un endpoint date-versioned estable para submissions; queda encapsulado como `endpointStatus=legacy_supported` + `migrationTarget=date_versioned_forms_submission_api_when_available`. Astro/WordPress/public site consumen Greenhouse (`/api/public/growth/forms/**` futuro), nunca HubSpot directo. Cada versión publicada declara `form_kind`, `risk_profile`, `data_classes`, `destination_policy`, `persistence_policy`, `analytics_policy`, `upload_policy` cuando aplique y boundary de acción de negocio; `quote_request`, `pricing_simulation`, `document_upload`, `diagnostic_intake` y `preference` no pueden colarse como writes comerciales/financieros implícitos. Backend = `policy compiler + submission orchestrator` que emite `render_contract`, `submission_contract` y `destination_plan`; la UI se condiciona por policy/propósito, no por HubSpot/vendor/destino. WordPress/Astro/Greenhouse Next.js son `host_surface`/`render_surface` consumidores; HubSpot es `destination`. Primer consumer WordPress con core portable Web Component/custom element y wrapper fino; Astro debe poder renderizar la misma definición/versión sin cambiar mapping/destinos. El renderer NO nace como iframe: debe vivir en el DOM del host para que GTM/dataLayer mida en la pagina padre; iframe solo fallback `measurement_degraded` con bridge `postMessage` allowlisted y sin PII/raw values. Full API parity es requisito de nacimiento: public render/submit APIs, Product APIs, commands/readers, Nexa/MCP/CLI/ops path plan antes de declarar completa cualquier UI/wrapper. Docs: `docs/architecture/GREENHOUSE_GROWTH_PUBLIC_FORMS_ENGINE_DECISION_V1.md` + `docs/architecture/GREENHOUSE_GROWTH_PUBLIC_FORMS_ENGINE_ARCHITECTURE_V1.md`. **Runtime ya existe:** backend/API parity (TASK-1229) + adapter HubSpot secure-submit (TASK-1230) + **renderer portable + 3 host surfaces (TASK-1231)** + **Turnstile `captchaToken` parity del renderer (TASK-1294)** + **identidad estable `formKey` (TASK-1297)**. El renderer es el Web Component `<greenhouse-form>` en `src/growth-forms-renderer/**` (vanilla TS, light DOM + ElementInternals; build `pnpm renderer:build` → `public/growth-forms/renderer-<channel>.js`); host surfaces = preview Greenhouse `/design-system/growth-forms-renderer`, widget Elementor WordPress (`efeonce-public-site-runtime`) y wrapper Astro (`efeonce-web`). Wrappers = adapters finos: NUNCA alteran campos/validación/destinos. Público gated por `GROWTH_FORMS_PUBLIC_API_ENABLED` (ON staging; producción activa para el primer bridge AEO específico en `/aeo-2/`). Desde TASK-1294 el renderer soporta `RenderContract.security.captcha` browser-safe para Turnstile invisible y envía `captchaToken`; desde TASK-1297 puede resolver por `form-key`. Runtime bridge AEO documentado en `docs/architecture/growth-public-forms-runtime-contract.md`; su versión publicada vigente es v5 (`fver-70c365c1-ea3b-4e84-b4b3-4fd852f951f4`) con `formKey=b120566a-dd1a-43c8-956a-4e0121e805b8`, `copy.submit`, placeholders `Selecciona país` / `Selecciona tamaño`, `corporate_email`, `emailPolicy.block_field` y `ui_policy_json.security.captcha`; el bridge WordPress replica temporalmente validación reactiva inline (`aria-invalid`, `/verify-email` debounced, success remoto) antes de Turnstile hasta que el renderer supere el bloqueo visual de Ohio.
- **Public Site AEO conversion bridge visual contract (2026-06-30):** en `/aeo-2/`, la separación visual del bloque de conversión vive en `.gh-aeo-conversion` como banda clara propia; `.gh-aeo-form-card` es solo host Elementor transparente y `.gh-aeo-growth-form-card` es la única card visible. No exponer kickers o metadata interna tipo `Growth Forms · Diagnóstico AEO` en la landing pública. El intento TASK-1298 de migrar a `<greenhouse-form>` fue revertido porque Ohio rompió los controles del renderer. Estado actual: pre-live parity verde (WP bridge/heroans guard + API slug/formKey/captcha fail-closed + fixture Ohio hostil + preview de composición real inyectado en memoria + focus/error/reduced-motion interaction preview + PNG frame review con pixel sampling), AEO v5 `fver-70c365c1-ea3b-4e84-b4b3-4fd852f951f4` publicada con placeholders `Selecciona país` / `Selecciona tamaño`, pero WordPress live sigue en bridge hasta cutover gobernado. El renderer pre-live vigente ya usa pares desktop `Nombre`/`Email` y `País`/`Tamaño`, campos largos full-width, shadow/focus/CTA hover tokenizados, first-click submit protegido contra blur layout shift y mobile 390 one-column. El bridge restaurado es piso visual, no techo: el renderer puede modernizar UI/UX, microcopy y microinteracciones, pero el save live exige backup Elementor, `heroans` guard, Kinsta purge, GVC/frame review y gates finales. Gate durable agregado: `pnpm public-website:verify-aeo-prelive-contract`; su frame review muestrea píxeles de inputs/selects/CTA desde bounding boxes reales para detectar campos grises, select chevron-wall y CTA oscuro. `pnpm codex:task-hook TASK-1298 --develop` debe seguir fallando mientras `live_cutover_pending_after_pre_live_parity` esté activo; para desbloquear hay que documentar `Cutover Unlock Evidence` en la task y usar embed por `form-key`, no por slug/`form=`.
