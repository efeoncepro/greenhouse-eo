...
- **Codex QA Stop hook vigente (2026-06-13):** `.codex/hooks.json` registra el hook local `.codex/hooks/qa-release-stop-hook.mjs` para `Stop`. Si Codex intenta cerrar un turno con lenguaje de cierre (`listo`, `terminado`, `implementado`, etc.) y hay cambios riesgosos en docs/source/scripts/skills/hooks sin evidencia QA en la respuesta, el hook pide continuar con `$greenhouse-qa-release-auditor`, `pnpm qa:gates --changed` o pathspec equivalente y verdict `PASS | CONDITIONAL PASS | BLOCK`. Es guardrail de cierre, no reemplaza la skill ni ejecuta QA pesado. Verificación: `pnpm codex:qa-hook:test`.
