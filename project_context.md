...
- **Hiring / ATS — Public Careers UI (TASK-354, code complete + production route live 2026-07-09):** `/public/careers/**` es la superficie pública Efeonce de vacantes, implementada con alta fidelidad al HTML local `~/Documents/carreers/Efeonce Carrers/Efeonce Careers.dc.html`. Listing/detalle consumen solo `PublicOpeningPayload` con ISR; apply es un Growth Form browser (`formKind='application'`, `efeonce-careers-application`, eventos `gh_form_*`) que postea al service de TASK-1367. El CV PDF opcional se adjunta por el asset pipeline privado (`hiring_application_cv_draft` -> `hiring_application_cv`) sin exponer URL pública. Flags `HIRING_PUBLIC_APPLICATIONS_ENABLED` y `NEXT_PUBLIC_TURNSTILE_SITE_KEY` están ON en Vercel staging/Production y producción sirve el bundle `433cfa2b0fd3` vía release control plane (`production-release.yml` run `28991488376` success). El 2026-07-09 se creó y publicó la vacante real `EO-OPN-0009` (`Account Manager / Especialista en Marketing`, demand `EO-TDM-0012`) usando los writers canónicos de Hiring, alineada al assessment `Account Manager L2`; el reader público y `/public/careers/EO-OPN-0009` responden 200. TASK-354 sigue en progreso hasta smoke de submit con browser/Turnstile: verificar `hiring_application` creada + dedupe genérico. GVC reproducible: `scripts/frontend/scenarios/task354-careers-runtime-audit.scenario.ts`.
