...
- **TASK-1373 Careers Apply Native Growth Form staging live (2026-07-13):** `/public/careers/[publicId]/apply` ya consume el Growth Form publicado `efeonce-careers-application` (`form_key=9f7a8fc0-6fa7-4670-8e2d-efe0ce354001`, version 2, `styleVariant=careers-html-fidelity`, surface `public-careers-nextjs`) vía `<greenhouse-form>`, con el custom `CareersApplyClient` preservado como rollback por `CAREERS_NATIVE_GROWTH_FORM_ENABLED`. Rollout sólo a `develop`/staging: Vercel `greenhouse-ldqkedyia` (`dpl_3hdhDYu6VxvadTHbXXH595gstjKj`) Ready en commit `838950916b270c47d1dc7a5f1bc36ce02b9d704f`, alias `dev-greenhouse.efeoncepro.com`; production OFF/ausente. Evidencia: API contract version 2 + `cvFile` + Turnstile required, submit sin captcha fail-closed `captcha_failed/missing_token`, GVC staging desktop/mobile PASS `.captures/2026-07-13T22-07-38_task354-careers-runtime-audit`, smoke sintético Growth Forms -> ATS + CV privado PASS, destinations `0`. El helper GVC ahora limita `x-vercel-protection-bypass` al origin capturado para no contaminar Sentry.
