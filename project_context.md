...
- **Greenhouse Loading Surface vigente (TASK-1037, Slice 1):** `GreenhouseLoadingSurface` (`src/components/greenhouse/primitives/GreenhouseLoadingSurface.tsx`) es la primitive inicial para elevar loading states mas alla de spinners/skeletons locales. Variants V1: `pageSkeleton`, `panelSkeleton`, `tableSkeleton`, `inlineAction`, `brandSplash`, `aiThinking`, `progressRail`. El Loading Lab interno vive como child route en `/admin/design-system/loaders` y se valida con GVC desktop+mobile separado del canon de color AXIS. Reutilizar stack no significa limitar capacidad visual: Framer/CSS/MUI/AXIS son el chasis; si una route/panel/IA necesita loader moderno, extender la primitive antes de crear otro loader local.
