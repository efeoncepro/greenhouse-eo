...
- **Nexa Brand Mark primitive vigente (2026-06-09):** `GreenhouseNexaBrandMark` (`src/components/greenhouse/primitives/`) es el contrato canónico para la identidad visual de Nexa con kinds `askNexaBadge|badgeIcon|inlineMark|monoMark`. Source visual: `public/images/nexa-mark/*`; regla de marca: arco + sparkle siempre juntos, nunca `tabler-sparkles` solo como Nexa. `kind='askNexaBadge'` renderiza el badge Midnight Navy con label `Pregúntale a Nexa` y se usa encima del input en `NexaGreetingsCard variant='compactContextual'`. `GreenhouseNexaAnimatedMark` suma runtime Rive (`@rive-app/react-canvas`) para reproducir un `.riv` cuando exista y caer al mark estático si falta o falla; animar de verdad requiere export desde Rive Editor, no el SVG crudo en runtime. Hoja viva: `/admin/design-system/nexa-brand`; scenario GVC: `design-system-nexa-brand`.
