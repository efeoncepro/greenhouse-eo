...
- **Growth Forms en WordPress Ohio — host layer compartida (2026-07-01, code-ready; rollout Kinsta pendiente):** para no pelear cada landing contra estilos globales Ohio, el runtime repo `efeonce-public-site-runtime` tiene una capa child-theme compartida `wp-content/themes/ohio-child/assets/css/growth-forms-host.css`, enqueued como `ohio-child-growth-forms-host` desde `inc/enqueue-and-layout.php`. Scope: `.eo-growth-form`, `.gh-growth-form-host`, `.gh-aeo-growth-form-host`, `.gh-aeo-growth-form-card` + `<greenhouse-form>`. La capa solo da contencion, tokens genericos y hardening de controles `.ghf-*`; NUNCA contiene fields, validacion, mapping HubSpot, Turnstile, destinos ni copy contractual. Antes de desplegar, refrescar drift con `pnpm public-website:export-live-code` + `pnpm public-website:diff-runtime -- --write` y leer `pnpm public-website:runtime-status`; si `releaseSafety.fullRepoDeploySafe=false` o `eo-elementor-widgets` aparece como `repo_pending_release`, no arrastrarlo en el rollout del child theme.
