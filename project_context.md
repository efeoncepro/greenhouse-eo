...
- **EPIC-026 / TASK-1376 — desacople modular propuesto + new-work contract activo (2026-07-10):** Greenhouse abrió un programa para reducir costo de build, presión local y acoplamiento de deployment. El target propuesto es modular monorepo + unidades desplegables incrementales, conservando modular monolith de dominio/datos, Vercel para el portal y PostgreSQL compartido en la primera fase. El ADR sigue `Proposed`: `TASK-1376` debe medir baseline p50/p95/costo/RSS, generar dependency graph y emitir `go|conditional-go|no-go` antes de mover rutas o crear proyectos. Mientras tanto el producto no se pausa: lo nuevo se construye en la topología actual pero extraction-ready bajo `docs/operations/MODULAR_MIGRATION_NEW_WORK_OPERATING_MODEL_V1.md` (primitive de dominio primero, browser/server split, adapters delgados; no anticipar `apps/*`/`packages/*`). Canon: `docs/architecture/GREENHOUSE_MODULAR_BUILD_RUNTIME_DECISION_V1.md` + `_ARCHITECTURE_V1.md`; programa: `docs/epics/to-do/EPIC-026-greenhouse-modular-build-runtime-decoupling.md`.
