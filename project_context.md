...
- **Extensibilidad**: el middleware es de propósito general (capa edge-global, singleton del framework). Hoy solo lleva el maintenance gate, pero es el lugar canónico para lógica request-level O(1) futura (geo/locale routing, rate limit, A/B por cookie, security headers globales, redirects masivos, bloqueo bot/IP). Toda responsabilidad nueva se **agrega a este mismo archivo** como función encadenada con su config SSOT, default-OFF + fail-open. NUNCA logica de dominio, auth fina ni IO/DB por request (eso vive en los guards por ruta). Doc canónica: `docs/documentation/plataforma/middleware-edge.md`.
