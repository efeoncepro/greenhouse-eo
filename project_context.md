...
- **Creative Workflow / Glitch — Seedance conserva el diseño, pero no sustituye la actuación (2026-07-11):** el key visual 4K canónico ya contiene el `ON AIR` dentro del estudio (hash confirmado contra el archivo entregado por el operador). La recuperación anterior que lo neutralizó en un adapter y lo recompuso en F/I queda **rechazada**: aun con composición correcta se veía pegado, el retime se leía como presión y el foley Gemini no correspondía al golpe de una yema sobre rejilla. Fal/Seedance 2.0 take S usó el PNG íntegro y sí preservó set, paleta, micrófono y practical; se rechazó completo por contacto sostenido/ambiguo y audio no aislado en dos golpes. El take T, con criterio de máximo dos frames, fue bloqueado antes de crear request por saldo Fal agotado (`HTTP 403 User is locked`), por lo que no hay candidato creativo vigente ni gasto pendiente. Al reanudar: recargar Fal, ejecutar T y revisar `hover → impacto 1–2 frames → rebote → aire → impacto → rebote`, con foley `piel → malla → cuerpo/cápsula`; se prohíben overlay/crop/máscara/tracking y retime que invente actuación. Canon: `ai-generations/2026-07-11_glitch-microphone-intro/review/take-s-seedance-source-keyvisual-review.md` y `recovery-plan-v2-integral-practical-and-foley.md`.
