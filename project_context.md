...
- **Creative Workflow / Glitch — recuperación V2: practical integrado y actuación real (2026-07-11):** el key visual 4K canónico ya contiene el `ON AIR` dentro del estudio (hash confirmado contra el archivo entregado por el operador). La recuperación anterior que lo neutralizó en un adapter y lo recompuso en F/I queda **rechazada**: aun con composición correcta se veía pegado, el retime se leía como presión y el foley Gemini no correspondía al golpe de una yema sobre rejilla. No hay candidato creativo vigente. La próxima toma parte del key visual intacto, conserva el practical desde el frame 1, genera/filma `hover → impacto 1–2 frames → rebote → hover → impacto → rebote`, y resuelve foley aislado `piel → malla → cuerpo/cápsula`; se prohíben overlay/crop/máscara/tracking de practicals y retime que invente actuación. Los espejos Codex/Claude de Motion Studio y Audio Studio, manual y ADR Proposed se ajustaron: un finish determinista sólo aplica a cambios que no alteran la verdad física/diegética. Sin runtime, publicación, archive ni gasto nuevo. Canon: `ai-generations/2026-07-11_glitch-microphone-intro/recovery-plan-v2-integral-practical-and-foley.md`.
