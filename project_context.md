...
- **EPIC-026 / TASK-1376 — baseline cerrado con `conditional-go` (2026-07-10):** Greenhouse confirma 1.269 entrypoints; local clean p50 138 s, warm p50/p95 102/124 s y RSS p95 7,51 GB; Vercel Ready p50/p95 4/7 min en ventana corta, con Billing FOCUS `not_configured`. El hotspot probado es Roadmap: tres traces arrastran 2.493 Markdown y generan artifacts analyzer de 8,83–9,61 MB. Primera prueba propuesta: TASK-1379 para materializar el índice y repetir A/B; ADR permanece `Proposed` y no autoriza `apps/*`, `packages/*` ni nuevos deployables. Toda task nueva mantiene `## Modular Placement Contract`. Canon: `docs/audits/platform/2026-07-10-greenhouse-build-dependency-baseline.md` + `docs/operations/MODULAR_MIGRATION_NEW_WORK_OPERATING_MODEL_V1.md`.
