...
- **Playwright CI smoke install (2026-06-22):** los smokes GitHub corren Chromium headless; el workflow instala `pnpm exec playwright install --with-deps --only-shell chromium`. No volver al install completo de Chrome for Testing salvo que un spec necesite browser headed/canal completo y se agregue cache/timeout explícito.
