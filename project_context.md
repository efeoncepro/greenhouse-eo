...
- **TASK-1175 Design Handoff Control Plane V2 (2026-06-20):** code complete local, rollout pendiente. TASK-1120 queda endurecida como control plane Full API Parity: migración additive `20260620003944557_task-1175-design-handoff-control-plane-v2.sql` agrega ownership/planning/links/evidence/node snapshots + estado `in_review`; commands/readers en `src/lib/design-system/handoff/**`; APIs programáticas `/api/design-system/handoff/**` con `executeApiPlatformCommand`; capabilities finas `design_system.handoff.{allowlist.manage,owner.assign,planning.update,link,evidence.attach,verify,drift.read}`; eventos outbox V2; reliability signals `design_system.handoff.{missing_evidence,node_drift,orphan_surfaces}`. Rollout pendiente por dependencia externa: aplicar TASK-1120+1175 migrations, aprobar/seedear un `file_key` Figma de producto y hacer smoke staging create -> link -> evidence -> in_review -> implemented. TASK-1176 sigue como UI consumer bloqueada hasta ese smoke.
