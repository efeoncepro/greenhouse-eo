...
- **Growth CTA & Popup Engine runtime + Experience System (actualizado 2026-07-18):** `growth.cta` ya tiene foundation/renderer productivos y Think live; WordPress/steady-state closure sigue en `TASK-1427`. Greenhouse owns definitions/versioning/targeting/suppression/priority/action routing/event ledger; renderer portable no-iframe preserva `dataLayer`/GTM; Growth Forms/Think/HubSpot son destinations/consumers. El contrato visible separa placement, experience kind, appearance (`style_variant`), density container-derived `full|condensed|peek` y `variant_id` experimental. V1 agrega solo `slide_in` no modal; TASK-1428/1429/1431/1430 cubren suppression, presentation, actions y cockpit sin una task por skin. **Delta TASK-1428 (2026-07-18, code-complete en shadow, sin push):** el data plane de suppression/exposición/kill-switch ya existe — `cta_visitor_state` + `cta_exposure_rollup` (Tier B agregado, jamás OLTP) + `cta_kill_switch_event` (migración aplicada), decisión server-side fail-closed con claim atómico interruptivo, kill switch global/per-surface por DB sin redeploy (`/api/admin/growth/ctas/kill-switch`, `growth.cta.pause`), `engineState ok|killed` aditivo al público y flag `GROWTH_CTA_SUPPRESSION_ENFORCEMENT_ENABLED` default OFF = shadow (arch §24 + ledger §Pendientes). Docs canónicos: `docs/architecture/GREENHOUSE_GROWTH_CTA_POPUP_ENGINE_DECISION_V1.md`, `docs/architecture/GREENHOUSE_GROWTH_CTA_POPUP_ENGINE_ARCHITECTURE_V1.md` y `docs/epics/to-do/EPIC-023-growth-cta-popup-cro-engine.md`.
