...
| Cómo crear o auditar publicidad/social con texto | `docs/operations/ADVERTISING_CREATIVE_AGENT_EXECUTION_V1.md` + skill espejo `efeonce-advertising-creative`; AXIS `axisAdvertising` y `efeonce.advertising-typography` son SSOT portable (`trial`), y el consumidor sólo traduce. Para social compone con `social-media-studio`; producir/revisar no autoriza publicar. MCP no sirve este manual mientras no exista una tool creativa federada real. |
