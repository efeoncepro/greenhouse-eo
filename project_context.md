...
| Cómo crear o auditar publicidad/social con texto o selección colaborativa | `docs/operations/ADVERTISING_CREATIVE_AGENT_EXECUTION_V1.md` + [Creative Typography Workbench público](https://axis.efeonce.org/references/creative-typography/) + skill espejo `efeonce-advertising-creative`; AXIS posee `axisAdvertising`/`efeonce.advertising-typography` (`trial`) y, en source, `efeonce.collaboration-selection` (`candidate`) + manifest agent-facing. El consumidor sólo traduce mediante adapter; si falta, reporta `pending adapter`. Para social compone con `social-media-studio`; producir/revisar no autoriza publicar. MCP no sirve este manual mientras no exista una tool creativa federada real. |
