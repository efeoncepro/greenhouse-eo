...
| Cómo descubrir y calificar licitaciones públicas de LicitaLAB | skill espejo `greenhouse-public-private-tenders` → `licitalab-radar-playwright.md` + `licitalab-mcp.md`; LicitaLAB ve contratación pública solamente y nunca sustituye Wherex/Ariba/Coupa ni origina `private_rfp`. `pnpm licitalab:radar` reutiliza una sesión Chrome aislada, pagina el listado y entrega códigos al MCP para evidencia documental. Discovery no postula ni escribe HubSpot; la promoción usa `public_opportunity`/`origin='public_tender'` y exige confirmación humana, asociación y readback |
