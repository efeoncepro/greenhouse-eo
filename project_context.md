...
- **NexaKnowledgeAnswerSurface vigente (TASK-1089, 2026-06-12):** `src/components/greenhouse/primitives/NexaKnowledgeAnswerSurface.tsx` es la composition primitive transversal para respuestas Nexa con evidencia. Variants `conversationTrace|overviewPanel`, kind inicial `knowledgeAnswerTrace`; primer consumer `/knowledge/mockup/answer-trace`. Reusa `NexaComposer kind='knowledgeAsk'`, `NexaSenderMark` y `GreenhouseThinkingBeat`; mantiene pregunta-burbuja, respuesta verificable, composer descendido y proof panel `Fuentes|Packet|Evals`. Es props-only/mock-data hasta `TASK-1085`; no consultar tablas de Knowledge ni endpoints desde la UI.
