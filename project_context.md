...
- **Tender Proposal Studio — foundation agentic y renderer planificados bajo EPIC-027 (2026-07-12):** el Deck Composer F1 determinista ya está shipped como CLI (`DeckPlan → selector → slot-fill → Chromium → PDF de N páginas`); la state machine existe sólo en TypeScript. No existe aún aggregate DB, API/UI, capabilities ni assets runtime. `TASK-1392` es el predecessor bloqueado: crea el aggregate/assets con `audience` (`internal` o `client_facing`) y el contrato agentic gobernado `contexto allowlisted → propuesta tipada/trazable → confirmación humana → command canónico`; un LLM no muta estado, assets ni gates. `TASK-1391` queda después como candidato bloqueado de `EPIC-027` para `tender-worker` Cloud Run Job: el agente no encola, invoca Cloud Run, ejecuta Chromium ni publica PDFs. No crear una tabla paralela hasta arbitrar ownership `RESEARCH-007 → Tender`, ni crear deployable sin la decisión `continue|pause|stop` del epic. Carruseles/posts/stories no ensanchan Tender: la capa reutilizable `format_spec → composition_spec → artifact_manifest` pertenece a Creative Studio (EPIC-028); cualquier handoff Tender futuro será minimizado/versionado y no moverá RFPs, material interno ni storage credentials. Canon: `docs/architecture/GREENHOUSE_TENDER_PROPOSAL_STUDIO_ARCHITECTURE_V1.md`, `agent-invariants/COMMERCIAL_TENDERS_AGENT_INVARIANTS.md`, tasks `1392 → 1391`.
