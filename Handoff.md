...
**EPIC-044 / TASK-1836, 2026-09-05:** rollout autorizado en curso. Primer commit51e285bb3 publicado: auth-server-00008-zwh y ops-worker-00649-6sv, 100%, gates interno/GC OFF; scheduler GC PAUSED. Gateway93819fa: revisión00030-87s, flags nativos OFF. Canary preparatorio detectó revoke OAuth no observado por gateway: fix jti firmado + ledger vigente probado localmente, en publicación antes de activar. Vercel reader pendiente de promoción main; config preparada OFF. Identidad canónica elegible y dry-run aplicado:false; sin enrollment/grant/token corporativo real. UI pública carga fuentes/CSP correctamente; UI completa y matriz clientes pendientes. Berel WIP preservado. Pendientes: consumers finales, canary y rollback reales. [Runbook](docs/operations/EFEONCE_INTERNAL_AUTH_ROLLOUT_RUNBOOK_V1.md).
