...
### Objetivo del turno
- Reabrir con diagnostico guiado por evidencia el overflow horizontal/vertical en `/people/[memberId]` sin seguir tocando el shell global.
- Scope actual: medir la ruta real con Playwright y limitar cualquier fix nuevo a `src/views/greenhouse/people/**` salvo evidencia fuerte en contra.
- Hallazgo en revisión: el root de `PersonView` sigue usando `MUI Grid container spacing={6}`, candidato a inflar ancho/alto de esta vista en particular.
...
- drawers asociados solo si la medición los identifica

### Verificacion
- Se instaló `@playwright/test` para medir la ruta real con navegador.
- Pendiente: identificar el nodo exacto que excede el viewport antes de editar más CSS.

### Riesgos o pendientes
- No mezclar este fix con los cambios locales de `People 360` que siguen sin commit.
- Evitar más cambios globales del layout mientras no haya evidencia de que el problema nace fuera de `People`.
...
### Objetivo del turno
- Aplicar un fix local a `/people/[memberId]` para cortar el overflow horizontal/vertical sin seguir moviendo el shell global del dashboard.
- Reemplazar los wrappers `MUI Grid` de la vista por layout local con `Box` + CSS grid en la superficie de persona.
...
### Verificacion
- `pnpm exec eslint src/views/greenhouse/people/PersonView.tsx src/views/greenhouse/people/tabs/PersonMembershipsTab.tsx` ✅
- `npx tsc --noEmit` ✅
- Intento de diagnóstico con Playwright/Chrome:
  - no quedó como dependencia del repo
  - Chrome bloqueó ejecución JS por Apple Events y la ruta en navegador limpio cae a Vercel SSO, así que no se pudo inspeccionar el DOM autenticado real desde automatización

### Riesgos o pendientes
- El fix nuevo queda acotado a `People`; no se agregaron más cambios globales de layout.
- Siguen locales y sin commit los cambios backend de `People 360` (`get-people-meta`, `permissions`, `get-person-detail`, `person-context`, tests).
