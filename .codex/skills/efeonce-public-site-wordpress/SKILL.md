...
- Agencia Creativa (`page_id 249582`) "Cómo trabajamos" module edge gutter (2026-06-23): the section `7489ca6` reuses a full-bleed sticky-scroll module whose `lp-container-offset-*` padding is home/front-page-scoped, so off-home it hugged the viewport edges. Applied a page-scoped fix in `wp-content/themes/ohio-child/assets/css/global-fixes.css`: `@media (min-width: 1025px) { body.page-id-249582 .elementor-element.elementor-element-7489ca6 { padding-left/right: clamp(24px, 3vw, 56px) } }` (≈52px @1728). Background stays full-bleed; tablet/mobile untouched; `position:sticky` intact. Full `.page-container` alignment (`calc((100vw - min(1344px,86vw))/2)` ≈192px) was tried first and rejected by the operator as too much. Also widened the two intro cards `ab7c19e`/`36eb5ec` to `width` 100%/93% via `Document::save()` (gap 147px→39px). Live edit backported to runtime repo `efeoncepro/efeonce-public-site-runtime` (`main`, commit `010a2e5`). Edit live via `scp` after backing up to `global-fixes.css.bak-<ts>`; always `wp kinsta cache purge --all` after.
- Custom Elementor widget `greenhouse_comparison_table` — **CANONICAL reusable primitive** (TASK-1224 complete 2026-06-23; plugin `eo-elementor-widgets` v0.6.0 in the runtime repo; LIVE on `/agencia-creativa/` page 249582). 2-column versus/comparison table. Architecture: semantic `<table>` (scope + sr-only `<caption>` + aria-labelled check/cross icons), per-column gradients painted on `<col>`/`<tbody>` backgrounds (keeps table semantics for AT), wrapper `.gh-ct-wrap` (container-type for `cqw` scaling, `max-width:1100px`) → `.gh-ct-surface` (rounded block, `overflow:hidden`+`clip-path`, soft glow shadow) → table. **Globe preset tokens — fidelity-matched pixel-by-pixel to AXIS Figma node `12479-18591`:** crimson header `#d11963` (deliberately more vivid than Figma `#ba1954`, operator choice), amber Globe header `#ffa300`, Agencia body deep wine→purple→indigo `#3a022c → #37103f → #341d4e` (Figma `#36002e→#331c4f`), Globe body coral→peach→rose-mauve `#f0804a → #f1a892 → #e597a5` (Figma `#ee7f49→#e796a5`), violet ribbon `#a233fa→#6b08c2` + fold `#2f0571`. Logo = white wordmark attachment `249612` `Globe_Logo.svg`. **Best-option ribbon = embedded corner-fold** (`.gh-ct-corner-ribbon` owned by the Globe header cell; `clip-path` triangular `face` over darker `fold--top`/`fold--left` bands = the wrap over the two edges; `role="img"`+`aria-label`, label `aria-hidden`). NOTE: earlier vertical-banner / 2-layer-occlusion ribbon experiments were rejected by the operator — the **embedded corner-fold is the canonical shape**. **Self-administrable theming (Elementor + agent-ready):** `theme_schema()` is a PUBLIC method = SINGLE SOURCE OF TRUTH mapping stable setting keys (`c_crimson`, `c_amber`, `c_amber_top`, `c_aub_top/mid/bot`, `c_globe_top/mid/bot`, `c_ribbon`, `c_ribbon_dark`, `c_ribbon_fold`, `c_radius`) → CSS custom properties. Elementor STYLE tab exposes a color picker per key + radius slider (empty inherits the preset); `render()` emits only the set overrides as inline CSS vars on `.gh-ct-wrap`, validated by `is_safe_css_color()`. Root markers `data-gh-schema="comparisonTable.v1"` + `data-gh-plugin-version` for agent governance/drift. **This `theme_schema()` contract is exactly what TASK-1225's `comparisonTable.v1` manifest must mirror — TASK-1225 is NOT executed yet, only prepared.** **Microinteractions** (reviewed with `motion-design` + `microinteractions-auditor`; all compositor-only `transform`/`opacity`, all behind `prefers-reduced-motion`): (1) scroll-driven entrance cascade — row `opacity` + check `scale` pop via `animation-timeline: view()`, `@supports`-guarded so unsupported browsers (Firefox) show content STATICALLY (never hidden); (2) row hover (fine pointer only) — subtle inset tint + Globe check grows + soft ring; (3) ribbon diagonal sheen sweep (clipped to the triangular face); (4) cursor-glow spotlight — compositor sprite translated via CSS vars set by `assets/js/comparison-table.js` (registered via `get_script_depends()`, fine-pointer + no-reduced-motion only, `mix-blend-mode:soft-light`). **Gotchas learned (load-bearing):** Ohio/Elementor table+img CSS wins specificity → scope all selectors under `.gh-comparison-table` (bare-class padding/img-width got overridden; logo rendered 0×0 until scoped); an SVG with `viewBox` but no width/height attrs does NOT resolve `width:auto` from height in flex/inline → set explicit `width`; **`transform` on `<tr>`/`<td>` (table-row/table-cell) is unreliable → animate row `opacity` + check `scale` on the icon spans, never row transforms**; scroll-driven animations MUST be `@supports (animation-timeline: view())`-guarded so initial `opacity:0` never leaves content hidden on non-supporting browsers; a stale fixed `?ver=` on the CSS cached a broken early state in the operator's browser → version CSS/JS by `filemtime()` (auto-bust). Deploy rail = `scp` the plugin dir + OPcache reset (drop `<?php opcache_reset();` in webroot, curl, delete) + `wp kinsta cache purge --all`; `php -l` (PHP) + `node --check` (JS) before upload. Docs: técnica = strategy doc `docs/documentation/public-site/wordpress-custom-widgets-react-strategy.md` (pilot marked delivered) + this fact; funcional = `docs/documentation/public-site/comparison-table-widget.md`; manual = `docs/manual-de-uso/public-site/comparison-table-widget.md`. Runtime repo commits on `main` (`86224bd`, `13ea66a`, `0ef3a5a`, `73b0768`, `6de0dcc`, `f72ce17`, `743f6be`).
- Plugin `eo-elementor-widgets` is the CANONICAL multi-widget container for ALL custom public-site Elementor widgets — NOT single-widget. To add another (Hero, Partner Proof, Feature Grid, CTA Band, …) do NOT create a new plugin: add a class file under `includes/widgets/` + one entry to `EO_Widgets_Loader::$widgets` (`file => Class`); the shared `greenhouse` category is already registered. Each widget declares its own assets via `get_style_depends()`/`get_script_depends()` (Elementor enqueues only when present). Current `register_styles`/`register_scripts` are coupled to the `comparison-table` handle — generalize to a per-widget handle when the 2nd widget lands (minor refactor). Theme-able/agent-administrable widgets should expose their own `theme_schema()` (same SSOT pattern) + `data-gh-schema`/`data-gh-plugin-version` markers for homogeneous manifest governance (TASK-1225 family). Recipe documented in `docs/documentation/public-site/wordpress-custom-widgets-react-strategy.md` → "Plugin contenedor multi-widget".
...
- Agencia Creativa "Cómo trabajamos" sticky-scroll module: `page_id=249582`, slug `agencia-creativa`. Section container `7489ca6` is a full-bleed flex row split into a left `49d5a98` (63%, the scrolling 7-step process built from `ohio_service_table` widgets + two intro cards) and a right `97e545e` (37%, the "CÓMO TRABAJAMOS / La diferencia no es…" block). Two behaviors to know before touching it:
  - **Sticky is `position:sticky`, NOT the Elementor Pro Sticky motion effect.** The custom class `-sticky-block` (defined in `Landing Custom CSS.css`) sets `position:sticky; top:0` on `d0ef9a7` (right block — stays pinned while the left column scrolls) and on `e6facfa` (the left illustration). The 7 steps and the cards are not sticky, so they scroll past. Do not look for `sticky`/`motion_fx` keys in `_elementor_data` — there are none.
  - **`lp-container-offset-left` / `lp-container-offset-right` are inert on non-home pages.** Their padding (`calc((100vw - var(--container-max-width))/2)` ≥1641px, or `calc(--clb-side-elements-height + --clb-spacer*2 - --clb-grid-gutter)` 1181–1640px, in `Landing Custom CSS.css`) is scoped ONLY to `body.home, body.front-page`. On home the container is full-bleed (`--container-max-width: 100%` → offset resolves to 0, content sits at the edge and that reads fine). When the same module is reused on a boxed page like `agencia-creativa` (`body.page-id-249582`, `.page-container` = `min(1344px, 86vw)`, inset ~192px @1728), those rules don't match and `--container-max-width` is undefined, so the offset classes do nothing → the module bleeds full-width (content at x≈20) while the rest of the page respects the container. That "pegada a los bordes" mismatch is the home-vs-boxed-page gotcha, not a width bug. Fix with a restrained symmetric edge gutter via page-scoped CSS in `global-fixes.css` (see Current Runtime Facts), keeping the pink background full-bleed; do not edit the home-scoped offset rules. **NOTE (operator 2026-06-23): full `.page-container` alignment (~192px @1728) was rejected as too much — the chosen fix is a modest gutter, NOT full boxing.**
  - **Card width inside the left column:** the two intro cards (`ab7c19e` rosada, `36eb5ec` morada) each live in a 50% sub-column. Their inner container `width` (not any gap setting) controls how wide they are; reduced/asymmetric values (78.15% / 72%) leave large side gutters and a wide middle gap. Widen via the native container `width` % through `Document::save()` and re-measure (Elementor's % is non-linear: card1's 78.15% rendered like card2's 72%). On 2026-06-23 set to 100% / 93% → cards ~489/492px, inter-card gap 147px→39px, symmetric.
