...
- `greenhouse-wp-bridge` current state: `wp-content/plugins/greenhouse-wp-bridge`, version `0.3.0` in the runtime repo. Production has active read-only inspection mode from 2026-06-14; authenticated `health`, `inspection/elementor-document/244079`, `inspection/block-document/249766`, and `inspection/ohio-widget-catalog` were smoke-tested as `200`, while anonymous health returns `401 ghwpb_auth_required`. v0.3.0 adds HMAC/shared-secret signature verification, body hash, timestamp window, replay guard and `draft/private` routes in code, but writes remain default-disabled until `GREENHOUSE_WP_BRIDGE_WRITES_ENABLED`, shared secret, staging/preview and least-privilege are ready. No publish, cache clear, backups or Abilities registration yet.
