...
- SEO metadata contract (2026-07-07): Yoast title is `Agencia SEO y posicionamiento web | Efeonce`; meta description is `Agencia SEO para marcas que necesitan tráfico orgánico, autoridad temática y visibilidad en IA. Diagnostica tu presencia en búsqueda con Efeonce.`; canonical is `https://efeoncepro.com/servicios/posicionamiento-seo/`; robots stays `index, follow`; Yoast focus keyphrase is `agencia seo`; `post_excerpt` is explicitly set to `SEO técnico, contenido y autoridad para que te encuentren en Google y la IA entienda tu marca. Diagnostica tu baseline con Efeonce.`; OG/Twitter title and description are aligned, and both social images explicitly use attachment `243360` (`og-home_Mesa-de-trabajo-1.webp`) until a page-specific SEO service social card is produced.
- Structured data contract (2026-07-07): Yoast owns `WebPage`, `BreadcrumbList`, `WebSite`, and `Organization`; the page-scoped inline JSON-LD inside `seohtml` must contain only `Service` (`@id=https://efeoncepro.com/servicios/posicionamiento-seo/#service`) and `FAQPage` (`@id=https://efeoncepro.com/servicios/posicionamiento-seo/#faq`, 9 visible FAQ entities). Do not reintroduce duplicate page-scoped `Organization` or `BreadcrumbList` nodes unless Yoast is intentionally disabled for this page.
...
- Latest backup before the SEO metadata/schema cleanup: `_gh_backup_before_task1343_seo_meta_schema_20260707T072341Z`.
...
- Latest SEO metadata/schema verification (2026-07-07): WP-CLI inspector confirmed explicit excerpt, Yoast title/metadesc/canonical/social image/focus keyphrase, inline `Service` + `FAQPage` JSON-LD with 9 FAQs, and `seohtml` md5 `b18d77516c899c948a1e6410f70f0632`; live head returned HTTP `200`, title `Agencia SEO y posicionamiento web | Efeonce`, canonical, robots `index, follow`, OG/Twitter image `og-home_Mesa-de-trabajo-1.webp`, and combined JSON-LD types `WebPage`, `BreadcrumbList`, `WebSite`, `Organization`, `Service`, `FAQPage`. Browser smoke desktop `1440` and mobile `390` confirmed `.gh-seo-landing`, `<greenhouse-form>`, native `#masthead.header-3`, no custom header, one footer, `scrollWidth==clientWidth`, and console errors `[]`.
