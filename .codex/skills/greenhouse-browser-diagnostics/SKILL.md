...
3. Capture browser evidence: screenshot, console messages, page errors, failed requests, non-2xx/3xx app responses, and relevant API payloads.
4. Save artifacts under `.captures/<route-or-scenario>-<iso>/` when the run produces screenshots, videos, traces, or JSON evidence.
5. If the UI reports operational drift, query the underlying source of truth with the repo's canonical tools: `pnpm staging:request`, `pnpm pg:doctor`, `pnpm pg:connect`, Vercel CLI, GitHub CLI, or focused runtime scripts.

Use `pnpm fe:capture --route=<path> --env=staging` when a visual/video capture is enough. Use a focused Playwright script when console/network/API evidence is required.
