...
| OAuth/resource metadata | root and path metadata, missing/expired/wrong issuer/audience token rejection, insufficient-scope rejection, authenticated initialize |
...
| Globe/creative capability | all provider evidence plus workspace/rights/credit/approval gates from Globe; never test with a real paid generation unless authorized |
...
5. A real authorized client completes MCP `initialize`; a missing provider scope denies before downstream dispatch.
