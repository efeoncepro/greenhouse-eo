...
- **Operating manuals travel through the protocol (Greenhouse `TASK-1804`).** The internal MCP and the gateway both
  ...
  `EXPECTED_GREENHOUSE_PLATFORM_TOOLS` (+ `computeFederatedNonSeoToolFindings`) — today `get_greenhouse_skill` plus
  the three `*_client_service_enablement` tools (TASK-1852). Manuals today (read the
  count from the manifest, never from here): `seo-spend-discipline`, `seo-visibility-reading`, `competitor-loop`,
  `seo-discovery-to-tracking`, `seo-technical-health`, `seo-prospect-diagnostic`, `client-service-enablement` (all
  internal). **Gateway deployed 2026-09-02**
