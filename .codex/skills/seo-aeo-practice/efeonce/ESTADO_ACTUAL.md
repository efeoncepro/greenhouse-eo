...
| **EPIC-022 · Search Visibility 360** | 🟡 **Motor real, sin cara** *(2026-08-05 fin de día: COMPLETE TASK-1299 schema `greenhouse_growth.seo_*` + TASK-1301 entitlement `seo_v1`/chokepoint `enforceSeoRunEntitlement` + TASK-1300 registry DataForSEO/ledger + TASK-1302 materializer GSC LIVE/`readKeywordOpportunities` + TASK-1305 `readSeoAeoGap`/quadrant 360 — primer quadrant real: **Berel #1.75 orgánico × AEO 44.5 → `riesgo`**, autoridad sin citabilidad = cross-sell al AEO. TASK-1645: 3 MCP tools code-complete; federación gateway TASK-1647 CODE COMPLETE — provider `greenhouse-seo` en el gateway `mcp.efeonce.org` con canary e2e real por HTTPS: el 360 de Berel respondió **`riesgo` / 50 keywords / AEO 44.5** end-to-end, entitlement Efeonce ok + `no_seo_data` honesto, deny anti-oracle 404; enable en prod gated por el release de greenhouse. Efeonce own-brand provisionada `EO-ORG-0007` con lente AEO propia + assignment `seo_v1`; **Berel provisionada Fase 0** (`cpma-berel-seo-contracted` + `seot-berel-fase0`, `seo_v1`); SKY con lente AEO ligada. La cara pública/cliente sigue sin existir — no venderlo como autoservicio)* | El nombre ya declara la estrategia: **vender visibilidad, no SEO** |
