...
> **📊 Medición GTM/GA4 — reference canónico (esta skill es la dueña designada).** Toda la documentación de Google Tag Manager + GA4 (modelo de eventos, recomendados, límites duros, dataLayer, consent mode) + la taxonomía de naming (object–action, anti-explosión, gobernanza) + la **house style `gh_<object>_<action>`** de Efeonce viven en **`docs/reference/measurement-gtm-ga4/`** (una sola copia, canónica; la leen Claude, Codex, humanos y Nexa). Cargarlo al instrumentar, taggear, medir eventos/conversiones o nombrar un evento nuevo. Empezar por `04-greenhouse-gh-event-convention.md` (coordenadas reales `GTM-NGHPGRLZ`/GA4 `486264460` + regla GA4-recomendado vs `gh_`). Complementa el módulo `07_MEASUREMENT_ANALYTICS.md`. **Para CONSTRUIR/publicar los tags/triggers/variables por API (workspace→preview→confirm→publish) → skill `greenhouse-gtm-ga4-operator`** (esta skill decide el "qué medir"; el operador lo ejecuta).
...
|---|---|---|
| Doctrina y arquitectura editorial de **Content Engineering** | **`content-marketing-studio`** (`../content-marketing-studio/references/content-engineering.md`) | Growth mide si la experiencia reduce fricción y mueve una decisión; no define la verdad, el formato ni su sistema editorial. |
