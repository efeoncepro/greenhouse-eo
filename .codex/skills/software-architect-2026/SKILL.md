...
The AXIS consumer pilot is runtime-complete as an opt-in fixture: `TASK-1591` verified Greenhouse/MUI and Globe/Tailwind adapters against packages `0.1.4`. Keep the distinction between pilot readiness and product-promotion readiness; before promotion require the exact consumer build, visual/accessibility evidence, rollback target, fresh-install evidence and release control-plane evidence. If the boundary, package source of truth, credential flow or rollout state changes, route the change through the applicable ADR and task rather than silently updating only a consumer.
