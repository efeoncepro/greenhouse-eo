...
- Five controlled pilot Services are live in `New` in portal `19893546`; all are `incomplete_core` and excluded from official KPIs until Maria Paz Haeger reviews activation facts. Workflow `1852406585` is active only as the activation-review queue: authenticated positive/negative tests passed, five pilot executions created five associated tasks and re-enrollment is disabled. This is not authorization for historical backfill, additional workflows or final dashboards. Load `anam-phase-3-forward-pilot-execution-2026-07-16.md` for IDs and rollback.
