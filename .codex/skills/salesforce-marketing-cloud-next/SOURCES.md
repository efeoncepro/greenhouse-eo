# Fuentes y vigencia

Fecha de revisión: **2026-09-16**. Abre la fuente primaria antes de convertir un dato perecible en claim, propuesta o diseño.

| Tema | Fuente primaria | Uso |
|---|---|---|
| Producto y narrativa | https://www.salesforce.com/marketing/agentic-marketing/ | Nombre y alcance público de Marketing Cloud Next |
| Data 360 como fundamento | https://help.salesforce.com/s/articleView?id=data.c360_a_datacloud_mktgnext.htm&type=5 | Data streams, modelado, identidad, segmentación y consumo |
| Ediciones y acceso | https://help.salesforce.com/s/articleView?id=mktg.mktg_editions.htm&type=5 | Growth/Advanced, requisitos, acceso desde Engagement/Account Engagement |
| Configuración de IA | https://help.salesforce.com/s/articleView?id=sf.mktg_admin_setup_einstein.htm&type=5 | Editions, permisos, Foundations y Agentforce |
| Uso de datos por IA | https://help.salesforce.com/s/articleView?id=sf.mktg_einstein_data_usage.htm&type=5 | Datos y modelos usados por Agentforce/Einstein |
| Límites y allocations | https://help.salesforce.com/s/articleView?id=mktg.mktg_admin_limits_ref.htm&type=5 | Límites y créditos; verificar siempre el contrato |
| Pricing público | https://www.salesforce.com/marketing/pricing/ | Precio inicial, edición y add-ons; validar país/moneda/fecha |
| Release notes | https://help.salesforce.com/s/articleView?id=release-notes.rn_marketing.htm | Estado GA/preview por release |
| Account Engagement + Next | https://help.salesforce.com/s/articleView?id=sf.mcae_mc_campaigns_dc_prereqs.htm&type=5 | Prerrequisitos, Connected Campaigns, Data 360 y entitlements |
| Dreamforce 2026 y AIforce | https://www.salesforce.com/news/stories/aiforce-announcement/ | Interfaces externas, gobernanza y estados de Claudeforce/Slackforce/Coworker |
| Agentforce job-ready agents | https://www.salesforce.com/ap/news/press-releases/2026/09/14/ph-salesforce-expands-agentforce-with-a-new-portfolio-of-ai-agents-built-for-high-value-work/ | Agentes de marketing/ventas y long-horizon runtime; separar GA, piloto y roadmap |
| AWS y Salesforce | https://www.salesforce.com/news/stories/aws-salesforce-enterprise-ai-expansion/ | Bedrock, Amazon Quick, zero-copy, voz y estados de disponibilidad |
| Google Cloud y Salesforce | https://www.salesforce.com/news/stories/salesforce-google-cloud-unify-infrastructure-and-agents/ | Gemini Enterprise, Hyperforce, MCP, Tableau y estados de disponibilidad |
| Novedades de Marketing Cloud desde Dreamforce | https://www.salesforce.com/marketing/whats-new/ | Campaign Agent, Palmata, Headless Marketing, Data Guardian, Budget Optimization y fechas GA |
| AIforce regional | https://www.salesforce.com/ap/news/press-releases/2026/09/16/sg-salesforce-unveils-aiforce-bringing-the-full-power-of-its-platform-to-any-interface/ | Capa de interfaz, contexto gobernado y acciones desde Claude, Slack y otras superficies |
| Missionforce y OpenAI | https://www.salesforce.com/news/stories/missionsforce-expansion-agents-nvidia-openai-partnerships/ | Capacidades gubernamentales, modelos y condiciones de entornos regulados |

## Jerarquía

1. Order form, Product & Services Documentation y contrato del cliente.
2. Salesforce Help y release notes de la release instalada.
3. Página oficial de pricing del país aplicable.
4. Trailhead y materiales de partner como orientación, no sustituto contractual.
5. Fuentes secundarias sólo para descubrir preguntas; no para prometer producto o precio.

No cites Winter ’27 preview como GA. Si una fuente oficial discrepa con el contrato o la org, registra la discrepancia y detén el claim hasta resolverla.
