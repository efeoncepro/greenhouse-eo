...
This reference summarizes the Chile payroll/legal concepts that are most relevant for Greenhouse audits. Always verify current rates, caps, tax tables, and legal interpretation against official sources before calculating or approving payroll.
...
1. Determine imponible remuneration:
   - base salary
   - fixed taxable bonuses
   - variable taxable bonuses
   - legal/conventional gratificacion when taxable
2. Exclude reasonable non-imponible allowances:
   - colacion
   - movilizacion
   - reimbursements/viaticos only if they compensate real work expenses and are reasonable
3. Apply caps where legally required:
   - AFP/pension and health cap in UF
   - accident insurance cap in UF
   - cesantia cap in UF
4. Calculate worker deductions:
   - AFP mandatory 10 percent plus AFP commission, subject to cap
   - Health 7 percent, with Isapre excess split as voluntary/excess
   - Seguro de cesantia worker share only when applicable
   - APV if configured
   - Impuesto Unico Segunda Categoria on the taxable base
5. Calculate employer costs:
   - SIS/employer social security components in force for the period
   - Seguro de cesantia employer share by contract type
   - mutual/accident insurance rate
6. Net pay:
   - taxable remuneration + non-imponible allowances - worker deductions - tax
