...
- **Enviar/sembrar un mensaje al chat PROGRAMÁTICAMENTE (TASK-1182):** para inyectar un mensaje en el thread (ej. un CTA de dominio que abre el chat con una pregunta pre-hecha — "Pregúntale a Nexa sobre este insight"), usar SIEMPRE el store de assistant-ui **`aui.thread().append({ role: 'user', content: [{ type: 'text', text }] })`** vía `useAui()`, **desde un componente montado DENTRO del `AssistantRuntimeProvider`**. Es el MISMO API que ya usan las follow-up suggestions del chat (`NexaThread` → `FollowupSuggestions`). **NUNCA** `runtime.thread.append()` sobre el objeto del outer `useLocalRuntime`: ese `.thread` es el placeholder "empty thread" → crashea en render ("call thread methods inside a useEffect") o no envía (silencioso). **NUNCA** `ThreadPrimitive.Suggestion` oculto + auto-click (frágil; no dispara). Patrón de seeding surface-aware: el CTA despacha `NEXA_FLOATING_OPEN_EVENT` con `{ focusRef, seedPrompt }`; `NexaFloatingButton` guarda `focusRef` (lo lee el adapter para el body → respuesta anclada, ver TASK-1182 §`focusRef`) y monta `NexaSeedAutoSend` dentro del provider, que envía el `seedPrompt` con `aui.thread().append` una vez (cuando `s.thread.isRunning` es false). Referencia viva: `NexaSeedAutoSend` en `NexaFloatingButton.tsx` + `FollowupSuggestions` en `NexaThread.tsx`.
