# Battlecard — plantilla

> Contenido completo por competidor: `modules/05_DISPLACEMENT.md`.
> Esto es la versión de **una página**, para llevar a la reunión.

---

## ⚠️ Antes de entrar — las tres trampas

1. 🔴 **"HubSpot es Leader en Gartner" → di CUÁL.** Es **Niche Player en el MQ de Sales Force Automation
   2025** (Leaders: Salesforce, Microsoft, Oracle; **Zoho es Visionary, por encima**).
   Es **Leader en B2B Marketing Automation, 5.º año** ✅ — **ese** es tu reporte.
   **El AE de Salesforce va a llegar con la lámina del primero.**
2. 🔴 **"Leader en el Forrester Wave 2026" → NO lo cites.** ❌ No verificado. La landing de HubSpot que lo
   promociona **cita el Wave de 2024**.
3. 🔴 **Contra Microsoft, Zoho, Pipedrive y Odoo → NO pelees el precio. Pierdes.**

---

## Plantilla

```
COMPETIDOR: ______________________          Fecha de verificación: __________

SU PRECIO (marca ✅ primaria / ⚠️ triangulado / ❌ no publicado)
  Licencia:            $______    [ ]✅ [ ]⚠️ [ ]❌
  Costos ocultos:      $______    (implementación · admin · support · add-ons · storage)
  TCO 3a estimado:     $______    Supuestos: _______________________________

DÓNDE GANO (con evidencia, no con adjetivos)
  1. ______________________________________________
  2. ______________________________________________
  3. ______________________________________________

🔴 DÓNDE PIERDO (dilo tú primero — la honestidad te compra el deal)
  1. ______________________________________________
  2. ______________________________________________

SUS 3 OBJECIONES         →  MI REBATE
  1. ____________________    ____________________
  2. ____________________    ____________________
  3. ____________________    ____________________

🚩 DESCALIFICO SI:  ______________________________________________
```

---

## Los golpes de una línea

| Contra | El golpe |
|---|---|
| **Salesforce** — Gartner | *"Correcto, en el MQ de **SFA**. El mismo Gartner nos puso **Leader en B2B Marketing Automation, 5.º año**. ¿Cuál de los dos cuadrantes describe **su** problema? **Si necesitan CPQ de ocho niveles y territorios, honestamente Salesforce es mejor.**"* |
| **Salesforce** — TCO | *"El delta no está en la licencia. Está en el **admin**."* 🔴 **Pero pregunta primero si ya lo tienen** |
| **Salesforce** — onboarding | *"Yo les digo **hoy** cuánto cuesta arrancar: $3.500, publicado. **Ellos les dicen 'contacta ventas'.**"* |
| **Salesforce** — Agentforce | *"Ellos cobran **$0,10 por acción**. Nosotros **$0,50 por conversación RESUELTA**. **Pagan cuando funciona, no cuando lo intenta.** Pregúntenle a su AE cuánto será su factura de Flex Credits el mes 14. **No lo sabe.**"* |
| **Salesforce** — portafolio | ❌ **No digas que Pardot está muerto.** ✅ Di: *"Tienen **tres** productos de marketing de Salesforce conviviendo, y su AE no les puede decir en cuál estarán en tres años. **Yo tengo uno.**"* |
| **Adobe / Marketo** | 🎯 *"Marketo les cobra por el **tamaño de su base**, no por uso. ¿Cuántos de esos 80.000 contactos tocaron un email este trimestre? **Están pagando por ochenta mil.**"* Y: ✅ **"Marketo no es un CRM."** |
| **Adobe** — el Wave | *"Cierto, y ese reporte mide **profundidad de capacidad**, no **costo de operarla**. ¿Su equipo de cuatro personas va a usar el 15% de Marketo y pagar el 100%?"* 🔴 **Si son casa Adobe real (AEM+Analytics+Target), retírate** |
| **Microsoft** | *"¿Tienen **D365 en producción** o tienen **Microsoft 365**?"* — en el 80% es lo segundo → **no hay sinergia que perder**. Y: *"Su equipo va a esperar **6-12 horas** para saber si una campaña funcionó. **Lo dice Microsoft Learn**, no yo."* 🔴 **Si corren D365 ERP → retírate** |
| **Zoho / Pipedrive** | *"Es el CRM correcto para el problema que tenían **hace dos años**. Si en 18 meses necesitan marketing, servicio y atribución sobre el mismo dato: **¿lo compran aparte y lo pegan con integraciones?** Si la respuesta es sí, **ellos son más baratos y tienen razón.**"* |
| **Odoo** | *"Odoo es un **ERP** con un módulo de CRM. **Yo no vendo ERP y no voy a fingir que sí.** Un ERP es un sistema de **registro**; ustedes necesitan uno de **generación**. **Quédense con Odoo para facturación** — se lo recomiendo — e integren HubSpot al front."* |
| **Sin CRM** | *"¿Cuántas oportunidades abiertas tienen **ahora mismo**? No la estimación — el número."* **(Si dudan, ganaste.)** 🎯 **Tu competidor no es Salesforce: es la indecisión** |
| **"No escala"** | *"'No escala' es una afirmación **sin unidades**. Díganme el número. Y **si no les damos el ancho, se los digo en esta reunión y no les vendo.**"* |
