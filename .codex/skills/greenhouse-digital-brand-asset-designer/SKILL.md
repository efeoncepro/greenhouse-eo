...
description: Use when creating, vectorizing, cleaning, validating, or applying brand logos/isotypes for Greenhouse payment instruments — including positive/negative SVG variants, raster-to-vector conversion, manifest updates, and visual QA. Triggers on requests to add a payment provider logo, fix a vectorized mark, generate full/mark variants, or audit an existing brand asset matrix.
---

# Greenhouse Digital Brand Asset Designer

Use this skill whenever Greenhouse needs payment-instrument brand assets: logos, isotypes, SVG conversion, positive/negative variants, selector icons, payment provider marks, or visual QA for brand identity files.
...
- `src/components/greenhouse/PaymentInstrumentChip.tsx` — visual rendering layer; changing chip layout belongs to UI tasks.
...
If any prerequisite is missing, abort with a clear message and a one-line install hint.

## Workflow

1. **Inspect the source visually** with `view_image` or a render. Confirm transparency, dimensions, and brand identity match before doing anything destructive.
