...
- **Hay defectos que ningún gate PUEDE ver, y hay que saber cuáles.** El proyecto no emite el preflight de
  Tailwind, así que la hoja del navegador aplica `b, strong { font-weight: bolder }`: un `<strong>` en un
  contenedor a 600 computa **900** — faux bold **sin que ninguna clase lo diga**. El gate escanea `className`,
  no elementos HTML: le es **estructuralmente invisible**. Apareció **tres veces el mismo día** en sitios
  distintos. Abierto, sin dueño.
...
**Y la suite no termina sola (2026-07-29, sin dueño asignado).** El canary `axis-pilot-canary` hace `server.kill('SIGTERM')` sobre el wrapper de `pnpm`, **no sobre el `vite` nieto**, que sobrevive reteniendo los pipes: `pnpm test` **no retorna** y cada corrida deja un huérfano escuchando en el puerto **4326** (llegaron a acumularse doce, uno de tres días). Workaround verificado: correr los canarios por separado y liberar el puerto antes de la corrida siguiente. Un runner que hay que matar a mano entrena a leer «se colgó» como normal — que es exactamente cómo un cuelgue real pasa desapercibido.
...
- ⚠️ **NUNCA** confíes en el default del navegador para `<strong>`/`<b>` en Globe: el proyecto **no emite el preflight de Tailwind**, así que `b, strong { font-weight: bolder }` computa **900** dentro de un contenedor a 600 (y 700 dentro de uno a 400) y pide un corte de Geist que no existe — **faux bold que ninguna clase declara**. El gate escanea `className`, no elementos HTML: **no puede verlo**. Declará el peso explícitamente (`font-semibold`). Detectado tres veces el mismo día (2026-07-29), **abierto y sin dueño**.
