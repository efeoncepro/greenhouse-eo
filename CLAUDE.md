...
- **Drift-guard (enforcement):** `src/components/theme/typography-drift.test.ts` (37 tests) falla CI si `runtime ≡ SoT ≡ DESIGN.md` divergen. Si cambiás un valor del SoT, **DEBÉS** actualizar DESIGN.md front-matter + V1 §3.2 en el mismo PR o el guard rompe (parity 3 capas).
...
- **Follow-ups:** (1) [abierto] rol semántico para el peso **500** (énfasis medio: nav/tabla/labels sutiles) — cambio visual, requiere aprobación + GVC; (2) [abierto] adapter **PDF** Geist 600/800: `register-fonts.ts` registra por **nombre de familia** (Geist=400 / Geist Medium=500 / Geist Bold=700), **no por peso** → faltan las familias SemiBold(600)/ExtraBold(800), así que un componente PDF que quiera esos pesos los **aproxima con Bold/Medium** (NO "cae a Helvetica" — eso es solo para familias sin registrar; es refinamiento de paridad, no bug). El **web no tiene gap** (next/font carga 400-800). Cerrarlo = agregar `Geist-SemiBold.ttf`+`Geist-ExtraBold.ttf` (mismo origen Google Fonts que 400/500/700) + registrar + migrar componentes PDF; (3) [abierto] adapter de **charts** desde el SoT (~47 archivos, sweep); (4) [✓ TASK-1038] lint rule `greenhouse/no-fontsize-inline-typography` (scopeada a `<Typography>`, warn) + rule tests cubiertos en CI (`pnpm test:lint-rules`). Spec: `docs/tasks/in-progress/TASK-1038-typography-scale-redesign.md`.
