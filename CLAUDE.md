...
- **Follow-ups abiertos:** (1) rol semántico para el peso **500** (énfasis medio: nav/tabla/labels sutiles); (2) adapter PDF que registre Geist **600/800** (hoy faltan en `register-fonts.ts` → caen a Helvetica) + adapter de charts desde el SoT; (3) lint rule `no-fontSize-inline` icon-vs-text (~1.351 inline hoy, mayoría íconos legítimos). Spec: `docs/tasks/in-progress/TASK-1038-typography-scale-redesign.md`.
