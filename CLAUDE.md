...
| MCP · tools (`src/mcp/**`) y entry/consentimiento por RP | `efeonce-mcp-platform` | `agent-invariants/MCP_TOOL_SURFACE_INVARIANTS.md` + `GREENHOUSE_MCP_ARCHITECTURE_V1.md` §22 + `EFEONCE_ID_RELYING_PARTY_ENTRY_AND_CONSENT_DECISION_V1.md`; first-party nunca debilita consentimiento, step-up ni audiencia MCP. `tool-manifest.ts` es el SSOT de lo que existe; el gateway decide qué cruza. `writes` ⊥ `spendsProviderBudget`. Gate: `pnpm mcp:manifest:check`. |
