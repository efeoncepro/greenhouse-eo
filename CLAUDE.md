...
| Efeonce Globe / Creative Studio (repo hermano `efeonce-globe`) · EPIC-028 · API Contract Spine (TASK-1481) · agregar capability / command / reader / provider adapter · trusted context / dispatch / SDK · boundary Globe↔Greenhouse | `greenhouse-globe` (+ `arch-architect` para forma/decisiones) | **Globe es un PRODUCTO COMERCIAL de Efeonce (ADR-010: *"now a commercial product, not an internal lab"*), NUNCA un lab/piloto interno; su ESTADIO DE ROLLOUT hoy es internal-only + runtime `internal_smoke` + externos gated por TASK-1480 — estadio ≠ naturaleza, y NUNCA dimensiones infra/UX/calidad "porque es interno".** · `architecture/EFEONCE_CREATIVE_STUDIO_AGENTIC_PLATFORM_{DECISION,ARCHITECTURE}_V1.md` + `epics/in-progress/EPIC-028-*.md` + `architecture/creative-studio/` (índice + arquitectura de Globe: spine, model lab, evaluation harness, connectivity, platform foundation — la doc gobernante de Globe vive en Greenhouse, NUNCA en `efeonce-globe/docs/**`; TASK-1492) + `operations/creative-studio/GLOBE_MODEL_FLEET_STATUS.md` (ledger de la flota de modelos — leer PRIMERO antes de asumir que un modelo no está integrado; Model Lab vs producción gobernada) |
