...
| **Radiografía AEO** — muestra de trabajo viva (`think.efeoncepro.com/muestras/…`): educación de prospecto + sales enablement. Runtime en el repo **`efeonce-think`**, NO acá; el cliente es un **payload JSON**, cero código | `seo-aeo` (oficio) · `seo-aeo-practice` (uso comercial) · `greenhouse-public-private-tenders` (consumer) · `commercial-expert` · `astro` (+overlay) | `docs/think/radiografia-aeo-architecture.md` (invariantes + gate 46 asserts) · `radiografia-aeo-manual.md` · `documentation/comercial/radiografia-aeo-muestra-de-trabajo.md` · `manual-de-uso/comercial/usar-radiografia-aeo-en-venta.md` |
| Content marketing: motor de contenidos (editorial/ops/atomización/distribución/medición) | `content-marketing-studio` (+ `digital-marketing`/`copywriting`/`seo-aeo`) | inline (`digital-marketing` módulo 02 le hace hand-off) · cliente **Berel** → `berel-content-production` |
...
| AI creative rights / enterprise governance | `greenhouse-ai-creative-rights-governance` + `legal-privacy-ip-operator` | AI data governance decision + provider terms |
...
- **UI Platform — contratos canónicos (load-on-demand):** los ~13 contratos de UI Platform (Primitive+Variants+Kinds, Figma Implementation Contract, Adaptive Sidecar, **Composition Shell**, **Adaptive Card density / The Seam**, contención de scroll horizontal, Floating Surface, Motion Primitive, Elevation/Shadow tokens, GVC V1.5) viven en **`docs/architecture/agent-invariants/UI_PLATFORM_AGENT_INVARIANTS.md`** + `docs/architecture/ui-platform/{PRIMITIVES,PATTERNS,STATE,MOTION,...}.md`. **Cargar ese doc + las skills de product-design al hacer CUALQUIER trabajo de UI.** Reglas cross-cutting que se quedan inline:
  - **Hook obligatorio de diseño UI (ANY UI work):** usar `greenhouse-ai-design-studio` como orquestador canónico y cargar solo lanes disponibles aplicables. Antes de JSX nuevo: persistir fuente/dirección visual, comparar 2–3 direcciones, elegir receta/Composition Shell/primitives/tokens y registrar wireframe, flow y motion cuando corresponda. `UI ready: yes` exige mapping, estados/copy, decision log y plan GVC premium. Después del primer fold: capturar y mirar desktop+390px; al cierre deben pasar `design-contract:lint`, `ui:code-lint`, `ui:visual-gate` y `ui:quality`. Scorecard: promedio >=4.5, piso 4 y >=4.5 en jerarquía, economía de superficies, impacto visual, fidelidad y resistencia a template genérico. Card-on-card, card wallpaper o ausencia de un momento visual dominante son `BLOCK` aunque lint/build pasen. Canon: `docs/ui/GREENHOUSE_PREMIUM_UI_DELIVERY_STANDARD_V1.md`.
...
**⚠️ Prender un flag es MULTI-RUNTIME, no "prenderlo en Vercel".** Hay **7 runtimes con env vars independientes** (Vercel + 6 Cloud Run, incluido el Job `artifact-worker`). **NUNCA** prendas/apagues un flag sin mapear antes dónde se LEE (`grep -rn "<FLAG>" src/ services/`) y aplicarlo en **todos**; lo **async** (email, projection reactiva, consumer del outbox, cron) vive en el **`ops-worker`, NO en Vercel**. **En Cloud Run el SoT es `services/<worker>/deploy.sh`**: declarar el flag ahí (los `deploy.sh` usan `--set-env-vars` **destructivo**, que borra toda var agregada out-of-band) **y además** aplicarlo en vivo con `gcloud run services update … --update-env-vars` para efecto inmediato. Hacer sólo lo segundo = el flag desaparece en el próximo deploy, **en silencio**. **SIEMPRE** verificar en la **revisión activa** + ejercitar el flujo real, y declarar el runtime en la fila del ledger. Runbook: `FEATURE_FLAG_STATE_LEDGER.md`.

**Caso fuente 2026-07-09/10:** `GROWTH_EBOOK_EMAIL_DELIVERY_ENABLED` se lee **sólo** en el `ops-worker`; prenderlo en Vercel habría dejado el email muerto. Y prenderlo sólo con `--update-env-vars` (revisión `00470`) duró hasta que la `00473` lo borró: el consumer reactivo registró `skip: flag OFF` y el email nunca salió, mientras la success card del form le prometía el email al usuario. El ledger decía ON; la realidad era OFF.
...
- `docs/services/README.md` — catálogo canónico de servicios que Efeonce ofrece/opera: outcome, alcance,
  entregables, evidencia, responsabilidades y continuidad; no sustituye specs, manuales ni catálogos runtime.
...
- **Design System catalog canónico:** `/admin/design-system` y sus contratos viven en [`docs/architecture/ui-platform/README.md`](docs/architecture/ui-platform/README.md). Carga ese canon antes de agregar tokens, primitives, labs o rutas; conserva sus gates de acceso, alcanzabilidad, GVC y separación Greenhouse/Globe.
...
- `GREENHOUSE_CANONICAL_PATTERNS_V1.md` — los 9 patrones de implementación transversales. Leer antes de inventar una forma propia para algo que ya tiene patrón; el índice del doc los lista.
...
🔴 **Tres más de ISSUE-172 (2026-09-12):** `lpad(x, N, '0')` **RECORTA** pasado N (un id público `prefijo || lpad(nextval::text, 5, '0')` colisiona tras 99 999) y un `INSERT … SELECT … ON CONFLICT DO NOTHING` periódico **evalúa el DEFAULT secuencial por cada fila candidata** (~71k valores/día quemados) → función plpgsql con UN solo `nextval` + `lpad(n, GREATEST(N, length(n)), '0')` + anti-join `NOT EXISTS`; `uuid = text` no existe (comparar `col::text`); un backtick en un comentario `--` cierra el template literal. **SIEMPRE** ejercitar cada SQL nuevo contra PG real antes de commitear. Detalle: `SQL_DATE_MATH_AGENT_INVARIANTS.md` → §"Ids secuenciales, casts y comentarios".
