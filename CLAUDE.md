...
| Efeonce Globe / Creative Studio (repo hermano `efeonce-globe`) · EPIC-028 · API Contract Spine (TASK-1481) · agregar capability / command / reader / provider adapter · trusted context / dispatch / SDK · boundary Globe↔Greenhouse | `greenhouse-globe` (+ `arch-architect` para forma/decisiones) | `architecture/EFEONCE_CREATIVE_STUDIO_AGENTIC_PLATFORM_{DECISION,ARCHITECTURE}_V1.md` + `epics/in-progress/EPIC-028-*.md` + (repo hermano) `efeonce-globe/docs/architecture/EFEONCE_GLOBE_API_CONTRACT_SPINE_V1.md` |
