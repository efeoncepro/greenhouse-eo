...
import { resolveSisterPlatformBinding } from './bindings'
import type {
...
export type SisterPlatformRouteContext = {
  requestId: string
  routeKey: string
...
type SisterPlatformRouteHandler = (context: SisterPlatformRouteContext) => Promise<Response>

export class SisterPlatformExternalApiError extends Error {
  statusCode: number
  errorCode: string

  constructor(message: string, options?: { statusCode?: number; errorCode?: string }) {
    super(message)
    this.name = 'SisterPlatformExternalApiError'
...
const parseScopeQuery = (request: Request): ScopeQuery => {
  ...
  if (!externalScopeType) {
    throw new SisterPlatformExternalApiError('Missing required query param: externalScopeType', {
      statusCode: 400,
  ...
  if (!externalScopeId) {
    throw new SisterPlatformExternalApiError('Missing required query param: externalScopeId', {
      statusCode: 400,
...
const loadConsumerByToken = async (token: string) => {
  ...
  const row = (result as SisterPlatformConsumerRow[]).find(candidate => safeEquals(candidate.token_hash, tokenHash))
...
const authenticateConsumer = async (request: Request) => {
  ...
  if (!token) {
    throw new SisterPlatformExternalApiError('Unauthorized', {
      statusCode: 401,
  ...
  if (consumer.credentialStatus !== 'active') {
    throw new SisterPlatformExternalApiError('Consumer credential is not active.', {
      statusCode: 403,
  ...
  if (consumer.expiresAt && new Date(consumer.expiresAt).getTime() <= Date.now()) {
    throw new SisterPlatformExternalApiError('Consumer credential has expired.', {
      statusCode: 403,
...
const enforceScopeAccess = ({
  ...
  if (!consumer.allowedGreenhouseScopeTypes.includes(greenhouseScopeType)) {
    throw new SisterPlatformExternalApiError('Resolved binding scope is not allowed for this consumer.', {
      statusCode: 403,
...
const enforceRateLimit = async (consumer: SisterPlatformConsumerRecord) => {
  ...
  if (Number(counts.requests_last_minute || 0) >= consumer.rateLimitPerMinute) {
    throw new SisterPlatformExternalApiError('Rate limit exceeded for the current minute window.', {
      statusCode: 429,
  ...
  if (Number(counts.requests_last_hour || 0) >= consumer.rateLimitPerHour) {
    throw new SisterPlatformExternalApiError('Rate limit exceeded for the current hour window.', {
      statusCode: 429,
      errorCode: 'rate_limited'
    })
  }
...
const normalizeError = (error: unknown) => {
  if (error instanceof SisterPlatformExternalApiError) {
    return error
  }

  return new SisterPlatformExternalApiError('Internal server error', {
    statusCode: 500,
    errorCode: 'internal_error'
  })
}

const attachCommonHeaders = (response: Response, requestId: string) => {
...
export const runSisterPlatformReadRoute = async ({
  request,
  routeKey,
  handler
}: {
  request: Request
  routeKey: string
  handler: SisterPlatformRouteHandler
}) => {
  ...
  try {
    ...
    auditState.sisterPlatformKey = consumer.sisterPlatformKey
    ...
    if (!binding) {
      throw new SisterPlatformExternalApiError('No active sister platform binding found for the provided external scope.', {
        statusCode: 404,
    ...
    if (binding.bindingStatus !== 'active') {
      throw new SisterPlatformExternalApiError('Resolved sister platform binding is not active.', {
        statusCode: 403,
    ...
    await recordRequestLog({
      ...
      errorCode: response.status >= 400 ? 'handler_error' : null
    })

    return attachCommonHeaders(response, requestId)
  } catch (error) {
...
export const getRequestedIntegrationKeys = (request: Request) => {
  const { searchParams } = new URL(request.url)
  const keysParam = searchParams.get('keys')

  if (!keysParam) {
    throw new SisterPlatformExternalApiError('Missing required query param: keys (comma-separated integration keys)', {
      statusCode: 400,
      errorCode: 'missing_integration_keys'
    })
  }
  ...
  if (keys.length === 0) {
    throw new SisterPlatformExternalApiError('No valid integration keys provided.', {
      statusCode: 400,
      errorCode: 'invalid_integration_keys'
    })
  }
