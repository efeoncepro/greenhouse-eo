...
export type InternalIdentityBindingPayload = {
  population: 'internal'; outcome: 'bound' | 'denied'; cacheTtlSeconds: 0
  contextVersion: 1; authorizationContextId: string; reason?: string
  profileId?: string; organizationId?: string; bindingId?: string; grantsVersion?: number; capabilities?: readonly string[]
}
export type EcosystemIdentityBindingPayload = (ExternalAccessResolution & { cacheTtlSeconds: number }) | InternalIdentityBindingPayload
...
export const getEcosystemIdentityBindingPayload = async ({
  ...
  const contextKeys = ['authorizationContextId','contextVersion','grantsVersion','audience']
  const allKeys = ['environment','subject','clientId',...contextKeys]

  if (allKeys.some(key => searchParams.getAll(key).length > 1) ||
    (!searchParams.has('authorizationContextId') && contextKeys.some(key => searchParams.has(key)))) {
    throw new ApiPlatformError('Invalid authorization context parameters', {statusCode:400,errorCode:'bad_request'})
  }
  ...
  try {
    ...
    if (contextId !== null) {
      ...
      if (!/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(contextId) ||
        rawVersion !== '1' || !/^[1-9][0-9]*$/.test(rawGv) || version !== 1 || !Number.isSafeInteger(gv) || gv < 1 || !clientId ||
        environmentId !== config.environmentId || audience !== config.mcpAudience) {
      ...
      const result = await contexts.resolve({id:contextId,version,issuer:config.issuer,environmentId,subject,clientId,audience,grantsVersion:gv})

      const data: InternalIdentityBindingPayload = result.allowed ? {
        population:'internal',outcome:'bound',cacheTtlSeconds:0,contextVersion:1,authorizationContextId:contextId,
        profileId:result.context.profileId,organizationId:result.context.organizationId,bindingId:result.context.bindingId,
        grantsVersion:result.grantsVersion,capabilities:result.capabilities
      } : {population:'internal',outcome:'denied',cacheTtlSeconds:0,contextVersion:1,authorizationContextId:contextId,reason:result.reason}
