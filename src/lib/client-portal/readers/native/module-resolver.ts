...
} from '../../visibility/client-portal-view-visibility'
...
/**
 * TASK-1685 Slice 2 — la allowlist de vistas base se mudó al primitive de visibilidad.
 *
 * Vivía acá desde `TASK-1679` Slice 5, pero este módulo es `server-only` y el menú y el ⌘K
 * son Client Components: dejarla acá obligaba a que el cliente tuviera su propia copia, que
 * es el defecto que `TASK-1685` vino a cerrar. La declaración canónica —con su rationale
 * completo— vive ahora en `visibility/client-portal-view-visibility.ts`, que es puro.
 *
 * Se re-exporta para no romper los consumers existentes. **NUNCA** re-declarar la lista acá.
 */
export { CLIENT_PORTAL_BASE_VIEW_CODES, isClientPortalBaseViewCode }

/**
 * Devuelve `true` si el viewCode es una vista base del portal, o si algún módulo asignado al
 * cliente lo expone.
 *
 * ⚠️ **Esto NO es el primitive de visibilidad.** Responde sólo la mitad de la pregunta: la
 * dimensión ORGANIZACIÓN. No conoce la dimensión PERSONA (`user_view_overrides`) ni el bypass
 * interno. Para decidir si alguien puede ver una vista, usar `canOpenClientPortalView` de
 * `visibility/resolve-client-portal-visibility.ts`. Este helper queda para consumers que
 * genuinamente preguntan por la organización (listados de admin, diagnósticos).
 */
