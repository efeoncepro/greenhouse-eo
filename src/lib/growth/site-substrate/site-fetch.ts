...
/**
 * TASK-1266 / TASK-1778 / TASK-1697 — Growth · Site substrate fetcher (server-only).
 *
 * SUSTRATO DE SITIO (movido desde `ai-visibility/probes/safe-fetch.ts` por TASK-1697, git mv
 * con historial): dice cómo se OBTIENE la evidencia de un sitio — nunca cómo se JUZGA. No
 * importa `growth/*`, no persiste, no conoce el grader. Consumers entran por el barrel
 * `@/lib/growth/site-substrate`; los nombres `Probe*` sobreviven como alias en los shims.
 *
 ...
 * identificable (`SiteFetchInit.userAgent` varía NUESTRO token, NUNCA suplanta el de un
...
/**
 * TASK-1778 — Endurecimiento de RED del fetcher (redirect containment + guarda DNS).
 * El flag es DEL FETCHER y viaja con él (TASK-1697): no es un flag de dominio grader.
 * Default OFF = red previa; ON (cutover 2026-08-27) = modo estricto. Lectura pura de env.
 */
...
export const isProbeFetchStrictNetworkEnabled = (env: NodeJS.ProcessEnv = process.env): boolean =>
  env[GROWTH_PROBE_FETCH_STRICT_NETWORK_FLAG]?.trim().toLowerCase() === 'true'
...
const blockedResult = (url: string, errorCode: BlockedReason, host: string): SiteFetchResult => {
 ...
 */
export const createSiteFetcher = (baseUrl: string, deps: SiteFetcherDeps = {}): SiteFetcher => {
  ...
  let robotsFetchMemo: Promise<SiteFetchResult> | null = null
  ...
  return async (path: string, init: SiteFetchInit = {}): Promise<SiteFetchResult> => {
