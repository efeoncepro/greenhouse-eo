 ...
 *
 * `ai_summary` signals are excluded from aggregation: they are meta-descriptions
 * generated by the AI Observer based on a *previous* sweep snapshot, so they
 * lag the real signals (often by ~1 hour) and can be stale relative to current
 * state. Letting the AI summary drive module status produces false "not_configured"
 * or "awaiting_data" badges when the underlying signals are healthy. The AI
 * narrative still appears in the signal list as evidence; it just doesn't
 * outrank concrete signals.
 */
export const aggregateModuleStatus = (signals: ReliabilitySignal[]): ReliabilitySeverity => {
  const eligible = signals.filter(signal => signal.kind !== 'ai_summary')
