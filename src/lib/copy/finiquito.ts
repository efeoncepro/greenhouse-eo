...
export const GH_FINIQUITO = {
  resignation: {
    clauses: {
      ...
      cuartoAltA: ({ declaredByDisplayName, declaredAt, declaredByTaxId }: FiniquitoMaintenanceObligationParams) =>
        `CUARTO ALTERNATIVA A: Las partes comparecientes declaran expresamente que el(la) trabajador(a) NO se encuentra afecto a retención por pensión de alimentos de acuerdo con lo establecido en la Ley N° 14.908, modificada por la Ley N° 21.389 de 2021. Declaración efectuada con fecha ${declaredAt}, registrada en sistema Greenhouse por operador HR ${declaredByDisplayName}${declaredByTaxId ? ` — RUT ${declaredByTaxId}` : ''}.`,

      cuartoAltB: ({ amount, beneficiary, declaredByDisplayName, declaredAt, declaredByTaxId }: FiniquitoMaintenanceObligationParams) =>
        `CUARTO ALTERNATIVA B: Las partes comparecientes declaran expresamente que el(la) trabajador(a) SÍ se encuentra afecto a retención por pensión de alimentos de acuerdo con lo establecido en la Ley N° 14.908, modificada por la Ley N° 21.389 de 2021, por un monto mensual de $ ${amount ?? 0} a favor de ${beneficiary ?? '(beneficiario pendiente de declarar)'}. Declaración efectuada con fecha ${declaredAt}, registrada en sistema Greenhouse por operador HR ${declaredByDisplayName}${declaredByTaxId ? ` — RUT ${declaredByTaxId}` : ''}.`,
