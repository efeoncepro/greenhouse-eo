...
/**
 * Server-safe pricing-build module (TASK-1212).
 *
 * Originalmente en `src/views/greenhouse/finance/workspace/quote-builder-pricing.ts`
 * (acoplado al tipo `QuoteLineItem` del editor). Se reubicó a `src/lib` para que el
 * command canónico `submitQuoteFromBuilder` lo reuse server-side sin un import lib→view
 * invertido. La vista re-exporta desde aquí, así que el shell + editor + tests siguen
 * intactos. Es lógica pura (solo imports `type`) — NO lleva `server-only` porque el
 * cliente también la importa via la re-exportación de la vista.
 *
 * Tipo `QuoteBuilderLineDraft` = subconjunto estructural de `QuoteLineItem` (el editor
 * pasa `QuoteLineItem[]`, asignable a `QuoteBuilderLineDraft[]`).
 */

export type QuoteLineSource = 'catalog' | 'service' | 'template' | 'manual'

export interface QuoteBuilderLineDraft {
  label: string
  ...
  // Passthrough opcionales de `QuoteLineItem` (el editor pasa el objeto completo);
  // la lógica de build NO los lee, pero los declaramos para mantener el draft como
  // mirror estructural fiel y aceptar literales inline del editor/tests.
...
export interface PersistedQuoteLineItem {
  label: string
  lineType: QuoteBuilderLineDraft['lineType']
  unit: QuoteBuilderLineDraft['unit']
  quantity: number
  unitPrice: number
...
export const lineRequiresSuggestedPrice = (line: QuoteBuilderLineDraft) => {
  const v2Type = line.metadata?.pricingV2LineType
...
const describeLine = (line: QuoteBuilderLineDraft): string => {
  const sku = line.metadata?.sku ?? line.roleCode ?? line.memberId ?? null
