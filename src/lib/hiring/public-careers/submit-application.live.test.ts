...
describe.skipIf(!hasPgConfig)('submitPublicHiringApplication — live PG (TASK-1367)', () => {
  ...
  afterAll(async () => {
    ...
    if (state.profileId)
      await runGreenhousePostgresQuery(`DELETE FROM greenhouse_core.identity_profiles WHERE profile_id = $1`, [
        state.profileId
      ]).catch(() => undefined)
    if (state.openingId)
  ...
  it('opening no publicado / inexistente → not_open (genérico)', async () => {
    const parsed = parsePublicHiringApplication({
      openingPublicId: 'EO-OPN-inexistente',
      firstName: 'X',
      lastName: 'Y',
      email,
  ...
  it('apply válido → accepted; persiste Person + facet (source/consent/links) + application', async () => {
    ...
    const facet = await runGreenhousePostgresQuery<{
