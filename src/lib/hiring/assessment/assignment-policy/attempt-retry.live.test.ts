...
describe.skipIf(!hasPgConfig || !canCleanUp)('assignment attempt retry — live PG (TASK-1755)', () => {
  beforeAll(async () => {
    // Fixture AISLADO por archivo (2026-08-23). Antes esto era una consulta que tomaba
    // `ORDER BY ip.profile_id LIMIT 2` sobre un pool compartido de 3 perfiles sintéticos, y los
    // tres archivos de assignment-policy tomaban LOS MISMOS DOS: al correr en paralelo se pisaban
    // las postulaciones y las propuestas se invalidaban entre sí (`..._proposal_stale`). Aislados
    // pasaban. Ver la razón completa en `live-test-identity.ts`.
    const profiles = await resolveLiveTestCandidateFixtures('assignment-attempt-retry', 2)

    // El helper crea lo que falte, así que esto ya no puede quedar corto — pero se conserva como
    // aserción barata: si alguna vez devuelve menos, el fallo dice «faltó fixture» en vez de
    // reventar más abajo con un error de dominio confuso.
  ...
  it('el ciclo completo contra PG: policy en `draft` bloquea, habilitarla ASIGNA en el intento 2', async () => {
    ...
    // `toMatchObject` en vez de `.status` a secas: cuando esto falla, el mensaje trae el
    // `reasonCode` — que es LA pregunta («¿bloqueado por qué?»). Con `.status` el fallo decía
    // «expected 'blocked' to be 'assigned'» y obligaba a instrumentar para averiguar la causa.
    expect(assigned.result).toMatchObject({ status: 'assigned' })
