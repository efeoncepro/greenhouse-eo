 ...
 *
 * ISSUE-172 (2026-09-12): vivía bajo `hiring/assessment/access-recovery/` y lo necesitaba también
 * la plataforma de correo (`delivery.ts`: reintento y revive nunca insisten contra un buzón
 * bloqueado). Importarlo desde ahí invertía la dependencia (email → hiring), así que el módulo
 * se mueve a `src/lib/email/` —su tabla es `email_deliveries`— y el path de hiring lo re-exporta
