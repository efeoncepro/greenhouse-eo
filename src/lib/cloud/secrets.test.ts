...
describe('getCloudSecretsPosture', () => {
  ...
  it('summarizes sources without exposing values', async () => {
    ...
    const migrator = posture.entries.find(entry => entry.key === 'postgres_migrator_password')
    ...
    expect(posture.runtimeSummary).toContain('via Secret Manager')
    expect(posture.toolingSummary).toContain('sin configurar')
    expect(nubox?.source).toBe('secret_manager')
    expect(nubox?.secretRefConfigured).toBe(true)
    expect(nubox?.classification).toBe('runtime')
    expect(nextAuth?.source).toBe('env')
    expect(slack?.source).toBe('secret_manager')
    expect(migrator?.classification).toBe('tooling')
  })
})
