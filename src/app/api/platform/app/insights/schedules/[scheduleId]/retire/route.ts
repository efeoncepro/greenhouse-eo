...
export async function POST(request: Request, { params }: { params: Promise<{ scheduleId: string }> }) {
  ...
  return runAppCommandRoute({ request, routeKey: 'platform.app.insights.schedules.retire', body, handler: async context => transitionAppInsightSchedule({ context, request, body, scheduleId, action: 'retire' }) })
}
