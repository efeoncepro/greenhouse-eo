...
// Collaborator self view of own employment contracts. Honest status only, scoped to
// the session's own identity profile (anti-IDOR). No legal text body.
export async function GET() {
  ...
  try {
    const items = await getOwnContractingSummary(identityProfileId, 'employment_contract')

    return NextResponse.json({ items })
  } catch (error) {
    return mapContractingError(error, 'my_contracts')
  }
}
