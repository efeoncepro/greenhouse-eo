...
import { requireIdempotencyKey } from '@/app/api/admin/globe/credit-funding/shared'

export const dynamic = 'force-dynamic'

export const POST = async (request: Request, { params }: { params: Promise<{ operationId: string }> }) => {
  try {
    ...
    if (!tenant || !can(buildTenantEntitlementSubject(tenant), 'platform.globe_credit_funding.reconcile', 'execute', 'all')) {
    ...
    if (!globeWorkspaceId || !operationId || !idempotencyKey) {
    ...
    const operation = await reconcileGlobeCreditFundingOperation({ globeWorkspaceId, operationId, idempotencyKey })
