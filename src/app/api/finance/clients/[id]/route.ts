...
export async function GET(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const rows = await runFinanceQuery<ClientProfileRow>(`
    SELECT
      ...
      COALESCE(fp_client.hubspot_company_id, fp_hubspot.hubspot_company_id, CAST(gc.hubspot_company_id AS STRING)) AS hubspot_company_id,
      ...
      COALESCE(fp_client.tax_id_type, fp_hubspot.tax_id_type) AS tax_id_type,
      COALESCE(fp_client.legal_name, fp_hubspot.legal_name, ${companyExpressions.nameExpr}, gc.client_name) AS legal_name,
      COALESCE(fp_client.billing_address, fp_hubspot.billing_address) AS billing_address,
      COALESCE(fp_client.billing_country, fp_hubspot.billing_country, 'CL') AS billing_country,
      ...
      COALESCE(fp_client.current_po_number, fp_hubspot.current_po_number) AS current_po_number,
      COALESCE(fp_client.current_hes_number, fp_hubspot.current_hes_number) AS current_hes_number,
      COALESCE(fp_client.finance_contacts, fp_hubspot.finance_contacts) AS finance_contacts,
      COALESCE(fp_client.special_conditions, fp_hubspot.special_conditions) AS special_conditions,
      COALESCE(fp_client.created_by, fp_hubspot.created_by) AS created_by,
  ...
  const invoices = await runFinanceQuery<InvoiceRow>(`
    ...
    LIMIT 50
  `, { clientProfileId, hubspotCompanyId })
