...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!can(tenant, 'finance.expenses.record_payment', 'create', 'tenant')) {
    return NextResponse.json({ error: 'No tienes permiso para registrar pagos de gasto.', code: 'forbidden' }, { status: 403 })
  }
