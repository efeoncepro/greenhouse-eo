...
const mockGetCostAllocationsByOrganization = vi.fn()
...
vi.mock('@/lib/finance/postgres-store-intelligence', () => ({
  ...
  getCostAllocationsByOrganization: (...args: unknown[]) => mockGetCostAllocationsByOrganization(...args),
...
describe('allocations route org-first contract', () => {
  beforeEach(() => {
    ...
    mockGetCostAllocationsByOrganization.mockResolvedValue([])
  ...
  it('lists allocations by organization when no legacy client bridge is needed', async () => {
    mockResolveFinanceDownstreamScope.mockResolvedValueOnce({
      clientId: null,
      clientProfileId: 'profile-1',
      hubspotCompanyId: 'hubspot-1',
      clientName: 'Sky Airline',
    ...
    const response = await GET(
      new Request('http://localhost/api/finance/intelligence/allocations?organizationId=org-1&year=2026&month=4')
    )

    expect(response.status).toBe(200)
    expect(mockGetCostAllocationsByOrganization).toHaveBeenCalledWith('org-1', 2026, 4)
    expect(mockGetCostAllocationsByClient).not.toHaveBeenCalled()
  })
})
