...
export async function GET(request: Request) {
  ...
  const incomeRows = await runGreenhousePostgresQuery<PnlRow>(
    `SELECT
  ...
  const expenseRows = await runGreenhousePostgresQuery<PnlRow>(
    `SELECT
       cost_category,
       COALESCE(SUM(total_amount_clp), 0) AS total_clp,
       COUNT(*) AS record_count
  ...
  const payrollRows = await runGreenhousePostgresQuery<PnlRow>(
    `SELECT
  ...
  const directLabor = roundCurrency(expenseByCat['direct_labor'] || 0)
  ...
  const netResult = roundCurrency(netRevenue - roundCurrency(totalExpenses))
