...
describe('createGreenhouseMcpHandlers', () => {
  it('preserves requestId and apiVersion on successful tool results', async () => {
    const handlers = createGreenhouseMcpHandlers({
      ...
      getSeoDomainOverview: vi.fn(),
      getSeoUrlVisibility: vi.fn(),
      getSeoBacklinkDetail: vi.fn(),
      getSeoKeywordDiscovery: vi.fn(),
      discoverSeoKeywords: vi.fn(),
      ...
      createInsightShare: vi.fn(),
      listInsightShares: vi.fn(),
      revokeInsightShare: vi.fn(),
      listInsightDeliveries: vi.fn(),
