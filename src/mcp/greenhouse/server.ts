...
export const createGreenhouseMcpServer = (
  ...
  collector.registerTool(
    'get_seo_prospect_diagnostic',
    {
      ...
      description:
        'Read SEO prospect diagnostics (a one-shot, provider-only diagnostic of a domain WITHOUT client access). Pass diagnosticId for full facts, or rootDomain/limit to list. Every figure is ESTIMATED (external provider, with capturedAt) — always report the lens and the as-of date, NEVER present a figure as measured, and NEVER assert the site is healthy: the diagnostic enumerates quantified loss, it does not certify health. ETV METHODOLOGY (TASK-1805) — each diagnostic carries etvMethodology (formula behind estimated_monthly_traffic; legacy_static_v1 today, improved_layout_clickstream_v2 mandatory from 2026-11-01T00:00:00Z) and that fact declares sampleRows/rowLimit/truncated: when truncated is true the sum is a FLOOR, not the total. Never compare traffic figures across different methodology versions.',
      inputSchema: {
