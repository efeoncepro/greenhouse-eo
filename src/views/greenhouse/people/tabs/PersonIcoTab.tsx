...
const PersonIcoTab = ({ memberId }: Props) => {
      ...
      {loading ? (
        ...
        <Grid size={{ xs: 12 }}>
          <EmptyState
            ...
            description={`No hay datos del ICO Engine para este colaborador en ${MONTH_SHORT[month]} ${year}. Las métricas se calculan a partir de las tareas asignadas en Notion.`}
          />
        </Grid>
      ) : (
        <>
          {/* KPI Row */}
          <Grid size={{ xs: 12 }}>
            <SectionErrorBoundary sectionName='person-ico-kpis' description='No pudimos calcular los KPIs del ICO Engine.'>
              <Grid container spacing={6}>
                {KPI_CONFIG.map(kpi => {
                  ...
                  return (
                    <Grid key={kpi.id} size={{ xs: 6, sm: 4, md: 2 }}>
                      <HorizontalWithSubtitle
                        title={kpi.format(value)}
                        subtitle={kpi.label}
                        icon={kpi.icon}
                        color={getZoneColor(metric?.zone ?? null)}
                      />
                    </Grid>
                  )
                })}
          ...
          <Grid size={{ xs: 12, md: 5 }}>
            <SectionErrorBoundary sectionName='person-ico-csc' description='No pudimos cargar la distribución CSC.'>
              <ExecutiveCardShell
                title='Distribución CSC'
                subtitle='Activos asignados por fase'
              >
                {cscEntries.length === 0 ? (
                  <EmptyState
          ...
          <Grid size={{ xs: 12, md: 4 }}>
            <SectionErrorBoundary sectionName='person-ico-radar' description='No pudimos cargar el radar de salud.'>
              <ExecutiveCardShell
          ...
          <Grid size={{ xs: 12, md: 3 }}>
            <SectionErrorBoundary sectionName='person-ico-velocity' description='No pudimos cargar la velocidad del pipeline.'>
              <ExecutiveCardShell
