...
// TASK-960 mockup — in-app MUI viewer for the Contractor Remittance Advice.
// Renders a RemittancePresentation struct. In implementation this same struct
// also feeds react-pdf (download) → zero content drift. Sober legal/financial
// aesthetic (mirrors generate-payroll-pdf tokens). No signature (a remittance
// advice does not require one).
...
import type { RemittancePresentation } from './remittance-data'
            ...
            {breakdown
              ...
              .map(row => (
                <Stack key={row.id} direction='row' justifyContent='space-between' alignItems='baseline'>
                  ...
                  <Typography
                    variant='h6'
                    sx={{ fontWeight: 700, fontVariantNumeric: 'tabular-nums', color: '#2E7D32' }}
                  >
                    {money(row.amount, row.currency)}
                  </Typography>
                </Stack>
