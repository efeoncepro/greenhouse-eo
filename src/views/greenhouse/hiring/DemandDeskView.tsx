...
const SENIORITIES = HIRING_PUBLIC_SENIORITIES
...
const formForTemplate = (value: string): DemandForm => {
  if (value === 'account-manager') {
    return {
      ...
      area: 'Growth',
      seniority: 'Junior',
      skills: 'Account mgmt, Upsell, QBR',
      summary: 'Lidera las cuentas clave de Growth y su expansión.'
    }
  }
  ...
  if (value === 'growth-designer') {
    return {
      ...
      area: 'Growth',
      seniority: 'Semi-senior',
      skills: 'Figma, CRO, Experimentación',
      summary: 'Diseña experiencias digitales que combinan craft, datos y velocidad de aprendizaje.'
    }
  }
  ...
  if (value === 'data-engineer') {
    return {
      ...
      area: 'Technology',
      seniority: 'Senior',
      skills: 'SQL, Python, BigQuery',
      summary: 'Construye productos de datos confiables para acelerar decisiones y automatización.'
    }
  }
...
const DemandKpi = ({
  ...
  <Paper
    ...
    sx={theme => ({
      ...
      borderColor: alpha(theme.palette[tone].main, 0.16),
      background: `linear-gradient(145deg, ${theme.palette.background.paper} 0%, ${alpha(theme.palette[tone].main, 0.035)} 100%)`,
      boxShadow: `0 18px 44px ${alpha(theme.palette.common.black, 0.045)}`,
      animation: 'ghHiringUp 300ms cubic-bezier(.2,0,0,1)',
      ...
      '&::before': {
        ...
        insetInline: 0,
        blockSize: 3,
        background: `linear-gradient(90deg, ${alpha(theme.palette[tone].main, 0.9)}, ${alpha(theme.palette[tone].main, 0.18)})`,
        opacity: 0.72
      },
      '&::after': {
        content: '""',
        ...
        blockSize: 124,
        borderRadius: '50%',
        background: `radial-gradient(circle, ${alpha(theme.palette[tone].main, 0.12)} 0%, transparent 68%)`,
        pointerEvents: 'none'
      },
      '&:hover': {
        borderColor: alpha(theme.palette[tone].main, 0.32),
        boxShadow: `0 22px 54px ${alpha(theme.palette.common.black, 0.075)}`,
  ...
  >
    ...
    >
      <Box sx={{ minWidth: 0 }}>
        <Typography variant='body2' color='text.secondary' fontWeight={700} sx={{ overflowWrap: 'anywhere' }}>
          {label}
        </Typography>
        <Typography
          className='gh-demand-kpi-value'
          variant='h3'
          sx={{ mt: 4.25, lineHeight: 1, fontVariantNumeric: 'tabular-nums', letterSpacing: '-.02em' }}
        >
          {value}
        </Typography>
        <Stack className='gh-demand-kpi-delta' direction='row' alignItems='center' spacing={0.75} sx={{ mt: 2 }}>
          {splitDelta(delta).positive ? (
            <i aria-hidden='true' className='tabler-trending-up text-success' style={{ fontSize: 14 }} />
          ) : null}
          <Typography
            variant='caption'
            color={splitDelta(delta).positive ? 'success.main' : 'text.secondary'}
            fontWeight={750}
          >
            {splitDelta(delta).value}
          </Typography>
          {splitDelta(delta).context ? (
      ...
      <Box
        ...
        sx={theme => ({
          ...
          boxShadow: `inset 0 0 0 1px ${alpha(theme.palette[tone].main, 0.16)}`
        })}
      >
        <i aria-hidden='true' className={icon} style={{ fontSize: 23 }} />
      </Box>
    </Stack>
  </Paper>
)
...
const DemandDeskView = ({
  ...
  const [form, setForm] = useState<DemandForm>(() =>
    initialDrawerTemplate ? formForTemplate(initialDrawerTemplate) : EMPTY_FORM
  )
  ...
  const availableAreas = useMemo(
    () =>
      [
        ...new Set(
          snapshot.openings.map(({ opening, demand }) => opening.publicArea ?? demand.businessUnit).filter(Boolean)
        )
      ] as string[],
    [snapshot.openings]
  )
  ...
  const visibleOpenings = useMemo(
    () =>
      snapshot.openings.filter(({ opening, demand }) => {
  ...
  const effectiveLoadError =
    loadError ||
    (demandView === 'error'
      ? 'Verifica tu conexión e intenta de nuevo. Si persiste, el equipo ya fue notificado.'
      : null)
    ...
    async (nextQuery = query) => {
      ...
      try {
        if (simulateLoadFailure)
          throw new Error('No pudimos cargar las demandas. Verifica tu conexión e intenta de nuevo.')
    ...
    <Stack spacing={4} sx={{ minWidth: 0, animation: 'ghHiringFade 240ms cubic-bezier(.2,0,0,1)' }}>
      <Grid container spacing={4}>
        <Grid size={{ xs: 12, sm: 6, lg: 3 }}>
          <DemandKpi
            label={copy.demand.activeDemands}
            value={pipelineMetrics.active}
        ...
        <Grid size={{ xs: 12, sm: 6, lg: 3 }}>
          <DemandKpi
            label={copy.demand.openPositions}
            value={pipelineMetrics.evaluation}
        ...
        <Grid size={{ xs: 12, sm: 6, lg: 3 }}>
          <DemandKpi
            label={copy.demand.applicants}
            value={pipelineMetrics.pendingDecision}
            delta='+1 esperando decisión'
            icon='tabler-gavel'
            tone='secondary'
          />
        </Grid>
        <Grid size={{ xs: 12, sm: 6, lg: 3 }}>
          <DemandKpi
            label={copy.demand.published}
            value={pipelineMetrics.selectedThisMonth}
      ...
      <Paper
        variant='outlined'
        data-capture='hiring-demand-toolbar'
        sx={theme => ({
          position: 'relative',
          overflow: 'hidden',
          p: { xs: 3, md: 3.25 },
          ...
          background: `linear-gradient(135deg, ${theme.palette.background.paper} 0%, ${alpha(theme.palette.primary.main, 0.025)} 48%, ${alpha(theme.palette.background.paper, 0.96)} 100%)`,
          ...
          '&::before': {
            ...
            insetBlock: 0,
            inlineSize: 3,
            background: `linear-gradient(180deg, ${theme.palette.primary.main}, ${alpha(theme.palette.primary.main, 0.18)})`,
            opacity: 0.78
          },
      ...
      >
        ...
        >
          <Stack
            ...
            useFlexGap
            flexWrap='wrap'
            sx={{ flex: '1 1 600px', minWidth: 0 }}
          >
            <Box sx={{ inlineSize: { xs: '100%', md: 300 }, flex: { xs: '1 1 auto', md: '0 1 300px' } }}>
              <TextField
                ...
                sx={theme => ({
                  '& .MuiOutlinedInput-root': {
                    ...
                    '&:hover': {
                      boxShadow: `0 8px 22px ${alpha(theme.palette.common.black, 0.055)}`
                    },
                    '&.Mui-focused': {
                  ...
                  input: {
                    ...
                    endAdornment: loading ? (
                      <InputAdornment position='end'>
                        <CircularProgress size={18} aria-label={copy.common.loading} />
                      </InputAdornment>
                    ) : undefined
                  }
                }}
            ...
            <FormControl
              ...
              sx={theme => ({
                ...
                '& .MuiOutlinedInput-root': {
                  ...
                  '&:hover': { boxShadow: `0 8px 22px ${alpha(theme.palette.common.black, 0.045)}` },
                  '&.Mui-focused': { boxShadow: `0 0 0 3px ${alpha(theme.palette.primary.main, 0.12)}` }
                }
              })}
            >
              <InputLabel id='demand-status-filter' sx={{ display: { xs: 'none', sm: 'block' } }}>
                Estado
              </InputLabel>
              <Select
                labelId='demand-status-filter'
                label='Estado'
                value={statusFilter}
              ...
              >
                ...
                <MenuItem value='active'>Abierta</MenuItem>
            ...
            >
              <InputLabel id='demand-area-filter' sx={{ display: { xs: 'none', sm: 'block' } }}>
                Área
              </InputLabel>
              <Select
                labelId='demand-area-filter'
                label='Área'
                value={areaFilter}
                onChange={event => setAreaFilter(event.target.value)}
                inputProps={{ 'aria-label': 'Área' }}
              >
            ...
            >
              ...
              <Box
                ...
                sx={theme => ({
                  ...
                  borderRadius: `${theme.shape.customBorderRadius.md}px`,
                  backgroundColor: theme.palette.background.paper,
                  boxShadow: `0 8px 18px ${alpha(theme.palette.common.black, 0.035)}`
                })}
              >
                {(
                  [
                  ...
                  <Box
                    ...
                    sx={theme => ({
                      ...
                      '&:hover': {
                        backgroundColor: demandView === value ? 'primary.dark' : 'action.hover'
                      }
                    })}
                  >
                    {label}
      ...
      <Paper
        ...
        sx={theme => ({
          ...
          background: `linear-gradient(180deg, ${theme.palette.background.paper} 0%, ${alpha(theme.palette.primary.main, 0.018)} 100%)`,
      ...
      >
        <Stack
          ...
          sx={theme => ({
            ...
            background: `linear-gradient(90deg, ${alpha(theme.palette.primary.main, 0.035)}, ${theme.palette.background.paper} 48%)`
          })}
        >
          <Box>
            <Stack direction='row' alignItems='center' spacing={1.5}>
              ...
              >
                <i aria-hidden='true' className='tabler-briefcase-2' style={{ fontSize: 17 }} />
              </Box>
              <Typography variant='h6'>{copy.demand.openingsTitle}</Typography>
            </Stack>
            <Typography variant='body2' color='text.secondary' sx={{ mt: 0.75 }}>
              1 opening activo · foco del programa
            </Typography>
          </Box>
        </Stack>
        {effectiveLoadError ? (
          ...
          <Stack alignItems='center' spacing={3} sx={{ px: 6, py: 13, textAlign: 'center' }}>
            ...
            <Typography color='text.secondary'>
              {query ? copy.demand.filteredEmptyBody : copy.demand.emptyBody}
            </Typography>
          </Stack>
        ) : (
            ...
            <Table
              ...
              sx={theme => ({
                ...
                '& .MuiTableCell-head': {
                  ...
                  borderBlockEnd: `1px solid ${alpha(theme.palette.divider, 0.92)}`
                },
                '& .MuiTableCell-body': {
                  px: 4,
                  py: 3.75,
                ...
                '& .MuiTableBody-root .MuiTableRow-root:focus-within': {
                  ...
                  boxShadow: `inset 3px 0 0 ${theme.palette.primary.dark}`
                }
              })}
            >
              <TableHead>
                  ...
                  <TableRow key={opening.openingId} hover>
                    <TableCell>
                      <Stack spacing={0.5}>
                        ...
                        <Typography variant='caption' color='text.secondary'>
                          {opening.publicId} · {opening.seniority ?? '—'}
                        </Typography>
                      </Stack>
                    </TableCell>
                    ...
                    <TableCell>
                      <GreenhouseChip
                        ...
                        kind='status'
                        variant='label'
                        tone={STATUS_TONE[opening.status]}
                        label={STATUS_LABEL[opening.status]}
                      />
                    </TableCell>
                    <TableCell>
                      <GreenhouseChip
                        ...
                        variant='label'
                        tone={
                          opening.publicationStatus === 'published'
                            ? 'success'
                            : opening.publicationStatus === 'paused'
                              ? 'warning'
                              : opening.publicationStatus === 'ready_for_review'
                                ? 'info'
                                : 'default'
                        }
                        iconClassName={
                        ...
                        label={PUBLICATION_LABEL[opening.publicationStatus]}
                      />
                    </TableCell>
    ...
    <>
      ...
      <Drawer
        ...
        slotProps={{
          backdrop: {
            sx: {
              animation: 'ghHiringFade 160ms cubic-bezier(.2,0,0,1)'
            }
          }
        }}
        PaperProps={{
      ...
      >
        <Stack sx={{ minBlockSize: '100%' }}>
          ...
          >
            <Box>
              ...
              <Typography color='text.secondary' sx={{ mt: 0.5 }}>
                {copy.demand.drawerSubtitle}
              </Typography>
            </Box>
            <IconButton aria-label={copy.common.close} onClick={requestClose}>
          ...
          <Stack
            component='section'
            role='region'
            aria-label={copy.common.demandFormRegion}
            tabIndex={0}
            spacing={4.5}
          ...
          >
            ...
            <Grid container spacing={3.5}>
              <Grid size={{ xs: 12, sm: 6 }}>
                <FormControl fullWidth required>
                  ...
                  <Select
                    labelId='hiring-area-label'
                    label={copy.demand.areaLabel}
                    value={form.area}
              ...
              <Grid size={{ xs: 12, sm: 6 }}>
                <FormControl fullWidth required>
                  ...
                  <Select
                    labelId='hiring-seniority-label'
                    label={copy.demand.seniorityLabel}
                    value={form.seniority}
                    onChange={event => updateForm('seniority', event.target.value)}
                  >
            ...
            <Box>
              ...
              <TextField
                fullWidth
                label={copy.demand.skillsLabel}
                value={form.skills}
                onChange={event => updateForm('skills', event.target.value)}
                helperText='Escribe y separa cada skill con una coma.'
              />
            </Box>
            <Typography variant='overline' color='text.secondary' sx={{ letterSpacing: '0.08em' }}>
              Contexto
            ...
            <Grid container spacing={3.5}>
              <Grid size={{ xs: 12, sm: 8 }}>
                <TextField
                  fullWidth
                  label={copy.demand.businessUnitLabel}
            ...
            <Grid container spacing={3.5}>
              <Grid size={{ xs: 12, sm: 6 }}>
                <FormControl fullWidth>
                  ...
                  <Select
                    labelId='hiring-mode-label'
                    label={copy.demand.modeLabel}
                    value={form.workMode}
                    onChange={event => updateForm('workMode', event.target.value as HiringPublicWorkMode)}
                  >
              ...
              <Grid size={{ xs: 12, sm: 6 }}>
                <FormControl fullWidth>
                  ...
                  <Select
                    labelId='hiring-manager-label'
                    label={copy.demand.owner}
                    value={form.manager}
                    onChange={event => updateForm('manager', event.target.value)}
                  >
                    {MANAGERS.map(([value, label]) => (
            ...
            <TextField
              type='date'
              label={copy.demand.targetDateLabel}
              ...
              <Stack spacing={2}>
                <Box>
                  <Typography variant='subtitle1' fontWeight={650}>
                    {copy.demand.internalCompensation}
                  </Typography>
                  <Typography variant='caption' color='text.secondary'>
                    {copy.demand.compensationHint}
                  </Typography>
                </Box>
                <Grid container spacing={3.5}>
                  <Grid size={{ xs: 6 }}>
                    <TextField
                      ...
                      onChange={event => updateForm('compensationMin', event.target.value)}
                    />
                  </Grid>
                  <Grid size={{ xs: 6 }}>
                    <TextField
                      ...
                      onChange={event => updateForm('compensationMax', event.target.value)}
                    />
                  </Grid>
                </Grid>
              </Stack>
            ...
            >
              <Stack
                ...
                alignItems='center'
                spacing={1.75}
                sx={{ px: 3.5, py: 2.25, backgroundColor: 'action.hover', borderBlockEnd: 1, borderColor: 'divider' }}
              >
                <i aria-hidden='true' className='tabler-eye text-success' />
                <Typography variant='overline' color='text.secondary' sx={{ letterSpacing: '.05em' }}>
              ...
              <Stack spacing={2} sx={{ p: 3.5 }}>
                ...
                <Typography variant='caption' color='text.secondary'>
                  {form.area || 'Growth'} · {form.businessUnit || 'Chile'} ·{' '}
                  {form.workMode === 'remote' ? 'Remoto' : form.workMode === 'hybrid' ? 'Híbrido' : 'Presencial'} ·{' '}
                  {form.seniority}
                </Typography>
                ...
                {skillTokens.length > 0 ? (
                  <Stack direction='row' spacing={1} flexWrap='wrap' useFlexGap>
                    {skillTokens.map(skill => (
                      <GreenhouseChip key={skill} size='small' kind='attribute' label={skill} />
                    ))}
                  </Stack>
                ) : null}
              </Stack>
            ...
            {discardOpen ? (
              ...
              >
                ...
                <Stack direction='row' justifyContent='flex-end' spacing={2}>
                  <Button variant='text' onClick={() => setDiscardOpen(false)}>
                    {copy.demand.discardContinue}
                  </Button>
                  <Button
                    color='error'
                    variant='contained'
                  ...
                  <Button
                    ...
                    startIcon={
                      submitting ? (
                        ...
                        <i aria-hidden='true' className='tabler-plus' />
                      )
                    }
                    sx={theme => ({
                      minBlockSize: 38,
