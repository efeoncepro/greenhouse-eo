...
describe('SampleSprintsExperienceView copy governance', () => {
  ...
  it('shows the approved approval state without enabling a duplicate approval action', async () => {
    const fetchMock = vi.fn(async () => new Response(JSON.stringify({
      ...
      approval: {
        ...
        status: 'approved',
        approvedAt: '2026-06-20T17:36:41.273Z'
      }
    }), {
      status: 200,
    ...
    expect(await screen.findByText('Sample Sprint aprobado')).toBeInTheDocument()
    expect(screen.getByText('El engagement ya está aprobado y puede operar. No se generó un segundo evento de aprobación.')).toBeInTheDocument()
    expect(screen.getByRole('button', { name: /Aprobar Sprint/i })).toBeDisabled()
