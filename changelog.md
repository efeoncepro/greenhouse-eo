...
- **TASK-777 backend parcial en progreso** — Canonical Expense Distribution & Shared Cost Pools. Se agrega fact canónico `expense_distribution_resolution` + policy/suggestions, resolver determinístico `expense -> distribution_lane`, CLI de materialización y signals de contaminación/unresolved. Abril 2026 fue rematerializado: SKY overhead baja a `$2.278.629,39`, ANAM queda en `$759.543,13`, y Deel/provider payroll deja de entrar como direct overhead de SKY. Cash/bancos/conciliación/payment orders no fueron mutados; drift CLP verificado `0`.
