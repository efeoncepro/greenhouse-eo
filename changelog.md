...
- **TASK-408 Slice 3A — templates auth migrados con fallback seguro.** Se agrega `selectEmailTemplateCopy()` para que los templates puedan consumir dictionary `es` sin degradar `en` mientras `en-US` siga como mirror. `VerifyEmail` y `MagicLinkEmail` usan `getMicrocopy().emails.auth.*` con snapshot byte-estable.
