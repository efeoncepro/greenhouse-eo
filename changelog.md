...
- **CI smoke-lane publisher hardening.** `pnpm sync:smoke-lane` ahora carga el shim canonico `scripts/lib/server-only-shim.cjs`, igual que los scripts operativos server-side. Esto corrige los warnings no bloqueantes `sync:smoke-lane <lane> failed` en Playwright causados por imports `server-only` desde `tsx`, sin cambiar el comportamiento de la suite E2E ni el modelo best-effort del publisher.
