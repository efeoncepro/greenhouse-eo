...
- **TASK-861 — HubSpot Production Release Drift Hardening.** Se endurece el Release Control Plane para que `hubspot-greenhouse-integration` no quede fuera del contrato anti-regresion: `concurrency-fix-verification.test.ts` ahora cubre los 4 workers Cloud Run, incluyendo workflow_call/dispatch/skip_tests de HubSpot y el wiring de `production-release.yml` con `expected_sha=${{ inputs.target_sha }}`. `platform.release.worker_revision_drift` agrega evidence `recommended_action` para drift HubSpot con el comando canonico `gh workflow run hubspot-greenhouse-integration-deploy.yml --ref main -f environment=production -f expected_sha=<release target_sha> -f skip_tests=false`; el watchdog Teams consume esa evidence si existe. Docs/skills/runbooks actualizados para prohibir SQL directo sobre `release_manifests` y exigir `/health`, `/contract` y watchdog `drift_count=0`. No cambia `push:main`, Vercel, region, URL, rutas, webhooks, payloads, secretos ni comportamiento runtime del bridge HubSpot.
