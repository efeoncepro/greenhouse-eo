...
- **TASK-408 Slice 3A — primer template auth migrado con fallback seguro.** Se agrega `selectEmailTemplateCopy()` para que los templates puedan consumir dictionary `es` sin degradar `en` mientras `en-US` siga como mirror, y `VerifyEmail` pasa a usar `getMicrocopy().emails.auth.verifyEmail` con snapshot byte-estable.
