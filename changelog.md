...
- **CI smoke-lane publisher hardening.** `pnpm sync:smoke-lane` ahora carga el shim canonico `scripts/lib/server-only-shim.cjs`, igual que los scripts operativos server-side, y Playwright publica con `GREENHOUSE_POSTGRES_PASSWORD_SECRET_REF=greenhouse-pg-dev-app-password` para usar el resolver canonico de Secret Manager. Esto corrige los warnings no bloqueantes `sync:smoke-lane <lane> failed` causados por imports `server-only` desde `tsx` y por referencias `secret:version` invalidas, sin cambiar el comportamiento de la suite E2E ni el modelo best-effort del publisher.
