...
| `efeonce-creator-influence-brief` | quote_request | Brief de Influencer Marketing, Creators y UGC | `/servicios/agencia-de-influencers/` · surface `fhsf-efeonce-creator-influence` | ✅ default | `gh_form_submission_accepted` → `generate_lead` | sí (previsto) | ✅ genérico | TASK-1598. Captura gobernada en Greenhouse; HubSpot directo conserva el gate vigente del sitio público. |
| `efeonce-content-marketing` | quote_request | Conversación de Content Marketing | `/servicio-marketing-de-contenidos/` · `fhsf-efeonce-content-marketing` | ✅ default | `gh_form_submission_accepted` → `generate_lead` | sí (previsto) | ◐ transporte sintético verificado; E2E pendiente | 2026-08-31: v3, Greenhouse-only, cero destinos/filas antes y después. Turnstile bloqueó el intento UI antes del POST; API rechazó honeypot y token ausente. `generate_lead` sintético (`smoke-test`, `measurement-smoke-content-marketing`) llegó a `G-KYPPY57M14` con 204. No es una aceptación de esta form ni confirmación Realtime; excluir smoke de métricas comerciales. [Evidencia](../../audits/public-site/2026-08-31-content-marketing-technical-closure.md). |
| `efeonce-hubspot-scope` | quote_request | Reunión de alcance HubSpot | `/servicios-contratar-hubspot/` · surface `fhsf-efeonce-hubspot-scope` | ✅ default | `gh_form_submission_accepted` → `generate_lead` | sí (previsto) | ⏳ smoke de conversión pendiente | Publicado 2026-08-30. Greenhouse-only; carga/API/validación verificadas sin crear un lead aceptado. |
...
> **Estado global (2026-07-07): LIVE.** Pipeline **genérico** publicado en `GTM-NGHPGRLZ` v2: trigger `CE - gh_form_submission_accepted` + tag `GA4 Event - generate_lead` (`measurementIdOverride=G-KYPPY57M14`) con params `form_slug`/`form_kind`/`surface_id`. Cubre TODOS los forms; se distinguen por `form_slug`. GA4: custom dimensions `form_slug`/`form_kind`/`surface_id` + key event `generate_lead` registrados. Verificado con Playwright (`/g/collect?...en=generate_lead`). Un form nuevo puede reutilizar ese pipeline sin crear otra etiqueta; publicar el form no prueba medición efectiva en su surface. Confirma `gh_form_submission_accepted` y el envío GA4 real antes de marcar su fila como verificada. Detalle + gotchas: [`LEARNINGS.md`](LEARNINGS.md).
...
>
> **Delta 2026-07-21 (TASK-1431/TASK-1510):** el param existente **`action_kind`** admite 5 valores (`open_growth_form` | `link_url` | `open_think_tool` | `book_meeting` | `open_meeting_scheduler`). CERO eventos/params nuevos para el CTA: la apertura del scheduler sigue el rail `greenhouse_cta_*` y usa `open_meeting_scheduler`; su funnel y conversión pertenecen al contrato `gh_meeting_*` descrito abajo. El destino (URL) JAMÁS viaja en telemetría — sólo `action_kind` allowlisted. `book_meeting` continúa navigation-only: ningún click crea Meeting/Contact/Deal ni equivale a una conversión.
...
| Surface id | Kind | Página / ubicación | Emite dataLayer | Evento GTM → GA4 | Key event | Tagging | Notas |
|---|---|---|---|---|---|---|---|
| `fhsf-efeonce-lead-gen-web` + binding `discovery` | native_meeting_scheduler | `/agenda/` piloto público `noindex`; otros placements por host aún no promovidos | ✅ renderer y funnel live; conversión no ejercitada | `gh_meeting_step_reached` → custom homónimo; `gh_meeting_booking_confirmed` → `generate_lead` con `lead_source=meeting_booking` | sólo `generate_lead` existente | ⏳ workspace 6 previewado; sin versión/publicación | El custom `gh_meeting_booking_confirmed` NO se reenvía a GA4. Piloto native-only liberado en `fbe8a9c76a74` (run `29854833210`): cero enlaces/copy HubSpot y recuperación por navegación mensual/`Reintentar`. Ledger `server_confirmed` es SoT; GA es mirror browser-reported. Pendientes: booking controlado/replay, evidencia live `/g/collect`, confirmación humana y publish. |

Allowlist funnel: `meeting_step`, `scheduler_key`, `surface_id`, `placement`, `availability_state`, `days_ahead_bucket`, `time_of_day_bucket`, `error_category`, `presentation_variant` y `activation_mode`. Las dos últimas describen contexto de exposición (`launcher|guided|split|command` e `inline|dialog|full_screen|page`), no pasos del funnel. Se prohíben PII, valores de campos, slot/timestamp/timezone exactos, receipt, idempotency/correlation/provider IDs, Teams URL, raw UTMs/referrer/query y provider body/error. `stage` no existe: sería redundante y podría contradecir `meeting_step`.

GTM pendiente: workspace 6 basado en versión publicada; reusa DLV `surface_id`/`placement` y contiene DLVs `meeting_step`, `scheduler_key`, `availability_state`, `days_ahead_bucket`, `time_of_day_bucket`, `error_category`, `presentation_variant` y `activation_mode`, más 2 triggers y 2 tags dedicados. GA4: registrar esas 8 dimensiones event-scoped; no crear otro key event ni usar `transaction_id`. Publish sólo tras preview, evidencia `/g/collect`, confirmación humana y snapshot.
