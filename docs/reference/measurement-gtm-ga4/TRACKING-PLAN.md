...
| `efeonce-web-agentica-ebook` | lead_magnet | Ebook "El fin de la web" | `/web-agentica` (Think, surface `fhsf-web-agentica-ebook`) | ✅ default | `gh_form_submission_accepted` → `generate_lead` | sí (previsto) | ⏳ pendiente | TASK-1375. Lead magnet ebook (descarga gated tokenizada). Emite además `download_url` en el accepted (browser-safe). Taggear al cerrar la landing TASK-1374. |
