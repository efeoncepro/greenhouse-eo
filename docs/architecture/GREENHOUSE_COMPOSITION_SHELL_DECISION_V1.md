...
## Delta 2026-06-14 (d) — Coreografía del cambio de densidad (reveal/unfold + rise + stagger) + frontera SSR del shell

La densidad ya se adaptaba (Delta c), pero el cambio entre modos **popeaba** (el contenido aparecía/desaparecía de golpe). Esta iteración lo convierte en una **coreografía escalonada** ("Transformer"), horneada en los card primitives — opt-in, SSR-safe, reduced-motion horneado, default byte-idéntico.

- **Reveal/unfold por pieza** (`MetricSummaryCard`/`MetricTrendCard`): el contenido que entra/sale entre densidades se DESPLIEGA — **texto** = unfold de altura (`height:0↔auto` + opacity, clipado por `overflow:hidden`); **chart** = fade + **rise** (sube `cardDensityRevealRisePx` = 10px, transform de compositor). **Stagger por pieza** en orden de lectura (`cardDensityRevealStaggerSec` = 0.06s: metricName→período, subtítulo→status). **Desync intencional** caja(`standard` 200ms)/contenido(`medium` 300ms) → secuencia, no unísono. Tokens en `card-density-motion.ts`, derivados del SoT de motion (cero hardcode).
- **Frontera dura del chart (no negociable):** el chart va SIEMPRE en modo **fade** (`animateHeight=false`). Animar `height:auto` sobre el `ResponsiveContainer` de Recharts crea un **loop infinito** (su ResizeObserver re-mide ↔ framer re-mide → `Maximum update depth exceeded`). El texto (sin ResizeObserver) sí hace unfold de altura.
- **Fix de distorsión:** el card adaptable usa `layout='position'` (NO `'size'`). Con `'size'`, framer escala la caja con `transform` y **el texto se estira a-desproporción** durante el morph de ancho. `'position'` no escala la caja; el ancho lo sigue el contenedor (continuo, sin distorsión).
- **Fix de loop en `useContainerDensity`:** el ResizeObserver solo hace `setState` cuando el ancho **CRUZA un breakpoint** (cambia el density bucket), no en cada pixel → mata el feedback **ResizeObserver ↔ framer `layout`** sobre el mismo nodo. Además baja los re-renders de "cada pixel" a "3 cruces".
- **Contención de scroll horizontal (clase TASK-742 / ISSUE-015), en la primitive:** la tabla sr-only del chart (`visuallyHidden`, `position:absolute`) lleva un contenedor `position:relative` (no escapa al viewport); el `Box` padre del `ResponsiveContainer` lleva `minWidth:0` + `overflowX:'clip'` (no propaga ancho stale, no clipa el tooltip vertical); el `split` del shell apila en xs (`{xs:'1fr', sm:clamp}`). Detalle canónico en CLAUDE.md/AGENTS.md (§Contención de scroll horizontal).
- **Hallazgo: la orquestación macro del shell vía `staggerChildren` de framer NO es SSR-safe.** Se evaluó hacer al shell el dueño de la secuencia con `parent variants + staggerChildren + when`. Se **descartó**: `staggerChildren` exige variant inheritance, que **no SSR-renderiza los estilos de los hijos** → hydration mismatch + viola never-hidden. La secuencia SÍ es dueña del shell, pero **SSR-safe**: el shell asigna el índice central (orden de DOM: dock→contenido→overlay) y cada región revela explícito (`compositionRegionReveal`). Re-coreografiar en cada morph (no solo al montar) requeriría un trigger imperativo post-mount aparte.
- **The Seam (Composition Shell × Adaptive Card) — demo de "secuencia macro":** el Lab suma un **driver continuo** (slider de ancho → morph fluido sin View Transition) y una **secuencia macro** imperativa (`useAnimate` + `stagger`, client-only → SSR-safe) que muestra varias cards entrando en cascada orquestada. La secuencia macro vive en el Lab (demo), **no es una primitive** todavía.
- **Evidencia:** `pnpm build` exit 0 · vitest 44/44 (card-density + composition-shell) · lint + tsc 0 · 0 `Maximum update depth` en carga + drag completo del slider (desktop + mobile 390px) · sin overflow de página. Commit `e21bcb145`.
