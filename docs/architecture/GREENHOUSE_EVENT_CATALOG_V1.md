...
| `hiring.assessment.assignment_superseded` | `supersedeAssignmentDeadEnd` (TASK-1771) | Una fila en callejón del carril automático **liberó su clave de idempotencia**. Payload `{assignmentId, applicationId, policyId, policyVersion, openingId, origin, triggerStage, attemptSeq, recordedOutcome, recordedReason, recoveryCount, actorUserId}`. Es **auditoría, no disparador**: no crea instancia, no manda correo y no tiene consumer reactivo — la fila superseded es la historia, y el evento responde quién la liberó y con qué evidencia. `recordedOutcome`/`recordedReason` son los del intento ORIGINAL: el supersede los conserva a propósito, porque son la explicación de por qué se bloqueó. |
...
| `proposal` | `commercial.proposal.created` (TASK-1392) | `commercial/tenders/proposals/store.ts` (from `createProposal`) | `{ version: 1, proposalId, ownerOrgId, clientOrganizationId, origin, publicOpportunityId?, deadlineConfidence, actorKind }` | Audit/observability; futuro TASK-1391 (prioridad de cola por deadline) |
| `proposal` | `commercial.proposal.state_transitioned` (TASK-1392) | `commercial/tenders/proposals/store.ts` (from `transitionProposalState`) | `{ version: 1, proposalId, ownerOrgId, fromState, toState, requiresHumanGate, actorKind, actorMemberId? }` | Audit; señal `deadline_at_risk`; futuro F1/F5 |
| `proposal` | `commercial.proposal.quote_attached` (TASK-1392) | `commercial/tenders/proposals/store.ts` (from `attachProposalQuote`) | `{ version: 1, proposalId, ownerOrgId, quoteId, actorKind }` | Audit; costura Proposal ⇄ Cotizador (gate de margen) |
| `proposal` | `commercial.proposal.rfp_ingested` (TASK-1392) | `commercial/tenders/proposals/assets.ts` (from `attachProposalAsset`/`ingestProposalRfp`) | `{ version: 1, proposalId, ownerOrgId, proposalAssetId, assetId, kind, audience, actorKind }` | Audit; futuro F1 (análisis de RFP) |
| `proposal` | `commercial.proposal.evidence_recorded` (TASK-1392) | `commercial/tenders/proposals/assets.ts` (from `recordProposalEvidence`) — **nunca el contenido, sólo ids/clasificación** | `{ version: 1, proposalId, ownerOrgId, evidenceId, classification, audience, hasSourceAsset, actorKind }` | Audit; proyección de render (TASK-1391) |
| `proposal` | `commercial.proposal.requirement_declared` (TASK-1392) | `commercial/tenders/proposals/assets.ts` (from `declareProposalRequirement`) | `{ version: 1, proposalId, ownerOrgId, requirementId, requirementKind, isBlocking, actorKind }` | Audit; matriz de admisibilidad F1 |
| `proposal` | `commercial.proposal.render_requested` (TASK-1391) | `commercial/tenders/proposals/render-jobs.ts` (from `requestProposalRender`) | `{ version: 1, renderJobId, proposalId, ownerOrgId, artifactPurpose, audience, catalogName, manifestHash, deadline, actorKind }` | Audit; dispatcher (prioridad de cola) |
| `proposal` | `commercial.proposal.render_completed` (TASK-1391) | `commercial/tenders/proposals/render-jobs.ts` (from `markRenderJobCompleted` vía transición del worker) | `{ version: 1, renderJobId, proposalId, ownerOrgId, state, failureCode: null, attempts, artifactPurpose, audience }` | Audit; futuros UI/Nexa (notificación de artefacto listo) |
| `proposal` | `commercial.proposal.render_failed` (TASK-1391) | `commercial/tenders/proposals/render-jobs.ts` (from `markRenderJobFailed` — incluye dead_letter) | `{ version: 1, renderJobId, proposalId, ownerOrgId, state, failureCode, attempts, artifactPurpose, audience }` | Audit; señales reliability (starvation/dead_letter) |
| `insight_report` | `insights.report.created` (TASK-1845) | `efeonce-insights/events.ts` (from `createInsightEdition`, misma tx que el INSERT) | `{ version: 1, reportId, reportCode, organizationId, actorKind }` | Audit/observability; biblioteca por org |
| `insight_edition` | `insights.edition.created` (TASK-1845) | `efeonce-insights/events.ts` (from `createInsightEdition`/`reviseInsightEdition`) | `{ version: 1, editionId, reportId, organizationId, editionVersion, audience, modules, outputs, periodStartUtc, periodEndUtc, requestHash, supersedesEditionId, actorKind }` — sin encargo completo ni evidencia | Audit; futuro worker de render (TASK-1846) |
| `insight_edition` | `insights.edition.state_transitioned` (TASK-1845) | `efeonce-insights/commands/generation.ts` + `lifecycle.ts` (misma tx que `insight_edition_transitions`) | `{ version: 1, editionId, reportId, organizationId, fromState, toState, requiresHumanGate, actorKind, transitionId }` | Audit; señales `insights.editions.*` leen la tabla, no el evento |
| `insight_edition` | `insights.evidence.sealed` (TASK-1845) | `efeonce-insights/commands/generation.ts` (fase collecting, tras `sealInsightEvidenceSnapshot`) | `{ version: 1, snapshotId, editionId, organizationId, snapshotHash, factCount, rejectionCount, asOfMax }` — conteos y hash, nunca hechos | Audit; futuro TASK-1846 (render sobre snapshot sellado) |
| `insight_edition` | `insights.edition.issued` (TASK-1845) | `efeonce-insights/commands/lifecycle.ts` (from `issueInsightEdition`, gate humano) | `{ version: 1, editionId, reportId, organizationId, editionVersion, issuedHash, actorKind }` | Futuro TASK-1848 (share/delivery sólo sobre emitidas) |
| `insight_render_run` | `insights.render.requested` (TASK-1846) | `efeonce-insights/events.ts` (from `requestInsightRender`, misma tx que el INSERT de run+outputs) | `{ version: 1, renderRunId, editionId, organizationId, audience, outputs, manifestHash, actorKind }` — nunca el manifest ni el plan | Audit; dispatcher/queue age (Slice 4) |
| `insight_render_run` | `insights.render.output_completed` (TASK-1846) | `efeonce-insights/render/store.ts` (`transitionOutput`, misma tx; lo dispara el worker vía `markInsightOutputCompleted`) | `{ version: 1, renderRunId, insightOutputId, editionId, organizationId, output, audience, state, attempts, failureCode: null, outputAssetId }` | Audit; futura notificación/UI (TASK-1849) |
| `insight_render_run` | `insights.render.output_failed` (TASK-1846) | `efeonce-insights/render/store.ts` (`transitionOutput`; incluye `dead_letter`) | `{ version: 1, renderRunId, insightOutputId, editionId, organizationId, output, audience, state, attempts, failureCode, outputAssetId: null }` | Audit; señal `insights.render.orphaned_output` lee la tabla, no el evento |
| `insight_share_grant` | `insights.share.created` (TASK-1848) | `efeonce-insights/events.ts` (from `createInsightShare`, o del despacho de correo con `source='delivery'`) | `{ version: 1, shareGrantId, editionId, organizationId, expiresAt, downloadOutputs, source, actorKind }` — **nunca token ni digest** | Audit |
| `insight_share_grant` | `insights.share.revoked` (TASK-1848) | `efeonce-insights/events.ts` (from `revokeInsightShare`, retirada de edición `edition_withdrawn` o fallo definitivo de envío) | `{ version: 1, shareGrantId, editionId, organizationId, reason, actorKind }` — nunca token ni digest | Audit |
...
## Delta 2026-06-25 — TASK-1229: `growth.forms.submission_accepted` (motor Growth Forms)

| Evento                             | Versión | Aggregate                                | Emisor                                                                                     | Consumer                                                                                            |
| ---------------------------------- | ------- | ---------------------------------------- | ----------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------- |
| `growth.forms.submission_accepted` | v1      | `growth_form_submission` (`fsub-{uuid}`) | command `submitForm` (`src/lib/growth/forms/commands.ts`), in-tx con la submission/consent | dispatcher de entrega (`dispatchPendingSubmissions`, ops-worker drain — ROLLOUT) **+ cualquier flujo interno** |
...
| Evento                                          | Versión | Aggregate                                  | Emisor                                                                                                  | Consumer                                                                                                              |
| ----------------------------------------------- | ------- | ------------------------------------------ | ------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------- |
| `growth.ai_visibility.lead_handoff_requested`   | v1      | `growth_ai_visibility_lead` (run_id)       | command gobernado `syncAiVisibilityRunToHubSpot` (auto: route de publish del snapshot · admin re-trigger) | `growth_ai_visibility_lead_handoff` (ProjectionDefinition, domain `growth`, lane `ops-reactive-growth`) → upsert HubSpot |

**Payload v1**: `{ schemaVersion: 1, runId, trigger: 'report_published' | 'admin_retrigger' }` — PII-free (el email/marca se re-leen de `grader_leads` en el consumer; NUNCA en el payload). El command es el primitive de Full API Parity (un consumer: reactive worker; otro: endpoint admin `POST /api/admin/growth/ai-visibility/runs/[runId]/lead-handoff`; futuros: Nexa via `propose → confirm → execute`, CLI). El WRITE a HubSpot ocurre SOLO en el reactive consumer (`executeLeadHandoff`), con cliente HubSpot **in-app directo** (TASK-1230, NO Cloud Run bridge), gated por flag + consent + score `completed`. Idempotente: upsert por email/dominio + `grader_leads.hubspot_synced_at`.

## Delta 2026-06-27 — TASK-1250: `growth.ai_visibility.report_email_requested` (entrega del informe por email)

| Evento                                          | Versión | Aggregate                                  | Emisor                                                                                                  | Consumer                                                                                                              |
| ----------------------------------------------- | ------- | ------------------------------------------ | ------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------- |
| `growth.ai_visibility.report_email_requested`   | v1      | `growth_ai_visibility_lead` (run_id)       | command gobernado `requestAiVisibilityReportEmail` (auto-finalize del worker · aprobación TASK-1244 · route admin de publish) | `growth_ai_visibility_report_email` (ProjectionDefinition, domain `growth`, lane `ops-reactive-growth`) → `dispatchAiVisibilityReportEmail` → email + adjunto PDF |

**Payload v1**: `{ schemaVersion: 1, runId, trigger: 'report_published' | 'admin_resend' }` — PII-free (email/marca/snapshot se re-leen de PG en el consumer; NUNCA en el payload). Mismo aggregate que el handoff. Emitido write-side desde los 3 puntos de publicación del snapshot (NUNCA on-read del GET de status público). El WRITE (envío) ocurre SOLO en el reactive consumer (`dispatchAiVisibilityReportEmail`), gated por flag `GROWTH_AI_VISIBILITY_REPORT_EMAIL_ENABLED` + consent + estado del reporte (`ready`/`partial`). Idempotente DB-level por `UNIQUE(report_id, email_type)` en `greenhouse_growth.grader_report_email_dispatches` (claim atómico antes de enviar). Adjunto = PDF público-safe de TASK-1273 (`renderAiVisibilityReportPdf`, variant `attachment`). Signal: `growth.ai_visibility.report_email_failed`.

## Delta 2026-06-29 — TASK-1275: `growth.ai_visibility.recommendation_status_changed` (estado del Plan AEO)

| Evento                                              | Versión | Aggregate                                                  | Emisor                                                              | Consumer                                                              |
| --------------------------------------------------- | ------- | ---------------------------------------------------------- | ------------------------------------------------------------------ | -------------------------------------------------------------------- |
| `growth.ai_visibility.recommendation_status_changed` | v1      | `growth_ai_visibility_recommendation_status` (`org:gap_key`) | command gobernado `setRecommendationStatus` (vista operador TASK-1276 / Nexa) | (futuro) CS health-score / QBR digest — sin consumer reactivo en V1 |

**Payload v1**: `{ schemaVersion: 1, organizationId, recommendationKey, fromStatus, toStatus, sourceRunId, updatedBy, reason }` — sin PII (gap key + estados + org/run id + user id + texto operativo). Emitido **transaccionalmente** (mismo `withGreenhousePostgresTransaction` que el UPSERT del current-state + el INSERT del history append-only), NUNCA best-effort fuera de la tx. Sólo en transición real: la idempotencia no-op (mismo status+reason) NO emite. `fromStatus` es `null` en el primer set. Habilita un futuro health-score/QBR digest y la señal "marcado `done` pero el re-grade posterior a `sourceRunId` regresó → revisar" sin reproceso. Aún sin ProjectionDefinition (no hay consumer reactivo en V1).

## Delta 2026-06-29 — TASK-1279: `growth.ai_visibility.report_send_requested` (cross-sell operador: envío informe + Lead HubSpot)

| Evento                                          | Versión | Aggregate                                          | Emisor                                                              | Consumer                                                                                                              |
| ----------------------------------------------- | ------- | -------------------------------------------------- | ------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------- |
| `growth.ai_visibility.report_send_requested`    | v1      | `growth_ai_visibility_report_send` (`send_id`)     | command gobernado `sendAeoReportAndCreateLead` (vista operador TASK-1276 / Nexa) | `growth_ai_visibility_operator_send` (ProjectionDefinition, domain `growth`, lane `ops-reactive-growth`) → `executeOperatorReportSend` → email público-safe + **Lead HubSpot** |

**Payload v1**: `{ schemaVersion: 1, sendId }` — PII-free (el recipient/consent/org/run se re-leen del send log `greenhouse_growth.grader_report_send_log` en el consumer; NUNCA en el payload). Emitido **transaccionalmente** (mismo `withGreenhousePostgresTransaction` que el claim atómico del audit, idempotente por `UNIQUE(run_id, lower(recipient_email))`). El WRITE (email + creación del **Lead** de HubSpot, objeto `leads` — NUNCA Deal) ocurre SOLO en el reactive consumer, con dos sub-pasos idempotentes (`email_status`/`lead_status`), gated por flag `GROWTH_AI_VISIBILITY_OPERATOR_SEND_ENABLED`. Consent gate server-side (prospecto exige `legitimate_interest` + consentimiento; cliente = servicio). Email = mismo path público-safe del lead magnet (snapshot + `renderAiVisibilityReportPdf`, marca Efeonce). Signal: `growth.ai_visibility.operator_send_failed`.

## Delta 2026-06-29 — TASK-1289: `growth.ai_visibility.business_model_overridden` (override del modelo de negocio del perfil AEO)
