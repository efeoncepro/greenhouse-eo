...
| App lane | ✅ `/api/platform/app/insights/{catalog, reports, reports/[id], editions, editions/[id], editions/[id]/{issue,revise,withdraw,recover}}` (`app-insights.ts` comandos + `app-insights-read.ts` lectura) — la única lane que emite y retira |
| Ecosystem lane | ✅ `/api/platform/ecosystem/insights/{catalog, reports, reports/[id], editions, editions/[id], editions/[id]/{revise,recover}}` (`ecosystem-insights.ts` comandos + `ecosystem-insights-read.ts` lectura); binding `internal` crea/revisa/recupera con `organizationId` explícito, binding org-scoped sólo lee; **no emite ni retira** por diseño |
