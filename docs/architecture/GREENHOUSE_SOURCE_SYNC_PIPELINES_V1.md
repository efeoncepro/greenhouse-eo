...
## Delta 2026-06-29 — TASK-1282 Search Console: redesign property-picker + token de operador + grant secretAccessor (verificado E2E)

Supersede parcialmente el Delta 2026-06-28 (per-org token). Tras feedback del operador, el flujo de conexion paso al patron **Semrush / operador-mediado**:

- **Token de OPERADOR, no per-org.** Un solo refresh token por operador interno (`search-console-token-operator-<userId>`), reutilizable entre todas las orgs que ese operador conecta. Las filas `search_console_connections` per-org sólo apuntan a ese secret. Razon: en el modelo real, el operador (ej. `jreyes@efeoncepro.com`) tiene acceso a las propiedades de muchos clientes (cada cliente comparte la suya con el operador); el token es del operador, no de la org. Per-org-token sigue siendo el diseño correcto el dia que un cliente se auto-conecte con SU cuenta.
- **Property-picker (no se tipea el site_url).** `oauth/start` ya no pide propiedad; el callback guarda el token y deja la conexion `pending`; el reader `listSearchConsoleSitesForOrg` llama `sites.list` y devuelve TODAS las propiedades de la cuenta; el operador elige del desplegable; el command `selectSearchConsoleProperty` valida server-side que la propiedad este en la cuenta (anti-binding ajeno) y marca `active`. Migracion aditiva: `site_url` nullable.
- **Modelo de consent: External + En produccion** (no Internal ni Testing). Internal sólo acepta `@efeonce.org` → bloquea cuentas externas de clientes. Testing expira los refresh tokens a 7 dias → rompe la conexion persistente. External+Produccion sin verificacion de Google = aviso "app no verificada" (aceptable B2B; ~100 grants hasta verificar). NO se uso proyecto GCP dedicado (el consent screen compartido con el Google-login del portal no es problema porque el login es Microsoft).
- **⚠️ Grant IAM doble (gotcha load-bearing).** El SA runtime `greenhouse-portal@` necesita DOS grants condicionados al prefijo `search-console-token-*`: `roles/secretmanager.secretVersionAdder` (escribir el token) **y** `roles/secretmanager.secretAccessor` (leerlo en el reader/list). `secretVersionAdder` NO incluye `versions.access` — sin el `secretAccessor`, `resolveSecretByRef` lanza PERMISSION_DENIED → el listado degrada a `query_failed` (sintoma "local funciona / staging falla"). Condiciones vivas: write `client-integration-tokens`, read `search-console-tokens-read`.
- **Verificado E2E en staging (2026-06-29):** Grupo Berel conectado (`sc-domain:berel.com`); el reader `readSearchConsoleAnalytics` devolvio datos reales (consulta "berel" 2.807 clics / 21.045 impresiones). Funcional: `docs/documentation/growth/conexion-search-console.md`; manual: `docs/manual-de-uso/growth/conectar-search-console.md`.
