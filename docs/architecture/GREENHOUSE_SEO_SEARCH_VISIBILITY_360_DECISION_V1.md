...
El 360 se extiende del nivel **marca** al nivel **página/topic cluster**: para una landing o cluster, análisis unificado de keywords · avg position · clicks · **grounded queries que cita la IA** · citation share · cuadrante 360 a ese nivel (el diferenciador más fuerte del producto; nadie lo da). No cambia ninguna de las 10 decisiones vinculantes: el cruce sigue siendo un **derived read** (join `org + url/cluster`), sin merge de tablas ni fusión de scoring (boundary §2 intacto). Materia prima ya existe: el grader captura citas con URL (`GrowthAiVisibilityCitation`); la extensión es atribución/lectura + entidad cluster + read unificado. Tasks `TASK-1311/1312/1313`. Detalle: `GREENHOUSE_SEO_MODULE_ARCHITECTURE_V1.md` §15. **Pillar page + topical authority** (refinamiento del cluster, mismo boundary): la pillar es el hub donde SEO y AEO convergen; `role` (`pillar`|`supporting`) en el member (delta `TASK-1312`) + `TASK-1314` (topical authority score componiendo rank/audit/citation, sin captura nueva). §15.1. **E-E-A-T** (capa de entidad/calidad conectiva, mismo boundary): ~70% de la materia prima ya existe en el probe layer del grader (eje entity KG/Wikidata/Reddit + json-ld + brand-intelligence LLM); gap = autor + rúbrica/rater 4 pilares YMYL-aware + trust; vive cerca del grader, SEO la consume; es **assessment** (medido vs evaluado, anti falso-0), no dial. `TASK-1315/1316/1317`. §16.

### Delta 2026-09-03 — metodología ETV: provenance adicional a la lente (TASK-1805)

La decisión 8 (medido ● vs estimado ◑) no cambia. Desde el release de `TASK-1805` toda cifra estimada que
provenga de ETV de DataForSEO Labs lleva además **con qué fórmula** se calculó: `etvMethodology`
(`legacy_static_v1 | improved_layout_clickstream_v2`, evidencia, policy version, comparabilidad) persistida por
fila, parte de la clave de idempotencia y del contrato de lectura. No es una tercera lente ni un valor
`mixed`: la lente sigue binaria y la fórmula es un atributo de la cifra estimada. Un DTO sirve una sola
fórmula; `not_available_for_method` es ausencia declarada, no cero. Detalle en
`GREENHOUSE_DATAFORSEO_ETV_METHOD_VERSIONING_DECISION_V1.md` y en `GREENHOUSE_SEO_MODULE_ARCHITECTURE_V1.md`
(§4.2, §5, §6, delta 2026-09-03).
