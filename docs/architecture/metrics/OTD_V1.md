...
## Delta 2026-06-20 — writeback `[GH] OTD` a Notion (TASK-927, display-only, flag OFF)

El bucket OTD freeze-aware (M2, `task_attributable_lateness_shadow.bucket_attributable`) se escribe de vuelta a Notion en una propiedad **read-only** `[GH] OTD` (select) de la DB Tareas de Efeonce + Sky, vía un **batch diario** (Cloud Scheduler `ops-otd-writeback` → ops-worker `/otd/writeback`; OTD es `now()`-dependiente, no event-driven). **Display-only**: cierra el loop del boundary Notion = OS / Greenhouse = motor (el operador ve el bucket GH-owned freeze-aware junto a la fórmula legacy `Indicador de Performance`), **NO toca el bono** (`otd_pct` intacto; el cutover del bono es M3, futura gateada).
