...
Cuando la generación se mueva a un worker, el umbral de `stuck_generation` pasa a ser el del job (TASK-1846 movió sólo el **render**; la generación sigue síncrona). Si la consulta falla, el reader degrada honesto: devuelve sólo `failed_recent` con `severity='unknown'` y `captureWithDomain(error, 'insights', …)`. DB vacía / flags `INSIGHTS_*_ENABLED` OFF → ambas `ok`.

**Delta 2026-09-16 (TASK-1846) — señal del render durable.** `insights.render.orphaned_output` (`kind=data_quality`, reader `src/lib/reliability/queries/insights-render-orphaned.ts`, wired en `get-reliability-overview.ts` como `insightsRenderOrphanedSignal`; steady 0; 0 → `ok`, 1–2 → `warning`, >2 → `error`). Cuenta outputs de `greenhouse_insights.insight_outputs` en `running` que nadie va a retomar: sin lease (reclamados por un worker previo al lease; no se reclaman solos para no producir dos finalizaciones) o con lease vencido hace más de 60 min (el Job `artifact-worker` no corre, `INSIGHTS_RENDER_ENABLED` OFF en la revisión activa o el Scheduler `ops-artifact-render-dispatch` detenido). Remediación: verificar el runtime vivo, no el ledger; para los sin lease, decidir reintento o `dead_letter`.

**TASK-1848 (code complete 2026-09-18, sin deploy) — señal de envíos por correo con resultado desconocido.** `insights.delivery.ambiguous` (`kind=data_quality`, reader `src/lib/reliability/queries/insights-delivery-ambiguous.ts`, wired en `get-reliability-overview.ts` como `insightsDeliveryAmbiguousSignal`; steady 0; 0 → `ok`, 1–3 → `warning`, >3 → `error`). Cuenta destinatarios de `greenhouse_insights.insight_delivery_recipients` en `ambiguous` (el correo pudo o no haber salido) más los `claimed` hace más de 30 min (un dispatcher reclamó y murió sin dejar resultado). Remediación: reconciliar contra el ledger de correo con `reconcileInsightDeliveryRecipient` (lane App `POST …/delivery-recipients/{id}/reconcile`); **nunca** reenviar a ciegas ni devolver a `pending` a mano: el correo puede llevar un enlace personal.
