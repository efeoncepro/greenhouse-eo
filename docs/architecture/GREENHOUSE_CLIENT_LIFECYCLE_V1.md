...
#### Delta 2026-08-06 — la reconciliación gobierna el eje ROL COMERCIAL, no el eje identidad

La reconciliación `organization_type ↔ lifecycle_stage` descrita arriba gobierna **un solo eje: el rol comercial**. No toca el eje identidad (`is_operating_entity`) ni el eje capabilities (`greenhouse_client_portal.module_assignments`). Contrato semántico completo: `GREENHOUSE_PERSON_ORGANIZATION_MODEL_V1.md` §`Organization Types`; espejo operativo (NUNCA/SIEMPRE): `agent-invariants/ORG_CLIENT_AGENT_INVARIANTS.md` §`Identidad vs rol comercial vs capabilities`.

- **`'other'` = sin rol comercial, NO "org a medio cocinar".** La heurística "type `other` ⇒ hay que promover" vale para prospects y clientes a medias (el bug class Grupo Berel), pero **NO** para la entidad legal operadora: lleva `'other'` porque no se vende ni se compra a sí misma, y su identidad la carga `is_operating_entity`. Verificado 2026-08-06: cero queries del repo usan `organization_type='other'` como criterio de negocio.
- **La operadora NUNCA debe alcanzar `lifecycle_stage='active_client'`.** Si lo alcanza, la regla `active_client ⇒ client/both` la promueve a cliente **por construcción** y la mete en los 5 readers client-facing (`api/finance/clients` y su detalle, `finance/canonical.ts`, `postgres-store-slice2.ts`, `client-onboarding/org-search.ts`) más la puerta de facturación `resolveFinanceClientContext` → autofacturación posible que contamina P&L/F29. Si un writer la lleva ahí, el bug está en el writer, no en la reconciliación.
- **`deriveOrganizationType` es MONÓTONA ⇒ bajar un rol exige puerta propia.** La rama `isClientCapable(currentType)` (`src/lib/account-360/organization-type.ts:74-88`) nunca degrada un rol ya adquirido: un `upsertCanonicalOrganization` corriente **perpetúa** el tipo. La puerta de bajada es `scripts/commercial/reset-organization-commercial-role.ts` (declara `currentType='other'` explícito y pasa igual por el writer canónico). `remediate-half-baked-orgs.ts` **no aplica**: su criterio (`lifecycle='active_client' AND type NOT IN ('client','both')`) sólo promueve.
- **Caso fuente**: `EO-ORG-0007` (Efeonce, `is_operating_entity=true`) tenía `organization_type='client'`, herencia del space de cliente de marzo 2026. No lo causó el dogfooding SEO (su script de provisión no escribe en `organizations`; la única transición registrada es `null → inactive` del 2026-04-21, backfill de TASK-535). Reseteada a `'other'`; `is_operating_entity`, los 2 `module_assignments` (`seo_v1`, `proposal_studio_v1`) y el canary SEO contra producción quedaron intactos. Daño consumado: 0 income, 0 contratos, 0 usuarios de portal.
- **Estado real de las defensas DB (verificado contra `pg_constraint`, 2026-08-06)**: el CHECK `organizations_type_lifecycle_consistent` **sigue sin aplicarse** — no existe en ninguna migración ni en la base viva. Sobre `greenhouse_core.organizations` sólo hay CHECKs de enum y **cero triggers**. Varios comentarios del código lo dan por activo: están stale. Tampoco existe índice único parcial que garantice **una sola** fila con `is_operating_entity=TRUE` (gap real frente al roadmap multi-entidad legal).

**⚠️ Reglas duras (delta)**:

- **NUNCA** promover una org a `'client'` para habilitarle un módulo de plataforma. Las capabilities viven en `module_assignments` y se resuelven por `organization_id` + `module_key` — `enforceSeoRunEntitlement` (`src/lib/growth/seo/entitlement.ts:212-214`) no lee `organization_type`.
- **NUNCA** agregar un valor de identidad al enum (`'internal'`, `'efeonce_internal'`, `'legal_entity'`) para "sacar la operadora de los listados de clientes". Mezcla dos ejes; el guard correcto es `!isOperatingEntity` (referencia viva: `src/app/(dashboard)/growth/aeo/page.tsx:107`). `efeonce_internal` es un `tenant_type` de usuarios, no un tipo de organización.
- **NUNCA** aplicar una remediación masiva de "orgs con type `other`" sin excluir `is_operating_entity=TRUE` del universo barrido.
- **SIEMPRE** que un writer o sweep nuevo pueda tocar la fila de la operadora, declarar explícitamente qué eje escribe y dejar los otros dos intactos.
