...
4. **Cohorte exacta.** Sólo pertenecen al run aplicaciones de la misma opening **sin desenlace terminal vigente**:
   quedan fuera `selected`, `not_selected`, `rejected`, `withdrawn` y `unresponsive`. `backup_selected` y la pausa
   —que tras la enmienda **ya no es un desenlace `on_hold`** sino la **etapa `decision_pending` sin desenlace
   declarado**— se muestran separados en el preview y sólo se superseden si la confirmación los incluye
   explícitamente. Otras openings de la persona no cambian.
5. **Comunicación por hechos persistidos.** Cada supersede al desenlace `not_selected` con causa allowlisted
   `capacity_filled` emite el evento canónico `hiring.application.decided`. El consumer de `TASK-1689` re-lee PG,
   deduplica por event/decision/application y elige la variante correspondiente: el agradecimiento de `rejected`
   (descarte) o «esta vez elegimos a otra persona» (sin selección por vacante completada). **No son el mismo
   template.** El run no envía correo directamente.
...
- Comunicación: `src/lib/hiring/notifications/**` + `src/emails/HiringDecisionEmail.tsx`; el email re-lee nombre,
  vacante, desenlace, causa y consentimiento vigente antes de enviar.
- **Un `EmailType` por desenlace; la causa modula el CUERPO, no el tipo.** La variante de capacidad nace con
  `EmailType` propio (`hiring_decision_not_selected`), distinto del histórico `hiring_decision_rejected`, porque el
  tipo es un **discriminante por el que el sistema ramifica**: kill-switch por tipo (`src/lib/email/delivery.ts:131`
  y `:336` bajo lock) y perfil de footer resuelto por `EmailType` (`TASK-1764`). Operativamente decide: un cierre por
  capacidad manda N correos de una vez y debe poder pausarse **sin** silenciar el correo de decisión individual — que
  es exactamente por lo que `hiring_decision_selected` y `hiring_decision_rejected` ya son tipos separados. El techo
  queda fijo: de los 6 desenlaces sólo 4 comunican, y las 3 causas viven dentro del cuerpo de `not_selected`.
- El envío es asíncrono y corre en el `ops-worker`, no en Vercel: seed de `email_type_config` y flags se declaran en
  `services/ops-worker/deploy.sh` (cuyo `--set-env-vars` es destructivo) y se aplican además en vivo.
...
| --- | --- |
| Cerrar la cohorte automáticamente al registrar la última selección | Rechazada: confunde selección con capacidad y produce un efecto externo irreversible sin revisar cohorte. |
...
- El mismo evento individual sostiene el descarte manual y el cierre por capacidad sin un segundo pipeline de email.
- Quien no quedó por un cupo lleno permanece en el Talent Pool y no arrastra un juicio que nadie emitió.
...
- `backup_selected` exige una elección visible adicional y no puede colapsarse con postulaciones sin desenlace.
- El cierre por capacidad depende del eje de desenlace + causa, que no existe todavía: no puede implementarse antes que `TASK-1765`.
...
- Tras un cierre por capacidad, un readback de la cohorte devuelve `not_selected` + `capacity_filled` en cada item y
  **cero filas `rejected`**; el embudo de equidad cuenta esas filas como concluidas por comparación, no como descartes.
