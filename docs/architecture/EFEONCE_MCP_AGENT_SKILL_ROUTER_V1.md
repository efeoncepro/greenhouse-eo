...
| Advertising/social craft without a federated creative tool | `efeonce-advertising-creative` locally; do not register an MCP manual against an unrelated tool | AXIS advertising contract + local creative workflow; MCP projection waits for a real creative capability |
