...
> **Updated:** 2026-04-21 — v2.33: **TASK-538 Unified Party Selector shipped**. El `QuoteContextStrip` deja de depender solo del preload legacy de organizations activas cuando el tenant trae `GREENHOUSE_PARTY_SELECTOR_UNIFIED`. Nuevo hook cliente `useParties()` consume `GET /api/commercial/parties/search` con debounce 250 ms, estados `loading/error/rate-limited` y adopción via `POST /api/commercial/parties/adopt`. La integración vive en el carril `QuoteContextStrip -> ContextChip -> QuoteBuilderShell`, no en una UI paralela. `organizationId` sigue siendo el handshake canónico hacia contactos, deals y persistencia de quotations; el selector unificado solo cambia cómo se resuelve ese anchor. Scope V1: `hubspot_candidate` sigue visible solo para `efeonce_internal`; edit mantiene `organizationLocked` y el fallback legacy permanece detrás del flag.
