...
- **La ÚNICA excepción es el ledger de gasto de proveedor**, y no es un cruce de dominio: `greenhouse_growth.seo_provider_spend_daily` es una tabla compartida **por TRANSPORTE**, porque una factura de DataForSEO es una sola aunque la gasten dos motores. Ahí la separación la hace la columna `consumer` (`seo|aeo`, dentro de la clave única) y **dos resolvers de presupuesto distintos** — `buildSeoProviderSpendMonthlySumSql` filtra `consumer = 'seo'`, `resolveAeoBudget` lee `consumer = 'aeo'`. Sigue prohibido todo JOIN/VIEW/FK entre tablas `seo_*` y `grader_*`, y **NUNCA** se abre un segundo ledger (`aeo_provider_spend_daily` no existe ni debe existir).
...
- Configuración por runtime: el worker requiere `DATAFORSEO_API_LOGIN` directo y `DATAFORSEO_API_PASSWORD_SECRET_REF`, aunque Vercel/local estén configurados. TASK-1341 agrega el preflight de deploy y readback de revisiones con tráfico en `services/ops-worker/dataforseo-config.mjs`. El error tipado `provider_configuration_missing` de discovery corta el batch antes del proveedor, preserva candidatos/costo anteriores y no incrementa `provider_calls` por ausencia de configuración. **Código local, rollout pendiente** al 2026-09-19; recuperación de env comprobada en [ISSUE-175](../issues/resolved/ISSUE-175-dataforseo-worker-login-missing.md).
- El **ledger de gasto** `seo_provider_spend_daily`, que el transporte escribe para los dos motores con la dimensión `consumer` separándolos (§6). Es el único acople de datos deliberado entre SEO y AEO, y por eso el adapter de AI Mode del grader importa `@/lib/growth/seo/register-provider-spend` por efecto: sin ese registro el transporte lanza cuando viene organización, y el `catch` del adapter lo convertiría en una observación `failed` — el grader perdería AI Mode justamente para los perfiles de cliente que la atribución existe para cobrar. Es el único import de `ai-visibility` hacia `growth/seo` y está declarado, no tolerado.
...
| | **Keyword gap competitivo** (qué búsquedas gana un competidor DECLARADO donde el cliente es invisible; TASK-1662) | DataForSEO Labs (`domain_intersection` ×2 por competidor: no-intersección = "no aparezco", intersección = "aparezco peor") — §4.2 `seo_competitors` + cobertura; el gap se **DERIVA al leer**, jamás se persiste |
| | **Top-N del SERP persistido** (quién ocupa cada ranura del SERP de tus keywords, día a día; propone competidores por recurrencia; TASK-1699) | La MISMA respuesta SERP que el rank capture YA paga (`depth 20` — costo marginal CERO, cero llamadas nuevas) — §4.2 `seo_serp_top_results`; la serie sólo existe hacia adelante (el SERP de ayer no se recompra) |
...
- `seo_keyword_set_members` — keyword + tags[] + `effective_from`/`effective_to` (membership append-only; NUNCA DELETE de una keyword) + **procedencia `created_by` / `source`** (TASK-1308, migración `20260807173706557`). La tabla nació escrita sólo por scripts de seed, así que no registraba quién agregó cada keyword; con `trackKeywords` gobernado y tres consumers (UI operador, Nexa, MCP) sobre un efecto diferido caro, *"¿por qué estamos pagando por esta keyword?"* necesita respuesta auditable — que es justo la pregunta que llega cuando el gasto sube. Ambas columnas son **nullable y sin backfill**: las filas de seed conservan `NULL` honesto (no se sabe quién) en vez de un valor inventado. `source` lleva **CHECK de vocabulario cerrado** (`operator_ui | nexa | mcp | seed | backfill`) — si el motor sólo entiende N valores, que el schema los enumere: un `source` nuevo debe romper el INSERT, no colarse invisible en toda lectura que agrupe por procedencia. 🔴 `source` y `tags` son dimensiones **ortogonales** y por eso son dos columnas: `tags` es clasificación de dominio que elige el operador; `source` es procedencia técnica del write. Meterla dentro de `tags` rompería cualquier filtro de dominio que lea ese arreglo.
  - **Intención declarada `intent` / `intent_declared_by` / `intent_declared_at`** (TASK-1659, migración `20260814221022082`). `source` responde *quién* metió la keyword; `intent` responde **por qué está en el set**: `target` (compromiso acordado con el cliente) u `opportunity` (demanda medida que se está empujando). Sin esa distinción, *"estoy en la 12 y quiero la 5"* y *"el cliente quiere rankear acá y estoy en la 60"* son la misma fila, y un objetivo aspiracional se lee como fracaso permanente en cualquier KPI agregado. **CHECK de vocabulario cerrado**, igual que `source`, y **ortogonal a él**: mezclarlos obligaría a un producto cartesiano (`operator_ui_target`, `mcp_opportunity`…) que ningún filtro podría leer.
    - 🔴 **Nullable y sin backfill, y eso es el contrato, no una omisión.** Marcar las filas previas como `opportunity` afirmaría que alguien las clasificó cuando nadie lo hizo, e inflaría el KPI de oportunidades. `NULL` dice la verdad —*no se declaró*— y obliga a los readers a tratar el caso explícito. Por la misma razón el command **no tiene default**: quien no declara escribe `NULL`, y es el caller quien declara (la lente Oportunidades manda `intent: 'opportunity'` porque ahí sí hay alguien diciéndolo).
    - 🔴 **Cambiar la intención NO es un `UPDATE` de la columna: cierra la membresía vigente y abre otra**, con `clock_timestamp()` (`NOW()` daría `effective_to = effective_from` y reventaría el CHECK, 23514). El dato que sostiene el reporte de avance no es *"esta keyword es un objetivo"* sino *"es objetivo desde marzo, y en marzo estaba en la 45"* — un `UPDATE` destruye exactamente eso. La tabla ya es append-only con ventanas: se reusa ese mecanismo. El cambio **no consume cupo** del techo (el conteo vigente no se mueve), así que reclasificar sigue siendo posible con el set lleno, que es cuando más falta hace.
    - `intent_declared_by` es distinto de `created_by`: uno es quién asumió el compromiso, el otro quién ejecutó el INSERT, y difieren cuando un agente mete la keyword por encargo. Un CHECK acopla ambas columnas de autoría a la existencia de `intent` — un compromiso sin responsable, o un responsable sin compromiso, no son estados válidos.
    - Outcome propio `intent_changed` en los tres lanes (`already_tracked` sería mentira: sí pasó algo; `tracked` sugeriría gasto nuevo). El evento outbox se emite también en este caso aunque `activeKeywordCount` no se mueva.
...
> **Delta 2026-09-03 (TASK-1805) — la metodología ETV es provenance ADICIONAL a la lente, no una lente nueva.**
>
> `lens: 'estimated'` sigue siendo binaria y derivada de la fuente. Lo que se agrega es **con qué fórmula** el proveedor calculó esa cifra estimada. Contrato de lectura: `readDomainOverview`, `readUrlVisibility` y `readVisibilityConcentration` filtran `etv_methodology_version = <servida>` en SQL **antes** de deduplicar; un DTO sirve UNA sola fórmula (`assertSingleEtvMethodology` lanza `mixed_etv_methodology` ante una serie mixta); el `ok: true` emite `etvMethodology { version, policyVersion, evidence, availableMethodologies[], comparability: single_methodology | not_available_for_method, breakpointDate: null, providerCutoffAt }` (`buildEtvMethodologyProvenance`, `etv-methodology/provenance.ts`); y cuando la fórmula servida no tiene filas el reader devuelve `{ ok: false, reason: 'not_available_for_method', requestedMethodology, availableMethodologies }` — **no es cero ni `no_market_data`**: es «hay dato, pero de otra fórmula». La fórmula servida sale de `GROWTH_SEO_ETV_READ_METHODOLOGY_VERSION` (`resolveEtvReadMethodology`; ausente = `legacy_static_v1`, `source: 'default'`); el parámetro `etvMethodology?` de los readers existe sólo para la comparación interna del evaluador y **nunca lo elige un caller externo**. El lane ecosystem (`/api/platform/ecosystem/growth/seo/domain-overview` y `url-visibility`) transporta `not_available_for_method` como `errorCode`; las tools MCP `get_seo_domain_overview`, `get_seo_url_visibility` y `get_seo_prospect_diagnostic` declaran el contrato en su descripción (manifest `8969c8d39c1f`). `ProspectDiagnostic.etvMethodology` viaja en el DTO del diagnóstico. El cruce con GSC no cambia: `compareEtvWithGscBenchmark` (evaluador) compara y nunca promedia.
...
- **Un cliente, familias como config** (no un cliente por familia): transporte compartido `postDataForSeoTask({ family, endpoint, tasks, organizationId, consumer })` + gate de familia + instrumentación por familia. **`consumer` es requerido por el tipo en TODAS las variantes** (TASK-1696): un caller nuevo no puede olvidarse de declarar quién gasta, y `serp` es la familia **compartida** — la compran el rank capture SEO (`consumer: 'seo'`, 20 callsites) y el adapter de AI Mode del grader AEO (`consumer: 'aeo'`).
- **El wrapper histórico `postDataForSeoSerpLiveAdvanced` queda CONGELADO** y documentado como puerta que **NO atribuye**: acepta `endpoint`/`tasks`/`timeoutMs` y nada más. Su único consumer productivo —el adapter `google-ai-overview-adapter.ts`— migró al transporte canónico, y un guard (`src/lib/ai/__tests__/dataforseo-legacy-wrapper-guard.test.ts`) rompe el build si otro módulo vuelve a comprar por ahí. Engordar el wrapper habría reabierto el punto ciego con la firma pareciendo correcta.
- **Cost tracking — `seo_provider_spend_daily` es la FUENTE ÚNICA de presupuesto, y es UN solo ledger para los dos dominios.** El contador lo escribe el **transporte** en cada llamada cobrada, así que una captura nueva no puede gastar sin quedar contabilizada por haber olvidado el registro; cubre además las llamadas que no dejan fila (tarea `on_page` async, consulta con cero resultados). Sumarlo además con el `provider_cost` de las tablas snapshot contaría el mismo gasto dos veces y agotaría los presupuestos a la mitad, en silencio. Ese `provider_cost` queda como procedencia por fila. Lo que se separa **no es la tabla sino el resolver de presupuesto**: una factura de proveedor es una sola, y dos tablas del mismo hecho serían dos verdades con una reconciliación manual que nadie va a hacer.
- **Cada fila declara quién consumió el dólar y de qué tipo es** (TASK-1696): `consumer` (`seo|aeo`, CHECK cerrado), `cost_basis` (`invoiced|estimated`) y `price_table_version`, acoplados por CHECK `(cost_basis = 'estimated') = (price_table_version IS NOT NULL)` — declaración y procedencia del número existen o faltan juntas. Hoy todas las filas son dólares facturados que el transporte lee del `cost` del proveedor; el día que entre un dólar estimado (presupuesto de tokens per-org) sin estas columnas nadie podría distinguirlos, y un total que suma facturado con estimado y se presenta como un número solo **no es un dato degradado, es un dato falso**.
- 🔴 **La clave única es de SEIS columnas y usa `NULLS NOT DISTINCT`:** `(organization_id, family, spend_date, consumer, cost_basis, price_table_version)`. No es exhaustividad decorativa — con la clave de 4 columnas (sin la base de costo) un dólar `estimated` **colisiona** con la fila `invoiced` del mismo día/familia/consumidor y entra por el `DO UPDATE`, que suma el monto pero **no toca `cost_basis`**: el dólar estimado queda reetiquetado como facturado, sin error y sin rastro — la mentira exacta que las columnas de honestidad existen para impedir. Se encontró ejercitando el SQL productivo contra PostgreSQL, no leyéndolo. `NULLS NOT DISTINCT` es lo que permite incluir `price_table_version` (NULL en toda fila facturada) sin que el UPSERT deje de acumular e inserte una fila por llamada. **NUNCA** el `ON CONFLICT` de `SEO_PROVIDER_SPEND_UPSERT_SQL` y la constraint de la base pueden divergir: viajan en el mismo commit, con test de paridad.
- **Atribución obligatoria por tipo.** Las 4 familias SEO exigen `organizationId` (el tipo lo impone y el runtime lo revalida) y el transporte **lanza** si el runtime no registró el contador de gasto — gastar sin contabilizar se descubre en la factura; un throw se descubre en desarrollo. ✅ `serp` mantiene `requiresOrganization: false` **POR DISEÑO, ya no por deuda de plomería**: el gasto AEO de un perfil ligado a un cliente **sí** entra hoy a su presupuesto (`ProviderAdapterContext` transporta la organización), y el `false` existe porque el grader **público** —prospecto sin organización— es un caso legítimo que el tipo no puede exigir. Ponerlo en `true` para "cerrar la deuda" rompería el camino público; hay test que lo afirma con la razón.
- **El gasto no atribuible no desaparece.** Cuando el perfil del grader no tiene organización no hay fila posible (el ledger tiene FK a `greenhouse_core.organizations`, y forzar una organización sintética sería peor que el hueco), pero la llamada queda contada en `growth.dataforseo.spend_ledger_drift` como no atribuible. "No está en el ledger" nunca significa "no ocurrió".
...
cablea al cliente público compartido"; el camino correcto es el grant por organización/persona que `TASK-1631`
ya materializó (2026-09-04) portado por el emisor propio y el gateway multi-issuer, EPIC-044: `TASK-1829`/
`TASK-1831` (actualizado 2026-09-04, TASK-1631)).

**Commands (write, capability-gated, audited, outbox):**
- `configureSeoTarget(orgId, { rootDomain, market }, actor)`
- `trackKeywords(seoTargetId, keywords[], actor, { source?, keywordSetName?, intent?, intentDeclaredBy?, env? })` — **IMPLEMENTADO (TASK-1308, 2026-08-07; intención declarada TASK-1659, 2026-08-14, migración `20260814221022082`)** en `src/lib/growth/seo/track-keywords.ts`. 🔴 **No es un INSERT: es un COMPROMISO DE GASTO DIFERIDO.** El write no cuesta nada; el rank capture diario (TASK-1303) le paga al proveedor por cada keyword vigente del set, en cada ciclo, hasta que alguien la deje de seguir. Por eso lleva **techo gobernado por target** (`GROWTH_SEO_TRACKED_KEYWORDS_PER_TARGET`, default 200 → outcome `capacity_exceeded` explícito, nunca silencio ni excepción), **entitlement per-org** (`seo_v2` vigente; NO consume allowance de site-audit — seguir no gasta hoy, gasta mañana), **outcome POR keyword** (`tracked|already_tracked|intent_changed|capacity_exceeded|invalid`, nunca un booleano), normalización + dedupe, idempotencia (`ON CONFLICT DO NOTHING` sobre el índice único parcial) y `FOR UPDATE OF` contra dos "Seguir" concurrentes. Append-only: sólo inserta — dejar de seguir es cerrar `effective_to` y es OTRO command, `untrackKeywords`, entregado en la misma task (abajo). App-lane `POST /api/admin/growth/seo/keywords/track` (capability `growth.seo.target.configure`) + lane ecosystem `POST /api/platform/ecosystem/growth/seo/keywords/track` (**sólo bindings de scope `internal`**: un binding cliente lee sus oportunidades pero no hace crecer su propia factura) + MCP tool `track_seo_keywords`, federada al gateway con el **scope de escritura del dominio** `efeonce.mcp.seo.write` — del DOMINIO y no de la capability: un scope por capability convertiría la lista de scopes de Entra en un espejo del `capabilities_registry`, editado a mano, que diverge; y el gateway NUNCA es autoridad de autorización. La regla es **un scope por clase de blast-radius** (`globe.credits.funding.ensure` tiene el suyo porque mueve dinero, no porque sea una capability). Lo fino ya se enforcea abajo: binding `internal` + entitlement per-ORG + techo. Evento `growth.seo.keyword_set.updated` emitido dentro de la transacción.
  - **Intención declarada (`intent`) — el segundo eje del set monitoreado (TASK-1659).** `intent: 'target' | 'opportunity'` responde **POR QUÉ** la keyword está en el set, y es **ortogonal a `source`**, que sólo dice de dónde vino el write. `target` = compromiso declarado con el cliente (puede estar en la posición 60 y eso NO es un fracaso: es la distancia que falta); `opportunity` = demanda ya medida que se está empujando. 🔴 **Sin default y sin un tercer valor `unknown`**: quien no declara escribe `NULL`, y la ausencia se propaga tal cual hasta la UI en vez de degradar a `opportunity` — asumir un valor para no romper callers viejos afirmaría que alguien clasificó la keyword cuando nadie lo hizo, y un enum `unknown` invitaría a escribir "no sé", que es distinto de no haber declarado nada. **Quien declara es el CALLER, no el command**: la lente Oportunidades (`KeywordOpportunitiesView`) manda `intent: 'opportunity'` explícito en cada "Seguir"; un caller que no pasa nada NO reescribe la historia de una membresía que alguien ya declaró. 🔴 **Los dos valores nunca se promedian en reporting**: un KPI que mezcla objetivos con oportunidades miente sobre ambos.
  - **`intentDeclaredBy` ≠ `actor`.** Uno es quién **asumió el compromiso**, el otro quién **ejecutó el INSERT**; difieren cuando un agente declara por encargo de una persona (default: `actor`). La autoría acompaña a la declaración o no se escribe ninguna de las dos — es el CHECK `seo_keyword_set_members_intent_authorship_check` expresado también del lado del command (`intent_declared_by`/`intent_declared_at` sólo existen con `intent`).
  - 🔴 **Cambiar la intención CIERRA la membresía vigente y ABRE otra (`clock_timestamp()`), NUNCA un `UPDATE` de la columna.** Un `UPDATE intent = 'target'` destruiría exactamente el dato que hace posible el reporte de avance: no es "esta keyword es objetivo", es "es objetivo desde marzo, y en marzo estaba en la 45". Reusa la ventana append-only de §4.1; el cierre va ANTES del INSERT y en la MISMA transacción, porque el índice único parcial `(keyword_set_id, keyword) WHERE effective_to IS NULL` no admite dos vigentes. `clock_timestamp()` y no `NOW()`, por la misma razón que `untrackKeywords`. Si la membresía anterior vivía en otro set del mismo target, la reclasificación la consolida en el set destino: el techo y el gasto son del **target**, no del set.
  - **Reclasificar NO consume cupo** del techo gobernado: el conteo vigente no se mueve (cierra una, abre una). Cobrarle techo a un cambio de intención bloquearía reclasificar justo cuando el set está lleno, que es cuando más falta hace.
  - **Outcome propio `intent_changed`** (+ `intent` / `previousIntent` en `SeoKeywordTrackOutcome`): no es `already_tracked` —sería mentira, sí pasó algo— ni `tracked`, que sugeriría gasto nuevo cuando el conteo vigente no se movió. El evento `growth.seo.keyword_set.updated` se emite **también** cuando lo único que cambió fue la intención (`intentChangedCount` en el payload), aunque `activeKeywordCount` quede igual: emitir sólo por `inserted > 0` dejaría esa transición invisible para todo downstream.
  - **Parity en las 3 lanes, con vocabulario validado en CADA frontera.** App `POST /api/admin/growth/seo/keywords/track` (body `intent`; ahí ejecutor y declarante son la misma persona), ecosystem `POST /api/platform/ecosystem/growth/seo/keywords/track` (body `intent` + `intentDeclaredBy`, sólo bindings `internal`) y MCP `track_seo_keywords` (`inputSchema` con ambos). Cada frontera valida contra `SEO_KEYWORD_INTENTS`/`isSeoKeywordIntent` y responde **400 explícito** ante un valor fuera del enum, **nunca** degradándolo a `undefined` en silencio —eso dejaría al caller creyendo que declaró algo—; el CHECK `seo_keyword_set_members_intent_check` de PostgreSQL es **backstop, no primera línea**. **Sin capability nueva** (sigue `growth.seo.target.configure`), **sin scope nuevo en Entra** (`efeonce.mcp.seo.write` ya cubre esta clase de blast-radius: declarar intención no compromete gasto adicional) y **sin flags nuevos**.
...
**Chokepoint único `enforceSeoRunEntitlement`** (`src/lib/growth/seo/entitlement.ts`) con **quota cap por-org** (gate de costo DataForSEO): entitlement → expiración (`expired` explícito) → allowance (site-audits/mes por tier) → budget (USD/mes por tier; el gasto sale **sólo** del ledger `seo_provider_spend_daily` vía `buildSeoProviderSpendMonthlySumSql`, con `consumer = 'seo'` — el `provider_cost` de los snapshots TASK-1299 quedó como procedencia por fila, jamás como sumando del gate). Acepta `estimatedCostUsd` y no deja pasar un run que exceda el budget restante. Consumer-agnóstico por diseño (mandato parity+MCP 2026-08-05): el plano fino de capability (`can()`) vive en el consumer; el MISMO gate sirve UI, Nexa, lane `app` y lane `ecosystem`/MCP. Knobs por env con default: `GROWTH_SEO_{CONTRACTED|TRIAL|PILOT}_{AUDIT_RUNS_PER_MONTH|MONTHLY_BUDGET_USD}`. Sanity live: `scripts/growth/_sanity-seo-entitlement.ts`.

🔴 **El presupuesto SEO cuenta SÓLO su propio consumo.** El fragmento canónico `buildSeoProviderSpendMonthlySumSql` filtra `sp.consumer = 'seo'` (TASK-1696), y ese filtro es parte del gate, no una optimización: sin él, el primer dólar que el grader AEO atribuya a una organización se descontaría del presupuesto SEO de **ese mismo cliente** sin que nada falle — el gate empezaría a bloquear capturas de rankings por un gasto que no es suyo, y `budget_exhausted` no se parece a su causa. El presupuesto del otro consumidor es `resolveAeoBudget` (`src/lib/growth/ai-visibility/budget.ts`), que lee la misma tabla con `consumer = 'aeo'`. **NUNCA** un consumer suma gasto con SQL propio: ni pantalla, ni tool MCP, ni señal.

**Lectura de atribución + lane ecosystem.** `readSeoProviderSpendByConsumer(organizationId)` es el reader canónico del gasto del mes cortado por `(consumer, family, cost_basis)`, expuesto en `/api/platform/ecosystem/growth/seo/provider-spend` y como tool MCP federada `get_seo_provider_spend`. El lane acepta **sólo bindings `internal`**, y no por permisos sino por **naturaleza del dato**: el gasto es lo que a Efeonce le **cuesta** servir a un cliente, no algo que el cliente haya consumido — un binding org-scoped leyendo "su" fila estaría leyendo nuestra estructura de costos. Un binding de cliente recibe **404 anti-oracle**, no 403: no debe aprender siquiera que el recurso existe.
...
1. **Costo DataForSEO (riesgo #1, $ + escalabilidad).** O(orgs × keywords × devices × días). Mitigación en dos planos, que no hay que confundir: el **control duro** es `enforceSeoRunEntitlement`, que bloquea antes de gastar (más batched Labs y GSC-first para bajar el numerador); la **detección temprana** es la señal `seo.provider.cost_over_budget` (kind `cost_guard`, steady 0), que **existe desde TASK-1696** y avisa al 80% del tope, antes de que el gate empiece a rechazar corridas. Hasta entonces el sobregiro sólo se manifestaba como corridas que fallaban con `budget_exhausted`, sin aviso previo — y nueve tasks (`1300`, `1301`, `1302`, `1303`, `1304`, `1308`, `1309`, `1651`, `1664`) la citaban como mitigación construida sin que existiera, cada una atribuyéndosela a otra. La señal necesita la dimensión `consumer`: una alarma que sólo viera gasto `seo` sub-reportaría exactamente el gasto del grader.
