...
**Reliability signals** (`/admin/operations`, subsistema Growth Health): `seo.rank.capture_lag` (steady=0), `seo.audit.stuck_tasks`, `seo.provider.cost_over_budget`, `seo.market_data.freshness` (TASK-1661; umbral tomado de la MISMA constante que el pre-check de gasto, para que señal y gasto no puedan divergir — sin dato es `warning`, no `error`: la ausencia de captura es un hecho esperable entre ciclos mensuales, no una falla del batch).
