...
es el error que este párrafo ya cometió una vez.** Inventario interno (as-of 2026-08-28): **25
tools (18 lecturas + 7 escrituras)** — `TASK-1696` agregó `get_seo_provider_spend` y `TASK-1662`
agregó `declare_seo_competitors` / `retire_seo_competitors` (writes) + `get_seo_keyword_gap`
(read), las 25 federadas en el código de `efeonce-mcp`. Desplegado en la revisión productiva del
...
Las 4 restantes (`get_seo_provider_spend` + las tres de `TASK-1662`) llegan **después** del
release `develop`→`main`: sus lanes (`provider-spend`, `competitors/{declare,retire}`,
`keyword-gap`) todavía no están en `main`, y desplegar el gateway antes las dejaría respondiendo
