...
- **Keyword discovery (TASK-1664, 2026-08-14 — code complete, rollout PENDIENTE: flag OFF + scheduler nace PAUSADO):** Cloud Scheduler `ops-seo-keyword-discovery-drain` (`*/10 * * * *`, declarativo en `deploy.sh`, **PAUSADO**) → `POST /seo/keyword-discovery/drain` en ops-worker → `drainKeywordDiscoveryRuns` → claim atómico `pending → running` por corrida (UPDATE condicional: el segundo worker matchea cero filas y responde `busy` sin gasto).
  ...
  - **Flag `GROWTH_SEO_KEYWORD_DISCOVERY_ENABLED`** (default OFF, subordinado a `GROWTH_SEO_ENABLED`): lo leen **DOS runtimes** — Vercel (enqueue + lanes) y ops-worker (drain). Dos frenos independientes: flag + scheduler pausado (patrón TASK-1661).
