...
Refina §5 (AI Architecture). El adapter Claude (`src/lib/workforce/contracting/ai/`) sigue siendo **advisory-only detrás del flag** (nunca aprueba/PDF/firma/email) — el flag NO cambia ese contrato; solo permite que el botón "Generar borrador IA" llame a Claude.

- **Modelo**: `claude-sonnet-4-6` (`getWorkforceContractingDraftModel`, override vía env `WORKFORCE_CONTRACTING_AI_MODEL`; no es un modelo Nexa pickable). `maxTokens=16384` (8192 truncaba el contrato bilingüe completo — el internacional caía con `prohibitedContentDetected ausente`).
- **Prompt v3** (`WORKFORCE_CONTRACTING_PROMPT_VERSION='workforce_contracting_ai_draft.v3'`, trackeado en el `ai_run` audit). Persona = especialista senior **nacional E internacional** (derecho del trabajo de la jurisdicción de la entidad contratante + acuerdos internacionales de prestación de servicios + compensaciones + traducción legal bilingüe). **Principio de jurisdicción canónico**: el contrato se rige por la jurisdicción de la **entidad contratante/matriz** (la operating entity, hoy Efeonce = Chile), **NO** por la del país de residencia del trabajador; **derivado dinámicamente del pack/entidad, no hardcodea Chile** (forward-compat si Efeonce abre entidad fuera de Chile). El idioma legalmente prevalente se deriva del `authoritativeLanguage` del caso (hoy es-CL en los 3 packs porque Efeonce es chilena). El drafting prompt enumera las cláusulas obligatorias/prohibidas del pack + declara la entidad contratante y el idioma prevalente.
