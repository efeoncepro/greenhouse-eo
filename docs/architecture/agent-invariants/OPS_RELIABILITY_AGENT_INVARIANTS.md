...
- **Tercera clase de mecanismo — alerta determinista de UNA señal de confiabilidad (TASK-1806, precedente `seo.etv_methodology.drift`):** distinta del AI Observer (LLM, pull, sólo alimenta una tarjeta de Admin Center, nunca push — TASK-638) y del dispatcher de watchdog de release (máquina de estados con dedup propia `greenhouse_sync.release_watchdog_alert_state` + escalamiento `warning→error→critical` + alertas de recuperación — `src/lib/release/watchdog-alerts-dispatcher.ts`, TASK-849). Usar esta clase simple cuando la señal de confiabilidad ya existe, es esencialmente binaria (error vs no-error) y el volumen de disparo es bajo: cron diario propio en `services/ops-worker/deploy.sh` que hace `POST` a un endpoint dedicado del `ops-worker` (ver `checkAndAlertSeoEtvMethodologyDrift()` en `src/lib/growth/seo/etv-methodology/drift-alert.ts` como plantilla) y llama `sendManualTeamsAnnouncement()` con un destino nuevo en `src/config/manual-teams-announcements.ts` que reutiliza el canal Teams físico correcto (no crear canal nuevo sin necesidad real). La cadencia del cron ES el mecanismo de dedup — sin tabla propia. Cuando la señal tiene niveles de severidad reales que importa distinguir, cuando hace falta alertar la recuperación además del problema, o cuando el volumen de instancias es alto y un recordatorio diario sin dedup por instancia sería spam, esta clase simple NO alcanza: usar el dispatcher completo de watchdog. **NUNCA** construir la máquina de estados completa (tabla propia de dedup + escalamiento) para una señal simple sin evidencia de que hace falta — es sobre-ingeniería que nadie mantiene.
- Chats verificados:
...
- **⚠️ UN solo servicio compartido por staging Y producción (canonizado live 2026-08-05, rollout TASK-1302):** el `ops-worker` **no** tiene gemelo por ambiente. Es un único servicio Cloud Run que sirve a los dos: **misma revisión, mismos Cloud Scheduler jobs, misma instancia Cloud SQL** (`greenhouse-pg-dev`). Está declarado textualmente en `services/ops-worker/deploy.sh` (§Environment): *"The ops-worker is a SINGLE Cloud Run service intentionally shared by both staging and production… This is the canonical topology, not a temporary shortcut."* `ENV` **sólo** selecciona qué secret refs de NextAuth/Resend se montan; NO parte la infraestructura.
  - **NUNCA** planifiques un flip "sólo staging" para algo que vive en el `ops-worker`: **no existe**. Prender un flag, despausar un cron o cablear una credencial ahí queda efectivo para **todos** los ambientes al mismo tiempo. Si necesitas rollout gradual real, gátealo **en datos** (per-org / per-perfil / opt-in persistido), NUNCA por ambiente.
  - **NUNCA** asumas que una capacidad worker-only necesita promoción a `main` para estar viva: el deploy se dispara desde `develop` (`.github/workflows/ops-worker-deploy.yml`), así que **queda LIVE al mergear a `develop`**, sin pasar por el release control plane. Ese blast radius se declara en la task **antes** del merge, no después.
  - **NUNCA** cuentes con "migro primero en staging y después en producción": hay **una sola** instancia Cloud SQL. Una migración aplicada es una migración aplicada en producción.
  - **SIEMPRE** configura los probes del worker compartido contra hosts públicos alcanzables por el propio worker. Un host staging protegido por SSO puede responder HTTP 302 y simular una indisponibilidad; para `portal_auth_health`, el destino canónico es `https://greenhouse.efeoncepro.com/api/auth/health` (caso `ISSUE-151`, 2026-08-12).
...
- **⚠️ Runtime nuevo ⇒ copia PROPIA de la config (canonizado live 2026-08-05 por TASK-1302; misma bug class que ISSUE-113):** un reader/cliente externo que hasta ahora sólo corría en rutas Vercel **no hereda nada** al empezar a correr también dentro del `ops-worker`. Cloud Run tiene su propio env: **flag + credenciales + `*_SECRET_REF` deben declararse otra vez** en `services/ops-worker/deploy.sh`. Caso fuente: TASK-1302 movió el reader de Google Search Console al worker y el worker **no tenía ninguna** de sus variables (`GROWTH_SEARCH_CONSOLE_ENABLED`, `GOOGLE_SEARCH_CONSOLE_OAUTH_CLIENT_ID`, el secret ref del client secret) — estaban todas en Vercel. Prender el módulo así habría hecho que `readSearchConsoleAnalytics` resolviera `disabled` y el batch degradara **TODAS** las organizaciones **en silencio**, reportando éxito. Precedente idéntico ya documentado en el propio `deploy.sh`: ISSUE-113 (`PERPLEXITY_ENABLED` ON con su secret ref nunca cableado en el worker ⇒ provider saltado sin ruido).
  - **NUNCA** des por cableada una variable porque exista en Vercel. **Tenerla en Vercel no sirve en Cloud Run.**
  - **SIEMPRE**, antes de prender el flag del módulo, corre `gcloud run services describe ops-worker --region us-east4 --format=json` y confirma que **todas** las variables que el reader lee están presentes en la **revisión activa**. El `grep -rn "<VAR>" src/ services/` te dice qué se lee; sólo el `describe` te dice qué existe.
  - **SIEMPRE** verifica además el binding de Secret Manager del SA `greenhouse-portal@` sobre cada secret nuevo (`ensure_secret_accessor_binding` en el deploy). En TASK-1302 el SA ya tenía `roles/secretmanager.secretAccessor` a nivel **proyecto**, así que el token per-org era legible — pero sin ese acceso el reader habría marcado la conexión del cliente como `revoked`, **rompiendo una integración que funcionaba**.
- **⚠️ El estado de pausa de un Cloud Scheduler job es DECLARATIVO, igual que las env vars (canonizado live 2026-08-05, TASK-1302):** el helper `upsert_scheduler_job` de `services/ops-worker/deploy.sh` recibe un 5º argumento `paused` y **re-aplica ese estado en CADA deploy** (`gcloud scheduler jobs pause` / `resume`). Por lo tanto despausar (o pausar) un job a mano **se revierte solo en el próximo deploy, en silencio** — exactamente la misma trampa que aplicar un env var sólo con `--update-env-vars` cuando el deploy usa `--set-env-vars` destructivo.
  - **NUNCA** dejes un job despausado (o pausado) sólo con `gcloud scheduler jobs resume|pause`. El SoT es el 5º argumento en `deploy.sh`; el comando en vivo sirve **únicamente** para el efecto inmediato.
  - **SIEMPRE** que pauses un job como rollback, pon **en el mismo movimiento** el 5º arg en `"true"` en `deploy.sh`. Si no, el próximo deploy lo vuelve a prender sin que nadie lo pida.
...
- Si el cambio toca el runtime que bundlea `ops-worker`, verificar build/deploy del worker o el drift guard del workflow. **Desde 2026-08-29 la declaración de rutas es GRUESA, no una enumeración de subdirectorios**: `services/ops-worker/**`, `services/_shared/**`, `src/lib/**`, `src/emails/**`, `src/types/**`, `src/config/**`, `src/components/**`, `src/i18n/**`, `src/@core/**`, `package.json`, `pnpm-lock.yaml`, `tsconfig.json`. La lista enumerada que vivía acá describía ~14 subdirectorios de un bundle de 1449 archivos: **NUNCA** la reintroduzcas (ver el bloque de cobertura de deploy más abajo).
...
- **⚠️ Worker @core boundary (canonizado live 2026-06-08, incidente ICO batch TASK-1048/1053):** los Docker builds de los 4 workers Cloud Run **EXCLUYEN `src/@core`** (la capa de tema AXIS/Vuexy) vía `.dockerignore` — los workers NO copian ni bundlean `@core`. Por eso **NUNCA importes `@core/theme/*` (ni `@menu`/`@layouts`/`@assets`) desde código `src/lib/**` que un worker bundlee** (dominio: `ico-engine`, `sync`, `finance`, `payroll`, `commercial-cost-attribution`, projections, materializers, PDF/Excel generators consumidos por projections). Con `--packages=external` el esbuild del worker externaliza el import → **silent startup crash** (`ERR_MODULE_NOT_FOUND` → container no escucha en 8080 → deploy failed). Causa raíz del incidente: `metric-registry.ts` (dominio) importó `@core/theme/axis-chart` para un mapa de color de UI.
  - **Delta 2026-09-04 (TASK-1828):** la misma frontera aplica al quinto deployable `auth-server` (`services/auth-server/Dockerfile` usa el mismo `--alias:@core` y `--packages=external`; el gate `worker:runtime-deps-gate` lo incluye).
...
- **⚠️ Worker runtime npm deps (hermano del @core boundary, canonizado live 2026-06-09, ISSUE-090):** los workers Node externalizan paquetes npm y los resuelven desde `node_modules` de la imagen. Por eso **TODO paquete npm importado directamente por un runtime worker DEBE estar en `dependencies`, NUNCA solo en `devDependencies` ni llegar por casualidad como transitive**. Los runners slim instalan `--prod`; `artifact-worker` instala el árbol completo por su ejecución source+tsx, pero conserva la misma regla de declaración directa. Si el paquete no está declarado, el contenedor puede caer con `ERR_MODULE_NOT_FOUND` o cambiar de resolución al variar otra dependencia. Causas reales: `pngjs` en el ops-worker y `playwright` directo en Artifact Worker pero sólo transitivo de `@playwright/test`.
  - **Guard canónico:** `pnpm worker:runtime-deps-gate` (`scripts/ci/worker-runtime-deps-gate.mjs`, wired en `ci.yml`) replica el grafo estático de los 4 workers + `auth-server` (5 entradas registradas), enumera paquetes externalizados y **falla loud** si alguno no está en `dependencies`. Mantener `SHIMMED_PACKAGES` en sync con los aliases Docker.
...
- **⚠️ Worker build inputs:** toda dependencia `file:` debe existir, estar trackeada, coincidir por SHA-512 con el lockfile, entrar a `.gcloudignore`/`.dockerignore` y copiarse antes de cada `pnpm install`. Los 5 workflows (4 workers + `auth-server`) observan `package.json`, lockfile, ignores y `vendor/**`. Ejecutar `pnpm worker:build-contract-gate`; canon completo en `GREENHOUSE_WORKER_BUILD_CONTRACT_V1.md`.
- **⚠️ Cobertura de rutas de deploy (canonizado live 2026-08-29, release `64bdd105c737`; gate `pnpm worker:deploy-path-gate`):** el workflow de cada worker decide **dos veces** si desplegar, y ambas decisiones se toman contra una lista de rutas — `on.push.paths` (si el workflow corre) y `WORKER_RUNTIME_PATHS` (el drift-check: si `git diff --quiet` no ve cambios en esas rutas, el step de deploy se **salta** y el job cierra `success`). Si el worker bundlea un archivo que la lista no menciona, el cambio entra a `main`, el manifest llega a `released`, **todo cierra verde y el worker sigue sirviendo la imagen anterior**. El síntoma aparece después y lejos, y apunta al dominio, nunca al deploy. Pasó exactamente así en `64bdd105c737`: job de 46 s, step `Deploy ops-worker` en `skipped`, `ops-worker` sirviendo `8adf8c2d3` — justo el código que ese release existía para corregir. Medido con el metafile de esbuild: el bundle son **1449** archivos, la lista cubría **24** prefijos y **696** archivos quedaban invisibles (`src/lib/postgres`, casi todo `src/lib/finance`, todo `src/lib/growth/seo`).
...
### Cloud Run `auth-server` — quinto deployable (EPIC-044, TASK-1828, desde 2026-09-04)

- El authorization server propio de Efeonce (`auth.efeonce.org`) es un **quinto** Cloud Run deployable junto a `ops-worker`, `commercial-cost-worker`, `ico-batch-worker` y el Job `artifact-worker`: service `auth-server` en `us-east4`, imagen `gcr.io/efeonce-group/auth-server` (Cloud Build; misma receta esbuild `--packages=external` + Node 22-slim que el `ops-worker`), SA propia `auth-server@efeonce-group` (`roles/cloudkms.signerVerifier` **sólo** sobre la llave `auth-server-es256` + `roles/cloudsql.client`), `cpu=1`/`512Mi`, `concurrency=80`, `timeout=30s`, `min=1` production / `0` staging. **Servicio ÚNICO compartido por staging y producción**, como el `ops-worker`: aplican las mismas reglas de arriba (no existe flip "sólo staging"; una migración aplicada es producción). Firma con **Cloud KMS HSM** (key ring `us-east4/auth-server`, `EC_SIGN_P256_SHA256`); la llave privada no existe fuera del HSM. Runbook: `docs/operations/runbooks/auth-server.md`.
  - **Ingress distinto a los workers:** `--ingress=internal-and-cloud-load-balancing` + `--allow-unauthenticated`. No es "IAM only": entra por el front door **compartido** del gateway MCP como segundo host (`efeonce-mcp/infra/terraform`, `enable_auth_host`; misma IP `34.111.78.237`, misma policy Cloud Armor). **NUNCA** le pongas `--no-allow-unauthenticated` "para alinearlo con los workers": el LB no adjunta identidad y el host dejaría de responder. **NUNCA** agregues `auth.efeonce.org` a `managed.domains` del certificado del gateway (re-provisiona `mcp.efeonce.org`): es un certificado aparte, `efeonce-auth-server-cert`.
  - **`/readyz` 503 con `AUTH_SERVER_ENABLED=false` NO es un deploy fallido.** El servicio vive (`/healthz` 200 siempre); con el flag OFF sólo declara que no está listo y el JWKS responde 404. El drift check del workflow y el deploy verifican `GIT_SHA` en la revisión activa, no `/readyz`. **NUNCA** leas un 503 de readiness con flag OFF como rollback pendiente; con flag ON, un 503 sí es incidente (revisar `checks` en el body: `postgres`, `kms`, `activeKey`).
  - **Env vars sólo en `deploy.sh`** (`services/auth-server/deploy.sh`, `--set-env-vars` destructivo): `AUTH_SERVER_ENABLED` (default `true` desde 2026-09-04), `AUTH_SERVER_ISSUER`, `AUTH_SERVER_ALLOWED_HOSTS`, `AUTH_SERVER_KMS_KEY`. **NUNCA** `gcloud run services update --update-env-vars` a mano: desaparece en el próximo deploy en silencio (misma trampa que el `ops-worker`). Ledger: `docs/operations/FEATURE_FLAG_STATE_LEDGER.md`.
  - **El deployer de CI necesita `roles/cloudkms.viewer` sobre la llave** (`github-actions-deployer@`, además de `iam.serviceAccountUser` sobre `auth-server@`): el preflight de `deploy.sh` hace `gcloud kms keys describe`; sin el viewer KMS responde como si la llave no existiera (`KMS key not found`) y el run falla **aunque la llave exista** (caso 2026-09-04, run `33870746218`). **NUNCA** "arregles" ese error creando otra llave.
...
- **NUNCA** crear handler Cloud Run sin pasar por `wrapCronHandler`. Sin él, pierdes runId estable, audit log consistente, captureWithDomain canónico, sanitización de error y 502 contract uniforme.
