...
>
> **Delta 2026-09-10 (TASK-1852) — habilitación de servicios, cohorte Berel/Sky.** Tres reglas que cruzan Commercial ↔ Client Portal ↔ Lifecycle (detalle en `GREENHOUSE_CLIENT_PORTAL_DOMAIN_V1.md` §5.4, `GREENHOUSE_CLIENT_LIFECYCLE_V1.md` Delta 2026-09-10 y `GREENHOUSE_CLIENT_SERVICE_EXPERIENCE_DECISION_V1.md` Delta 2026-09-10): (1) **NUNCA** inferir "módulo contratado" desde `business_line` ni desde el nombre del servicio: la única evidencia es `engagement_commercial_terms.bundled_modules`, que `declareCommercialTerms` valida en la misma transacción contra `greenhouse_client_portal.modules` activos (`effective_to IS NULL AND effective_from <= CURRENT_DATE`); (2) **NUNCA** colapsar `person_invitation_pending` (persona `status='invited'` con invitación diferida sin entregar) con `person_not_authorized_in_organization` — tienen dueño y acción distintos; (3) **NUNCA** tratar un destino Teams registrado (`recipient_kind='chat_group'`, `ready`) ni una política de preferencias aplicada (`client_service_default_v1`) como evidencia de mensaje enviado: registrar y preferir no envían nada. El módulo contratado por Sky es Creative Hub (`creative_hub_globe_v1`); su ruta `/creative-hub` la construye TASK-1857.
