...
| Connection pooling Postgres V1 data-driven + V2 PgBouncer GKE Autopilot contingente | Accepted | PostgreSQL / Cloud SQL / Vercel + Cloud Run runtimes | [GREENHOUSE_POSTGRES_CONNECTION_POOLING_V1.md](GREENHOUSE_POSTGRES_CONNECTION_POOLING_V1.md) | TASK-846 (V1 deployed) + TASK-847 (V2 contingente). V1: defense-in-depth 3 capas sin multiplexer. V2: PgBouncer GKE Autopilot deploy si signal `runtime.postgres.connection_saturation` sustained > 60%. Pivot crítico: Cloud Run NO soporta TCP raw → multiplexer va en GKE. Trigger: PG saturation 103% + Sentry weekly report 7 errors NEW. |
