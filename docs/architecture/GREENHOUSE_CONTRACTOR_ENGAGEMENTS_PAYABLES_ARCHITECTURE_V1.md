...
## Contrato vigente — compensación, episodios y recuperación (2026-09-03)

- `rate_amount` representa la tarifa **bruta** recurrente. Un acuerdo líquido debe conservar su evidencia y convertir el importe con la política tributaria aplicable; la tasa del caso histórico no se convierte en default global.
- `updateContractorEngagement` actualiza términos del engagement y emite su evento. No reescribe `rate_amount_snapshot`/`gross_amount` de submissions ni bruto/retención/neto de payables ya creados.
- `createContractorWorkSubmission` acepta el bruto explícito del command. Sin él, deriva cantidad × tarifa para hour/day y tarifa completa para las otras tarifas. **No prorratea por `service_period_start/end`**. El composer self-service envía `grossAmount: null`; su cálculo automático no constituye una política de meses parciales.
- Para una excepción proporcional autorizada: registrar fechas, base, fórmula, monto y actor; crear/ajustar el envío según su state machine; `submitContractorWorkSubmission` → `reviewContractorWorkSubmission(approve)` → `createContractorPayableFromSubmission`. No duplicar con off-cycle un trabajo ya representado por envío/payable. El caso agosto usó 12/31 por confirmación expresa, no como regla de calendario universal.
- La PK `contractor_engagement_id` y el `public_id` son contratos distintos. El POST Finance pasa `contractorEngagementId` directamente a `getContractorEngagementById`, que filtra por PK; el formulario off-cycle actual no resuelve `EO-CENG-…`. El error de lookup no autoriza crear otra identidad. Resolver por reader y comprobar episodio es diagnóstico; un selector por ID público sigue pendiente de producto.
- Un reingreso contractor conserva el `profile_id` longitudinal y crea un episodio de relación/engagement con fechas y condiciones propias. El cierre de servicio anterior no borra pendientes: `ending` puede coexistir con pagos históricos abiertos; no se fuerza `ended` ni se trasladan activos documentales, envíos u obligaciones al nuevo episodio.
- La recuperación de member/asignación es un command separado y no cambia la naturaleza contractual ni los importes. Las guardas de salida histórica y del consumer `member.updated` se rigen por la [decisión de recuperación de reingreso](GREENHOUSE_WORKFORCE_REENTRY_RECOVERY_DECISION_V1.md). Se verifica después de los consumers, además del readback transaccional.
- Trabajo aprobado y boleta recibida son hechos separados. `invoice_asset_missing` continúa bloqueando readiness cuando corresponde; no se desactiva `requiresInvoice` ni se usa boleta de otra etapa para simular cumplimiento.

Fuentes verificadas: `src/lib/contractor-engagements/{store.ts,work-submissions/store.ts,payables/store.ts,payables/readiness.ts}`, `src/views/greenhouse/contractors/ContractorSubmissionComposer.tsx` y `src/app/api/finance/contractor-payables/route.ts`. Operación y evidencia: [compensación](../documentation/hr/contratistas-compensacion.md), [manual Finance](../manual-de-uso/finance/pagos-a-contractors.md), [auditoría del reingreso](../audits/payroll/VALENTINA_REHIRE_IDENTITY_RECOVERY_2026-09-03.md). La auditoría es evidencia fechada, no sustituto de un readback vigente.
