...
## Delta TASK-1145 — `personal` suma "tu recibo de pago ya está disponible"

- Cierra el follow-up de pago de TASK-1144. El resolver `personal` ahora compone una **señal de recibo de pago** vía el reader canónico `pgMemberPayslipReadyForRecentPeriod(memberId)` (`src/lib/payroll/postgres-store.ts`): `EXISTS` de un `payroll_entries` activo cuyo `payroll_period` está en `status='exported'` y cae en el **mes operativo reciente** (actual o anterior, calendario operativo canónico `getOperationalPayrollMonth` — NUNCA `new Date()` naive para el mes). Query validada contra PG real (gate TASK-893): dummy → `ready:false`, member real con período exportado → `ready:true`.
- **Regime-neutral:** el copy es `'Tu recibo de pago más reciente ya está disponible, ¿lo revisamos?'` — **"recibo", NUNCA "liquidación"** (un colaborador Deel no recibe una liquidación chilena). El prompt es **informativo/positivo**: NO lleva `hint` (no es atención/pendiente/kpi). Orden: atrasos > **recibo** > desempeño > aprobaciones > vacaciones (cap 4).
- **Allowlist categórica intacta:** el texto dice solo "tu recibo está disponible", **NUNCA el monto** (el `EXISTS` ni siquiera lee el neto). El monto + desglose vive en el tool `explain_my_pay` (TASK-1146), que es el camino entitled cuando el colaborador pregunta "¿cuánto cobré y por qué?".
- **Degradación independiente:** la 3ra fuente (`pgMemberPayslipReadyForRecentPeriod`) corre en el mismo `Promise.allSettled` que vacaciones + ICO → si el payroll store falla, el resto de los prompts personales siguen.

## Procedencia

TASK-1078 (Tier 1/1.5 — floating chat + `NexaContextScope`) · **TASK-1087 (Tier 2 — data-aware)** · **TASK-1139 (Tier 2.1 — hint UI + entrypoint + cache)** · **TASK-1141 (registry + contexto `personal`)** · **TASK-1143 (contexto `finance`)** · **TASK-1144 (`personal` + performance ICO)** · **TASK-1145 (`personal` + recibo de pago disponible)**.
