...
5. **Estado de `services`:** `requestApproval` y `declareSampleSprint` marcan non-regular services como `status='pending_approval'`; `approveEngagement` los vuelve `active`. Desde TASK-836 el runtime tiene `services_status_check`; la migración `20260620172000000_sample-sprint-service-status-check.sql` declara explícitamente `pending_approval` como estado válido para evitar drift entre el contrato de engagement y el CHECK de `greenhouse_core.services`.
