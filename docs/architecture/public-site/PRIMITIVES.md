...
| `Glitch` | `planned/block` | `semantic-widget` | Planned plugin `efeonce-editorial-blocks`, block `efeoncepro/glitch-drop` | Weekly blog series `Glitch de la semana` on `efeoncepro.com/blog` | Functional/technical contract `docs/documentation/public-site/glitch-drop-gutenberg-block.md`; authoring rules `docs/documentation/public-site/gutenberg-post-authoring-recipes.md` | Pending implementation. V1 must verify inserter, save/reload without invalid block, editor iframe styles, front-end render, mobile 390px no overflow, and no automatic conversion of real quotes |
