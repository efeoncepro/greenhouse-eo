...
## Delta 2026-05-28 — TASK-947: Detail page canonical `/nexa/insights/[id]`

### 9. Routing canonical del detail page

**Decisión arquitectónica (post arch-architect Greenhouse 4-pillar + greenhouse-ico 5-pillar eval)**: el detail page de un Nexa Insight vive en **`/nexa/insights/[id]`** top-level cross-domain, NO bajo `/agency/insights/...` ni `/finance/insights/...` ni `/people/[id]/insights/...`.

**Justificación 4-pillar**:

| Pilar | Justificación |
|---|---|
| **Safety** | Anclar a un route_group concreto fuerza cross-tenant violations cuando un EFEONCE_ADMIN entra desde Person 360 o cuando un client_user llega desde un email digest. Capability dedicada `nexa.insights.read` (NO heredada del route_group). |
| **Robustness** | Semánticamente neutral — el dato (signal AI enrichment) es **cross-domain por diseño**. Mantener `/agency/...` viola el patrón "domain boundaries" (ICO/Finance/Identity insights cohabitan en el mismo modelo). |
| **Resilience** | Weekly Executive Digest email + Teams notifications generan links que el receptor abre días después — permalink top-level estable es contrato. Bajo route_group rompe cuando la composición de menú se reordena. |
| **Scalability** | TASK-944 introduce `/finance/insights/[id]` como alias del mismo shell. TASK-449 había propuesto `/[tenant]/nexa/insights/[id]`. **Convergencia canonical**: `/nexa/insights/[id]` top-level + aliases bajo dominios específicos reusan el componente, cero duplicación. Precedente canonical: `/admin/...` como lane cross-domain. |

### 10. Drill key canonical — Dispatch prefix `EO-AIS-*` / `EO-AIE-*`

El URL acepta dos shapes de ID con dispatch en el resolver del page:

| Prefix | Semántica | Caso de uso canonical |
|---|---|---|
| `EO-AIS-*` | **Signal-anchored** — estable cross-period (TASK-943 append-only-aware) | Cards "Ver causa raíz" en las 5 surfaces → default = ver narrativa actual del signal vigente. Si el LLM-worker regenera el enrichment, el link sigue válido apuntando al actual. |
| `EO-AIE-*` | **Enrichment-anchored** — snapshot LLM específico atado a `(signal_id, run_id, model_id, prompt_hash, processed_at)` | Share button (TASK-449) genera permalink por enrichment_id explícito. Semántica "esto vio el operador cuando capturó la URL". |
| `EO-FAIE-*` | **Finance Enrichment** (path separado `finance_ai_signals`) | Dispatch al helper sibling Finance Signal Engine + alias `/finance/insights/[id]`. |

### 11. Resolver canonical del page

```text
GET /nexa/insights/[id]
├─ requireServerSession + can(subject, 'nexa.insights.read', 'read', 'tenant')
├─ id.startsWith('EO-AIS-') → signal-anchored
│   └─ Lookup ai_signals_current (PG serving) → resolve latest enrichment
│       └─ render con anchor estable
│
├─ id.startsWith('EO-AIE-') → enrichment-anchored (3-tier lookup)
│   ├─ ai_signal_enrichments (current) → found? render
│   ├─ ai_signal_enrichment_history (TASK-914) → found?
│   │   render con banner amber "versión histórica del <fecha>; ver actual →"
│   └─ No history → notFound() (404 semántico)
│
├─ id.startsWith('EO-FAIE-') → Finance helper sibling dispatch
│
└─ Otherwise → notFound()
```

### 12. Helper canonical `readNexaInsightDrill(id, subject)`

Vive en `src/lib/ico-engine/ai/nexa-insight-drill-reader.ts`. Server-only. Discriminated union return forza al consumer manejar todos los states:

```typescript
type NexaInsightDrillResult =
  | { state: 'current'; enrichment: AiSignalEnrichmentRecord; lifecycleTimeline: HistoryEntry[]; relatedEnrichments: TopAiLlmEnrichmentItem[] }
  | { state: 'superseded'; enrichment: AiSignalEnrichmentRecord; currentEnrichmentId: string; supersededAt: string }
  | { state: 'expired'; signalId: string; resolvedAt: string }
  | { state: 'not_found' }
  | { state: 'degraded'; reason: 'pg_serving_stale' | 'history_unavailable' | 'bq_view_unavailable' }
```

**Subject-aware filter**: si el subject NO es EFEONCE_ADMIN y NO es assigned member del space_id/member_id del insight → `state: 'not_found'` (NO 403 — anti-oracle TASK-872 pattern).

### 13. Capability canonical `nexa.insights.read`
...
| action | `read` |
| scope | `tenant` |
| grants V1 (runtime.ts dual-plane TASK-873) | EFEONCE_ADMIN + FINANCE_ADMIN + HR_MANAGER + DEVOPS_OPERATOR + assigned member (scope-filtered en helper) |

Capabilities ortogonales para mutations (TASK-449 V1.3):
...
### 14. 12 UI states canonical (TASK-946 framework mapping)

| Loader state | UI state | Render |
|---|---|---|
| `current` | `ready` | Full: narrativa + lifecycle timeline + recommended action + related |
...
### 15. Hard rules canonical (anti-regresión)

1. **NUNCA** crear detail page bajo route_group de dominio (`/agency/...`, `/finance/...`, `/people/...`). Canonical = `/nexa/insights/[id]` top-level.
2. **NUNCA** consumer downstream compone su propio drawer/modal/detail.
3. **NUNCA** crear URLs canonical ancladas al `enrichment_id` para cards "Ver causa raíz" current. Cards usan `signal_id`; `enrichment_id` reservado para share.
4. **NUNCA** retornar `403` desde el detail page. `notFound()` siempre (anti-oracle TASK-872).
5. **NUNCA** read directo de `ico_engine.ai_signals` raw ni `ico_ai_signal_enrichments` PG desde el page. Pasa por `readNexaInsightDrill`. VIEW canonical TASK-943.
6. **NUNCA** colapsar UI states `not_found` + `expired` + `superseded` + `stale` en un único "Sin datos". Mapping explícito TASK-946 framework.
7. **NUNCA** mostrar narrativa superseded sin banner explícito.
8. **NUNCA** invocar `Sentry.captureException` directo. Usar `captureWithDomain(err, 'home', { tags: { source: 'nexa_insight_detail' } })`.
9. **NUNCA** seed `nexa.insights.read` sin grant en `runtime.ts` mismo PR (invariant TASK-873).
10. **NUNCA** emit URL `/agency/insights/*` desde loader/componente nuevo.
11. **NUNCA** romper el dispatch prefix. Mantener `EO-AIS-*` / `EO-AIE-*` / `EO-FAIE-*`.
12. **SIEMPRE** email/Teams notification que incluya link a insight usa `/nexa/insights/[id]`.
13. **SIEMPRE** cuando emerja consumer cross-surface nuevo, navegar al canonical.
14. **SIEMPRE** que el LLM-worker regenere enrichment, el URL `/nexa/insights/EO-AIS-*` sigue válido apuntando al current.

### 16. Composabilidad cross-task

| Task | Cómo compone con el detail page canonical |
|---|---|
| **TASK-449** (Read/Pin/Dismiss/Share) | Acciones inline en detail page + en cards. Share button genera permalink anclado por `enrichment_id`. |
| **TASK-944** (Finance toggle) | Alias `/finance/insights/[id]` reusa el mismo `NexaInsightDetailView` shell. |
| **TASK-945** (lifecycle timeline) | Timeline embebido vía `lifecycleTimeline[]` del helper. Time-travel via `?at=YYYY-MM-DD`. |
| **TASK-946** (12 UI states) | Detail page **canoniza** el mapping; las 5 surfaces lo adoptan post-cancanization. |
| **TASK-698** (Nexa Conversation Drawer) | Ortogonal. CTA cross-link "Pregúntale a Nexa sobre esto" abre drawer con prompt prefill. |
| **Weekly Executive Digest** | Email deep-links a `/nexa/insights/[id]` — funcionales por primera vez. |

### 17. Reliability signal canonical `home.insights.drill_404_rate`

V1.1 (TASK-947 Slice 5):

- kind=`drift`, subsystem rollup `delivery`.
- Severity: warning >0.5% requests/24h, error >5%, steady=0 post-deploy.
- Detecta drift entre links emitidos (Bento + Digest emails + Teams) y enrichments accesibles.

### 18. Bug class fuente

Live 2026-05-28 (post TASK-943 verification): 404 sistemático en `/agency/insights/EO-AIE-0D5D0740` confirmado en Vercel staging cuando operador clickea "Ver causa raíz" en Home Nexa Insights bento. RCA: drift TASK-696 (Smart Home v2 redesign construyó `drillHref` apuntando a página que nunca se materializó). TASK-947 cierra el bug + canoniza el routing para 5 surfaces + Weekly Digest + Teams deep-links.

Implementation: TASK-947 V1 MVP (`to-do/`, 8 slices).
