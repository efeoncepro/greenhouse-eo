...
|---|---|---|
| **Adaptive Sidecar** (asistencia/inspección/review/preview/edición contextual que preserva contexto) | `AdaptiveSidecarLayout`, `ContextualSidecar`, `adaptive-sidecar-controller`. **Animación de entrada del panel inline `panelEntrance` (TASK-1079):** `appear` (default, byte-idéntico al histórico: fade + nudge + scale + blur, aparece en su lugar) \| `slide` (entra/sale por traslación pura `x 100%→0` desde el borde, empujando el contenido — sin scale/blur; el grid anima `grid-template-columns` en paralelo). Demostrado en el lab `/platform/adaptive-sidecar/mockup` (toggle Appear/Slide). | [GREENHOUSE_ADAPTIVE_SIDECAR_DECISION_V1.md](../GREENHOUSE_ADAPTIVE_SIDECAR_DECISION_V1.md) + [GREENHOUSE_ADAPTIVE_SIDECAR_UI_PLATFORM_V1.md](../GREENHOUSE_ADAPTIVE_SIDECAR_UI_PLATFORM_V1.md) |
| **Composition Shell** (substrato de coreografía de layout: regiones singleton `primary/aside/lead/dock/overlay` + composiciones nombradas `single/leadPlusContext/split/focused` + morph in-place View Transitions + reflow por size class M3; domain-neutral, opt-in; del que Adaptive Sidecar / NexaMomentComposition / OrganizationWorkspaceShell son consumers; NO reemplaza `LayoutContent`) | `CompositionShell`, `composition-shell-controller` (`resolveComposition`, `resolveCompositionLayout`, `reduceCompositionShellState`, `createCompositionShellEvent`) — morph vía `startViewTransition` + motion tokens (no fork); regiones = query containers (seam a Adaptive Card / TASK-1115). **Fluidez (TASK-1117, opt-in `fluidity='rich'`):** stagger de entrada (`composition-shell-motion`) + morph interrumpible (`morphStrategy='interruptible'`, framer-motion `layout`) + promoción shared-element (card→lead) + drawer temporal en compact `split`. **Hardening (TASK-1119):** guard dev-time del singleton VT (`composition-shell-vt-guard`) + telemetry opt-in (`onTelemetry`) + lint `greenhouse/no-ad-hoc-layout-morph` (namespace reservado `gh-region-*`) + baseline GVC durable. Default `baseline` byte-idéntico a V1 | [GREENHOUSE_COMPOSITION_SHELL_DECISION_V1.md](../GREENHOUSE_COMPOSITION_SHELL_DECISION_V1.md) + [GREENHOUSE_COMPOSITION_SHELL_UI_PLATFORM_V1.md](../GREENHOUSE_COMPOSITION_SHELL_UI_PLATFORM_V1.md) · lab `/design-system/composition-shell` |
| **Adaptive Card density** (capacidad COMPARTIDA, no un componente card nuevo — TASK-1115; hermana del Composition Shell) | `card-density` (`useContainerDensity` + `resolveCardDensity`/`resolveCardDensityRequest` + `isCardDensityAtLeast`) desde `@/components/greenhouse/primitives`. El card se adapta a SU PROPIO ancho (container query `inline-size`) con modos `full`/`condensed`/`peek` — **condensación honesta** (versión real más chica, NUNCA clip/overflow/`$0`; el dato clave nunca desaparece). El card NO hereda del shell: el **seam es la container query** (cuando una región del shell condensa, el ancho del card cambia → su fit mode se reevalúa solo). Generaliza el density contract de tablas (TASK-743) + reusa el patrón `size→behavior` de `resolveAdaptiveSidecarMode`. Adopción **aditiva opt-in** (`density?: CardDensityRequest`, default `full` byte-idéntico): `MetricSummaryCard` (condensed oculta subtitle, peek = title+value) + `MetricTrendCard` (condensed reduce el chart + oculta metricName/period, peek sin chart). SSR-safe (modo inicial `full`, sin hydration mismatch). **Morph fluido del cambio de densidad (reveal/unfold + stagger, 2026-06-14):** el contenido que entra/sale entre modos se DESPLIEGA en vez de popear — texto = unfold de altura (`height:0↔auto`), chart = fade + rise (sube `cardDensityRevealRisePx`); cascada por pieza en orden de lectura (`cardDensityRevealStaggerSec`) + desync caja(200ms)/contenido(300ms). Tokens en `card-density-motion.ts` (`cardDensityLayoutTransition`/`cardDensityRevealTransition`/`cardDensityRevealRisePx`/`cardDensityRevealStaggerSec`), derivados del SoT, reduced-motion horneado. **Reglas duras:** el card adaptable usa `layout='position'` (NO `'size'` → framer no escala la caja, el texto no se deforma); el chart va SIEMPRE en modo fade (`animateHeight=false`) — `height:auto` sobre el `ResponsiveContainer` de Recharts crea un loop infinito por su ResizeObserver (`Maximum update depth`); `useContainerDensity` solo hace setState al CRUZAR un breakpoint (anti-loop ResizeObserver↔framer `layout`). NUNCA fork ni construir sobre `AdaptiveSidecarLayout` | [GREENHOUSE_COMPOSITION_SHELL_DECISION_V1.md](../GREENHOUSE_COMPOSITION_SHELL_DECISION_V1.md) (§Delta Adaptive Card + §Delta coreografía 2026-06-14) + [GREENHOUSE_OPERATIONAL_TABLE_PLATFORM_V1.md](../GREENHOUSE_OPERATIONAL_TABLE_PLATFORM_V1.md) (precedente TASK-743) · lab `/design-system/card-density` |
| **Card entrance "al armarse"** (capacidad de ENTRADA del contenido — TASK-1110 Slice A; hermana de la densidad) | `entrance?: CardEntrance` (`'none'`\|`'assemble'`) en `MetricTrendCard` (default `'none'` = legacy byte-idéntico). `'assemble'` = el dato **se construye frente al usuario** al montar: el número **cuenta** 0→valor (`AnimatedCounter animateFrom`) y el chart **se dibuja solo** (Recharts area draw-in). Pensado para cuando una card materializa en una respuesta de Nexa moment. **Conteo confiable:** `animateFrom` cuenta al MONTAR **sin gate de `isInView`** → elimina la race del IntersectionObserver frío que hacía SALTAR el número en flujos de re-monte/composición. **never-hidden + reduced-motion horneados:** SSR / no-JS / `prefers-reduced-motion` muestran el valor FINAL de una (sin contar); `suppressHydrationWarning` cubre el mismatch SSR(value)↔cliente-motion(0) intencional; reduced-motion leído síncrono (`matchMedia`, sin el lag del hook). Solo propiedades de compositor. Es la ENTRADA del **contenido** (frontera de motion: distinta del morph estructural de regiones por View Transitions y del morph de densidad por framer `layout` — nunca dos capas sobre la misma propiedad del mismo nodo). Tokens en `card-density-motion.ts` (`CardEntrance`, `cardAssembleRisePx`, `cardAssembleTransition`). **`MetricSummaryCard` NO lo tiene** (su `value` es `ReactNode` estático sin chart → su "armado" es el fly-in del wrapper/grupo, p.ej. Motion Primitive `stagger` — no un prop no-op). **Pendiente (end-to-end TASK-1110):** `NexaAnswerBubble` renderiza su chart/métrica con Recharts inline propio (NO reusa `MetricTrendCard`) → para que la RESPUESTA del moment se arme, wirear ahí el mismo ensamble o converger a la primitive | [GREENHOUSE_COMPOSITION_SHELL_DECISION_V1.md](../GREENHOUSE_COMPOSITION_SHELL_DECISION_V1.md) + `docs/tasks/in-progress/TASK-1110-*` (Delta 2026-06-14) · lab `/design-system/card-density` (Secuencia macro) |
...
| **Floating Action Dock** (acciones persistentes ancladas al viewport) | `NexaFloatingButton`, `ScrollToTop` + safe-area CSS vars; Nexa FAB usa mark animado + `GreenhouseSpectrumBeam` brand solo en hover/focus detrás del botón | HISTORIAL Deltas 2026-06-06c / 2026-06-13h |
| **Brand logo variations** (logos externos portados desde AXIS Figma con variants como kinds) | `GreenhouseBrandLogoMark` (variants `isotype` / `contained` / `lockup`; kinds Gemini, Adobe, Firefly, Photoshop, Illustrator y After Effects declarados en el controller). La primitive gobierna kind, tamaño, a11y y placement; los assets viven bajo `public/images/logos/axis/` con política SVG/PNG y cache-buster. | [BRAND_LOGO_VARIATIONS.md](./BRAND_LOGO_VARIATIONS.md) · HISTORIAL Delta 2026-06-18 · lab `/design-system/brand-logos` |
...
| **Border beam / spectrum / shiny / spotlight aura** | `GreenhouseBorderBeam` (effects `beam` / `spectrum`) + `GreenhouseSpectrumBeam` (primitive fina del efecto spectrum) + `GreenhouseShinyBorder` (surface/CTA shiny con highlight radial, glow inferior y contenido elevado) + `GreenhouseSpotlightCard` (card con spotlight atado al puntero; kinds `blue` / `purple` / `green` / `red` / `orange` / `nexaBrand` / `nexaBrandCore` / `nexaBrandSignal` / `nexaBrandGlass`); variants `ambient` / `interactive` / `progress`; spectrum palettes `axis` / `nexa`; border kinds `nexaSurface` / `promptDock` / `evidencePeek` / `approvalCard` / `asyncOperation`; intensities `subtle` / `medium` / `strong`; overlay decorativo con radio heredado, colores AXIS/MUI/brand SSOT y reduced-motion; no Tailwind config ni HEX locales | HISTORIAL Delta 2026-06-13g · lab `/design-system/border-beam` |
...
| **Nexa expression cues** | `NexaExpressionCue` (registry gobernado `cue -> treatment` para intenciones `ready` / `reviewing` / `risk` / `idea` / `source` / `next_step` / `opportunity` / `missing_context` / `blocked` / `sensitive`; contexts `chatText` / `answerSurface` / `stateChip` / `emptyState` / `promptDock`; treatments `nexaMark` / `fluentAsset` / `tablerIcon` / `statusDot` / `textOnly` / `none`; degrada dominios sensibles finance/payroll/legal/security/contractual a iconografía sobria o texto; `NexaExpressiveText` acepta segmento `type: 'cue'` y conserva plain text accesible) | HISTORIAL Delta 2026-06-18 (TASK-1132) · lab `/design-system/nexa-chat` · assets curados `public/images/nexa-expression-cues/*` con notice MIT |
...
| **Roadmap timelines** (roadmaps, release plans, horizontes de producto/onboarding) | `GreenhouseRoadmapTimeline` (variants `horizontal` / `stacked` / `compact`; kinds `productRoadmap` / `releasePlan` / `implementationPlan` / `clientOnboarding` / `custom`; estados `complete` / `active` / `pending` / `blocked` + aliases del prompt `done` / `in-progress` / `upcoming`) | HISTORIAL Delta 2026-06-13k · lab `/design-system/roadmap-timeline` · scenario `design-system-roadmap-timeline` |
...
`GreenhouseBorderBeam` es la primitive canónica para patrones de borde perimetral animado inspirados en "border beam" y "rainbow borders button". `GreenhouseSpectrumBeam` es la primitive fina del efecto `spectrum`: anillo completo animado + aura blur amplia, sin poseer el botón, card o caja que lo usa. `GreenhouseShinyBorder` trae el patrón "shiny borders button" como surface/CTA tokenizada: highlight radial superior, glow inferior y contenido elevado, sin Tailwind ni HEX locales. Toman la intención visual del prompt shadcn/Tailwind, pero la implementan como overlay decorativo Greenhouse: MUI `sx`, colores desde `theme.axis.*` / `theme.palette.*`, radio heredado, `prefers-reduced-motion` y sin extender Tailwind.
...
- Para el patrón shiny realism, usar `GreenhouseShinyBorder` cuando la CTA completa necesita highlight radial + glow inferior. No recrear blobs/gradientes locales.
...
Specimens vivos: `/design-system/border-beam`, `data-capture='border-beam-button-specimen'`, `data-capture='border-beam-shiny-button-specimen'`, `data-capture='border-beam-nexa-spectrum-box'` y `data-capture='border-beam-nexa-brand-spectrum-box'`. La variación lab-only del composer que usa `GreenhouseSpectrumBeam` vive con los otros composer en `/design-system/nexa-chat`: `data-capture='nexa-composer-spectrum-unified'`; inicia inactive/sin texto y cambia al estado activo cuando el usuario escribe (no aplicado al chat runtime).
...
- `NexaFloatingButton` consume `GreenhouseNexaAnimatedMark` como FAB global dentro de `ShellFloatingActionDock`. Su hover/focus usa `GreenhouseSpectrumBeam` en paleta Nexa detrás del FAB, con fade-out lento, animación pausada en idle y reduced-motion sin transición larga; no mover el glow al foreground ni mezclarlo con la sombra navy del botón.
