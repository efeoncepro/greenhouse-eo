...
| **Nexa brand marks** | `GreenhouseNexaBrandMark` (kinds `askNexaBadge` / `badgeIcon` / `inlineMark` / `monoMark`) | HISTORIAL Delta 2026-06-09d · lab `/admin/design-system/nexa-brand` · assets `public/images/nexa-mark/*` |
...
- `GreenhouseNexaBrandMark` es la primitive canónica para el mark de Nexa. La unidad mínima visual es arco + sparkle; nunca usar `tabler-sparkles` solo como sustituto de Nexa. `kind='askNexaBadge'` renderiza el badge Midnight Navy con label `Pregúntale a Nexa` para entradas conversacionales; el texto usa tipografía de control (`button`/label-md) para tener presencia sin convertirse en headline. `badgeIcon`, `inlineMark` y `monoMark` cubren usos más compactos. El source visual vive en `public/images/nexa-mark/`; la hoja viva `/admin/design-system/nexa-brand` debe mostrar nuevos kinds/sizes con GVC antes de migrarlos a producto. Nuevos kinds deben preservar el contrato de marca + `aria-label`.
