...
| **Composition Shell** (substrato de coreografía de layout: regiones singleton `primary/aside/lead/dock/overlay` + composiciones nombradas `single/leadPlusContext/split/focused` + morph in-place View Transitions + reflow por size class M3; domain-neutral, opt-in; del que Adaptive Sidecar / NexaMomentComposition / OrganizationWorkspaceShell son consumers; NO reemplaza `LayoutContent`) | `CompositionShell`, `composition-shell-controller` (`resolveComposition`, `resolveCompositionLayout`, `reduceCompositionShellState`) — morph vía `startViewTransition` + motion tokens (no fork); regiones = query containers (seam a Adaptive Card / TASK-1115) | [GREENHOUSE_COMPOSITION_SHELL_DECISION_V1.md](../GREENHOUSE_COMPOSITION_SHELL_DECISION_V1.md) + [GREENHOUSE_COMPOSITION_SHELL_UI_PLATFORM_V1.md](../GREENHOUSE_COMPOSITION_SHELL_UI_PLATFORM_V1.md) · lab `/design-system/composition-shell` |
