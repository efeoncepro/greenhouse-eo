...
## Delta 2026-08-26 — TASK-1751: el estado de un control (nombre accesible, congelado, ofrecido) y el gate que se equivoca

La primera captura premium de la superficie de rendición de assessment destapó cuatro contratos de plataforma, ninguno específico de Hiring. Los cuatro pasaban `lint`, `typecheck`, `build` y la suite del dominio en verde:

1. **Un `placeholder` NUNCA es el nombre accesible de un campo.** El textarea llevaba años colgando su nombre del placeholder; sólo se vio al ocultarlo en solo lectura, cuando cayó el check `label` de axe. Corolario para agentes: un `label` verde no prueba que exista etiqueta, prueba que hay *algún* texto del que colgar el nombre. Contrato: [`FORMS.md` § Estado de un control](./FORMS.md#estado-de-un-control--nombre-accesible-y-campo-congelado-task-1751) regla 1.
2. **Un campo congelado necesita señal visual propia.** Al pasar de `disabled` a `readOnly` desaparece el gris que ponía el navegador (estilo de UA) y el campo parece editable: si el módulo no declaraba `:disabled`, tampoco declara `:read-only`. Misma sección, regla 2.
3. **`readOnly` sobre `disabled` es una decisión de accesibilidad**, con el matiz de que `readonly` no aplica a `checkbox`/`radio` por spec HTML — la asimetría es correcta y debe declararse. Misma sección, regla 3.
4. **Un gate que exige romper el patrón correcto está mal configurado, no el código.** El check de layout marcaba los botones del stepper fuera del viewport en 390px; su contenedor ya declara `overflow-x: auto`, que es el scroller contenido canónico del repo. La salida fue declarar `allowHorizontalScrollSelectors` en el scenario (nombre no descubrible: el probe interno lo consume como `allowHScrollSelectors`), no aplanar el scroller. Contrato: [`GREENHOUSE_PREMIUM_UI_DELIVERY_STANDARD_V1.md` § A gate that demands breaking the correct pattern is misconfigured](../../ui/GREENHOUSE_PREMIUM_UI_DELIVERY_STANDARD_V1.md#a-gate-that-demands-breaking-the-correct-pattern-is-misconfigured) + refuerzo en el invariante de contención de scroll horizontal.

Se suma un contrato de oferta de acción: **"una acción que no puede tener efecto no se ofrece"** — con respuestas faltantes el CTA de envío no se renderiza en vez de mostrarse deshabilitado, porque un control gris invita a buscar cómo habilitarlo cuando la acción correcta es otra; aplica igual al "Reintentar" de un error no reintentable (`actionable: false`). Contrato: [`PATTERNS.md` § Una acción que no puede tener efecto no se ofrece](./PATTERNS.md#una-acción-que-no-puede-tener-efecto-no-se-ofrece-task-1751). Reglas duras para agentes en [`UI_PLATFORM_AGENT_INVARIANTS.md`](../agent-invariants/UI_PLATFORM_AGENT_INVARIANTS.md#estado-de-un-control-nombre-accesible-congelado-y-ofrecido-task-1751-medido-2026-08-26).

## Delta 2026-08-10 — TASK-1388 + TASK-1686: reequilibrio del chrome de navegación interna

El sidebar interno pasó de 12 grupos top-level a **3 zonas `isSection`** (`Operación` con dominios acordeón `Agencia · Comercial · Finanzas · Personas` / `Administración` con Admin Center / `Recursos` con Knowledge + Design System), con acordeón nativo Vuexy (un dominio abierto). Las hojas personales `/my/*` salieron del sidebar al dropdown del avatar, servidas por el builder canónico `src/lib/navigation/my-nav-items.ts` (`buildMyNavItems`); alcanzabilidad declarada con `via: 'avatar-dropdown'` en el manifest. **`NavSearch` fue eliminada** — la única palette ⌘K es la `CommandPalette` de TASK-696, montada globalmente vía `GlobalCommandPalette` (audiencia routeGroup+authorizedViews, recientes client-side). A11y del chrome cerrada (focus ring del rail, región scrollable con role/label, NavToggle como IconButton con aria-label); gap conocido: los triggers de submenú de `@menu` no exponen `aria-expanded` (estado canónico = clase `ts-open`). TASK-1686 agrega el predicado **colaborador puro** (`isMyUser && !isInternalPortalUser && !isClientUser`): rail = `/my` + sección Mi Ficha (builder) + plataforma concedida, avatar semántico (`ButtonBase` + aria), cero destinos cliente; my+client/client/internal conservan salida byte-a-byte. Estado vigente: nota de status en [`GREENHOUSE_SIDEBAR_ARCHITECTURE_V1.md`](../GREENHOUSE_SIDEBAR_ARCHITECTURE_V1.md) + §Sidebar Composition de [`GREENHOUSE_IDENTITY_ACCESS_V2.md`](../GREENHOUSE_IDENTITY_ACCESS_V2.md); specs: `docs/tasks/complete/TASK-1388-vertical-menu-restructure.md` y `TASK-1686-pure-collaborator-navigation.md`.
