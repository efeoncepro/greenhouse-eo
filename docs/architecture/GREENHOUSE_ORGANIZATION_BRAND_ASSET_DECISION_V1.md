...
## Delta 2026-06-09 — AI logo generation (gpt-image-2)

The logo pop-up gains a third acquisition path next to upload and operator-URL: **generate the logo with AI**.

- **Command:** `generateOrganizationLogoDraft` (server-only, `src/lib/account-360/organization-logo-generation.ts`) → route `POST /api/organizations/[id]/brand-assets/logo/generate`.
- **Provider:** OpenAI `gpt-image-2`, **opaque background** (operator chose max model precision over transparency). gpt-image-2 is **raster only** — PNG/WebP/JPEG, **no SVG**. Output is a 1024×1024 PNG.
- **Persistence:** the generated bytes land as an `organization_logo_draft` private asset (same context as upload). The command does NOT attach — the operator reviews the result and applies it through the existing `attachOrganizationLogoAsset` flow.
- **Guardrails (unchanged contract):** reuses the `organization.brand_asset` capability gate at the route boundary and **fails fast on `is_operating_entity = TRUE` BEFORE the paid OpenAI call**. Efeonce/legal/institutional logos are never generated here.
