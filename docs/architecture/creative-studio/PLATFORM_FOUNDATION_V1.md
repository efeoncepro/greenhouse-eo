...
The bootstrap deliberately does not choose or provision the database tenancy implementation, identity issuer, storage lifecycle, credit currency, provider credential set or production environment. Each requires an approved implementation task, threat model and runtime evidence.
