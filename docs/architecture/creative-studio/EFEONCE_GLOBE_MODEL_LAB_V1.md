...
La persistencia es un puerto workspace-scoped, `ExperimentStorePort`, con un impl `InMemoryExperimentStore` por defecto (clave `workspaceId::experimentId`). El store durable de tenencia lo reemplaza **TASK-1465** — declarado, no omitido.
...
**Es un fence de SEGURIDAD, no el credit ledger comercial.** El ledger durable, append-only, de créditos de estudio es **TASK-1468**. El comentario del código lo dice explícito: siendo per-proceso e in-memory, el fence **resetea al reiniciar** — aceptable para un Lab interno acotado, y reemplazado por el ledger durable antes de cualquier exposición externa. El wiring lee el cap diario de `GLOBE_LAB_DAILY_CAP_CREDITS` (default 500, acotado 1..100_000 en `readStudioRuntimeConfig`, `apps/studio-web/src/app.ts`).
...
- **NUNCA** usar `LabSpendFence` como ledger comercial de créditos. Es un fence de seguridad in-memory que resetea al reiniciar; el ledger durable append-only es TASK-1468.
...
`correlationId` propagado extremo a extremo, un `SpineAuditEventV1` por dispatch, y errores que nunca filtran el motivo interno verbatim. La reserva se libera correctamente ante fallo de runner. **Riesgo residual load-bearing**: tanto el `ExperimentStorePort` (`InMemoryExperimentStore`) como el `LabSpendFence` son **per-proceso e in-memory** — un reinicio pierde experimentos y el conteo del cap diario. Es intencional para el piloto interno acotado, pero **no es resiliente para producción**: el store durable de tenencia (TASK-1465) y el credit ledger durable (TASK-1468) son prerequisito de cualquier exposición externa. Un crash a mitad del `execute` (entre `reserve` y `settle`) puede dejar una reserva colgada hasta el reset del proceso, sin recovery.

### Scalability — Medio-Alto

El patrón "un primitive canónico, muchos consumers" escala por construcción: el Lab se montó sobre el spine sin tocar transporte ni harness, y HTTP/SDK/CLI/worker lo operan idénticos. Schemas versionados (`schemaVersion: '1'`) permiten evolución sin romper consumers. El `LabRunnerPort` desacopla el crecimiento de proveedores del dominio. **Riesgo residual**: el `execute` es **síncrono e in-process** — un run largo con proveedor real sostendría un request web, violando el invariante 4; la escalabilidad real depende del dispatcher async a Cloud Run Job (diferido con TASK-1464/adapter real). El fence in-memory tampoco escala a múltiples réplicas del servicio (el cap diario no es compartido entre procesos) hasta el ledger durable.
...
| **Store durable de tenencia** (workspace/experimentos persistidos) | TASK-1465 | `InMemoryExperimentStore` (resetea al reiniciar) |
| **Credit ledger comercial** (durable, append-only) | TASK-1468 | `LabSpendFence` in-memory de seguridad |
