...
1. **Todo el payload de browser de Globe migra a una aplicación cliente tipada y componetizada, compilada a assets estáticos.** Aplica a las cinco superficies actuales y a toda superficie humana futura. Se termina la composición por strings y la serialización por `.toString()`. El cliente compila con `lib.dom` y el mismo `strict` + `exactOptionalPropertyTypes` + `noUncheckedIndexedAccess` del monorepo.
   ...
   **Stack pineado, verificado contra el registry npm el 2026-07-25 — todas las versiones EXACTAS, sin `^`:**
   ...
   - **`vite@7.3.x`, NO `8.x`.** Vite 8.0 (2026-03-12) no fue un major de bump de Node: fue un **cambio de motor** — reemplazó Rollup (~10 años de batalla) + esbuild por **Rolldown**, un proyecto Rust de 2 años y 10 meses, en producción desde hace cuatro meses. Riesgos abiertos y verificados: [rolldown#9330](https://github.com/rolldown/rolldown/issues/9330) reporta **~7× de memoria en dev**, abierto y sin resolver desde 2026-05-09; Rolldown aplica semántica CJS estricta donde esbuild era permisiva, produciendo `TypeError: e is not a function` **en el browser, en producción, pasando CI**; y Rolldown declara API estable pero **output no determinista entre minors** (chunking/DCE). Vite 7.3 recibe *"important fixes + security patches"* por política oficial. Precedente: **Angular pinea exacto `vite@7.3.6`**; Qwik sigue capado en `<8`. Como Globe **no tiene Vite hoy**, empezar en 7 no cuesta migración — y es además el piso documentado de React Router 8 (`Vite 7+`), cuya compatibilidad con Vite 8 no está confirmada. **Gatillo de subida a Vite 8: `rolldown#9330` cerrado + un trimestre de soak, revisable en Q4-2026.**
...
**La conclusión operativa** es la que ya está en la Decisión punto 1: **el riesgo real no es Vite, es Rolldown**, y la respuesta correcta a Rolldown en 2026 no es evitar Vite sino **entrar por `7.3.x` y esperar dos trimestres**. Test de gobernanza a 12 meses: si a junio 2027 el fondo de USD 1M no desembolsó a mantenedores no-Cloudflare y no apareció fundación ni TSC, el riesgo de gobernanza sube de amarillo a naranja y esta ADR se revisa.
