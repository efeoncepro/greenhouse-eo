...
1. **Todo el payload de browser de Globe migra a una aplicación cliente tipada y componetizada, compilada a assets estáticos.** Aplica a las cinco superficies actuales y a toda superficie humana futura. Se termina la composición por strings y la serialización por `.toString()`. El cliente compila con `lib.dom` y el mismo `strict` + `exactOptionalPropertyTypes` + `noUncheckedIndexedAccess` del monorepo.
   ...
   **Stack pineado, verificado contra el registry npm el 2026-07-25:** `vite@8.1.5` (Rolldown es el bundler por defecto en estable desde Vite 8.0, 2026-03-12; el paquete opt-in `rolldown-vite` está retirado) · `react@19.2.8` · `react-router@8.3.0` en **framework mode con SSR apagado** (v8.0 del 2026-06-17; el equipo declaró ciclo mayor anual y releases *"regulares, predecibles y sobre todo aburridas"*, y su propia doc dice que el modo no obliga a SSR) · `babel-plugin-react-compiler@1.0.0` **pineado exacto**, habilitado **después** de que `eslint-plugin-react-hooks` pase limpio sobre el módulo de canvas — el compilador asume las Rules of React y un editor vectorial es justo donde más se rompen. `@tanstack/react-router` es sustituto aceptable de React Router si Discovery prefiere su type-safety; **`@tanstack/react-start` NO** (ver alternativas).
...
**Nota de gobernanza a registrar (no bloqueante, sí monitoreable):** **Cloudflare adquirió VoidZero** —Vite, Vitest, Rolldown, Oxc— el **2026-06-04**, y **Astro** el **2026-01-16**. Ambos siguen open source con compromiso público de neutralidad. Globe corre en **GCP**, así que su cadena de build pasa a tener dueño corporativo que compite con su cloud. No cambia la decisión; entra como dependencia nueva a vigilar. **Riesgo técnico abierto a monitorear:** [rolldown#9330](https://github.com/rolldown/rolldown/issues/9330) reporta ~7× de footprint de memoria en **dev** con Vite 8 (abierto, p2) — afecta la máquina del desarrollador, no el build de Cloud Run, pero con un editor vectorial el repo es grande. Y Rolldown declara API estable pero **output no determinista entre minors** (chunking/DCE): pinear la minor de Vite y subir deliberadamente.
