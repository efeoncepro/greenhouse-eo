...
- [EFEONCE_GLOBE_CREATIVE_PRODUCER_ARCHITECTURE_V1.md](EFEONCE_GLOBE_CREATIVE_PRODUCER_ARCHITECTURE_V1.md) — catálogo gobernado de rutas del Creative Producer (keystone del cluster TASK-1500…1505): SSOT por ruta (capability + constraints discriminados por modalidad + specialty + modos de entrada) y naming **modelo real público / casa interna operator-only** (`globe.producer.route.reveal_house`; slug/costo/margen nunca salen). Funcional: [`efeonce-globe-producer-catalog.md`](../../documentation/creative-studio/efeonce-globe-producer-catalog.md) · Manual: [`efeonce-globe-producer-catalog.md`](../../manual-de-uso/creative-studio/efeonce-globe-producer-catalog.md). Incluye además el **output side** (`TASK-1503`): retrieval tenant-safe `hash → bytes servible` (ficha + grant HMAC efímero, bytes por ruta aparte, `authorizeOwnedOutput` como puerta única contra los `outputHashes` retenidos — nunca `authorizedInputHashes`) + asset actions (`favorite` idempotente por estado deseado, `copyAsReference` con `derived-internal` inforjable y `parentRights` heredado). Funcional: [`efeonce-globe-producer-retrieval-assets.md`](../../documentation/creative-studio/efeonce-globe-producer-retrieval-assets.md) · Manual: [`operar-retrieval-assets-globe.md`](../../manual-de-uso/creative-studio/operar-retrieval-assets-globe.md). Y la **expansión de capabilities** (`TASK-1504`, code-complete 2026-07-22): `CREATIVE_CAPABILITIES` 10 → 14 (`video-frames`, `video-motion-control`, `audio-change-voice`, `audio-translate`) y `GLOBE_CAPABILITIES` 12 → 13 (`globe.voice.preset.manage`); catálogo de rutas a **1.1.0** con 8 rutas (4 nuevas dedicadas); **multi-output** modelado en el manifest (`ExperimentAttemptManifestV1.outputs?: LabOutputDescriptorV1[]` con `sha256`/`mediaType`/`mimeType` y **retención por-output**; el `outputsRetained` plano queda por compatibilidad) con `resolveEditSource` eligiendo el output **por modalidad** desde la capability hija; y el **voice preset registry** (`globe.voice.preset.{register,list,get}`, capability + grant en el mismo PR, `ui`/`mcp` `policy-blocked`; el vendor voice id vive sólo en el adapter). Cierra un **fail-open** del seed original: `ref/motion/loop-v1` declaraba los modos `frames`/`motion-source` sobre un motor text-to-video sin campo de referencia, así que los keyframes se descartaban **en silencio y después de reservar crédito**; ambos modos se movieron a rutas dedicadas y `assertInputModeSatisfied` ahora cuenta referencias **por tipo de medio** antes del fence. Motores verificados en vivo: **Veo 2.0** para `video-frames` (Veo 3.0 fast y estándar no soportan `lastFrame`), Seedance 2.0 `reference-to-video` para motion control, ElevenLabs **voice-changer** para change-voice (el slug `speech-to-speech` no existe) y ElevenLabs dubbing para translate. **Estado honesto:** kill switch OFF, `GLOBE_LAB_PROVIDER=fake`, canario facturable pendiente de gate humano — **no desplegado ni verificado en runtime**. Funcional: [`efeonce-globe-producer-capabilities.md`](../../documentation/creative-studio/efeonce-globe-producer-capabilities.md) · Manual: [`operar-capabilities-producer-globe.md`](../../manual-de-uso/creative-studio/operar-capabilities-producer-globe.md).
- [EFEONCE_GLOBE_ASSET_GOVERNANCE_WORKER_DECISION_V1.md](EFEONCE_GLOBE_ASSET_GOVERNANCE_WORKER_DECISION_V1.md) — **ADR-007**: ingest privado y outputs generados pasan por el mismo pipeline asíncrono durable. El request sólo valida/encola; un Cloud Run Job keyless ejecuta malware → C2PA → rights en orden estricto, con leases/evidencia tenant-scoped, streaming acotado, discovery desde tenancy persistida y canarios live obligatorios antes de rollout.
