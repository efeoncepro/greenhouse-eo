...
| `COMMERCIAL_Q2C_CANONICAL_CLOSE_ENABLED` + `COMMERCIAL_Q2C_CONTRACT_ONLY_ENABLED` | TASK-1206 | OFF/default en todos los environments (verdad live: `vercel env ls`) · **code-complete + gated** | (1) `COMMERCIAL_Q2C_CANONICAL_CLOSE_ENABLED=true` en staging → `/api/finance/quotes/[id]/convert-to-invoice` delega en el comando canónico `closeQuoteToCash` (income idempotente + audit Q2C + contrato + party + eventos) en vez del materializer legacy (sin audit Q2C); (2) staging smoke: un cierre `simple_invoice` controlado sobre una cotización emitida real → verificar income + contrato + audit Q2C + outbox + signals `commercial.quote_to_cash.*` en steady; (3) prod tras sign-off Commercial/Finance. `COMMERCIAL_Q2C_CONTRACT_ONLY_ENABLED` se prende solo si el operador habilita la estrategia `contract_only` (deal suspendido sin AR + SLA) — política aparte que requiere sign-off (revenue leakage si no se resuelve en SLA). | Cierre canónico Quote-to-Cash (TASK-1206). Default OFF → el path visible sigue bit-for-bit el legacy. `contract_only` NUNCA es cierre terminal: deja audit `status='suspended'` + signal `contract_only_sla_breach`. |
