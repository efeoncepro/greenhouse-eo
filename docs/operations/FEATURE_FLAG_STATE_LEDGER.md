...
| `GROWTH_AI_VISIBILITY_ASYNC_EXECUTION_ENABLED` | TASK-1234 | OFF/default en todos los environments (verdad live: `vercel env ls`) · **code-complete + gated** | (1) deploy del ops-worker a staging (`ENV=staging bash services/ops-worker/deploy.sh`) → crea el Cloud Scheduler `ops-growth-grader-drain` (*/5) + monta los flags `GROWTH_AI_VISIBILITY_*` (OFF) + `OPENAI/ANTHROPIC_API_KEY_SECRET_REF` + sube el TIMEOUT del worker a 3600s; (2) `vercel env add GROWTH_AI_VISIBILITY_ASYNC_EXECUTION_ENABLED=true Preview(develop)` + redeploy → el endpoint admin pasa a enqueue+poll (202+runId); (3) smoke real: encolar un run `full` multi-provider (Gemini 3) y verificar que el worker lo ejecuta sin timeout, las observations persisten incrementalmente y los signals `growth.ai_visibility.run_execution_lag`/`run_stuck_running` quedan en steady; (4) prod fuera de scope (release control plane posterior). NOTA: en el worker los flags de provider también deben prenderse para que el `full` real llame providers (mismo set que Vercel). | Cutover inline → async del AI Visibility Grader (TASK-1234). Default OFF → el endpoint ejecuta inline como hoy (sólo `light`/OpenAI cabe en el timeout Vercel). Con ON encola y el worker Cloud Run drena sin límite de duración (única vía para runs `full` multi-provider). Revert (<5 min): flag a false → endpoint vuelve a inline. |
