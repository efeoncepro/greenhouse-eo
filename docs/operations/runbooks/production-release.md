...
- El change-gate de `auth-server` compara el SHA y el valor servido de `AUTH_SERVER_INTERNAL_AUTH_ENABLED` y `EXTERNAL_IDENTITY_CANARY_ENABLED` contra el GitHub Environment objetivo. Si cambia sólo uno de esos knobs, `deploy_needed=true`; no se debe forzar un cambio de código para transportar configuración.
