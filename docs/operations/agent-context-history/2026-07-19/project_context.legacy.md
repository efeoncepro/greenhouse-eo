...
- **Secondary Tidal Teal (2026-07-18, code-complete local):** secondary ya no es lime/green. El SoT
  vive en `axisRamp.secondary` (`#DDF9F5→#083F3D`, anchor `500 #12AFA2`) y
  `greenhouseSecondaryPalette`: light `700 #0B726C + white`, dark `400 #3BCBBD + Midnight`.
  Componentes consumen `theme.palette.secondary.*`; raw ramp sólo para Labs/charts deliberados.
  ADR `GREENHOUSE_SECONDARY_TEAL_COLOR_DECISION_V1.md`, Visual Direction y baseline
  `design-system.colors` son canónicos. El flag `NEXT_PUBLIC_GREENHOUSE_SECONDARY_TEAL_ENABLED`
...
prepress -> release`. Cada asset declara `channelMode` y `brandMode`; offline no es off-branding.
  ...
  **Delta Campaign Layout Compiler V1 (2026-07-19):** `pnpm creative:layout` convierte el contrato YAML/JSON
  en plan, SVGs editables por capas, masters, manifests/hashes, contact sheet y QA. Sus modos
  `plan|compile|check` son determinísticos y no llaman a proveedores; `compile` bloquea anchor/layout/finish
  pendientes y mantiene release humano separado. Canon técnico:
  `docs/architecture/GREENHOUSE_CAMPAIGN_LAYOUT_COMPILER_V1.md`. El replay High Frequency pasó `3/3` sin nueva
  inferencia y quedó bajo el gate de fidelidad `0,002`.
...
anatomía/cultura -> scorecard -> derivados -> integración CMS`. El modelo exacto debe quedar demostrado por
  tooling; referencias declaran rol y no autorizan copia; un error anatómico o gesto culturalmente ambiguo es
  blocker; y un degradado sólo se usa cuando codifica una transición narrativa. Primera aplicación:
  `WAG-V01-C15` para el pillar Web Agéntica, `gpt-image-2` high, `2048×1152`, score `49/50`, integrado como
  featured `251553` y OG/Twitter `251554`. La metodología es estable, pero ninguna skin se convierte en dirección universal del
  blog hasta probarla en al menos dos portadas adicionales.
...
- **Método de infografía editorial determinística + canon Efeonce (actualizado 2026-07-18):** cuando una pieza
  depende de copy, cifras, ejes, conectores, taxonomías o logos exactos, `content-marketing-studio` carga
  `references/deterministic-editorial-infographics.md` y gobierna `contrato -> SVG source -> delivery SVG directo
y/o raster justificado -> QA -> manifest -> integración`. Para Efeonce carga además
  `efeonce/EFEONCE_EDITORIAL_INFOGRAPHIC_SYSTEM.md`: shell estable, arquetipo por relación, paleta auditada, sello
  canónico `efeoncepro.com` desde Artifact Composer y benchmark shareable. Source y delivery SVG se separan;
  rasterizar deja de ser obligatorio. Hay preset JSON, contrato y auditor ejecutable espejados Codex/Claude;
  comando canónico: `pnpm content:editorial-svg:audit -- <delivery.svg...>`.
  `design-studio` dirige composición y `dataviz-design` conserva encoding analítico complejo.
...
- **HubSpot as a Service / ANAM tiene canon visible en Greenhouse (2026-07-16):** este repositorio es el SSOT
  ...
  `docs/architecture/kortex/hubspot-as-a-service/anam-next-session-handoff-2026-07-16.md`.
  ...
  Los adjuntos no-agent están clasificados por contrato: sólo el ledger de facturación y la segmentación de
  clientes son candidatos de migración; la segmentación ya cerró un primer slice exacto/aprobado, mientras
  facturación continúa `NO-GO` hasta identidad/mapping/dry-run/aprobación. Los demás son
  insumos de configuración, referencia o exclusión. Canon:
  `docs/architecture/kortex/hubspot-as-a-service/anam-attachment-hubspot-classification-2026-07-16.md`.
  Slice Company live: `segmento_de_mercado_anam` (label `Segmento de mercado`, 22 opciones) y `region_de_chile`
  están pobladas en 471/1.023 Companies con match exacto y clave Company única; `sector_estrategico` está poblada
  en 65/1.023 mediante cuatro mappings directos. Se retuvieron 22 records bajo 11 claves Company duplicadas,
  3 casos de fuente ambigua y 527 no emparejados. Imports `77871653` y `77871743` terminaron con 0 errores y el
  readback de los 471 IDs reconcilió. Esto habilita análisis de composición de Companies, no todavía ventas por
  sector/región: la cobertura Deal→Company es sólo 629/1.240 (`50,73%`) y los KPI de monto requieren >=95%.
  Canon/rollback: `anam-sector-geography-kpi-slice-change-set-2026-07-16.md`.
  Primer slice Deal→Company vigente: 34/645 candidatos high-confidence por convergencia explícita de Companies a
  través de Contacts fueron aprobados como tabla exacta, importados como Primary en `77872707` y leídos de vuelta
  34/34 (una Company por Deal, type ID `5`, 0 errores, 0 records nuevos). La cobertura global quedó en 50,73%.
  Los 113 matches sólo por dominio, 498 held y Companies duplicadas siguen sin tocar y approval-gated. El
  conector HubSpot actualmente expuesto apunta a `48713323`, por lo que
  operaciones ANAM deben verificar y usar el perfil `anam-19893546`/portal `19893546`. Canon:
  `anam-deal-company-association-remediation-dry-run-2026-07-16.md`.
  Con aprobación posterior, Growth `19708354` recibió tres diagnósticos históricos parciales: valor comercial
  ganado por segmento `340896790` (14 categorías, CLF 41.830,35), sector estratégico `340897291` (2 categorías,
  CLF 34.204,13) y región de sede `340897635` (12 regiones, CLF 41.830,35). Todos filtran `Ganado` exacto más
  dimensión Company conocida. No son facturación, revenue reconocido, penetración TAM/SAM ni KPIs oficiales;
  la publicación oficial sigue bloqueada por cobertura Deal→Company de 50,73%.
  Los dos candidatos convergen en `Company -> Cuenta/Unidad ANAM -> Billing Event`, con Deal/Service asociados
  sólo por claves determinísticas. Código ANAM/CeCo no es una propiedad única de Company. Modelo propuesto:
  `docs/architecture/kortex/hubspot-as-a-service/anam-account-unit-billing-event-converged-model-2026-07-16.md`.
  El diseño posterior reemplaza SharePoint como dependencia primaria: intake administrado -> assets/GCS privados
  -> profiler tenant-scoped no-write -> aprobación. SharePoint queda como adaptador opcional. El ETL no crea
  Services desde filas billing. Canon vigente: `anam-managed-billing-intake-ui-2026-07-16.md` y
  `client-billing-intake-data-model-spec-v1.md`; el operating model SharePoint conserva evidencia del adapter.
  ...
  Services del piloto controlado; cada uno tiene Company única, Deal de origen, key/source line item, owner
  heredado y TCV/ARR preservados. Para construir paneles, el operador autorizó valores de activación sintéticos
  conspicuamente marcados en-record; los cinco calculan `fields_ready`, pero siguen fuera de KPIs oficiales hasta
  ratificación/reemplazo por ANAM. Maria Paz Haeger es reviewer de activación. Retención piloto `21152855` contiene
  Portafolio `340874128`, Radar `340874425`, cohorte recurrente elegible `340877391` = `2` y ARR elegible
  `340877588` = `22` UF; Fidelización piloto `21152950` contiene Cola `340874258`, seguimiento `340877942` = `2`
  y retrasos `340878184` = `1`. Los action reports muestran Härting + Hidrogistica por `delivery delayed OR
renewal upcoming`; no son GRR/NRR, NPS ni health score. Canon:
  `anam-phase-3-pilot-dashboard-execution-2026-07-16.md`; ledger original/rollback:
...
- **Pillar Web agéntica publicado (2026-07-18):** el post WordPress `249387`, `El fin de la web “solo para humanos”: cómo preparar tu sitio para los agentes de IA`, está público en `https://efeoncepro.com/aeo/web-agentica-agentes-ia/` y soporta editorialmente `/desarrollo-sitios-web/`. AEO (`156`) es categoría raíz por `PDR-015`; Yoast Premium conserva 301 desde las tres URLs publicadas y el archivo histórico bajo `/loop-marketing/aeo/`, pero todos los enlaces propios ya apuntan directamente a `/aeo/`. El corte vigente valida 99 bloques semánticos y siete infografías SVG art-directed light/dark y desktop/mobile; cada pieza usa un arquetipo explicativo propio y concentra fuente/fecha, wordmark oficial y `efeoncepro.com` exclusivamente en el footer. La relación es bidireccional: artículo→landing `3` y landing→artículo `1`, verificada a 1440/390 sin overflow. Portada C15 media `251553`/`251554`; canonical, index/follow, schema, breadcrumbs, sitemap, media, purge y QA live completos. El manifest general deriva la portada del submanifest C15 para evitar drift. Canon: `docs/public-site/decisions/PDR-015-aeo-categoria-editorial-raiz.md`, `docs/public-site/WEB_AGENTICA_PILLAR_CLUSTER_BRIEF_V1.md`, `WEB_AGENTICA_PILLAR_GUTENBERG_SPEC_V5.json`, `WEB_AGENTICA_EDITORIAL_VISUAL_SYSTEM_V1.md`, manifest y `docs/audits/public-site/2026-07-18-web-agentica-pillar-publication.md`. Deuda explícita: los siete diagramas complejos todavía no tienen descripción larga dedicada.
- **Creative Workflows es soporte científico/editorial, NO implementación operativa (2026-07-15):** [PDR-014](docs/public-site/decisions/PDR-014-creative-workflows-territorio-editorial-pillar-cluster.md) fija una Pillar educativa y 12 satélites bajo la tesis **“un sistema de decisiones creativas humanas vuelto ejecutable”**. El [brief maestro](docs/public-site/CREATIVE_WORKFLOWS_PILLAR_CLUSTER_BRIEF_V1.md) prepara contenido, fuentes, preguntas y distribución para sostener Creative Studio futuro; sus secciones, diagramas, checklists o templates conceptuales **no son product specs, workflows ejecutables ni child tasks**. La implementación sólo puede nacer después desde RESEARCH-009 + arquitectura vigente + EPIC-028 y tasks formales en el repositorio de Creative Studio. Estado editorial vigente: V5 está **publicada** en `https://efeoncepro.com/creative/creative-workflows/` (`post 251363`, autor `1`, categoría `Creative`, 114 bloques, cinco imágenes, seis captions, tabla nativa y featured/OG `251370`). Los diagramas textuales V3 (`251393` frontera y `251392` autonomía) usan tipografía determinista, reservan la safe area del chrome Ohio, evitan cruces internos de conectores/copy y enlazan a media completa en mobile; V1/V2 permanecen superseded. `kind=table`, captions, `linkDestination` y `public-website:wpcli --input-file` son capacidades gobernadas del Content Factory/rail SSH. Canon de esta iteración: `docs/public-site/CREATIVE_WORKFLOWS_PILLAR_VISUAL_ENRICHMENT_V5.md`; el corte E-E-A-T V4 permanece como evidencia histórica.
...
- **Growth CTA & Popup Engine runtime + Experience System (actualizado 2026-07-18):** `growth.cta` ya tiene foundation/renderer productivos y Think live; WordPress/steady-state closure sigue en `TASK-1427`. Greenhouse owns definitions/versioning/targeting/suppression/priority/action routing/event ledger; renderer portable no-iframe preserva `dataLayer`/GTM; Growth Forms/Think/HubSpot son destinations/consumers. El contrato visible separa placement, experience kind, appearance (`style_variant`), density container-derived `full|condensed|peek` y `variant_id` experimental. V1 agrega solo `slide_in` no modal; TASK-1428/1429/1431/1430 cubren suppression, presentation, actions y cockpit sin una task por skin. **Delta release `d5db8b568` (2026-07-18):** TASK-1428+1429 EN PRODUCCIÓN — suppression enforcement ON (staging+prod, verificado E2E por visitante), Tier B rollup, kill switches en DB sin redeploy (`/api/admin/growth/ctas/kill-switch`, `growth.cta.pause`), slide_in no modal live en el renderer 1.1.0 con identidad pseudónima consent-aware y `viewed` visibility-gated (corte en TRACKING-PLAN). Primera campaña interruptiva pendiente de decisión de operador; quedan TASK-1430/1431 (arch §24-25 + ledger). Docs canónicos: `docs/architecture/GREENHOUSE_GROWTH_CTA_POPUP_ENGINE_DECISION_V1.md`, `docs/architecture/GREENHOUSE_GROWTH_CTA_POPUP_ENGINE_ARCHITECTURE_V1.md` y `docs/epics/to-do/EPIC-023-growth-cta-popup-cro-engine.md`.
...
- **Kortex / ANAM chat landing + HubSpot as a Service (actualizado 2026-07-16):** la landing `https://anam-2.hubspotpagebuilder.com/agente-anam` esta live desde HubSpot Developer Project `kortex-cms-react` (`projectId=103589049`, `deployedBuildId=22`) y documentada en `docs/architecture/kortex/hubspot-cms/anam-chat-landing.md`. UX vigente: canal oficial de atencion, CTA `Iniciar chat` e intents `Cotizar / Seguimiento del Servicio / Requerimientos de Calidad`; `anam_intent` + refresh del widget es el seam soportado y el prefill del composer no es confiable. La operación integral se enruta por las skills espejo, el canon técnico, la documentación funcional y el manual ANAM. Modelo vivo: Contact -> Lead -> Company -> Deal/line items -> Service, con Ticket como caso/SLA y Billing Event propuesto. Inventario vigente: 291 Leads, 506 line items, 10 Quotes sin adopción operativa, **6 Services** (5 pilotos + 1 muestra excluida), 18 Tickets y 0 Invoices. Para facturación se prefiere una superficie ANAM autenticada sobre `client_billing_*`, assets/scan privados, `space_id` de sesión, aprobación humana y dataset ligado a `19893546`; SharePoint es adaptador opcional. ANAM es dueño de source/CRM data; Greenhouse sólo opera el control plane y jamás proyecta filas a su CRM, Finance, Income o Account 360. El siguiente slice de construcción es el profiler no-write; no equivale a que hayan desaparecido la ratificación Service, asociaciones pendientes, KPI oficiales, materializer, Tickets/SLA u otros backlogs. Product read está verificado con 110 scopes, 65 propiedades y 22 Products; Kortex aún debe corregir el generador de URL de scopes. Operación CMS: `hs project validate/upload --profile anam`, manteniendo el default CLI de Kortex/Efeonce.
...
- **Color como capa gobernada con guard mecánico (actualizado 2026-07-18):** Restraint v1 conserva Core Blue `#0375DB`, feedback AA (info `#1F6FD4`, success `#157F47`, warning `#FFB703`, error `#DC2E39`) y neutrales AXIS. La cláusula de secondary verde de TASK-1053 fue supersedida por `GREENHOUSE_SECONDARY_TEAL_COLOR_DECISION_V1`: Tidal Teal ramp `#DDF9F5→#083F3D`, anchor `500 #12AFA2`, semantic light `700 #0B726C + white` y dark `400 #3BCBBD + Midnight`, ambos AA. **Contrato para agentes:** cambiar color = editar el SoT (`axis-tokens`/`axis-semantic`/`axis-secondary`/`axis-chart`), todo deriva; NUNCA override local en views ni HEX inline. Drift/contrast quedan guardados por `axis-semantic-drift.test` + `axis-semantic-contrast.test` + paridad DESIGN.md/V1. AXIS Figma requiere reconciliación upstream, pero la regeneración debe preservar el override Tidal Teal hasta entonces.
...
## Delta 2026-07-16 TASK-1385/1422 Redacción asistida IA del aviso público de vacantes

- **Capability nueva del dominio Hiring (backend 1385 + UI 1422, ambas complete en develop local):** el copy público (`public_*`) de una vacante se redacta asistido por IA con el patrón propose→confirm de 1361 — la IA **propone COPY desde inputs allowlist-safe** (`VacancyPromptInput`: rol/skills/hechos públicos/competencias del template; NUNCA presupuesto/rate/notas internas — test negativo con sentinels) y **el confirm humano aplica** vía `updateHiringOpening` (capability `hiring.opening.write`, `publicCopyOverride` con la edición humana). Ledger 1361 reusado con kind `opening_public_copy` (dedupe por digest, terminal-once); capability nueva `hiring.opening.ai_assist` (propose, tier operador); provider `claude-sonnet-5` seam `HIRING_VACANCY_AI_COPY_MODEL`; prompt con voz Efeonce + checklist anti-sesgo de avisos. **UI en el Publication Desk** (`/agency/hiring/publication`): selector de vacante + CTA `✨ Redactar con IA` (ready/locked/pending) + drawer con bloque "Lo que la IA verá" → form editable → Aplicar/Descartar; GVC loop 5 iteraciones → 0 errores + enterprise rubric PASS (desktop+mobile+teclado+reduced-motion). **El publish sigue humano** (gate 422 intacto). Flag `HIRING_VACANCY_AI_ENABLED` **OFF en staging/prod** (fila en `FEATURE_FLAG_STATE_LEDGER.md`; dev local ON); rollout = flip staging + smoke desde el desk → prod tras revisión del primer aviso. Specs: arch delta en `GREENHOUSE_HIRING_ATS_ARCHITECTURE_V1.md`; contratos UI en `docs/ui/{wireframes,flows,motion}/TASK-1422-*`; manual `operar-hiring-desk.md` §Redactar el aviso con IA; skill binding en `greenhouse-talent-people-operator/references/greenhouse-runtime.md` §AI-assisted vacancy copy (mirrors Claude+Codex). Follow-ups: edición manual sin IA, Nexa actionKey, en-US del aviso.
