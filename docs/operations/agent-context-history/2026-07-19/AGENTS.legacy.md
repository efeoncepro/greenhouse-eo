...
- **Hook obligatorio de diseno UI — ANY UI work:** para CUALQUIER trabajo de UI, invocar primero `greenhouse-ai-design-studio` como orquestador canonico y cargar solo sus lanes disponibles aplicables (`greenhouse-product-ui-architect`, `modern-ui-greenhouse-overlay`, `greenhouse-portal-ui-implementer`, `greenhouse-ux-content-accessibility`, `greenhouse-microinteractions-auditor`, `greenhouse-ui-enterprise-review`). Antes de JSX nuevo, persistir fuente/direccion visual, comparar 2–3 direcciones, elegir una receta, registrar wireframe en `docs/ui/wireframes/` y crear flow/motion cuando sus triggers apliquen. `UI ready: yes` exige mapping, estados/copy, decision log, plan GVC premium y todos los checks de readiness. Despues del primer fold, capturar y mirar desktop+390px antes de cablear exhaustivamente; al cierre deben pasar `design-contract:lint`, `ui:code-lint`, `ui:visual-gate` y `ui:quality`. El scorecard exige promedio >=4.5, piso 4 y >=4.5 en jerarquia, economia de superficies, impacto visual, fidelidad y resistencia a template generico. Card-on-card, card wallpaper o una pantalla sin momento visual dominante son `BLOCK` aunque lint/build pasen. Canon: `docs/ui/GREENHOUSE_PREMIUM_UI_DELIVERY_STANDARD_V1.md`.
