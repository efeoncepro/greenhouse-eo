...
En GitHub Actions, los smoke lanes usan el Chrome del runner (`PLAYWRIGHT_CHROMIUM_CHANNEL=chrome`), verifican `google-chrome --version` y apagan video (`PLAYWRIGHT_VIDEO=off`). Esto evita descargar el Chrome for Testing completo o depender del `ffmpeg` del bundle Playwright antes de cada corrida, dejando el smoke enfocado en la ruta real y no en el bootstrap del browser.
