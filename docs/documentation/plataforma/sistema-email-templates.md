...
`VerifyEmail` y `MagicLinkEmail` son los primeros templates individuales migrados a dictionary. El copy en espanol vive en `getMicrocopy().emails.auth.*`; el copy ingles queda como fallback legacy mientras `en-US` sea mirror de `es-CL`.
