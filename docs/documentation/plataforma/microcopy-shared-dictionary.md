...
- TASK-408 Slice 3A agrega `selectEmailTemplateCopy()` y migra `VerifyEmail` a `emails.auth.verifyEmail` para espanol, preservando fallback ingles legacy.
...
| Copy de templates migrados | `emails.<grupo>.<template>` + `selectEmailTemplateCopy()` | VerifyEmail `es` desde dictionary, `en` fallback legacy |
...
- Snapshot baseline de `VerifyEmail` estable despues de mover copy `es` a `emails.auth.verifyEmail`.
