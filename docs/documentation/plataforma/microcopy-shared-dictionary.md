...
| Copy de templates migrados | `emails.<grupo>.<template>` + `selectEmailTemplateCopy()` | VerifyEmail/MagicLink/PasswordReset/Invitation `es` desde dictionary, `en` fallback legacy |
...
- Snapshot baseline de `VerifyEmail`, `MagicLinkEmail`, `PasswordResetEmail` e `InvitationEmail` estable despues de mover copy `es` a `emails.auth.*`.
