...
- [Página "En mantenimiento" + Modo Mantenimiento](plataforma/pagina-mantenimiento.md) — la página institucional `/maintenance` (familia 404/401/coming-soon, 5 mensajes rotativos, ilustración Efeonce) + el gate de mantenimiento env-driven (`MAINTENANCE_MODE`, default OFF, fail-open) aplicado por el primer `middleware.ts` del repo: 503 honesto, allowlist, bypass de operador `?gh_bypass`
- [Middleware Edge de Greenhouse](plataforma/middleware-edge.md) — la única capa edge-global (Next.js `middleware.ts`, singleton del framework) que corre antes del routing: qué la distingue de los guards por ruta, su responsabilidad actual (gate de mantenimiento), para qué MÁS sirve (geo/locale, rate limit, A/B, security headers, redirects masivos), qué NO meterle (cero DB/IO por request) y cómo extenderla sin crear un segundo middleware
