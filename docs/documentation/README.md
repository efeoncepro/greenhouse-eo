...
- [Captura Visual con Playwright](plataforma/captura-visual.md) — herramienta `pnpm fe:capture` para grabar `.webm` + frames PNG + GIF opcional de cualquier ruta del portal usando agent auth canónico. Por qué existe (reemplazo del patrón `_cap.mjs` ad-hoc), 3 niveles de uso, 5 capas de safety, qué entrega y quién debería usarla
