...
2. **La salida del banco (estación 7) — construida tras flag (TASK-977), pendiente de activar.** El motor de liquidación se extendió con una rama para el contratista (`provider_payroll`): al pagar la orden, resuelve el expense del contratista (materializado al quedar "listo"), registra el egreso como `labor_cost_external` y rebaja el banco. Está **detrás de un flag desactivado por default** (`CONTRACTOR_PAYABLE_SETTLEMENT_ENABLED`) hasta validación en staging + visto bueno de Finanzas. **Mientras el flag esté OFF, el pago al banco sigue bloqueado** (`out_of_scope_v1`). El path de nómina quedó intacto. Nota contable: la retención SII queda como pasivo a remesar **por separado** (F29) → para honorarios el gasto queda "parcialmente pagado" hasta construir esa remesa.
