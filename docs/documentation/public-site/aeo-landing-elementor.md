...
> **Delta 2026-06-30 (TASK-1298) — migración bridge → `<greenhouse-form>`.** La sección `convers` dejó de usar el bridge HTML temporal (form + validación + Turnstile + submit embebidos en un blob de ~32 KB). Ahora el único widget `html` de la sección contiene el embed del **renderer portable**:
>
> ```html
> <greenhouse-form form-key="b120566a-dd1a-43c8-956a-4e0121e805b8" surface="fhsf-efeonce-aeo-diagnostic" locale="es-CL" color-scheme="light" appearance="bare">…fallback no-JS…</greenhouse-form>
> ```
>
> + el script `renderer-latest.js`, envuelto por la card aprobada (**Opción A**: la card `.gh-aeo-growth-form-card` es la única superficie visible; el renderer va transparente dentro). El identificador es el `form_key` estable/opaco (TASK-1297), **no** el slug ni el `formGuid` de destino. La descripción del bridge más abajo queda como **referencia histórica**; el contrato vigente es el renderer. Backup del estado previo: post meta `_gh_aeo_backup_20260630_task1298_convers_migration` (raw `_elementor_data`). Hash `heroans` estable (`e0b951b2456a83578cd9e22005900521`). Tematización: ver el guardrail "Tematización del renderer" abajo.
...
| Header | `convers`, `.gh-aeo-section-header` | Eyebrow `Diagnóstico gratuito`, H2 `Mide cómo aparece tu marca en la IA` y bajada `Recibe en 24-48 h un diagnóstico personalizado con score, prompts críticos y competidores citados.` |
| Form host/card (vigente, TASK-1298) | `convers`, widget `html`, `.gh-aeo-growth-form-card` | La card visible `.gh-aeo-growth-form-card` envuelve al renderer portable `<greenhouse-form>` (**Opción A**). Dentro de la card: título (`.gh-aeo-growth-form-title`), lead, trust (`.gh-aeo-growth-form-proof`), privacidad y direct-link = markup WP; los campos los pinta el renderer. El widget `html` ya **NO** contiene form, JS, Turnstile ni metadata de destino: solo el embed + el script + un `<style>` scoped de tematización. |
| Campos (vigente) | renderer `<greenhouse-form>` (light DOM, clases `.ghf-*`) | Campos, labels, placeholders, validación reactiva, email-gate, Turnstile invisible y submit los provee el renderer desde el `render_contract` publicado (resuelto por `form_key b120566a-…`). WordPress NO declara campos ni copy de campo (sale del `field_schema` del contrato, dominio de TASK-1297). |
| CTA principal (vigente) | renderer `.ghf-btn` | `Solicitar diagnóstico gratis →` desde `render_contract.copy.submit` (v4 `fver-dbdd6a02-…`). El renderer hace verify-email reactivo → Turnstile invisible → submit gobernado; WordPress no ejecuta submit. |
| Trust/privacidad | `.gh-aeo-growth-form-proof`, `.gh-aeo-growth-form-privacy` | Mantener `Sin costo`, `Sin compromiso`, `Sin permanencia`, `Datos protegidos`, la salida `¿Prefieres coordinar directo? Agenda una conversación →` y la nota `Usaremos tus datos para preparar el diagnóstico y contactarte. Ver política de privacidad.` |
...
- El formulario gobernado vigente es `efeonce-aeo-diagnostic` (`fdef-efeonce-aeo-diagnostic`, versión publicada `fver-9507f6a7-431d-4215-a699-9c713328b69b` / v3; v2 `fver-bc5a1cfe-76eb-4658-9fe9-ab0c8fb0a657` deprecada) y la surface es `fhsf-efeonce-aeo-diagnostic`. Destination HubSpot: portal `48713323`, form GUID `8649e76c-8b01-41f3-9b0c-5713d7b4dba6`.
- El campo `email` usa `validator=corporate_email` y la versión publicada conserva `validation_schema.emailPolicy={mode:"block_field",field:"email"}`. El **renderer** consulta `/api/public/growth/forms/<formKey-o-slug>/verify-email` de forma reactiva/debounced antes de pedir Turnstile; si el correo es Gmail/free/disposable, muestra el error inline en el campo email y no llama `/submit`. (La resolución por `formKey` o `slug` la unifica `resolveFormSlugFromRef`, TASK-1297.)
- El **renderer** trae por construcción las capacidades de Growth Forms: errores por campo con `role="alert"`, `aria-invalid`, `aria-describedby`, estado `Verificando correo…`, success del email solo después de `/verify-email` real, anti-doble-submit, skeleton de carga y estados honestos (`Formulario no disponible` / `No pudimos cargar… Reintentar`). Required: `firstName`, `email`, `brandWebsite`; `country` y `companySize` opcionales según el contrato publicado.
...
- **MIGRADO (TASK-1298).** La landing ya usa `<greenhouse-form form-key="b120566a-…" surface="fhsf-efeonce-aeo-diagnostic" locale="es-CL" color-scheme="light" appearance="bare">` + `renderer-latest.js`. El renderer emite `captchaToken` (TASK-1294) porque el contrato v4 declara `security.captcha` (TASK-1296) y el `GET` público lo serializa. `color-scheme="light"` es obligatorio (banda clara; sin él, un visitante con OS dark vería el form oscuro). El contenido interno de `<greenhouse-form>…</greenhouse-form>` es el fallback no-JS (el renderer no auto-renderiza `noScriptFallback`). Validación, mapping, consent y surface siguen 100% en Greenhouse.
- **Tematización del renderer (transversal).** El renderer no dibuja card; se integra con tokens `--ghf-*`. **Hallazgo TASK-1298:** el renderer envuelve el contenido en `<div class="ghf-root ghf-scope">` y su CSS base re-declara los tokens en `greenhouse-form, .ghf-scope`, así que un override solo en `greenhouse-form` **no propaga** al contenido. Por eso el `<style>` scoped de AEO targetea **ambos**: `​.gh-aeo-conversion greenhouse-form, .gh-aeo-conversion greenhouse-form .ghf-scope { --ghf-font: <DM Sans>; --ghf-bg: transparent; }`. La **causa raíz se corrigió en el renderer** (`FormRendererOptions.hosted` — el wrapper interno ya no lleva `.ghf-scope` cuando está dentro de un host `<greenhouse-form>`); cuando ese fix llegue a prod, el override `.ghf-scope` de AEO queda inerte pero el de `greenhouse-form` sigue aplicando (forward-compatible, no requiere re-save).
...
- No exponer metadata interna en la landing. El kicker técnico `Growth Forms · Diagnóstico AEO` no debe renderizarse; la card pública empieza con `Solicita tu diagnóstico AEO`.
- El bloque debe mantener `letter-spacing:0` en lead, labels, inputs, trust bullets y links. Los dos títulos siguen con contrato de heading: el H2 de sección y `.gh-aeo-growth-form-title` (markup WP, conservado) deben computar `letter-spacing:-0.045em`. **Gate obligatorio tras tocar esta sección: `pnpm public-website:verify-aeo-form-typography`** — reescrito en TASK-1298: ahora espera el mount del renderer y lee los controles por sus clases light-DOM (`.ghf-label`/`.ghf-input`/`.ghf-btn`), valida `letter-spacing` de título WP, `overflowX==0` desktop+390 y que la familia del renderer sea el stack DM Sans (`--ghf-font` scoped, no `system-ui`). `lead`/`proof` siguen leyéndose del markup WP.
...
| FAQ accordion | `faqlist`, widget `ohio_accordion`, `.gh-aeo-faq-accordion` | Primitive Ohio canonica para FAQ. `block_layout=outline`, 9 tabs, primera pregunta abierta por defecto. Tratamiento visual vigente: lista editorial ligera, sin card exterior/interior, ancho maximo `840px`, filas cerradas de ~58px desktop y ~54-66px mobile. |
...
- Copy vigente: las preguntas deben responder decisiones, no solo definiciones. Mantener el set orientado a objeciones: qué optimiza AEO, 5 niveles, diferencia con SEO, qué recibe en el diagnóstico, precio, tiempo, contrato/permanencia, industria específica y HubSpot.
