...
| Proof strip | `whylogo`, `.gh-aeo-why-proof-widget` | Texto centrado `Marcas que ya confían en nosotros`, separado visualmente del panel navy; widget `greenhouse_logo_marquee` (`whylogom`) con 7 logos únicos en 3 sets idénticos; meta tipo `TeamAvatarGroup` con discos solapados en color de Berel, Sky y Bresler, más un cuarto disco navy `+120` detrás del último logo con texto micro casi blanco. El copy visible posterior debe ser solo `Chile · Colombia · México · Perú` con ícono flat de mundo en navy compacto; no volver a `+120 marcas - 4 países`, al mono/dashed box genérico ni al círculo teal. El marquee debe mantener fades laterales, loop continuo `translate(-33.333%)`, set más ancho que el viewport, gap visual/fase ~55px y reduced-motion sin animación/duplicados. |
...
- Follow-up de meta proof why 2026-07-02: a pedido del operador, `whylogometa` movió el conteo `+120` desde texto visible a un cuarto disco dentro del `TeamAvatarGroup`, con fondo navy, texto micro casi blanco y solape uniforme detrás de Bresler. Se eliminó la palabra visible `marcas` y se reemplazó `4 países` por `Chile · Colombia · México · Perú` en navy compacto con globe inline. Cambio Elementor vía `Document::save()` con backup `_gh_backup_before_aeo_why_proof_avatar_countries_20260702T115614Z`, settings backup `_gh_backup_page_settings_before_aeo_why_proof_avatar_countries_20260702T115614Z`; CSS plugin `logo-marquee.css` desplegado con backups remotos timestamped y Kinsta purgada. Evidencia final: `.captures/aeo-why-proof-meta-2026-07-02T12-08-15-864Z/`, `pnpm public-website:verify-aeo-why-proof-meta`, `pnpm public-website:verify-aeo-wordpress-guards` y `pnpm public-website:verify-aeo-live-contract` verdes.
