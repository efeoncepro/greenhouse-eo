...
> **Relacionados:** [Playbook de landings Ohio + Elementor](../../manual-de-uso/public-site/wordpress-ohio-elementor-landing-playbook.md), [Layout Ohio + Elementor](./wordpress-ohio-elementor-layout.md), [Custom Elementor Widgets y React](./wordpress-custom-widgets-react-strategy.md), [Public Site Primitives Registry](../../architecture/public-site/PRIMITIVES.md)
...
- `/aeo/` resuelve como archivo de la categoría raíz AEO (`156`) desde 2026-07-18 y su canonical de archivo es `https://efeoncepro.com/category/aeo/`. Es una superficie editorial, no la landing comercial; no usarla como destino canonical del servicio salvo una decisión explícita que reemplace el PDR vigente.
...
| Cards | `marketa`, `marketb`, `marketc`, `.gh-aeo-metric-card` | Conservan las tres estadisticas aprobadas; cards blancas sin top accent teal, con borde hairline, sombra suave, fuente con logo/wordmark y minima altura mobile para evitar clipping de Elementor. El titular interpretativo de cada card usa `.gh-aeo-card-takeaway` y debe separarse visualmente de la prueba/cita. |
| Statement inferior | `marketc`, `.gh-aeo-lead` | Frase final como barra navy: `SEO te hacía competir por el ranking. AEO te hace competir por la recomendación: que la IA te entienda, te cite y te elija — antes de que exista un clic.` |
...
- Verificar mobile `390px`: las metric cards necesitan `height:auto` + `min-height` porque Elementor puede colapsarlas cuando se combinan container flex y `overflow:hidden`. Estado vigente: marker CSS `gh-aeo-market-card-takeaway-spacing-v1`, `.gh-aeo-card-takeaway` como bloque y `min-height:286px` en mobile para que source marks sigan visibles.
- Tratamiento visual vigente de cards: markers `gh-aeo-market-card-polish-no-top-accent-v1` y `gh-aeo-market-card-polish-specificity-v1`; no reintroducir la linea superior teal en `.gh-aeo-metric-card::before`. El grid desktop usa `gh-aeo-market-quote-watermark-v1`: SVG transparente de comilla de cierre (`”`) en navy con opacidad baja, colocado mayormente a la derecha de la tercera card para que la silueta se lea como comillas y no como manchas tapadas; mantenerlo libre del rail social fijo. Oculto en tablet/mobile (`max-width:1023px`) para evitar ruido y solapes.
- Las fuentes de las tres cards se renderizan con `.gh-aeo-source-line`: HubSpot usa el SVG servido desde WordPress (`/wp-content/uploads/greenhouse-axis/hubspot-logotype.svg`), SparkToro usa el SVG correcto entregado por marca y servido desde WordPress (`/wp-content/uploads/greenhouse/aeo-market-logos/sparktoro-logo.svg`, fuente versionada en `docs/assets/public-site/aeo-market-logos/sparktoro-logo.svg`), y McKinsey se mantiene como wordmark inline accesible. No volver a representar SparkToro como texto local naranja.
- Copy cards vigente: HubSpot usa `El tráfico que antes llegaba solo ya no está garantizado.` + `El tráfico orgánico de clientes HubSpot cayó 27% interanual. La búsqueda tradicional ya no alcanza por sí sola.`; McKinsey usa `Tu comprador ya le pregunta a la IA qué elegir.` + `Uno de cada dos consumidores ya usa búsqueda con IA, y la mayoría la prefiere para decidir compras.`; SparkToro usa `Aparecer una vez no es una estrategia.` + `Los motores de IA casi nunca repiten la misma lista de marcas. Sin sistema, tu visibilidad depende del azar.`
...
| Pieza | Element ID / clase | Contrato |
| --- | --- | --- |
| Root | `pipelin`, `.gh-aeo-pipeline` | Seccion clara con halo suave; la ruta vertical central esta deshabilitada para no partir visualmente las cards. |
...
| Proof grid | `pipelin`, `.gh-aeo-grid-2 gh-aeo-pipeline-proof-grid` | Dos proof tiles compactas: desktop en dos columnas, mobile apiladas. Markers CSS vigentes `gh-aeo-pipeline-compact-proof-tiles-v1` y `gh-aeo-pipeline-proof-update-v1`. |
...
| Card 2 | `pipelin`, `.gh-aeo-metric-card` | Proof tile compacta: label `Compra B2B`, KPI `94%`, copy Forrester sobre compradores B2B usando IA y source mark con PNG transparente Forrester + `2026`. |
...
- Preservar el modo secuencial de las cards: prueba de conversion -> prueba de adopcion de IA en compra B2B -> conclusion.
...
- No reintroducir la linea vertical central entre cards: `.gh-aeo-pipeline::before` queda apagado por `gh-aeo-pipeline-proof-update-v1`.
...
- Mantener tracking de display solo en el H2. Eyebrow, lead, labels metricos (`Conversión`, `Compra B2B`), source marks/anios, copy y statement inferior deben quedar con `letter-spacing: normal/0` y sin uppercase forzado.
- Semrush usa el SVG real AXIS/primitive `semrush-logotype.svg` inline en Elementor (`viewBox="0 0 363 44"`, no texto local). Forrester usa el PNG transparente subido a WordPress (`/wp-content/uploads/2026/07/forrester-logo-transparent-*.png`) dentro de `.gh-aeo-source-logo-forrester` con `role="img"`/`aria-label="Forrester"` y `<img alt="">`; no volver al combo HubSpot/Docebo ni al wordmark textual.
- El párrafo Forrester lleva ajuste óptico propio (`.gh-aeo-pipeline-proof-copy-forrester p`, `17px` desktop / `16px` mobile + line-height proporcional) porque el texto más corto se percibía más grande que el Semrush aunque ambos computaran `18px`.
...
- Copy vigente: lead `Cuando tu marca es la que los motores de IA nombran, ganas algo más que visibilidad: ganas la conversación de compra antes que tu competencia. Y el tráfico que llega desde ahí llega más decidido a comprar.`; card Semrush `Los visitantes que llegan desde motores de IA convierten cerca de 4,4 veces más que los de búsqueda orgánica: llegan pre-calificados por el propio motor.`; card Forrester `Los compradores B2B ya usan IA para investigar, comparar y decidir. La oportunidad es aparecer en esas respuestas antes de que lleguen a ventas.`
...
| Card icons | `serv1cat`-`serv4cat`, `.gh-aeo-service-card-icon` | PNG 3D decorativos/contextuales, `alt="" aria-hidden="true"`: measure/create/distribute/optimize. Estado live v2: adjuntos WP `250708`-`250711`, fuente repo en `docs/assets/public-site/aeo-service-icons/v2/`; rollback v1 `250642`-`250645`. |
...
| Proof strip | `whylogo`, `.gh-aeo-why-proof-widget` | Texto centrado `Marcas que ya confían en nosotros`, separado visualmente del panel navy; widget/primitiva publica `greenhouse_logo_marquee` (`whylogom`) con 7 logos únicos en 3 sets idénticos; meta `BrandProofAvatarGroup` canonizado como pattern público en `docs/architecture/public-site/PRIMITIVES.md`: discos solapados en color de Berel, Sky y Bresler, más un cuarto disco claro/frosted `+90` detrás de Bresler con count slate renderizado por `::after` en DM Sans. El copy visible posterior debe ser solo `Chile · Colombia · México · Perú` con ícono flat de mundo en navy compacto; no volver a `+120 marcas - 4 países`, fondo navy para el count, texto count grande/DOM normal, mono/dashed box genérico ni círculo teal. El marquee debe mantener fades laterales, loop continuo `translate(-33.333%)`, set más ancho que el viewport, gap visual/fase ~55px y reduced-motion sin animación/duplicados. |
...
| Campos | renderer `.ghf-*` | `Nombre completo`, `Email corporativo`, `Marca / sitio web`, `País`, `Tamaño de empresa`, `Principal competidor`. Placeholders vigentes: `ej. María González`, `nombre@tuempresa.com`, `tuempresa.com`, `Selecciona tu país`, `Selecciona un rango`, `marca de tu competencia`. `País`, `Tamaño de empresa` y `Principal competidor` siguen opcionales; el suffix `(opcional)` lo agrega el renderer desde el contrato de validación, no desde el label. |
...
| Trust/privacidad | `.gh-aeo-growth-form-proof`, `.gh-aeo-growth-form-privacy` | Mantener en estado formulario `Sin costo`, `Sin compromiso`, `Sin amarres`, `Tus datos están seguros`, la salida `¿Ya quieres hablar del servicio? Agenda una reunión` con ícono de calendario alineado, y la nota `Tratamos tus datos según nuestra política de privacidad.` En hover los links no se subrayan ni usan fondo/sombra; cambian a teal y el icono de calendario tiene micro-lift sutil. En estado Success Card, este footer externo se oculta para evitar CTA duplicado. |
...
- El formulario gobernado vigente es `efeonce-aeo-diagnostic` (`fdef-efeonce-aeo-diagnostic`, `formKey` `b120566a-dd1a-43c8-956a-4e0121e805b8`, versión publicada `fver-bfc40c59-8d95-4d38-8ae5-0da7dc4ab468` / v16; v15 `fver-1139a7f7-4e62-4fb7-8e5c-be024652d217`, v14 `fver-2cc79ff4-6dcc-404e-b79b-094bd0a81e29`, v13 `fver-1f727049-6600-4d68-8089-1718b9edd54e`, v12 `fver-f933f877-c1ff-4e76-9832-2078ca64c6dd`, v7 `fver-f2f8abde-3b11-42b3-bf78-a309ef7678ad`, v6 `fver-9ec43a66-5372-45b7-829d-2c9e6381e27d` y v5 `fver-70c365c1-ea3b-4e84-b4b3-4fd852f951f4` deprecadas) y la surface es `fhsf-efeonce-aeo-diagnostic`. La v16 conserva `copy.submit`, Turnstile, destino, Success Card, placeholders `ej. María González`, `Selecciona tu país` / `Selecciona un rango` y `style_variant=diagnostic_premium`; además declara `successBehavior.steps=[]` para no renderizar pasos por fallback. Destination HubSpot: portal `48713323`, form GUID `8649e76c-8b01-41f3-9b0c-5713d7b4dba6`.
...
- El link directo `Agenda una reunión` debe renderizarse como conjunto separado del texto previo: row flex, gap visible entre pregunta y link, gap interno icono/texto, icono calendario de 16px. El hover de `Agenda una reunión` y `política de privacidad` no debe usar subrayado, fondo, borde ni sombra; feedback aprobado = cambio a teal. En `Agenda una reunión`, el icono puede hacer micro-lift/scale sutil; reduced-motion lo apaga. Focus visible de teclado conserva outline. Markers live: `gh-aeo-meeting-link-spacing-v1`, `gh-aeo-link-hover-modern-v1`, `gh-aeo-link-hover-minimal-v1`, `gh-aeo-link-hover-signal-final-v1`.
- Success Card AEO vigente: título `Tu informe de visibilidad va en camino.`, body `Lo estamos preparando y te llegará por correo apenas esté listo.`, apoyo `Si quieres avanzar antes, en la reunión revisamos oportunidades concretas para mejorar tu visibilidad en IA.` y una sola acción principal `Agenda una reunión`. No mostrar pasos numerados en AEO ni mantener el footer externo de agenda/privacidad debajo de la Success Card; tampoco reintroducir marco teal interno, línea superior ni focus frame persistente sobre la card estática. El host live usa markers `gh-aeo-success-card-polish-v1`, `gh-aeo-success-card-compact-steps-v1`, `gh-aeo-success-card-recraft-popper-v1` y `gh-aeo-success-card-borderless-v1`, backups `_gh_backup_before_aeo_success_card_polish_20260703T071500Z`, `_gh_backup_settings_before_aeo_success_card_polish_20260703T071500Z`, `_gh_backup_before_aeo_success_card_compact_steps_20260703T072200Z`, `_gh_backup_settings_before_aeo_success_card_compact_steps_20260703T072200Z`, `_gh_backup_before_aeo_success_card_recraft_popper_20260703T115248Z`, `_gh_backup_settings_before_aeo_success_card_recraft_popper_20260703T115248Z`, `_gh_backup_before_aeo_success_card_borderless_20260703T121057Z` y `_gh_backup_settings_before_aeo_success_card_borderless_20260703T121057Z`.
- Readiness del formulario: cuando el formulario está listo, `Todo listo para solicitar el diagnóstico` debe quedar centrado bajo `Empezar con mi diagnóstico`, no alineado al borde izquierdo de la columna de campos. Host marker vigente: `gh-aeo-readiness-centered-v1`; backup `_gh_backup_before_aeo_readiness_centered_20260703T122245Z`.
...
- El formulario AEO live es Growth Forms v16 `fver-bfc40c59-8d95-4d38-8ae5-0da7dc4ab468` con `style_variant=diagnostic_premium`, validacion de email corporativo, Turnstile fail-closed, dropdowns premium y Success Card `presentation='success_card'`. WordPress no captura datos.
...
- Narrative cards market 2026-07-03: se reescribieron las tres cards para que cada dato tenga un titular interpretativo (`.gh-aeo-card-takeaway`) separado de la prueba/cita, y se actualizó la barra inferior a ranking vs. recomendación. CSS page-scoped marker `gh-aeo-market-card-takeaway-spacing-v1`; mobile 390 usa `height:auto !important` + `min-height:286px !important` para mantener visibles HubSpot/McKinsey/SparkToro. Luego se retiro la linea superior teal de las cards (`gh-aeo-market-card-polish-no-top-accent-v1`, `gh-aeo-market-card-polish-specificity-v1`) y se agrego un watermark editorial desktop-only con SVG transparente de comilla de cierre (`gh-aeo-market-quote-watermark-v1`), ajustado hacia la derecha de la tercera card para que se entienda como comillas sin chocar con el rail social. Backups live: `_gh_backup_before_aeo_market_narrative_cards_20260703T135550Z`, `_gh_backup_before_aeo_market_narrative_compact_20260703T135841Z`, `_gh_backup_before_aeo_market_takeaway_spacing_20260703T140207Z`, `_gh_backup_before_aeo_market_takeaway_spacing_tighten_20260703T140732Z`, `_gh_backup_before_aeo_market_mobile_card_height_20260703T140853Z`, `_gh_backup_before_aeo_market_card_polish_20260703T141657Z`, `_gh_backup_before_aeo_market_card_polish_specificity_20260703T141835Z`, `_gh_backup_before_aeo_market_quote_watermark_20260703T142031Z`, `_gh_backup_before_aeo_market_quote_watermark_20260703T142228Z`, `_gh_backup_before_aeo_market_quote_watermark_20260703T142636Z`, `_gh_backup_before_aeo_market_quote_watermark_20260703T142819Z`, `_gh_backup_before_aeo_market_quote_watermark_20260703T143041Z`, `_gh_backup_before_aeo_market_quote_watermark_position_20260703T143555Z` y sus settings backups equivalentes. Evidencia: `.captures/aeo-market-narrative-copy-2026-07-03T14-09-08-137Z/`, `.captures/aeo-market-card-polish-watermark-2026-07-03T14-30-58-155Z/`, `.captures/aeo-market-quote-watermark-position-2026-07-03T14-36-53-362Z/`, `pnpm public-website:verify-aeo-market-sparktoro-logo`, `pnpm public-website:verify-aeo-wordpress-guards` y `pnpm public-website:verify-aeo-live-contract` verdes; `heroans` preservado.
- Logo SparkToro market 2026-07-02: se reemplazo el wordmark textual naranja por el SVG correcto compartido por el operador, versionado en `docs/assets/public-site/aeo-market-logos/sparktoro-logo.svg` y servido desde `/wp-content/uploads/greenhouse/aeo-market-logos/sparktoro-logo.svg`. El widget `marketc` mantiene `.gh-aeo-source-logo-sparktoro` con `role="img"`/`aria-label="SparkToro"` y `<img alt="">`; CSS page-scoped ajusta el logo a `height:18px`, sin tracking ni texto fallback. Backup `_gh_backup_before_aeo_sparktoro_logo_svg_20260702T114406Z`; settings backup `_gh_backup_page_settings_before_aeo_sparktoro_logo_svg_20260702T114406Z`; `heroans` preservado. Evidencia: `.captures/aeo-market-sparktoro-logo-2026-07-02T11-50-38-995Z/`, desktop/mobile 390 con SVG cargado, `pageOverflowX=0`, `sectionOverflowX=0`; `pnpm public-website:verify-aeo-market-sparktoro-logo`, `pnpm public-website:verify-aeo-wordpress-guards` y `pnpm public-website:verify-aeo-live-contract` verdes.
- Verificacion de la seccion pipeline: root `pipelin` con `.gh-aeo-pipeline-optimized`, grid `.gh-aeo-pipeline-proof-grid`, proof tiles compactas, source marks Semrush/Forrester presentes, Semrush renderizado como SVG AXIS inline real, Forrester cargando como PNG transparente desde WordPress y Playwright desktop/mobile sin overflow.
...
- Proof update pipeline 2026-07-03: se retiro la linea vertical central (`.gh-aeo-pipeline::before`) y se reemplazo la card derecha de `Lead source ~15% / Docebo` por `Compra B2B / 94%`, con copy Forrester sobre compradores B2B usando IA para investigar, comparar y decidir. El source mark de Forrester usa el PNG transparente versionado en `docs/assets/public-site/aeo-pipeline-logos/forrester-logo-transparent.png` y subido a WordPress como attachment `250802` (`/wp-content/uploads/2026/07/forrester-logo-transparent-20260703144832.png`), con `role="img"`/`aria-label="Forrester"` y `<img alt="">`. Follow-up óptico: el párrafo Forrester recibió clase `.gh-aeo-pipeline-proof-copy-forrester`; queda en `17px` desktop y `16px` mobile para compensar que el bloque más corto se percibía más grande que el Semrush. Backups live `_gh_backup_before_aeo_pipeline_proof_update_20260703T144349Z`, `_gh_backup_before_aeo_pipeline_forrester_logo_20260703T144832Z`, `_gh_backup_before_aeo_pipeline_forrester_type_tune_20260703T145203Z` y `_gh_backup_before_aeo_pipeline_forrester_type_tune_20260703T145515Z` + settings backups; `heroans` preservado. Evidencia: `.captures/aeo-pipeline-proof-update-2026-07-03T14-44-43-131Z/`, `.captures/aeo-pipeline-proof-update-2026-07-03T14-49-02-970Z/`, `.captures/aeo-pipeline-proof-update-2026-07-03T14-55-35-424Z/`, desktop/mobile 390 con logo cargado, `pageOverflowX=0` y pseudo-elemento apagado.
...
- Iconos 3D service v2 2026-07-02: se reemplazo el set inicial por cuatro PNG generados con el motor nativo de imagen de Codex, recortados desde sprite, limpiados a alpha y guardados en `docs/assets/public-site/aeo-service-icons/v2/`. La direccion visual baja el microdetalle: siluetas grandes Clay3D mate, navy/blanco y acentos teal/cyan tipo tech/neon muy contenidos. Se subieron a WordPress como adjuntos `250708`-`250711` (`aeo-service-icon-*-v2.png`) y se actualizaron solo los `src` de `serv1cat`-`serv4cat`; rollback simple a `250642`-`250645`. Backup `_gh_backup_before_aeo_service_native_icons_v2_20260702T113628Z`; `heroans` preservado. Verificacion: `.captures/aeo-service-native-icons-v2-2026-07-02T11-37-29-202Z/`, desktop 1440/mobile 390 con 4 PNG v2 cargados, `alt=""`, `aria-hidden="true"`, sin solape con kicker/title, `pageOverflowX=0`, `sectionOverflowX=0`; `pnpm public-website:verify-aeo-live-contract` verde.
...
- Follow-up de meta proof why 2026-07-02: a pedido del operador, `whylogometa` movió el conteo a un cuarto disco dentro del `TeamAvatarGroup`. La iteración vigente usa `+90` como count visible, disco claro/frosted detrás de Bresler, solape real por z-index/margen negativo y `::after` en DM Sans desplazado dentro de la porción visible del disco para evitar borde derecho o texto achatado. Se eliminó la palabra visible `marcas` y se reemplazó `4 países` por `Chile · Colombia · México · Perú` en navy compacto con globe inline. Cambio Elementor vía `Document::save()` con backup `_gh_backup_before_aeo_why_proof_count_90_20260702T143756Z`, settings backup `_gh_backup_page_settings_before_aeo_why_proof_count_90_20260702T143756Z`; CSS plugin `logo-marquee.css` desplegado con backups remotos timestamped y Kinsta purgada. Evidencia final vigente: `.captures/aeo-why-proof-meta-2026-07-02T14-42-11-797Z/`, `pnpm public-website:verify-aeo-why-proof-meta`, `pnpm public-website:verify-aeo-wordpress-guards` y `pnpm public-website:verify-aeo-live-contract` verdes.
