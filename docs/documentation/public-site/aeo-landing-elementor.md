...
| Campos | `.gh-aeo-growth-form-fields` | `Nombre`, `Correo corporativo`, `Sitio web de tu marca`, `País principal`, `Tamaño de la empresa`, `Competidor principal (opcional)`. Placeholders vigentes: `nombre@empresa.com`, `tuempresa.com`, `Selecciona país`, `Selecciona tamaño`, `Ej. marca competidora`. |
| CTA principal | `.gh-aeo-growth-form-button` | `Solicitar diagnóstico gratis →`; respeta validación reactiva por campo, consulta `POST /verify-email` para bloquear correos no corporativos, luego ejecuta Turnstile invisible y finalmente `POST https://greenhouse.efeoncepro.com/api/public/growth/forms/efeonce-aeo-diagnostic/submit`. |
...
- El formulario gobernado vigente es `efeonce-aeo-diagnostic` (`fdef-efeonce-aeo-diagnostic`, `formKey` `b120566a-dd1a-43c8-956a-4e0121e805b8`, versión publicada `fver-70c365c1-ea3b-4e84-b4b3-4fd852f951f4` / v5; v4 `fver-dbdd6a02-7e89-4d65-b29e-7228b7475a94` deprecada) y la surface es `fhsf-efeonce-aeo-diagnostic`. La v5 conserva `copy.submit`, Turnstile y destino, y alinea placeholders del renderer con el bridge (`Selecciona país`, `Selecciona tamaño`). Destination HubSpot: portal `48713323`, form GUID `8649e76c-8b01-41f3-9b0c-5713d7b4dba6`.
...
- El renderer canonico `<greenhouse-form>` ya emite `captchaToken` desde TASK-1294 cuando el contract declara `security.captcha`; desde TASK-1296 el form AEO v3 declara esa metadata en `ui_policy_json` y el `GET` publico productivo ya la serializa en `render_contract.security.captcha`. TASK-1297 tambien dejo `formKey` productivo. Aun asi la landing sigue usando un host bridge HTML scoped con Turnstile invisible porque el intento TASK-1298 rompio visualmente el form dentro de Ohio. Al migrar, usar `<greenhouse-form form-key="b120566a-dd1a-43c8-956a-4e0121e805b8" surface="fhsf-efeonce-aeo-diagnostic" locale="es-CL">` y conservar validacion, mapping, consent y surface en Greenhouse.
- El bridge actual es el piso visual aprobado, no el techo estetico. El renderer puede modernizar el formulario (jerarquia mas premium, helper copy util, foco visible, validacion/pending/success mas claros y microinteracciones sobrias), pero no puede degradar el baseline: campos blancos con borde, selects limpios con placeholders `Selecciona país` / `Selecciona tamaño`, CTA teal, trust inline, una sola superficie y mobile 390 sin overflow. El renderer pre-live vigente ya conserva dos columnas desktop para pares escaneables (`Nombre`/`Email`, `País`/`Tamaño`) y deja campos largos full-width; mobile 390 apila a una columna. Antes de cualquier nuevo save live, verificar con screenshots/GVC revisadas y gates mecanicos: `pnpm public-website:verify-aeo-prelive-contract` (WordPress sigue en bridge + `heroans` protegido + API publica por slug/formKey + captcha fail-closed + tipografia + bridge/live baseline + renderer bajo CSS hostil tipo Ohio + renderer inyectado en memoria dentro de `/aeo-2/` sin mutar WordPress + review PNG fresh/nonblank + muestreo de píxeles en inputs/selects/CTA).
- TASK-1298 permanece bloqueada para cutover live mientras `pnpm codex:task-hook TASK-1298 --develop` falle por `live_cutover_pending_after_pre_live_parity`. Ese bloqueo es intencional: solo se puede retirar tras documentar `Cutover Unlock Evidence` en la task con aprobacion explicita para mutar WordPress, backup/restore Elementor, frames pre-save revisados, Kinsta purge y gates post-save.
...
- El bloque actual debe mantener `letter-spacing:0` en lead, labels, inputs, trust bullets y links. Los dos titulos del formulario tienen contrato de heading: el H2 de seccion y `.gh-aeo-growth-form-title` deben computar `letter-spacing:-0.045em`. Ojo: Ohio/Elementor tiende a dejar reglas page-scoped con `!important` que pisan el HTML bridge; verificar siempre el valor computado en navegador, no solo la presencia de la regla en el widget. Marcadores vigentes: `gh-aeo-form-typography-spacing-v1`, `gh-aeo-form-title-letter-spacing-html-v1` y `gh-aeo-form-title-letter-spacing-specificity-v1`. Gate obligatorio tras tocar esta seccion o preparar migracion: `pnpm public-website:verify-aeo-prelive-contract`.
- Los selects del bridge necesitan `appearance:none` + caret scoped; el theme Ohio puede repetir flechas nativas si se elimina ese CSS. En el renderer, el hardening vigente debe mantener `.ghf-select` con `background-image:none`/`background-repeat:no-repeat` bajo host CSS hostil.
