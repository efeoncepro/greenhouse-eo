...
> **Ultima actualizacion:** 2026-08-31 por Codex (TASK-1799 — Content Marketing)
...
- **Produccion: ACTIVO de forma acotada para AEO `/aeo-2/`, Think `/brand-visibility` e Influencer `/servicios/agencia-de-influencers/`; Content Marketing `/servicio-marketing-de-contenidos/` tiene definición y embed publicados, con smoke de captura aceptada pendiente.** AEO usa `efeonce-aeo-diagnostic`; Think usa `fdef-ai-visibility-grader` con `successBehavior.kind="tokenized_report"`; Influencer usa `efeonce-creator-influence-brief` y acepta submissions gobernadas, aunque su entrega directa a HubSpot permanece deshabilitada por el gate vigente. En Think, el submit persiste la submission y el renderer entrega al host un `status_url`; un consumer reactivo crea el grader run y el status devuelve el token cuando el reporte esta listo. La verdad live de flags y dispatch es `vercel env ls` + Cloud Run `ops-worker`; el ledger es estado humano, no evidencia runtime.
...
## Formulario de Content Marketing y Content Ops

La landing `/servicio-marketing-de-contenidos/` usa el motor desde el rollout del 2026-08-31.
El visitante completa dos pasos: nombre, email laboral y empresa; después, modo de trabajo y contexto
opcionales. Si elige un modo antes de empezar, el formulario lo recibe como selección inicial.
Puede volver al paso anterior sin perder lo escrito. Una vez que empieza a escribir, las interacciones
de la landing no reemplazan sus datos.

La presentación `content_marketing` mantiene el diseño aprobado con encabezado, ayuda y progreso por
paso. Los textos, campos, consentimiento y validación siguen gobernados en Greenhouse. WordPress sólo
hospeda el componente y su card. La variante se distribuyó con un bundle fijado para esta landing;
no significa que todos los formularios del sitio o el bundle global hayan cambiado.

Este formulario está configurado para atención en **Greenhouse solamente**; su publicación no habilita
una entrega directa a HubSpot ni un envío de correo. Se verificaron carga, validación y navegación entre
pasos sin enviar un lead aceptado. La prueba de captura completa y medición GA4 de conversión sigue
pendiente. Identidad y versión del rollout:
...
- En **produccion** hay consumers con grados distintos de verificación: Content Marketing tiene publicación y navegación verificadas, pero no envío aceptado ni conversión GA4 probados. No generalizar el estado de AEO, Think o Influencer a cualquier embed sin revisar flags, host surface, CORS, Turnstile, destino y smoke.
