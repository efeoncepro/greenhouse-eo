...
## Identidad propuesta por el operador y elegibilidad

El operador propuso su correo `jreyes@efeoncepro.com` con Efeonce como organización. Reader PG: identidad canónica activa `identity-greenhouse-auth-client-user-user-efeonce-admin-julio-reyes`, tipo `efeonce_internal`; el perfil HubSpot anterior está merged hacia ella. Los tres registros coincidentes con Efeonce son organization_type `other`: dos prospect y uno inactive. Ninguno cumple el command `bindExternalOrganization` (`client|both` + `active_client` + active/status active).

No se modificó la clasificación comercial ni se creó binding/invitación. El correo de prueba ya está indicado; falta decisión sobre ampliar el contrato para un canary interno explícito o elegir una organización cliente elegible. Cambiar las filas para simular un cliente no es una solución válida. CI 33934410366 sigue en Test al verificarlo; no se reinicia ni se declara verde.
