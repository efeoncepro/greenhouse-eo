...
- **Delta 2026-09-19 (decisiones de Berel y del operador; registro en [`BEREL_COLORES_DE_TEMPORADA_2027_2026-09-16.md`](BEREL_COLORES_DE_TEMPORADA_2027_2026-09-16.md), deltas del 19-sep):** **BL-1 cerrado** — la pieza (N28, `/articulos/color-berel-2027`) se publica el **martes 29-sep-2026**, junto con el hub de Colores de Temporada 2027 y Raíces de la piel. **BL-3 cerrado por decisión del cliente** — Berel no quiere que firme ninguna persona: `author` y `publisher` son la `Organization` «Pinturas Berel» (`url` `/somos-berel`), lo que también corrige el `Person` «Equipo Editorial Berel» de §8.3; los expertos se citan solo por disciplina, sin nombres, así que la palanca de colaborador nombrado de §4.3 y el `author` `Person` recomendado en §8.3 **quedan descartados**. Los gráficos principales son material de marca de Berel; Efeonce desarrolla solo el del color en diversas luces y el del color en diversos acabados. BL-4 y BL-5 no se revisaron en esta actualización.
