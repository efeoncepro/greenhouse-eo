...
- `TASK-1670` — [creada 2026-08-08, backend-data/integration] **Kernel compartido de site probes + hallazgos de sitio en el audit.** El audit es passthrough de OnPage, y OnPage no cubre tres cosas de Capa 1: acceso de **crawlers de IA** en `robots.txt`, **ausencia** de JSON-LD y salud de sitemap. Los tres ya existen probados en el probe layer del grader (`TASK-1266`), encerrados en el motor AEO. Se extraen a un kernel neutral con superficie pública única —que **no persiste nada**, y por eso la frontera §1.1 queda intacta— y se conectan al collect como hallazgos de **sitio**, detrás de flag. Cierra el punto ciego más caro: hoy un sitio que bloquea `OAI-SearchBot`/`PerplexityBot`/`ClaudeBot` puntúa 95/100 y se presenta como sano (−23,1% de tráfico medido, Rutgers/Wharton dic-2025). Prerequisito del entregable descargable de la auditoría: un artefacto con nuestro nombre no debe declarar sano un sitio invisible para la IA. Fuera de alcance `core_web_vitals` (Lighthouse = laboratorio) y `llms-txt` (ROI marginal).
