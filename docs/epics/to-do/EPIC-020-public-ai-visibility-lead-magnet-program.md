...
- `TASK-1778` 🔄 **in-progress — code complete en develop (2026-08-27), rollout pendiente** — **Endurecer el fetcher de sitio para uso comercial.** Dueña de `ISSUE-164`. Los defectos de la auditoría 2026-08-26 (contención de redirects prometida y no implementada, host validado sin resolver DNS, tope que no protegía memoria, truncado sin rastro) ya tienen fix en develop: contención de redirects a la familia del sujeto + guarda DNS anti-SSRF gated por `GROWTH_PROBE_FETCH_STRICT_NETWORK_ENABLED` (default OFF, cutover pendiente); tope de bytes real por stream (4 MiB) + rastro `truncated`/`observable` con probes de presencia degradando a `skipped` (nunca «no tiene datos estructurados» sobre cuerpo parcial o shell JS), y `robots.txt` obedecido con nuestro UA (`GreenhouseAEOGrader`) — todo esto viaja con el código sin flag. 🔴 **Sigue bloqueando el flip a `prod: ON`** de `GROWTH_AI_VISIBILITY_PROBES_ENABLED` y `..._BRAND_INTELLIGENCE_ENABLED` hasta la corrida real en staging (`ISSUE-164` open). **P1.**
