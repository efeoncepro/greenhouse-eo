...
- Impact: `Muy alto`
- Effort: `Alto`
- Status real: `Cara pública LIVE en think.efeoncepro.com (verificado runtime 2026-08-05); registro reconciliado: 52 childs reales — 38 complete, 14 abiertas (medido 2026-09-01) (paridad Epic:↔child list verificada por `pnpm epic:lint`). Falta el smoke E2E del loop`
- Rank: `TBD`
- Domain: `cross-domain`
- Owner: `unassigned`
...
- `TASK-1778` ✅ **complete (2026-08-27, rollout ejecutado)** — **Endurecer el fetcher de sitio para uso comercial.** Dueña de `ISSUE-164` (resuelto 2026-08-27). Los defectos de la auditoría 2026-08-26 (contención de redirects prometida y no implementada, host validado sin resolver DNS, tope que no protegía memoria, truncado sin rastro) quedaron cerrados y desplegados: contención de redirects por salto (familia del sujeto + subdominios descendientes + upgrade http→https; cross-dominio-registrable bloqueado) + guarda DNS anti-SSRF, gated por `GROWTH_PROBE_FETCH_STRICT_NETWORK_ENABLED` — **ON en el ops-worker** (revisión `ops-worker-00598-459`, worker único compartido staging+prod: cubre el path async del intake público, la cadena real del issue) **y en Vercel `staging`**; Vercel Production con el próximo release develop→main (regla ISSUE-150; hasta entonces solo el path inline de prod — runs `light` de admin — conserva la red vieja). Tope de bytes real por stream (4 MiB) + rastro `truncated`/`observable` con probes de presencia degradando a `skipped`, y `robots.txt` obedecido con nuestro UA (`GreenhouseAEOGrader`) viajan sin flag y ya están desplegados. Evidencia: corrida real `EO-GRUN-00048` verde con el flag ON (13 probes, cero `blocked_*` falsos) + 7 dominios vivos de cartera (6 ok; bancochile ya fallaba con la red vieja — evidencia para `TASK-1281`). **El sustrato de probes quedó defendible y desplegado: deja de bloquear el flip a `prod: ON` de `GROWTH_AI_VISIBILITY_PROBES_ENABLED` y `..._BRAND_INTELLIGENCE_ENABLED`.** Residuales fechados en `FEATURE_FLAG_STATE_LEDGER.md` § Pendientes (revisión Sentry 2026-08-29 + flip prod Vercel con el release). **P1.**
...
- `TASK-1330` ✅ `complete` 2026-09-01 — short links del reporte (distribución comercial).
...
- `TASK-1861` 📋 `to-do` — **AEO Grader operable por MCP**: correr el grader (gasto gobernado, tope diario por persona, idempotencia con scope por organización), leer resultados (runs, score, informe, evidencia, cola de revisión) y obtener el informe web tokenizado y su PDF, por 12 tools con **autoridad humana delegada** (lane `app` + exchange RFC 8693, patrón `TASK-1852`), federadas en `mcp.efeonce.org`, con dos manuales MCP y una skill local. Cierra el hueco que el non-goal "Nexa/MCP por construcción" dejó abierto (ver Delta 2026-09-10 en § Non-goals).
- `TASK-1863` 📋 `to-do` — **AEO Grader multi-mercado**: una marca con N mercados (país, idioma, competidores versionados con alias, cadencia propia), runs siempre de un mercado, **lotes que seleccionan varios mercados** de forma atómica, catálogo único de mercados de Growth (hoy hay cuatro mapas y un fallback silencioso a Estados Unidos), prompts `es`/`pt-BR`/`en`, geo nativa por motor y matriz entre mercados sin overall combinado. Sky (Chile, Perú, Argentina, Brasil, Uruguay, Colombia) es el primer tenant. Las tools de `TASK-1861` nacen multi-mercado sobre estos commands.
- `TASK-1867` 📋 `to-do` — **evidencia completa de cada respuesta.** El grader mide presencia, competidores y tono sobre los primeros 600 caracteres de cada respuesta, porque es lo único que persiste: en el panel Sky (2026-09-11, 300 observaciones) ninguna respuesta cupo entera y `raw_evidence_pointer` —diseñado para el texto completo— está nulo en todas. Persiste el texto íntegro, mide sobre él y versiona el score (`ai_visibility_score_v3`, detrás de flag) para no comparar ventanas de medición distintas. P1: los informes ya se entregan a clientes.
- `TASK-1868` 📋 `to-do` — **precisión de los detectores deterministas.** Los probes estructurales toman la página HTML de un SPA (200 en cualquier ruta) por `llms.txt`, `robots.txt` o `sitemap.xml` reales, y la compuerta de revisión humana compara términos de riesgo como substring ("demandadas" activa "demanda"). Ambos medidos en el panel Sky; corrige los dos con fixtures del caso real y readiness `ai_readiness_score_v2`.
- `TASK-1282` ✅ `complete` 2026-09-01 / `TASK-1283` 🚧 `in-progress` — conexión Search Console multi-tenant (OAuth + per-org) y su UI. Declaran `Epic: EPIC-020`; **EPIC-022 depende de su rollout** (`TASK-1302`). Si el programa decide que GSC es infraestructura de SEO, mover ambas a EPIC-022 — decisión de alcance, no de higiene.
...
> **Delta 2026-09-10 — el non-goal de arriba no se cumplió solo.** El discovery midió que ninguna de las 47 tools del MCP de Greenhouse corre ni lee un run del grader, que el lane `ecosystem/growth` sólo tiene `seo/**` y que el run sólo se dispara desde rutas con sesión humana. "Por construcción" suponía un contrato programático que no existía para el run ni para la lectura operador, y la identidad máquina del lane ecosystem no puede pasar `can()` sobre `growth.ai_visibility.run.operator`. La exposición MCP sigue sin ser trabajo Nexa-específico, pero **sí** requería trabajo: lo cubre `TASK-1861` (lane `app` con autoridad humana delegada, tools federadas y manuales). Nexa queda habilitada sobre ese mismo contrato como follow-up.
