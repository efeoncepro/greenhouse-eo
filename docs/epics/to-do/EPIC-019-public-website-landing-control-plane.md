...
| `TASK-1865` | `4` | `to-do` | Landing Performance Marketing en `/servicios/performance-marketing/` (working route) para Performance & Commerce Distribution, construida desde cero, con 301 desde la legacy `242862` (`/servicio-gestion-campanas-publicitarias/`), que no se parcha. Trece módulos semánticos, firma interactiva `Clics / Ventas` sin cifras, reuso de ComparisonTable, social trust, Growth Form, Growth CTA y scheduler nativo. PDR-022 y brief SEO/AEO; dirección visual propuesta y `UI ready: no`. |
