...
| `TASK-1167` | `1.1` | `complete / staging verified` | Public Site GitHub repo control plane: `public-site-github-control-plane.v1`, admin reader/commands for `efeoncepro/efeonce-web`, allowlisted GitHub Actions commands default OFF, and signal `public_site.astro_ci_failed` that reports the real red `CI` state. Zero deploy/rollback/cutover. |
...
| `TASK-1860` | `4` | `to-do` | Landing Trade Marketing & BTL de la línea Channel & Commerce en `/servicios/trade-marketing/` (working route): doce módulos semánticos, reuso de ComparisonTable, social trust, Growth Form, Growth CTA y scheduler nativo; posición en el medio entre plataformas y agencias. PDR-021 y brief SEO/AEO con demanda real de Chile; dirección visual propuesta y `UI ready: no`. |
| `TASK-1865` | `4` | `to-do` | Landing Performance Marketing en `/servicios/performance-marketing/` (working route) para Performance & Commerce Distribution, con 301 desde la legacy `242862` (`/servicio-gestion-campanas-publicitarias/`) y contención previa de sus contadores en cero y de su claim de partners no verificado. Trece módulos semánticos, firma interactiva `Clics / Ventas` sin cifras, reuso de ComparisonTable, social trust, Growth Form, Growth CTA y scheduler nativo. PDR-022 y brief SEO/AEO; dirección visual propuesta y `UI ready: no`. |
| `TASK-1862` | `4` | `to-do` | Landing ASO en `/servicios/aso/` (working route), tercera superficie del grupo `Visibilidad` junto a SEO y AEO: página de expansión y citabilidad en español, no de captura. Once módulos semánticos, reuso de ComparisonTable, social trust, Growth Form, Growth CTA y scheduler nativo; costura con las landings SEO y AEO en fase C. Publicación por fases atada a la extensión Search & App Visibility (`Proposed`). PDR-023 y brief SEO/AEO; dirección visual propuesta y `UI ready: no`. |
