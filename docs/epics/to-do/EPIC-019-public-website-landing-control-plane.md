...
| `TASK-1116` | `1` | `to-do` | WordPress bridge plugin foundation: private namespace, Abilities API registrations, auth/signature verification, health endpoint, metadata contract and draft-only write path in `efeonce-public-site-runtime:wp-content/plugins/greenhouse-wp-bridge`. Depends on Greenhouse binding/deploy dry-run from `TASK-1122`. |
