...
- Impact: `Muy alto`
- Effort: `Alto`
- Status real: `2026-09-08: emisor nativo y gateway multi-issuer productivos. TASK-1813 completa; TASK-1832 conserva observación sintética/cleanup propio y TASK-1841 el primer cliente real. TASK-1844/U19 completa para una identidad interna: SQL expand/contract, Codex/Claude Code/hospedado/Desktop, refresh, revocación y rollback verificados; PR 230/main 45f6910e3, orquestador 34281143424 success, manifest released y watchdog 5/5. Fixtures retiradas y conexiones internas definitivas conservadas. Sólo conexión hospedada Claude del canary sustituida bajo autorización; el epic mantiene sus demás pendientes.`
- Rank: `TBD`
...
| **U19** | [TASK-1844](../../tasks/complete/TASK-1844-efeonce-mcp-internal-multi-organization-authority.md) | **Complete, producción verificada 2026-09-08.** Autoridad interna multiorganización por objetivo explícito. Extiende U11/U05 ya productivas sin reimplementarlas: contexto como ancla del actor, reader Greenhouse por request, consentimiento v2 sin elevación silenciosa y matriz A/B permitidas, C denegada, revocación y concurrencia. Sin wildcard/lista en JWT ni scope por organización. | U11/U05 desplegadas; U10 no bloquea la implementación, pero cierra antes del rollout interno |
| **U20** | [TASK-1864](../../tasks/to-do/TASK-1864-mcp-self-sufficient-agent-surface.md) | Superficie agéntica autosuficiente: el gateway envía `instructions` derivadas del manifiesto (primeros 512 caracteres autocontenidos, hoy vacías en `efeonce-mcp/src/mcp.ts:137-143`), contrato estructurado `next` que cruza el gateway sin prosa por tool, digest de superficie que incluye instructions, kit de cliente generado (plugin Claude Code, skill/plugin Codex que sólo enrutan a los manuales) y eval end-to-end de agentes con Claude Code y Codex (una frase → flujo completo; disciplina de confirmación medida). Primeros escenarios: lectura SEO y AEO Grader (`TASK-1861`/`1863`). Registrada 2026-09-11; sin implementación. | U10 (clientes certificados); `TASK-1861` sólo para el escenario AEO |
| **U21** | [TASK-1869](../../tasks/to-do/TASK-1869-mcp-operational-diagnostics-readbacks.md) | Diagnóstico operativo por MCP: registry donde cada dominio publica su corte agregado read-only (Hiring primero, más un corte transversal de flags por runtime), expuesto por el lane ecosystem y federado como tools de dominio `platform`. No duplica `get_platform_health`, que ya cubre las señales de confiabilidad. Cierra la dependencia de la máquina del operador para diagnosticar producción. Sin PII, sin escrituras, binding `internal` con 404 anti-oracle y **sin scope OAuth nuevo**. Descarta explícitamente el SQL ad-hoc por MCP y el acceso directo a Cloud SQL desde el runtime de un agente. Flag `PLATFORM_DIAGNOSTICS_LANE_ENABLED` OFF por defecto. | U00 |

Una sola task ejecutable posee cada unidad. `TASK-1836` es U11, `TASK-1837` es U12, `TASK-1838` es U13, `TASK-1839` es U14, `TASK-1840` es U15, `TASK-1841` es U16, `TASK-1842` es U17, `TASK-1843` es U18, `TASK-1844` es U19 y `TASK-1864` es U20; `TASK-659` y `TASK-658` permanecen relacionadas (ver
...
4. **U06 (ui-ux) desde que U03 fija el contrato de flujo.** Con skills de product design y GVC; nunca bloquea
   U01–U05.
5. **U07 certifica con población externa sintética controlada por Efeonce** y sesiones interactivas del operador
   (matriz de tokens con Claude, Codex y ChatGPT; cuentas M365/Google; Chrome/Safari). U08 corre en paralelo desde
