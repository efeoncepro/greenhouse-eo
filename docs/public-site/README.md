...
- **[Creative Workflows Pillar Gutenberg Spec V6](CREATIVE_WORKFLOWS_PILLAR_GUTENBERG_SPEC_V6.json)** — corte live vigente desde 2026-07-16: convierte la FAQ de cuatro `H3 + párrafo` en cuatro disclosures nativos `core/details` dentro del H2 `Preguntas frecuentes`, preservando respuestas completas en HTML y simplificando el TOC. QA anónimo desktop/mobile PASS; `FAQPage` no se emite.
- **[Creative Workflows FAQ Disclosure Decision V1](CREATIVE_WORKFLOWS_FAQ_DISCLOSURE_DECISION_V1.md)** — diagnóstico runtime WordPress/Ohio/Gutenberg: existen `core/details`, `core/accordion`, Essential Blocks accordion y Yoast FAQ; la decisión segura para este caso es `core/details` gobernado por Content Factory, sin custom block ni JS adicional.
