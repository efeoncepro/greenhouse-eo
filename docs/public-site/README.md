...
- **[Creative Workflows Pillar Gutenberg Spec V6](CREATIVE_WORKFLOWS_PILLAR_GUTENBERG_SPEC_V6.json)** — corte local/dry-run, **no publicado**: convierte la FAQ de cuatro `H3 + párrafo` en cuatro disclosures nativos `core/details` dentro del H2 `Preguntas frecuentes`, preservando respuestas completas en HTML y simplificando el TOC. Requiere autorización humana explícita para cualquier write live.
