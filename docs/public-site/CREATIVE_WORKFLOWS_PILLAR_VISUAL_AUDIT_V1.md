...
Esta auditoría verifica que las cuatro imágenes:
...
- son legibles como raster editorial sin depender de texto interno;
...
responde `200`, no presenta overflow horizontal y carga las tres imágenes de cuerpo sin errores.
...
pnpm public-website:content-factory:run -- \
  --spec docs/public-site/CREATIVE_WORKFLOWS_PILLAR_GUTENBERG_SPEC_V4.json \
  --out tmp/creative-workflows-pillar-draft-v4.json
```
...
- `blockCount=111`;
- `hasMedia=true`;
- tres bloques de cuerpo `core/image` con IDs `251366`, `251367` y `251368`;
...
- El tema Ohio no muestra el featured dentro del cuerpo del single post; lo usa correctamente como featured/OG. Las tres imágenes contextuales sí aparecen en el cuerpo.
...
3. TOC con 21 enlaces internos y destinos válidos;
4. featured/OG JPEG `1440×757` responde `200` y Yoast emite `summary_large_image`;
5. canonical WordPress único; rutas equivalentes en Think responden `404`.
