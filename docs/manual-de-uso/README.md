...
- [Fondear los créditos de Globe (propose → confirm)](creative-studio/fondear-creditos-globe.md) — el runbook del carril gobernado de fondeo mensual (TASK-1566/ADR-015): las dos capas (ledger = caja, política = regla — cargar la caja sin subir el tope no habilita nada), el plan legible con el delta antes de aprobar, el paso a paso con las **dos correcciones medidas** (el confirm exige clave de idempotencia PROPIA; el anti-replay del broker es por propuesta), la tabla de estados (`confirmed` sin completar = anomalía, no reintentar a ciegas) y los NO: nunca confirmar con la persona agente, nunca reintentar un confirm tras timeout sin leer el estado, nunca los scripts legacy de firma cliente.
