...
## 2026-07-23 — Globe Producer: live feed server-authoritative cerrado internal-only

- `TASK-1525` quedó cerrada como base backend/feed: el reader `globe.producer.feed.live.list|changes` corre
  internal-only detrás de `GLOBE_PRODUCER_LIVE_FEED_ENABLED`, con cursor/watermark, batch SQL tenant-safe y
  errores DB sanitizados.
- El fix final en `efeonce-globe` `ed5e993` resolvió una incompatibilidad real de PostgreSQL en `stable_key`
  (`||` vs `->>`), verificada con reproducción read-only contra Cloud SQL y smoke humano same-tab en
  `https://globe.efeoncepro.com/producer`: sesión `200`, reader `200`, 10 items reales image/audio/video.
- No se promovieron rutas ni se declaró commercial ready. `TASK-1526` queda como dueño de la paridad UI aprobada,
  cards keyed, viewer multimodal, reauth UX y GVC/live canary visual.
