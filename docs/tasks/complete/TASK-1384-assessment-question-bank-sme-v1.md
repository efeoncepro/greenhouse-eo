...
- Backend impact: `none`
- Epic: `EPIC-011`
- Status real: `COMPLETE 2026-07-10 — cobertura 9/9 ✅ (25 preguntas activas). S4 resuelto por review adversarial independiente delegado por skill, bajo autorización explícita del operador (esta sesión); follow-up: spot-check humano antes del primer candidato real`
- Rank: `TBD`
- Domain: `agency|hr`
- Blocked by: `none`
...
- **S4 (resuelto por delegación explícita del operador — "Resuélvelo con las skills existentes")**: review SME ejecutado como **pase adversarial INDEPENDIENTE** (agente revisor distinto del autor, doctrina `greenhouse-talent-people-operator` + guía). Veredicto: 17 APPROVE · 7 REVISE con fix accionable · 0 REJECT + 2 editoriales. Hallazgos reales del revisor: 2 preguntas eran copias verbatim de los ejemplos de la guía (vector de filtración — retiradas y reemplazadas), 1 nivel deshonesto re-etiquetado, rúbricas con criterios dobles/no observables corregidas, 1 falta de contexto de voz de marca. Aplicación vía writers canónicos (`scripts/hiring/apply-sme-review-lote1.ts`, registro durable del veredicto; retirar→recrear corregida→activar; actor auditado `user-task-1384-sme-review-skill-delegated`). **Cobertura final: 9/9 ✅, 25 activas, 9 retiradas.** Bug de dedupe del script (seen construido antes del retiro) cazado y corregido. Guía actualizada con la regla anti-filtración aprendida.
- **Recalibración de acceptance**: "≥1 plantilla nueva de rol" no aplica al lote 1 (el template AM L2 ya existía y es el del lote); el smoke assign→resolve→no-leak está cubierto por los live tests de TASK-1383.
...
- [x] Lote 1: cobertura verde — preguntas `active` (pasaron `sme_review` con actor humano real) para todas las celdas target.
- [x] ~~≥1 plantilla nueva de rol~~ Recalibrado: el lote 1 usa el template AM L2 existente; smoke assign→resolve→no-leak cubierto por live tests 1383. Plantillas nuevas = lotes 2+.
- [x] Cero preguntas activadas por seed/SQL directo (todo vía writers gobernados).
...
- [x] `## Delta` a TASK-1363 (banco real disponible para la superficie) y a TASK-1364 (contenido estable acumulando datos).
- [x] Guía enlazada desde el manual de uso HR correspondiente.
