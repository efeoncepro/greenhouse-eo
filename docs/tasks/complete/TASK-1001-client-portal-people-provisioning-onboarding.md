...
## Cierre 2026-06-04 — núcleo COMPLETE (opción A: invitación real → TASK-1010)

✅ **Núcleo cerrado:** helper SSOT `inviteClientPortalUser` (idempotente, additive, emite `role.assigned` v1 in-tx) + `suggestClientPortalRole` + `listClientPortalPersonCandidates` + capability dedicada `client.lifecycle.portal_user.invite` + `PortalUsersPanel` cableado al ítem canónico `provision_client_users_access` del timeline. **GVC del panel verificada enterprise** (estado ready, filas nombre/email·cargo, select de rol sugerido, "Con acceso" verde). Tests + grant-coverage + parity verdes.
📄 **Docs:** sección "Invitación de personas al portal cliente" en `docs/documentation/agency/alta-de-cliente.md` + manual.
📌 **Diferido a TASK-1010** (split aprobado por operador): **invitación real end-to-end** (email + activación de cuenta verificados en vivo) — la GVC NO clickeó "Invitar" para no emailear personas reales; el path de email/activación nunca se ejecutó live. Requiere flag ON + persona de prueba controlada.
