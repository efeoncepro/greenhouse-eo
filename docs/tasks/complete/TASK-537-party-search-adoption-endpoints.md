...
Fase C del programa TASK-534. Expone los endpoints `GET /api/commercial/parties/search` (unifica organizations ya materializadas + HubSpot companies candidates desde `greenhouse_crm.companies`, con search live suplementario cuando el mirror viene atrasado) y `POST /api/commercial/parties/adopt` (materializa un candidate en un click, con bootstrap completo si el lifecycle mapea a `active_client`). Backend-only — la UI del selector vive en TASK-538.

## Outcome

- Shipped en `2026-04-21`.
- `GET /api/commercial/parties/search` ya resuelve parties materializadas visibles por tenant y, para actores `efeonce_internal`, candidates HubSpot desde `greenhouse_crm.companies` con suplemento live vía `hubspot-greenhouse-integration` cuando el mirror no alcanza.
...
- `greenhouse_crm.companies` sigue siendo la fuente local primaria de candidates, pero `/search` y `/adopt` pueden suplementar con read live vía `hubspot-greenhouse-integration` cuando el mirror viene atrasado.
