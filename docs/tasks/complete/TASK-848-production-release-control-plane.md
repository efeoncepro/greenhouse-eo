...
## Closure 2026-05-10 — Control Plane V1.1 COMPLETO end-to-end

**SHIPPED en `develop`** vía 4 commits incrementales V1.0 (2026-05-10) + 5 follow-ups V1.1 SHIPPED el mismo día. Todos los acceptance criteria del control plane completos.

**V1.0 entregado** (4 commits originales TASK-848):

- Slice 1 (`824eacab`) Foundation — migration `release_manifests` + `release_state_transitions` + 3 capabilities + 7 outbox events versionados v1 + spec V1 + DECISIONS_INDEX entry
- Slice 3 partial (`d845dbfb`) Concurrency fix Opcion A en 3 worker workflows — kills bug class del incidente 2026-04-26 → 2026-05-09
- Slice 7 partial (`f1b85a86`) 2 critical reliability signals (`stale_approval` + `pending_without_jobs`) + subsystem `Platform Release` + module `platform`
- Slice 6 skeleton (`a0b004e4`) Rollback CLI `production-rollback.ts` + runbook canónico `production-release.md`

**V1.0 score 4-pillar**: Safety 8/10, Robustness 9/10, Resilience 8/10, Scalability 10/10.

**V1.1 follow-ups SHIPPED 2026-05-10** (5 tasks compactadas de 6 originales por arch-architect):

| Task | Slice | Entrega |
|---|---|---|
| TASK-849 (watchdog) | 6 commits | Detector cron `*/30` + 3rd reader `worker_revision_drift` + alertas Teams + GitHub App auth canonical |
| TASK-850 (preflight CLI) | 9 commits | CLI `pnpm release:preflight` con 12 checks fail-fast + composer pattern TASK-672 + 3 sub-capabilities granulares |
| TASK-851 (orchestrator + worker SHA) | 5 commits | Workflow `production-release.yml` 8 jobs + worker workflow_call canonical + EXPECTED_SHA verify post-deploy |
| TASK-853 (Azure gating) | 4 commits | 2 Azure workflows refactored 5 jobs canonicos + Bicep apply gated por diff o force flag + WIF subjects documentados |
| TASK-854 (dashboard + 2 signals) | 3 commits | `/admin/releases` read-only + 2 signals `deploy_duration_p95` + `last_status` (5 of 5 signals canonicos) |

**Hito: subsystem `Platform Release` con 5 of 5 reliability signals canonicos**:

1. `platform.release.stale_approval` (drift)
2. `platform.release.pending_without_jobs` (drift)
3. `platform.release.worker_revision_drift` (drift)
4. `platform.release.deploy_duration_p95` (lag)
5. `platform.release.last_status` (drift)

**Acceptance criteria globales V1.1** (cumplidos):

- [x] State machine canonica 8 estados + transition matrix V1 §2.3 + parity test TS↔SQL
- [x] 3 capabilities granulares least-privilege seedadas (TASK-848) + 3 sub-granulares preflight (TASK-850) + watchdog.read (TASK-849)
- [x] 7 outbox events versionados v1 emitiendo en cada transición (manifest-store atomic)
- [x] Concurrency fix Opcion A aplicado a 3 worker workflows production
- [x] Watchdog cron `*/30 * * * *` activo con dedup state + Teams alerts + GitHub App auth
- [x] Preflight CLI 12 checks fail-fast con composer pattern TASK-672 canonical
- [x] Orchestrator workflow_dispatch con 8 jobs + worker SHA verification end-to-end
- [x] Azure gating 2 workflows con Bicep apply gated + WIF subjects 3 patterns documentados
- [x] Dashboard `/admin/releases` read-only con cursor pagination + drawer + microcopy es-CL canonical
- [x] 5 of 5 signals subsystem `Platform Release` visibles en `/admin/operations`

**V1.2 backlog documentado en spec V1 §Pendiente para V1.2**:

- Capability `platform.release.read_results` granular para FINANCE_ADMIN observability
...
- Rollback automatizado Azure (V2 contingente con `what-if` mandatory + restore desde commit previo + smoke test)

**Hard Rules** canonizadas en CLAUDE.md secciones:
...
**Docs canonizadas**: 6 secciones CLAUDE.md + 6 secciones AGENTS.md + 6 entries DECISIONS_INDEX + Delta GREENHOUSE_RELEASE_CONTROL_PLANE_V1 con 5 sub-deltas + 5 manuales operador + 5 docs funcionales + runbook ampliado §6.1/§6.2/§6.3.
