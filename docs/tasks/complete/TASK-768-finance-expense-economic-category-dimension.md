...
## Open Questions — RESUELTAS pre-execution (2026-05-03)

Resueltas con la opción más robusta, segura, resiliente, escalable. Cero bandaids.

- **Q1 — `EconomicCategory` enum: 9 valores suficientes?** ✅ **RESUELTO: 11 valores (extendido)**. Lista canónica: `labor_cost_internal | labor_cost_external | vendor_cost_saas | vendor_cost_professional_services | regulatory_payment | tax | financial_cost | bank_fee_real | overhead | financial_settlement | other`.
   - **Rationale**: la lista de 9 conflate `vendor_cost` (que mezcla SaaS e.g. Adobe/Vercel con servicios profesionales e.g. Beeconta/contadora — ortogonales para análisis de cost-of-tooling vs cost-of-services). Split en `vendor_cost_saas` + `vendor_cost_professional_services` se justifica por blast radius del Member Loaded Cost (TASK-710-713) que distingue tool-cost de service-cost al modelar full absorption costing.
   - **Adicionalmente** introduzco `bank_fee_real` para distinguir comisiones bancarias **operativas** (mantención de cuenta, comisión de transferencia) vs FX fees que **debería poder cargar a labor cost externo** (los FX fees de Global66 pagando a Daniela/Andrés son cost-of-payroll, no overhead bancario).
   - **NO agrego** `marketing_spend` (subset de `vendor_cost_saas` o `vendor_cost_professional_services` según naturaleza), `client_reimbursement` (esto es revenue side — pertenece al simétrico de income), ni `intercompany_transfer` (no aplica hoy; cuando emerja con TASK-710b o sucesor, se agrega via migration aditiva sin breaking change).
   - **Backwards-compatibility plan**: si se necesita agregar un valor en el futuro, seguir patrón TASK-571: ADD VALUE TO ENUM atomic + helper `mapLegacyToCanonicalEconomicCategory()` para retrocompat.

- **Q2 — Ubicación del backfill?** ✅ **RESUELTO: Node script con resolver TS canónico**, ejecutado via `pnpm tsx --require ./scripts/lib/server-only-shim.cjs scripts/finance/backfill-economic-category.ts`.
   - **Rationale**: PL/pgSQL function requiere duplicar el rules engine en SQL (regex + identity lookups + accounting_type map) — viola single source of truth. PLPython no está habilitado en Cloud SQL. Admin endpoint transient acopla scope de migración al runtime web (mal encaje). Node script con `--dry-run` + `--batch-size` + `--limit` es: (a) reusa el resolver TS canónico (cero duplicación), (b) idempotente, (c) auditable (escribe `economic_category_resolution_log` con rule matched + confidence), (d) operable desde local sin dependencias serverless, (e) testeable con vitest.
   - **Implementación**: el script obtiene Cloud SQL Connector vía `getDb()`, lee batches de 500 vía Kysely, llama a `resolveEconomicCategoryBulkBatch()` (con cache de identity lookups), persiste UPDATE per-row dentro de transacción de batch. Failures individuales se enqueuen en `economic_category_manual_queue` y NO bloquean el batch.

- **Q3 — Cutover_date del CHECK?** ✅ **RESUELTO: derivado, atomic VALIDATE post-backfill clean** (mismo patrón TASK-708/728/766).
   - **Rationale**: pinning rígido pre-backfill rompe atomicity. Pattern probado 4 veces. La migration de constraint detecta `COUNT(*) WHERE economic_category IS NULL` post-backfill: si = 0 (caso esperado para Greenhouse pequeño), `VALIDATE CONSTRAINT` corre atomic en la misma transacción. Si > 0 (rows en manual queue), constraint queda NOT VALID hasta que UI de slice 6 las resuelva, momento en el cual una migration follow-up ejecuta el VALIDATE — todavía atomic, solo diferido.

- **Q4 — Schema de lookup tables?** ✅ **RESUELTO: `greenhouse_finance`** para ambas (`known_regulators`, `known_payroll_vendors`).
   - **Rationale**: aunque `team_members` y `identity_profiles` viven en `greenhouse_core` (porque son objetos canónicos 360 cross-domain), las lookup tables que esta task introduce son **artefactos de classification financial-only**. Ningún módulo fuera de Finance las consumirá. Ubicarlas en `greenhouse_finance` cumple el principio domain-per-schema sin contaminar el core con accidental coupling.

- **Q5 — Rename `expense_type` → `accounting_type`?** ✅ **RESUELTO: NO renombrar en TASK-768. Documentar alias semántico en CLAUDE.md.**
   - **Rationale**: rename físico es invasivo (BQ exports tienen ese nombre, dashboards SQL ad-hoc, types.d.ts, scripts de operadores). El nombre `expense_type` no es semánticamente incorrecto; es solo un nombre subóptimo. Renombrarlo en TASK-768 acopla scope (refactor cosmetic + dimension split). Documentación canónica en CLAUDE.md sección nueva: "`expense_type` es la dimensión taxonómica fiscal (legacy name); `economic_category` es la dimensión analítica/operativa (TASK-768)". Si emerge necesidad real de rename, follow-up TASK con migration `ALTER TABLE ... RENAME COLUMN expense_type TO accounting_type` con backwards-compat shim de VIEW.

- **Q6 — Mode lint rule?** ✅ **RESUELTO: `error` desde commit-1**. Patrón TASK-766.
   - **Rationale**: tolerancia cero. CHECK constraint asegura no-NULL post-cutover. Cualquier callsite que use `expense_type` para análisis económico produce KPI sesgado por construcción (porque la realidad es que `expense_type='supplier'` puede ser labor externa, vendor SaaS, o servicios profesionales — 3 buckets económicos distintos). Override block exime: SII reports, IVA engine (TASK-532), VAT ledger (TASK-533), el resolver mismo, tests del resolver, y archivos `legacy/`. Lint rule + CHECK constraint + reliability signal forman defensa-en-tres-capas.

- **Q7 — Símetrico para `income.income_type`?** ✅ **RESUELTO: SÍ, paridad completa**.
   - **Rationale**: el task scope ya incluía la columna en income aditivamente; los slices del scope ya están redactados con paridad. Ahora se confirma que el resolver canónico maneja AMBOS lados (expense + income) con rules engine compartido (rules específicas para income: client_reimbursement, factoring_proceeds, partner_payout, internal_transfer_in, etc.). Income enum: `service_revenue | client_reimbursement | factoring_proceeds | partner_payout_offset | internal_transfer_in | tax_refund | financial_income | other`. Misma estructura: lookup table `known_income_categorization_hints` si emerge necesidad. Cero ambigüedad en el scope.

**Decisión adicional pre-execution (no en spec original)**: introduzco una tabla append-only `greenhouse_finance.economic_category_resolution_log` para auditar el backfill (rule matched, confidence, timestamp, runner, batch_id). Es mecanismo de transparencia — el operador podrá ver cómo se resolvió cada fila pre-cutover. Eliminable post-validación pero útil durante rollout.
