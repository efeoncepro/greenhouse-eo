...
**Paso 3 (causa raíz) — supuesto corregido**: la spec asumía "verificar que `secrets:rotate` destruye superseded; si no, agregarlo". **Corrección**: `scripts/secrets/rotate.ts` NO destruye superseded **a propósito** (línea 257: disable manual tras soak, preserva rollback). Auto-destroy-on-rotate ahí sería **inseguro** (pierdes rollback si la rotación sale mal). La causa raíz NO es el helper genérico — es el **productor** Frame.io que rotó 76× sin limpieza. Requisito documentado en **TASK-020** (Delta 2026-05-25): el flujo OAuth de Frame.io debe hacer keep-N + destroy on-rotate cuando se productivice. Helper `secrets:rotate` queda **sin cambios** (su patrón es el correcto para secrets críticos que rotan raramente).
