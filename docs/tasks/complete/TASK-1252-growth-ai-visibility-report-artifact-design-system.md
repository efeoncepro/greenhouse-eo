...
## Delta 2026-06-28 — tercer eje `entity` disponible para render (TASK-1267)

- TASK-1267 sumó un **tercer eje ortogonal** de readiness: `entity` (Knowledge Graph / Wikidata / Reddit-UGC), computado en `ReadinessScore.entity` (engine). El report builder hoy solo mapea `.structural`/`.agentic` al `ReportReadiness`/`PublicReportReadiness` → **el eje `entity` se persiste pero todavía NO se renderiza**. Cuando esta task materialice el render de readiness, debe contemplar los **tres** ejes lado a lado (percepción vs structural vs agentic vs entity), honrando la misma honest degradation (`status='empty'`/`severity='sin_dato'`) — ej. el eje entity sale `sin_dato` si los probes de entidad están OFF o la KG api key no está configurada. Extender el builder (`toReportReadinessAxis` para `readiness.entity`) + los contratos (`ReportReadiness.entity` / `PublicReportReadiness.entity`) es parte del render del eje.
