...
### Complemento TASK-1389 (2026-08-10) — campo `surface` en el manifest + contrato de superficies

- `ChildRouteDeclaration` en `route-reachability-manifest.ts` suma el campo **opcional `surface: NavSurface`** (`sidebar|avatar|command-palette|shortcuts|none`): además de `parent` + `via` + `reason`, una declaración puede fijar **en qué superficie vive** el destino. Las 13 declaraciones `/my/*` declaran `surface: 'avatar'`; una `/my/*` con `surface: 'sidebar'` es violación del gate `pnpm nav:budget` (cross-check del manifest).
- Alcanzabilidad y superficie son contratos **complementarios**: este manifest responde "¿cómo se LLEGA a la ruta?"; el **Contrato de Asignación de Superficies de Navegación** (`docs/architecture/agent-invariants/NAVIGATION_SURFACE_ALLOCATION_CONTRACT.md`, TASK-1389) responde "¿en qué SUPERFICIE vive?" e impone el presupuesto del rail interno (`MAX_TOP_LEVEL_SLOTS=8`, `MAX_INTERACTIVE_DEPTH=2`; gate `pnpm nav:budget` en severidad `error`, medido contra el árbol real vía `VerticalMenu.budget.test.tsx`).
