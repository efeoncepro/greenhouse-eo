...
## Nota de alineación 360 y vigencia parcial

Esta task es históricamente importante, pero debe leerse hoy alineada con:
- `docs/architecture/GREENHOUSE_360_OBJECT_MODEL_V1.md`
- `docs/architecture/GREENHOUSE_INTERNAL_IDENTITY_V1.md`

Partes que siguen vigentes:
...
1. **Email y `notion_display_name` no son identidad canónica**
   - pueden servir como señales de bootstrap o fallback de reconciliación
   - no deben reemplazar `identity_profile_id` ni `member_id`

2. **El objeto `Collaborator` ya no debe depender de match textual como diseño principal**
   - la dirección vigente del repo usa:
     - `team_members.member_id`
     - `team_members.identity_profile_id`
     - `identity_profile_source_links`

3. **El sistema de identidad no debe quedarse en silo de Team**
   - debe enriquecer People, Payroll, Finance, Agency y futuros módulos sobre el mismo objeto canónico

4. **Cualquier extensión futura de esta task**
   - debe tratar `team_members` como objeto canónico compartido
   - no debe crear un nuevo maestro de persona dentro de otro módulo
...
- [ ] Auth con NextAuth.js funcionando (`docs/tasks/complete/CODEX_TASK_Microsoft_SSO_Greenhouse.md`)
...
- Click dispara el modal de solicitud de expansión (mismo modal del dashboard, ver `docs/tasks/complete/CODEX_TASK_Client_Dashboard_Redesign.md` sección ghost slot)
