...
- **Slice 4 (CODE-COMPLETE)** — render real del nodo Figma: cliente server-only `figma-render.ts` (REST `/v1/images` + `/v1/files/nodes`, timeout, degrada honesto), API `GET /api/design-system/figma-nodes/preview` (mismo gate, fail-closed AXIS), y el editor hace fetch debounced del render real cuando la URL parsea a un nodo AXIS válido (fallback de identidad AXIS sin token / si la API falla). Token vía `resolveSecret('FIGMA_API_TOKEN')` → secret `greenhouse-figma-api-token`. lint+tsc+build verdes. **Pendiente operativo único**: provisionar el secret `greenhouse-figma-api-token` (PAT Figma read-only con acceso a AXIS) — apenas exista, el preview pasa a render real sin deploy de código. Caché durable del thumbnail (download→asset) NO se construyó: no hay surface que muestre el thumbnail fuera del editor (el preview es transitorio, la URL temporal de Figma alcanza); es enhancement futuro si emerge ese consumer.
