...
- **Drift de datos pre-existente (ajeno a TASK-957)**: **Melkin Hernández** tiene `member.contract_type='contractor'` pero su comp-version activa dice `contract_type='indefinido'` y `deel_contract_id=NULL`. Hoy el motor lo rutea bien por `payroll_via='deel'` (passthrough), pero si `payroll_via` se limpiara lo calcularía como empleado dependiente chileno sobre un sueldo USD/internacional (incorrecto). La señal `double_rail_overlap` NO lo captura (no hay engagement), pero `payroll.contract_taxonomy.invalid_tuple_drift` SÍ. **Resuelto vía → TASK-958** (2026-05-31): remediación payroll-safe del drift de tupla + VALIDATE del CHECK `compensation_versions_contract_pay_regime_check` (estaba NOT VALID → grandfathered la fila de Melkin) + señal `payroll.deel_member_without_contract_id`. Confirmado con el operador: Melkin es contractor internacional de Nicaragua vía Deel → target `contractor`; el cambio es payroll-neutral (passthrough Deel).
