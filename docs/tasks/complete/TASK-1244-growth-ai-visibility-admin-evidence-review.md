...
`growth.ai_visibility.report.review` se declaró en `src/config/entitlements-catalog.ts` sin su fila en
...
- **HTTP smoke de los 3 endpoints contra `pnpm dev` + agent auth (la capa que el dry-run de comando NO tocaba).** 8 casos sobre runs sembrados: `GET /reviews` (admin) → 200 + run en la cola + `reviewReasons` presentes; `GET /reviews` (collaborator sin capability) → **403** (guard de capability funciona en HTTP); `POST reject` sin reason → 422; `POST approve` → 200 + `reportToken`; `POST reject` sobre aprobado → 409; `POST approve` run inexistente → 404; sin sesión → 401; cola limpia post-aprobación. **El smoke atrapó un bug real que los unit tests (mockean el comando) no podían: los errores 4xx devolvían `code:internal_error` + mensaje genérico "algo salió mal de nuestro lado, inténtalo en unos minutos" + `actionable:true`** — engañoso para errores estructurales (`invalid_transition`/`not_reviewable`/`not_found`: reintentar NO resuelve) y violando el contrato canónico ("NUNCA Reintentar con `actionable=false`"). Era el anti-patrón `canonicalErrorResponse('internal_error', { statusOverride })` (espejado de la publish route hermana). **Fix (causa compartida, no parche local):** el mismo anti-patrón estaba en **6 routes** del dominio AI Visibility (review approve/reject + run-detail + score + report-read + publish-TASK-1239 + POST runs). Sweep completo a códigos canónicos: `grader_run_not_found` (404, false) único para run/score not-found, `grader_report_not_releasable` (409, false) para el publish gateado, `grader_run_invalid_input` (400, true) para validación del POST, `grader_report_not_reviewable`/`grader_report_invalid_review_transition` (409, false) + `grader_report_review_reason_required` (422, true) para el gate de revisión — todos con mensaje es-CL honesto. Lead-handoff intacto (solo 502 genuino). TASK-1247 (UI) ya puede leer `code`/`actionable` honestos.
- **Smoke de STAGING contra el bundle desplegado** (`dev-greenhouse` env, agent auth superadmin vía SSO bypass, commit `11b6d89b9` deploy `success`). Verificado end-to-end sobre el deploy real (no localhost): `GET /reviews`→200 + run en la cola; `POST reject` sin reason→**422** `{code:grader_report_review_reason_required, actionable:true, "Indica el motivo del rechazo…"}`; `POST approve` run inexistente→**404** `{code:grader_run_not_found, actionable:false, "No encontramos ese análisis…"}`; `POST approve`→**200** `state:approved`; `POST reject` sobre aprobado→**409**. Auth + capability + `await params` + contrato de error honesto confirmados en el deployment activo. Cola real de revisión limpiada post-smoke (runs sintéticos pendientes borrados; los decididos quedan fuera de la cola con sus artefactos append-only).
