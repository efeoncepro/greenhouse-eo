...
- Epic: `EPIC-045`
- Status real: `COMPLETE 2026-09-16 — en producción desde 2026-09-15 (release 9c094688309d, INSIGHTS_GENERATION_ENABLED ON en staging y Production; emisión/IA OFF hasta TASK-1846), gateway efeonce-mcp v1.5.0 desplegado (rev 00053-dsk) con scope efeonce.mcp.insights.write en Entra, skill servida verificada en producción y con agente sin contexto, ensayo de rollback (migrate:down + up) ejecutado 2026-09-16 00:24–00:25Z con readbacks y canaries posteriores (EO-INS-000002 staging, EO-INS-000003 producción). Diferido a EPIC-046 P01: integración con Berel/Sky reales. Verificación de cliente opcional: tools/list por sesión MCP con token humano`
- Rank: `TBD`
...
- [ ] Berel SEO/contenidos y Sky diseño/ICO se integran con EPIC-046 consumiendo readers de los productores, nunca el BFF client-portal; conservar unidad, período, frescura y proyección en snapshot/identidad de reutilización. — Cubierto por esta task: adapters consumen sólo readers dueños (boundary test prohíbe `client-portal`), unidad/período/frescura/proyección viajan en el snapshot (verificado en staging y producción con la org sintética). **Diferido a EPIC-046 P01 (fuera del alcance de cierre de esta task):** asignar `insights_v1` a Berel/Sky reales y correr su primer canary; no requiere código nuevo de esta unidad.
...
- [x] API interna, App/Ecosystem y MCP ejercitan allow/deny y misma semántica; base-only read no permite crear ni emitir. — Evidencia: `insights-lanes.test.ts` (app + ecosystem con mocks) y, en staging real 2026-09-15, app (persona `agent-client`, 200 catálogo / 202 create / 200 idempotente / 409 conflicto) y ecosystem (consumer del gateway: catálogo, lista, detalle con evidencia, 202 create, 404 deny) con la misma tabla de errores; el provider REAL del gateway (`dist`, `scripts/greenhouse-insights-canary.mjs`) pasó contra staging. Gateway desplegado 2026-09-15 (revisión `00053-dsk`, provider real verde contra staging) y scope en Entra; manual servido verificado en producción por el lane y evaluado por un agente sin contexto (request aceptado, 202). Un `tools/list` por sesión MCP con token humano queda como verificación de cliente opcional (no cambia el contrato ya ejercitado en todas las capas que esta task posee).
...
- [x] Migración/readback, pruebas live serializadas cuando corresponda, flags y rollback se verifican antes de activación; issue permanece bloqueado sin outputs validados. — Evidencia: migración aplicada + readback, `test:live` verde, flags registrados (generación ON en staging y Production desde 2026-09-15; emisión e IA OFF), activación por lane verificada en staging y producción, `issue` bloqueado por puerto (test), y **ensayo de rollback ejecutado 2026-09-16 00:24–00:25Z** en la instancia compartida con el Down definitivo (readback tras down: schema ausente, fila de `pgmigrations` ausente, módulo y asignación intactos, 4 capabilities deprecadas; readback tras up: 7 tablas, 14 transiciones, 3 clases de retención, 13 triggers, 4 capabilities vivas, módulo y asignación intactos, `run_on` 2026-09-16 00:25:31Z, secuencia reiniciada; canaries posteriores: staging create 202 `EO-INS-000002` + replay 200, producción create 202 `EO-INS-000003`).
...
- Ensayo de rollback ejecutado 2026-09-16 00:24Z–00:25:31Z (tercer intento, con el Down definitivo y permisos habilitados por el operador): `migrate:down` OK (630 tablas introspectadas), readback intermedio `schema_exists=0 / migration_row=0 / module_row=1 / assignment=1 / audit_events=1 / deprecated_caps=4`, `migrate:up` OK (637 tablas), readback final `tables=7 / editions=0 / matrix=14 / retention=3 / module=1 / assignment=1 / live_caps=4 / triggers=13`, secuencia de códigos reiniciada (`EO-INS-000001` consumido por el readback). Las 4 ediciones sintéticas previas (`EO-INS-000012..15`) se perdieron por diseño. Canaries posteriores: staging app lane create 202 → `EO-INS-000002` (replay 200 `idempotent:true`); producción ecosystem create 202 → `EO-INS-000003`. La sesión que ejecuta TASK-1846 fue avisada antes y después.
- Verificación de cliente opcional (no bloqueante): `tools/list` por sesión MCP con token humano.
...
- [x] Lifecycle, carpeta, Status real y acceptance actualizados con evidencia; sin rollout no se declara complete. — 2026-09-16: rollout en producción verificado; un criterio diferido explícitamente a EPIC-046 P01.
- [x] TASK_ID_REGISTRY, README y EPIC-045 sincronizados; remover blockers obsoletos en dependientes. — TASK-1846/1847/1848/1849/1672 desbloqueadas el 2026-09-15.
- [x] Arquitectura técnica, documentación funcional y manual/runbook actualizados proporcionalmente. — Barrido documental 2026-09-15 (`060cf1157`) + `EFEONCE_INSIGHTS_IMPLEMENTATION_RECORD_V1.md`.
- [x] Handoff/changelog y contratos UI/API/MCP reflejan disponibilidad real.
- [x] Regresiones, señales, rollback y gates documentales pasan; no commit/push/deploy automático. — suite 14,123 verde en el cierre de código, señales `insights.editions.*` ok, rollback ensayado, gates documentales 0/0; commits/push/deploy autorizados explícitamente por el operador.
