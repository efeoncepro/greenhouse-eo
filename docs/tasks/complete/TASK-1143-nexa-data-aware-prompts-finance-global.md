...
Resolver `finance` en el registry (TASK-1141): arranca los prompts del dashboard `/finance` desde las anomalías del ledger (descuadre/saldos viejos/gastos sin anclar/chequeos degradados) reusando `getFinanceLedgerHealth` (cero recompute). `entityKind='finance_scope'` + sentinel `finance-global`. **Anti-oracle**: gate por route_group `finance` (test). Counts/estados, nunca montos. Copy es-CL revisado (greenhouse-ux-writing, 0 voseo).
Gates: tsc 0 · lint 0 · 7/7 tests focales · doc gate verde. Pendiente rollout: GVC en `/finance` con drift real.
