...
**GVC ✅ verificado (2026-06-04, contra staging):** scenario `client-onboarding-wizard-runtime` extendido (llena dirección + OC en step 4 + clip de la sección Finanzas en Confirmar). Frame leído con bar enterprise 2026 (greenhouse-ui-review + modern-ui): el resumen Confirmar muestra los 5 campos persistidos + país `México` auto-derivado + OC vigente condicional, jerarquía label/valor limpia, estados honestos, copy 'OC vigente' (no creación de entidad). Loop convergió: fullPage ilegible (artefacto sidebar fixed) → clip → crisp. `data-capture` agregados a finance-billing-address/requires-po/po-number + Section de Confirmar. **Desbloquea TASK-1005** (AI preflight ya puede razonar sobre campos que el runtime persiste).
