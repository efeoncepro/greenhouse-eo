...
## Delta 2026-07-10 — Baseline recalibration pre-ejecución (verificado contra PG vivo + código)

- **`members.active`/`status` NO son columnas GENERATED** (verificado live: `is_generated='NEVER'` en todas las columnas de members, cero triggers; el propio primitive SCIM ejecuta `UPDATE members SET active=TRUE` en la lane de reactivación — imposible sobre una generated). El `Generated<>` de `db.d.ts` es semántica de kysely-codegen para "tiene DEFAULT", no SQL `GENERATED ALWAYS` — el Delta 2026-07-08 leyó mal esa marca (exactamente la trampa "db.d.ts no es SoT" que esta misma spec advierte). **La regla se conserva como gobernanza** (la activación pasa SOLO por `completeWorkforceMemberIntake` + readiness), no como imposibilidad física.
- **"Member nace no-activo" se recalibra a "no activado operacionalmente" = `active=TRUE + workforce_intake_status='pending_intake'`** (el patrón canónico SCIM/TASK-872). Crear con `active=FALSE` rompería el flujo: `syncOperatingEntityMembershipForMember` con member inactivo NO crea la membership (skip/deactivate) y la lane `identity_access` de readiness bloquearía el intake para siempre. El gate real de payroll/capacity es `workforce_intake_status != 'completed'` detrás de `PAYROLL_WORKFORCE_INTAKE_GATE_ENABLED` (invariante TASK-872), no la columna `active`.
- **El core del cascade D-2 es module-private y OID-shaped** (`resolveMemberIdCascade` no exportado, exige `externalId`). Refactorizar el archivo SCIM = blast alto en la superficie del incidente 2026-06-01 → se implementa un **core hermano source-neutral** en `src/lib/workforce/hiring-activation/member-core.ts` que espeja las lanes aplicables (por `identity_profile_id` → por email sin profile → reactivación de inactivo → INSERT espejo del SCIM) con los mismos guards de drift (block, nunca auto-merge). La **discoverability D-2 queda garantizada por construcción**: el member nace con `identity_profile_id` poblado y la lane #1 del SCIM lo encuentra y backfillea `azure_oid` cuando llegue Entra (cero duplicación).
- **`ready_to_activate` NO se persiste** (readiness es live; persistirlo invita drift) — el detail reader lo computa con `resolveWorkforceActivationReadiness`. Estados persistidos: `pending_hr_review|blocked|member_created|onboarding_open|active|cancelled`.
- **El checklist de onboarding llega solo**: el consumer reactivo `hr_onboarding_auto_create` escucha `member.created` y crea la instancia idempotente. `open-onboarding` la asegura explícita (idempotente) y captura los ids en el mapping.
- **Capabilities (mapa final):** cola/review/complete → `hiring.activation.review` (la única nueva); create-member → `workforce.member.intake.update` (reusada); open-onboarding → `hr.onboarding_instance:create` (reusada). Sin caps de access/login: el bridge NO crea `client_users` ni `user_role_assignments` (out of scope, follow-up de access provisioning).
