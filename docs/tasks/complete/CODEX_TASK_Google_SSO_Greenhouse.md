...
Este task queda como referencia histórica del rollout inicial de Google SSO.

Estado real del repo al 2026-03-14:
- Google SSO ya está implementado y funcionando junto con Microsoft SSO y `credentials`
- el principal canónico de auth es `greenhouse.client_users`
- cualquier trabajo futuro en este frente debe tratarse como hardening, UX o provisioning incremental
...
- **Auth implementada hoy:** NextAuth.js v4 con 3 providers (`credentials`, Azure AD, Google)
- **Archivos existentes que se modifican:**
  - `src/lib/auth.ts` — authOptions con providers y callbacks
  - `src/lib/tenant/access.ts` — helpers de auth y vinculación de identidad
...
- **Tabla BigQuery vigente:** `efeonce-group.greenhouse.client_users` ya existe con campos de Microsoft y Google

---

## Alcance de la tarea

### 1. Nota de alineación sobre BigQuery

El cambio histórico consistía en agregar estas columnas. En el repo actual ese delta ya fue absorbido por `greenhouse.client_users`:
...
No debe ejecutarse hoy sobre `greenhouse.clients`.
...
### 4. Helper de auth: nota de alineación

En la implementación real, estas funciones viven en la capa de acceso de tenant/auth y operan sobre `greenhouse.client_users`, no sobre `greenhouse.clients`:

```typescript
export async function getClientByGoogleSub(sub: string) {
  const [rows] = await bigquery.query({
    query: `
      SELECT * FROM \`efeonce-group.greenhouse.client_users\`
...
if (account?.provider === 'google') {
  ...
  if (!client.google_sub) {
    await linkGoogleIdentity(client.user_id, googleSub, googleEmail)
  }

  await updateLastLogin(client.user_id, 'google_sso')
  return true
}
```
...
if (account?.provider === 'google') {
  ...
  if (client) {
    ...
    token.role = client.primary_role_code || client.role
    token.projectIds = client.project_scopes || client.notion_project_ids
...
- [ ] Columnas `google_sub` y `google_email` agregadas a `greenhouse.client_users`
