...
- [x] Existe una ADR aceptada que decide Cloud Run+ALB o Vercel+Cloud Run para la release actual y declara horizonte,
      alternativas, costo, consecuencias, reversibilidad, confidence y triggers de reconsideración.
      → `EFEONCE_GLOBE_FRONTEND_HOSTING_FRONT_DOOR_DECISION_V1.md` (ADR-004): Cloud Run web/BFF + Global ALB.
- [x] El ADR identifica inequívocamente dónde termina `globe.efeoncepro.com`; `globe-api-internal` permanece privado
      y su Google ID-token audience no se deriva del dominio browser.
      → ALB + serverless NEG → `globe-studio-internal`; API IAM-private, audience `run.app`.
- [x] El drift Next.js objetivo versus Node nativo vivo queda resuelto o asignado con owner/gate anterior a 1505.
      → adoptado el servidor Node nativo; Next.js `superseded` para el shell interno (ADR §Decision.2).
- [x] El dominio internal-only, la escalabilidad/HA y Production/clientes externos quedan modelados como gates distintos.
      → tres gates: URL internal-only / HA (gate `TASK-1465`) / Production (`TASK-1480`).
- [x] Existe una task sucesora de implementación; la ADR no se interpreta como autorización de apply.
      → `TASK-1507` registrada (`to-do`, blocked by 1506); ADR §"authorizes no apply".
- [x] El owner de sesión/OAuth durable bajo `apps/studio-web` queda dentro de `TASK-1465` o en una child task explícita.
      → asignado a `TASK-1465` (Delta 2026-07-20); `maxScale > 1` hard-gated ahí.
- [x] La task sucesora incluye ownership Terraform/deploy de servicios, ingress, scale/env, OAuth, DNS/TLS,
      observabilidad, costo, least privilege, smoke y rollback. → cubierto por el scope de `TASK-1507`.
- [x] `TASK-1469`, `TASK-1474`, `TASK-1475` y `TASK-1505` expresan dependencias coherentes con la decisión.
      → 1469/1465/1475/1505 con Delta 2026-07-20; 1474 vía el front-door ordering contract de EPIC-028.
- [x] Ningún recurso, DNS, OAuth, IAM, secreto, flag, tráfico o cliente externo cambia durante esta task.
      → sólo documental; runtime verificado read-only, sin apply.
...
- [x] `Lifecycle` y carpeta sincronizados con el estado real (`complete` + `complete/`).
- [x] `docs/tasks/README.md`, `docs/tasks/TASK_ID_REGISTRY.md` y EPIC-028 sincronizados.
- [x] `docs/architecture/creative-studio/DECISIONS_INDEX.md` registra la ADR (ADR-004).
- [x] `GLOBE_RUNTIME_HANDOFF.md` declara decisión, límites y siguiente paso ejecutable (tomar `TASK-1507`).
- [x] La task sucesora (`TASK-1507`) queda registrada, pero ningún apply/cutover queda implícitamente autorizado.
- [x] `greenhouse-qa-release-auditor` y `greenhouse-documentation-governor` revisan el cierre (gates docs/closure abajo).
...
- ~~¿El dominio internal-only sale inmediatamente tras la ADR o junto al primer slice funcional de Producer?~~
  **Resuelto:** el custom domain (`TASK-1507`) se secuencia **antes del rollout interno de `TASK-1505`**, no
  inmediatamente; hasta entonces el `*.run.app` + SSO es la base URL estable (decisión del operador 2026-07-20).
- ~~¿La arquitectura objetivo conserva Next.js o adopta el servidor Node nativo para esta release?~~
  **Resuelto:** adopta el servidor **Node nativo** para el shell interno (Next.js `superseded`). El host + framework
  del **frontend cliente comercial** queda como decisión diferida (ADR-004 §Revisit triggers): Vercel + Next.js sobre
  edge global es candidato vivo, a decidir al construir `TASK-1505` y antes de `TASK-1480`.
