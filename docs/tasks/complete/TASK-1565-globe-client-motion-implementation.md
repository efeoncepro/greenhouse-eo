...
- ⚠️ **CORREGIDO 2026-07-25 — el "defecto de accesibilidad vigente" que esta task declaraba NO existe.**
  Al escribir el gate leí el CSS real: `.pf__actions` se revela con `:hover` **y** `:focus-within`, y
  `@media (hover: none)` las deja **siempre visibles en touch**. O sea son alcanzables por teclado y por
  dedo, y bajo `reduce` sólo pierden la transición. La afirmación salió de leer el bloque del prototipo
  (que las fija visibles bajo `reduce`) y asumir que el mío no tenía equivalente — sin abrir el archivo.
  El prototipo resuelve el problema de otra forma, no mejor: fijar las acciones bajo `reduce` **conflaciona
  dos preferencias no relacionadas** (quien pide menos movimiento no necesariamente tiene problema con el
  hover), mientras `@media (hover: none)` apunta exactamente al caso real. **Se mantiene el enfoque actual.**
...
### Slice 2 — ~~Corregir el defecto vigente~~ → RETIRADO, no existía

El defecto declarado no existe (ver `Current Repo State`). El CSS ya cubre teclado (`:focus-within`) y touch
(`@media (hover: none)`), que es el problema real. Lo que **sí** queda de este slice: un assert del canary que
afirme esa cobertura, para que la próxima superficie no la pierda.
...
| Una superficie nueva anima sin declarar reduced-motion | accesibilidad | **Alta** sin gate | el gate del Slice 1, ya calibrado contra falsos positivos | falla de build |
...
| 2 | retirado — no había defecto que corregir | — | n/a |
...
- [ ] Con `reduce` activo: las acciones de la card son **alcanzables** — por `:focus-within` con teclado y
      siempre visibles bajo `@media (hover: none)`. (Corregido: no se exige `opacity: 1` permanente bajo
      `reduce`; ese era el enfoque del prototipo y conflaciona dos preferencias distintas.)
