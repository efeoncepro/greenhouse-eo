...
1. **Persistencia durable en vez de in-memory** (autorizado por el operador). El spec difería las
   anotaciones a `TASK-1465`, que **ya shipeó y no las cubrió**, y `TASK-1508` dejó los servicios en
   **3 réplicas**: un store in-memory no queda "volátil" sino **no determinista** (una estrella
   escrita en una réplica es invisible en otra). Se implementó `AssetAnnotationStorePort` con doble
   in-memory (default de tests) **más** `DurableProducerAssetStore` + migración
   `0003_producer_asset_annotations.sql`, wired en `main.ts`. `Migration posture` pasa de `none` a
   **una migración aditiva**; el rollback sigue siendo flag OFF + revert.
2. **Constantes propias en vez de extender `GLOBE_PRODUCER_READERS`.** Ese mapa ya existía
   (`TASK-1500`) para los readers del **catálogo**, que responden a otra capability. Meter ahí el
   output side conflaciona dos autoridades en un vocabulario. Se crearon
   `GLOBE_PRODUCER_ASSET_READERS` / `GLOBE_PRODUCER_ASSET_COMMANDS` en
   `contracts/src/producer-assets.ts`.
3. **`ProducerOutputMediaType` propio.** El spec tipaba el handle con `'model-3d'`, que no pertenece a
   `LabInputMediaType` (`image|video|audio|text`). El tipo nuevo es `image|video|audio|model-3d`;
   `copyAsReference` sigue usando `LabInputMediaType` y **falla cerrado antes de mintear** para
   `model-3d` (no es referenciable).
4. **`OutputRetrievalPort.retrieve({ sha256, mediaType })`** en vez de `retrieve(sha256)`. El store es
   content-addressed: no puede conocer el medio. El `mediaType` lo aporta el dominio (derivado de la
   capability) y se usa **solo como fallback** — el `Content-Type` servido sale del objeto real, así un
   run multi-output no miente en el cable.
5. **Auth antes del kill switch en la ruta**, coherente con `/v1/commands` y `/v1/readers` (que
   autentican antes de decidir política). El kill switch corre inmediatamente después y **outranks** al
   grant: apagado responde `policy_blocked`, no "tu grant está mal".
6. **`handlerErrorToApiCode` pasó a exportado** para que la ruta de serving reuse el **mismo** mapeo
   error-de-dominio → código canónico que el dispatch, sin duplicarlo.
7. **Los scripts `test` de los packages enumeran archivos a mano**: se registraron los tres tests
   nuevos. Un test no registrado nunca corre y la suite queda verde por no haber mirado. Queda como
   follow-up volverlos glob.

### Open Questions — resueltas

- **Capability única `globe.producer.assets.operate`:** sí, y **confirmado que NO reusa**
  `globe.lab.experiment.run` (esa es de gasto y api-mode; el retrieval debe ser humano-alcanzable sin
  autoridad de gasto). Partirla en dos queda para cuando un consumer exija granularidad.
- **Grant en query, no en header:** sí — la UI necesita `src` directo. Es aceptable porque el grant
  **no es un bearer autosuficiente**: la ruta autentica antes y re-chequea propiedad después, y el
  grant está atado a `(workspace, experimento, hash, disposition)` con TTL corto y nunca se loggea.
- **`copy-as-reference` persiste el handle:** sí, idempotente por
  `(workspace, experimento, attempt, hash)`; la idempotencia vive en SQL (`ON CONFLICT DO NOTHING` +
  re-lectura), no en un read-then-write que entre réplicas es una carrera.
- **mimeType autoritativo:** el `content-type` del objeto GCS, con default por `mediaType` cuando falta.
...
- Migration posture: `additive` — `packages/database/migrations/0003_producer_asset_annotations.sql` (dos tablas nuevas, sin backfill ni cambio destructivo). Recalibrado en §Delta 2026-07-22: el spec la difería a TASK-1465, que ya shipeó sin cubrirla, y con 3 réplicas un store in-memory es no determinista.
...
- [x] Source of truth, contract surface y consumidores nombrados con paths/objetos reales.
- [x] Invariantes tenant-safe, private-ingest, integridad y grant-hygiene explícitos y con test.
- [x] Migration/backfill/rollback posture explícito (una migración aditiva `0003` — ver §Delta — + flag OFF + revert).
- [x] Evidencia runtime listada (retrieval real + negativos cross-workspace/input-hash).
- [x] Errores canónicos, sin leak de bytes/secretos/vendor; grant nunca loggeado.

## Capability Definition of Done — Full API Parity gate

- [x] **Lógica en el primitive, no en la UI.** Autorización tenant-safe + private-ingest +
  disposition + favorite + copy-as-reference viven en `packages/domain/src/producer-assets.ts`.
- [x] **Modelada como reader/command**, no como click-handler: readers `output.get`/`asset.list`,
  commands `asset.favorite`/`asset.copyAsReference`.
- [x] **Read** como reader canónico; **write** como command con command semantics, autorización
...
- [x] **Capability + grant en el MISMO PR:** `globe.producer.assets.operate` en `GLOBE_CAPABILITIES`
...
- [x] **Camino programático declarado:** HTTP `/v1/readers` + `/v1/commands` + `GET /v1/outputs/:sha256`;
  SDK/MCP/CLI como clientes de la surface http; `ui`/`mcp` `policy-blocked` hasta gate.
- [x] **Write apto para propose→confirm→execute:** copyAsReference/favorite son commands gobernados
  server-side (no integración Nexa-específica).
- [x] **Un primitive, muchos consumers:** UI (1505), Workbench (1474), SDK/MCP/CLI, y 1501/1504
  (referencias) sin lógica duplicada.
- [x] **Parity check = SÍ:** el output side tiene contrato gobernado a nivel capability.
...
- [x] Reader `globe.producer.output.get` devuelve `ProducerOutputHandleV1` (descriptor + grant,
  **sin bytes**) para un output propio retenido; `experimentId` cross-workspace o desconocido ⇒
  ...
  `not_found`.
- [x] `GET /v1/outputs/:sha256` streamea bytes con `Content-Type` + `Content-Disposition`
...
- [x] La ruta de serving **reusa** el mismo helper de autorización que el reader (un primitivo, dos
  transportes) — sin política duplicada.
- [x] `globe.producer.asset.favorite` idempotente por estado deseado explícito; solo sobre outputs
  propios.
- [x] `globe.producer.asset.copyAsReference` produce `ProducerReferenceHandleV1` con
...
- [x] `globe.producer.asset.list` devuelve favorites + referencias del workspace del caller.
- [x] Capability `globe.producer.assets.operate` en `GLOBE_CAPABILITIES` + grant al service principal;
...
- [x] Kill switch `GLOBE_PRODUCER_ASSETS_ENABLED` default OFF ⇒ `policy_blocked`.
- [x] Ningún path reserva crédito ni corre un provider.
- [x] `cd ../efeonce-globe && pnpm check && pnpm build` verdes.
...
- [x] `Lifecycle` del markdown sincronizado con el estado real (`in-progress` al tomarla, `complete`
  al cerrarla).
- [x] el archivo vive en la carpeta correcta (`to-do/`, `in-progress/` o `complete/`).
- [x] `docs/tasks/README.md` + `docs/tasks/TASK_ID_REGISTRY.md` sincronizados.
- [x] `Handoff.md` actualizado (retrieval + asset actions live; flags/secretos pendientes de prender).
- [x] `changelog.md` actualizado (capability nueva del output side del Producer).
- [x] chequeo de impacto cruzado: TASK-1505 (consume retrieval/favorite/copyAsReference),
...
- [x] cierre documental proporcional (arquitectura Producer §retrieval + doc funcional + manual)
  vía `greenhouse-documentation-governor`.
- [x] §retrieval/asset-actions agregada a
