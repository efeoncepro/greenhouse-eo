...
- La lane específica de observabilidad de tests quedó extraída a `TASK-249` para volver ejecutables el inventario del suite, los artifacts de CI, el summary de GitHub Actions y la policy inicial de coverage sin mezclarla con logging estructurado ni security headers.
- `TASK-249` ya quedó implementada: `vitest.config.ts` tiene coverage reporter, CI publica artifacts de testing/coverage y GitHub Actions Summary ya expone totales del suite. Los gaps residuales de esta task quedan limitados a `tsc --noEmit`, `audit` y logging estructurado.
