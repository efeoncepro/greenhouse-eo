...
- **Worker payout** = el `net_payable` del payable existente (dinero al worker, o a Deel que lo reenvía). Sin cambios al invariante `net = gross − withholding`.
- **Provider fee** = obligación **separada** cuando es un egreso real al provider: nuevo `payable_source_kind='provider_fee'` (enum additivo) + `beneficiary_type='provider'` (ver D-795-3), `economic_category` = categoría canónica de fee de plataforma (resolver TASK-768; si no existe, **se agrega al enum TASK-768**, NUNCA string libre).
- **FX spread/fee** = **NO es un payee separado** → se registra como metadata + se clasifica a su `economic_category` de costo FX al materializar (resolver TASK-768). No genera payable.
...
**Por qué.** Separa movimientos de dinero (auditable), preserva la semántica de CHECK de TASK-793, y le da a Finance clasificación de costo limpia (worker vs fee vs FX) sin mezclar. Es el patrón canónico "state machine + CHECK + audit trio".
