...
- 🔴 **`proposals.quote_id` — la COSTURA con el cotizador** *(corrección del operador, 2026-07-12; ADR §3 reescrito)*. **`Proposal` es distinto de `Quote`, pero debe tener sinergia DIRECTA con el módulo de cotizaciones.** La frontera anterior era **pasiva** (*"el Proposal no calcula el precio"*) y dejó dos agujeros que **ya se materializaron**:
  - **"Nunca un GO sin margen" era una regla SIN MECANISMO** — la `Proposal` no conocía su costo, así que el gate de `fit_review` **no se podía hacer cumplir**.
  - **El precio se escribió A MANO** (la económica de SKY: 5.200.000 / 6.900.000 / 260.000 CLP, sin trazabilidad al loaded cost). **Una cifra de dinero client-facing sin origen trazable es la misma clase de problema que una estadística sin `evidenceRef`** — y el dominio prohíbe la primera mientras toleraba la segunda.
  - **Canónico:** `proposals.quote_id` (nullable hasta que haya precio) → **la oferta económica ES la Quote, renderizada.** El cotizador (`submit-quote-from-builder` + `finance/pricing/{pricing-engine-v2,costing-engine}` sobre **loaded cost**) sigue siendo **el único que calcula**. **El Proposal no calcula el precio, pero TAMPOCO lo transcribe: lo REFERENCIA.**
...
- Invariantes que no se pueden romper:
  ...
  - 🔴 **El gate `fit_review` LEE el margen de la `Quote` vinculada** — ahí se hace cumplir *"NUNCA un GO sin margen sobre loaded cost"*. **Sin `quote_id`, el gate NO puede evaluarse y la transición FALLA CERRADA.** *(No aprobar por omisión: hoy la regla existía en el ADR **sin mecanismo**, porque la `Proposal` no conocía su costo.)*
  - 🔴 **Al entrar a `packaging`, la `Quote` se CONGELA en un snapshot inmutable** que el artefacto renderiza — **mismo patrón que `proposal_evidence`**. El PDF debe ser reproducible: **una cotización viva que cambia después de emitir el PDF haría que el artefacto MIENTA sobre su propio precio.** El `PricingFull` del deck lee **el snapshot**, nunca la tabla viva.
  - **El Proposal NO calcula el precio, pero TAMPOCO lo transcribe: lo REFERENCIA.** El cotizador es el único que calcula (sobre loaded cost). **NUNCA** reimplementar pricing en el dominio de propuestas.
...
- Parsing/IA profunda del RFP, requisito-set/admisibilidad, fit score, HubSpot deal, producción de copy/squad/diagnóstico o submit. El agente F0 propone intake estructurado; el fan-out lector pertenece a F1.
- ⚠️ **Matiz sobre el cotizador (corregido 2026-07-12):** lo que queda fuera de scope es **el ADAPTER** —construir/derivar una `Quote` desde la `Proposal`, o el flujo de autoría del precio— **NO la COSTURA**. La costura (`proposals.quote_id` + el gate de margen en `fit_review` + el snapshot congelado en `packaging`) **entra en esta task**: sin ella, *"nunca un GO sin margen"* sigue siendo **una regla sin mecanismo**, y el precio se seguirá escribiendo a mano en un PDF contractual **sin trazabilidad al loaded cost**. Ver ADR §3.
