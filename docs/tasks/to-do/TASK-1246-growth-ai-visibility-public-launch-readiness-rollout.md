...
3. **Riesgo flag-ahead-of-code — DEGRADADO tras verificar el código (no era un peligro abierto):** el sondeo 2026-07-01 confirma que los flags ON en prod **están respaldados por código + schema reales**, no adelantados:
   - `SERVER_VALIDATION_ENABLED` (TASK-1253) → `submitForm` **sí re-valida server-side** (`revalidateAndNormalizeFields`, commands.ts:289) + `validators/` registry existe.
