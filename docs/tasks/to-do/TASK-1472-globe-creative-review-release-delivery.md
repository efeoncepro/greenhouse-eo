# TASK-1472 — Globe Creative Review, Release and Delivery
...
Implementar review humano, anotaciones, artifact manifest, release gates y paquetes de entrega recuperables.
...
Cerrar la brecha entre output del modelo y pieza profesional liberada.
...
- Boundary: `Globe Creative Review, Release and Delivery`
...
- Contrato nuevo o modificado: `candidate list/get readers; annotate/request-change/approve/reject/release/revoke/deliver commands`
- Backward compatibility: `gated`
- Full API parity: `review/release primitives are shared; irreversible release/delivery uses human propose-confirm-execute authority`
...
- Persistir candidates, reviews, decisions y supersession.

### Slice 2

- Generar artifact manifest con lineage, QA, rights y route history.

### Slice 3

- Implementar release/delivery package con revoke y rollback.
...
La ejecución comienza desde Greenhouse con `pnpm codex:task-hook TASK-1472 --develop` cuando el operador apruebe su goal. El plan puede modificar el repositorio hermano en los paths owned, pero lifecycle, checkpoints, QA y cierre permanecen en esta spec canónica.
...
|---|---|---|---|---|
| Entregar artefacto incorrecto o sin evidencia | Globe/Greenhouse | medium | gate binario, allowlist, audit y rollback antes de ampliar | release sin review/manifest |
...
- [ ] Sólo un candidate aprobado y con gates verdes puede liberarse.
- [ ] Toda entrega incluye manifest y checksums.
- [ ] Revocar release no borra evidencia ni assets fuente.
- [ ] API/SDK/conformance prueban review/release/revoke, authorization and replay; MCP may propose but cannot
      bypass the human release confirmation policy.
