...
- `src/lib/growth/ai-visibility/probes/structural/json-ld.ts` — patrón canónico de probe que parsea JSON-LD del HTML (reusa `extractJsonLdBlocks`/`flattenJsonLdNodes`/`jsonLdTypes` de `@/lib/growth/site-substrate` — `html.ts`; el path viejo `probes/html.ts` queda como shim, TASK-1697 2026-08-27). El author probe reusa este parser para detectar `Person`/`Author`/`sameAs`.
...
- `src/lib/growth/site-substrate/site-fetch.ts` — `createSiteFetcher` + `resolveSubjectSite` (SSRF-guarded, read-only al host del sujeto). Casa canónica desde TASK-1697 (2026-08-27); el path viejo `probes/safe-fetch.ts` sobrevive como shim con el alias `createProbeFetcher`. Los probes de trust/author sobre el sitio propio reusan este fetcher.
...
- **Structural (eje `structural`, TASK-1266):** `probes/structural/json-ld.ts` (schema.org en HTML, con el parser del sustrato `site-substrate/html.ts` — TASK-1697), robots-txt, llms-txt, sitemap, core-web-vitals.
...
- **Probe substrate:** `grader_probe_results` (tabla, `probe_kind` TEXT libre + `axis` CHECK structural/agentic/entity), `store.ts` (upsert idempotente), `registry.ts` (ensamble por eje/flag), `contracts.ts` (`Probe`/`ProbeContext`/honest degradation), `site-substrate/site-fetch.ts` (SSRF; ex `probes/safe-fetch.ts`, hoy shim — TASK-1697), `entity-fetch.ts` (allowlist externa).
...
- `probes/entity/author.ts`: probe(s) read-only que sobre el HTML del sujeto (reusando `resolveSubjectSite` + `createSiteFetcher` —alias `createProbeFetcher` en el shim— + el parser JSON-LD `extractJsonLdBlocks`/`jsonLdTypes`) detectan: `Person`/`Author` schema, author bylines/pages, `sameAs` (perfiles verificables), y credenciales declaradas (`hasCredential`/`knowsAbout`/`jobTitle`). Honest degradation `null ≠ 0`; evidencia public-safe (conteos, tipos, presencia).
...
- **Reuso, no re-fetch ciego.** El author/trust probe reusa: `resolveSubjectSite` + `createSiteFetcher` (SSRF, sujeto; sustrato `@/lib/growth/site-substrate`), `extractJsonLdBlocks`/`jsonLdTypes` (parser JSON-LD), el descubrimiento de about de `fetch-site-content.ts`, y `createEntityApiFetcher` + allowlist si hay verificación externa. Es principalmente **composición de primitives existentes**, no infra nueva.
