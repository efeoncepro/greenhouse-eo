...
- [ ] Existe `NexaChatProvider` y el provider Gemini está extraído detrás de ella, con default = Gemini y comportamiento idéntico (tests de paridad verdes).
- [ ] Existe el adapter Anthropic con tool-use conversacional (2-pass `tool_use`/`tool_result`), reusando `executeNexaTool` sin tocarlo.
- [ ] El output de cada adapter es `{ text, toolInvocations }` con el `raw.packet` idéntico → señales TASK-1085 + persistencia + Answer Trace funcionan sin cambios.
- [ ] Existe el router interno (`selectProvider`, función pura) por intención + failover; el provider elegido se loggea.
- [ ] La selección de modelo NO está expuesta al usuario (no hay UI de selección; `requestedModel` es solo override de ops).
- [ ] Flags `NEXA_AUTO_ROUTER_ENABLED` / `NEXA_PROVIDER`; activar "auto" es config sin deploy; default = Gemini.
- [ ] El tool `search_knowledge` y las Answer Rules NO fueron modificados (diff vacío en esos archivos).
- [ ] Eval de paridad: cada provider cita + respeta no-invención en `confidence='none'`.
...
- [ ] Delta en `GREENHOUSE_NEXA_ARCHITECTURE_V1.md` + `CLAUDE.md` (provider abstraction + router).
