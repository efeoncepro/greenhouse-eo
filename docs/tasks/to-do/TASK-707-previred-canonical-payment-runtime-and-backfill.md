# TASK-707 — Previred Canonical Payment Runtime & Backfill (umbrella)

## Delta 2026-04-28 (b) — Split en 707a / 707b / 707c por riesgo de write-path

Después del cierre de **TASK-706** (Previred Processor UX & Bank Semantics, mergeada `55c14844`) y de la auditoría de impacto cruzado, esta task se **descompone en 3 sub-tasks** para minimizar el blast radius del write-path canónico + backfill histórico + componentización en una sola sesión.

**Razón del split**: TASK-707 mezcla 3 capas con perfiles de riesgo muy distintos:

1. **Detection + canonical state runtime (write-path nuevo)** — riesgo medio, blast radius local. Pagos Previred nuevos entran por carril correcto sin tocar histórico.
2. **Backfill histórico** — riesgo alto. Muta filas ya consumidas por `commercial_cost_attribution`, `client_economics`, `client_labor_cost_allocation_consolidated` (TASK-709). Recomputar mal cambia P&L histórico de marzo/abril.
3. **Componentization runtime** — riesgo medio. Requiere payroll context y solo aplica cuando aparezca el segundo pago Previred del lifecycle con desglose disponible. No urgente hoy (1 fila pendiente al 2026-04-28).

Tomar las 3 en una sesión es la receta para un bug silencioso que cambie el P&L histórico vía (2) sin que sea evidente. El split deja (1) ejecutándose y observable durante 7+ días antes de habilitar (2). (3) espera a que el lifecycle real del módulo lo amerite.

**Sub-tasks creadas (2026-04-28)**:

| Sub-task | Scope | Status | Bloqueada por |
|---|---|---|---|
| [`TASK-707a`](TASK-707a-previred-detection-and-canonical-state-runtime.md) | Detection + canonical state runtime (sin backfill, sin componentización efectiva) | `to-do` P1/Medio | none |
| [`TASK-707b`](TASK-707b-previred-historical-backfill-and-rematerialize.md) | Backfill histórico marzo/abril + downstream rematerialize controlado | `to-do` P1/Alto | TASK-707a + ≥7 días observación |
| [`TASK-707c`](TASK-707c-previred-componentization-runtime.md) | Componentization runtime (`pending_componentization` → `componentized` por payroll anchor) | `to-do` P2/Medio | TASK-707a + TASK-707b |

**Esta task umbrella (`TASK-707`)** queda como referencia histórica + orchestration. Cierra cuando las 3 sub-tasks estén `complete`.
...
- Lifecycle: `to-do` (umbrella — split en TASK-707a/b/c)
- Priority: `P1`
- Impact: `Alto`
- Effort: `Alto` (suma de las 3 sub-tasks)
- Type: `umbrella` (orchestration de sub-tasks)
- Epic: `[optional EPIC-###]`
- Status real: `Split en TASK-707a/b/c (2026-04-28). Esta umbrella cierra cuando las 3 sub-tasks estén complete. Coordinación TASK-708 verificada (Previred no rompe invariante). Patrón remediación reusable disponible si emerge cohorte histórica.`
- Rank: `TBD`
- Domain: `finance`
- Blocked by: `none`
