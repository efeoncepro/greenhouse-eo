...
| `TASK-813` | `to-do` | HubSpot p_services (0-162) Bidirectional Sync Activation + Phantom Seed Cleanup (auditoría 2026-05-06: 30 filas en `core.services` seedeadas el 2026-03-16 como cross-product `service_modules × clients` con `hubspot_service_id IS NULL`, 16 servicios reales en HubSpot 0-162 nunca sincronizados, 3 huérfanos sin org GH — Loyal/Motogas/Aguas Andinas. Sync code existe pero nunca se invocó. Activar webhook inbound + Cloud Scheduler safety-net + 3 reliability signals nuevos en subsystem Commercial Health. Soft dep TASK-555, hard dep TASK-801. Diseño aprobado por skill arch-architect). | `docs/tasks/to-do/TASK-813-hubspot-services-bidirectional-sync-phantom-seed-cleanup.md` |
