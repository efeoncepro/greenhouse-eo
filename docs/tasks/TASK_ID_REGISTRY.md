...
| `TASK-867` | `to-do` | Offboarding Work Queue Projection + UX Modernization — follow-up V2 de TASK-862/TASK-863. Crea proyeccion read-only `OffboardingWorkQueue` para componer caso + ultimo calculo + ultimo documento + prerequisitos + proximo paso, elimina N+1 fetch en `HrOffboardingView` y moderniza `/hr/offboarding` como cola operacional con summary, progreso accesible, estados y accion principal sin reemplazar write paths existentes. | `docs/tasks/to-do/TASK-867-offboarding-work-queue-projection-ux-modernization.md` |
