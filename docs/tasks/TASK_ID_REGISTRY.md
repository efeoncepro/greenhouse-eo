...
| `TASK-777` | `to-do` | Canonical Expense Distribution & Shared Cost Pools (formaliza el resolver `expense -> distribution lane` sobre `economic_category` + payroll/payment anchors; separa `member_direct`, `shared_operational_overhead`, `shared_financial_cost`, `regulatory_payment`, `provider_payroll`, `treasury_transit` y `unallocated`; corta la contaminación actual de `operational_pl`/`member_capacity_economics` donde Deel, Previred y fees financieros hoy inflan overhead; deja recomendación explícita para abril 2026 y gate de cierre para mayo) | `docs/tasks/to-do/TASK-777-canonical-expense-distribution-and-shared-cost-pools.md` |
| `TASK-778` | `to-do` | Treasury-Grade Cash Position Contract (refactor `/finance/cash-position` que viola IAS 7 §6 mezclando assets+liabilities+accounts in transit en KPI "Posición neta"; introduce `account_treasury_class` enum + 8 surfaces canónicas: Cash on Hand reconciliado / Runway / 13-week rolling forecast / AR aging / AP scheduling / FX exposure / IAS 7 categorización Operating-Investing-Financing / Working Capital DSO-DPO-CCC; reliability signals `cash_position.kpi_drift` + `unreconciled_accounts`; lint rule `no-untokenized-cash-aggregation`; benchmarks Mercury/Brex/Stripe Treasury/Kyriba; auditoría origen 2026-05-03 vía skill greenhouse-finance-accounting-operator) | `docs/tasks/to-do/TASK-778-treasury-grade-cash-position-contract.md` |
