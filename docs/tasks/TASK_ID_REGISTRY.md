...
| `TASK-862` | `to-do` | Final Settlement V1 Closing: Renuncia Voluntaria Legalmente Ratificable — cierra el motor V1 de finiquito por renuncia (art. 159 N°2 CT). 3 frentes: (1) emitir 3 componentes declarados-no-calculados (`monthly_gratification_due`, `used_or_advanced_vacation_adjustment`, `payroll_overlap_adjustment`) + split feriado pendiente vs proporcional; (2) PDF legal rewrite con clausulas narrativas PRIMERO-QUINTO + Ley 21.389 pension alimentos + Ley Bustos cotizaciones al dia + reserva derechos + ministro de fe + huella + logo empleador + watermark PROYECTO; (3) pre-requisitos data (carta renuncia gate, domicilio TASK-784, pension alimentos declarada). 7 slices A-G incluido revision legal externa antes de flip a produccion. Out of scope: otras causales (V2). | `docs/tasks/to-do/TASK-862-final-settlement-resignation-v1-closing.md` |
