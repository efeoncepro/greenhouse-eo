...
| `TASK-863` | `to-do` | Finiquito Prerequisites UI: Carta Renuncia Uploader + Ley 21.389 Form en HrOffboardingView — follow-up de TASK-862. Los 2 endpoints canonicos `/resignation-letter` + `/maintenance-obligation` (Slice C) viven pero NO tienen UI dedicada; operador HR debe llamarlos via DevTools. Agrega Card "Pre-requisitos del finiquito" en la fila del caso con 2 chips de estado, 2 dialogs modales (upload + form Alt A/B) y gating client-side del boton "Calcular finiquito". 5 slices A-E. NO toca backend. | `docs/tasks/to-do/TASK-863-finiquito-prerequisites-ui.md` |
...
| `TASK-865` | `to-do` | Production Worker Environment Isolation — corrige drift donde workflows `push:develop` de workers mutan los mismos servicios Cloud Run productivos que el orquestador. Define contrato staging/prod, agrega guardrails en workflows/deploy.sh y tests anti-regresion para que production workers solo cambien via `production-release.yml` o break-glass explícito. | `docs/tasks/to-do/TASK-865-production-worker-environment-isolation.md` |
