...
| `TASK-842` | `to-do` | Finance FX Drift Auto-Remediation Control Plane — convierte `finance.account_balances.fx_drift` de detector reactivo + backfill manual a control plane autocorrectivo en `ops-worker`: reader detallado, planner con politica financiera segura, rematerialización canónica, scheduler bounded, run tracking/auditoría y regresión del drift `santander-clp` 2026-05-01. | `docs/tasks/to-do/TASK-842-finance-fx-drift-auto-remediation-control-plane.md` |
