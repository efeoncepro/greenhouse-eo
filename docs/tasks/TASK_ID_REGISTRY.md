...
| `TASK-771` | `in-progress` | Finance Supplier Write Decoupling + BQ Projection vía Outbox (desacopla POST/PUT supplier de BQ inline; mueve sync a consumer reactivo `provider.upserted` en ops-worker; reliability signal `finance.providers.bq_sync_drift`; backfill 3 suppliers huérfanos figma/microsoft/notion; recovery del incidente 2026-05-03 "Error al crear proveedor") | `docs/tasks/in-progress/TASK-771-finance-supplier-write-decoupling-bq-projection.md` |
