...
| `TASK-891` | `to-do` | Person 360 Relationship Drift Reconciliation Write Path — follow-up declarado en TASK-890 ADR §7. Command auditado server-only para que EFEONCE_ADMIN cierre relacion legacy (effective_to + status='ended') y abra nueva relacion (contractor/eor/honorarios) en una sola transaccion atomica + outbox event v1. Reason >=20 chars. UI dialog en /admin/operations integrado al signal alerta. Auto-escalation severity warning → error post 30d. Bulk + auto-reconcile + delegacion a HR son V1.1+. P1 / Alto / Medio. Domain: identity\|hr. Spec: `to-do/TASK-891-person-relationship-drift-reconciliation-write-path.md`. | `docs/tasks/to-do/TASK-891-person-relationship-drift-reconciliation-write-path.md` |
