...
| `TASK-892` | `in-progress` | Offboarding Closure Completeness Aggregate — modela `closureCompleteness` server-side sintetizando 4 capas ortogonales (case lifecycle + member runtime + Person 360 relationship + payroll scope). primaryAction se deriva de pendingSteps[0] priorizado canonical (NO mas hardcoded por lane). Reliability signal nuevo `hr.offboarding.completeness_partial`. Bug class fuente: Maria case executed + drift Person 360 muestra boton "Cerrar con proveedor" obsoleto. Validado arch-architect Greenhouse overlay. In-progress 2026-05-15 directo en develop. P1 / Alto / Medio. Domain: hr\|identity\|payroll\|reliability. | `docs/tasks/in-progress/TASK-892-offboarding-closure-completeness-aggregate.md` |
