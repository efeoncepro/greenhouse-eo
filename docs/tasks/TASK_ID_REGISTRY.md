...
| `TASK-895` | `to-do` | Leave Accrual Participation-Aware (TASK-893 V1.1a) — extiende `src/lib/hr-core/leave-domain.ts` componiendo `resolvePayrollParticipationWindowsForMembers` (TASK-893) mes a mes del year. Crea primitive year-scope `LeaveAccrualEligibilityWindow` que filtra `rule_lane='internal_payroll'` para feriado legal CL Art 67 CT (excluye contractor/honorarios/external). Cierra bug class sobreacumulacion en transiciones contractor→dependent. Flag `LEAVE_PARTICIPATION_AWARE_ENABLED=false` default. Signal `hr.leave.accrual_overshoot_drift`. Backfill audit script dry-run. P1 / Alto / Medio. Domain: hr\|payroll. Skills consultadas: arch-architect + greenhouse-payroll-auditor (2026-05-16). | `docs/tasks/to-do/TASK-895-leave-accrual-participation-aware.md` |
