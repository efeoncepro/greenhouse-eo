...
| `TASK-812` | `to-do` | Compliance Exports Chile — Previred + LRE como proyecciones payroll versionadas (corregida 2026-05-09: no asumir Previred posicional ni LRE XML/XSD; Slice 0 congela fuentes oficiales vigentes, variante, delimitador/encoding/headers, mappings y fixtures antes de implementar; registry de artefactos con `specVersion`, `sourceSnapshotHash`, `sha256`, totales y validation result; generadores read-only sobre `payroll_entries`; mappings declarativos; capabilities `hr.payroll.export_previred`/`hr.payroll.export_lre`; reliability drift; blocker hard TASK-707a para paridad completa con `payment_order`, dependency TASK-784 resuelta para RUT canónico). | `docs/tasks/to-do/TASK-812-compliance-exports-chile-previred-planilla-lre-libro.md` |
...
| `TASK-839` | `to-do` | ISSUE-068 Fase 5 — Wire Admin Center governance mutation paths a writes reales con outbox + audit log + UI degraded mode honesto. Activa la capa de governance overlay (defaults por rol + overrides personales + startup policy) que TASK-404 declaró pero nunca quedó wireada. La infraestructura DB ya está lista (TASK-838). 7 slices: discovery + helpers canónicos atómicos (UPSERT + audit + outbox) + endpoints con capability granular + UI degraded + Playwright smoke + capabilities granulares seedeadas + 2 reliability signals. Blocked by: nada (TASK-838 cerrada). | `docs/tasks/to-do/TASK-839-issue-068-fase-5-admin-center-governance-wire-up.md` |
| `TASK-840` | `to-do` | ISSUE-068 Fase 6 — Deprecated capabilities cleanup oportunista. Helper canónico `markCapabilityDeprecated` con pre-check de grants activos + endpoint admin + scan reporter script + disciplina canonizada (cuando se remueve capability del TS catalog, migration que la marque deprecated). Housekeeping. Blocked by: TASK-839 (necesita writes activos para que emerjan candidates). | `docs/tasks/to-do/TASK-840-issue-068-fase-6-deprecated-capabilities-cleanup.md` |
| `TASK-841` | `in-progress` | Nubox ops-worker Config + Freshness Hardening — corrige el cutover incompleto Vercel cron → Cloud Run: `ops-worker` no tiene `NUBOX_API_BASE_URL` ni credenciales Nubox, provocando `raw_sync` y `quotes_hot_sync` fallidos aunque Cloud Scheduler este ENABLED. Incluye config/secret contract, replay controlado, freshness semantics raw/conformed/Postgres y reliability guard contra falsa salud. | `docs/tasks/in-progress/TASK-841-nubox-ops-worker-config-freshness-hardening.md` |
