...
| `TASK-773` | `in-progress` | Outbox Publisher Cloud Scheduler Cutover + Reliability + E2E Pre-Merge Gate (migra `outbox-publish` Vercel cron → Cloud Scheduler + ops-worker; state machine `pending→publishing→published/failed/dead_letter`; 2 reliability signals `sync.outbox.unpublished_lag`+`dead_letter`; lint rule custom `finance-route-requires-e2e-evidence`; cierra clase de bugs invisibles staging cuando flujos write-then-projection no se procesan) | `docs/tasks/in-progress/TASK-773-outbox-publisher-cloud-scheduler-cutover.md` |
