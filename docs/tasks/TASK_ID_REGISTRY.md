...
| `TASK-857` | `in-progress` | GitHub Webhooks Release Event Ingestion — follow-up de Release Control Plane V1.1: endpoint GitHub firmado para `workflow_run`/`workflow_job`/`deployment_status`, dedupe por `X-GitHub-Delivery`, payload redacted y reconciliacion contra `release_manifests` sin reemplazar el watchdog. | `docs/tasks/in-progress/TASK-857-github-webhooks-release-event-ingestion.md` |
