...
| `TASK-758` | `in-progress` | Payroll Receipt Render Contract Hardening (4 regímenes: chile_dependent, honorarios, international_deel, international_internal — helper canónico `resolveReceiptRegime` + `buildReceiptPresentation` puro server-safe con exhaustiveness `never`-check; cierra bug raíz `isChile=payRegime==='chile'` que afecta a 3/4 regímenes; F6 PeriodReport+Excel sale a TASK-782) | `docs/tasks/in-progress/TASK-758-payroll-honorarios-receipt-render-contract-hardening.md` |
...
| `TASK-782` | `to-do` (blocked by TASK-758) | Payroll Period Report + Excel Honorarios Disaggregation (follow-up operador-facing de TASK-758: corrige `PeriodReportDocument` y `generate-payroll-excel.ts` para separar honorarios de Chile dependiente bajo subtotales propios — hoy mezclan retención SII con descuentos previsionales bajo "Total descuentos Chile"; reusa `resolveReceiptRegime` exportado por TASK-758 — single source of truth de clasificación de régimen) | `docs/tasks/to-do/TASK-782-payroll-period-report-excel-honorarios-disaggregation.md` |
