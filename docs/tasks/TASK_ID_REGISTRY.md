...
| `TASK-841` | `to-do` | Nubox ops-worker Config + Freshness Hardening — corrige el cutover incompleto Vercel cron → Cloud Run: `ops-worker` no tiene `NUBOX_API_BASE_URL` ni credenciales Nubox, provocando `raw_sync` y `quotes_hot_sync` fallidos aunque Cloud Scheduler este ENABLED. Incluye config/secret contract, replay controlado, freshness semantics raw/conformed/Postgres y reliability guard contra falsa salud. | `docs/tasks/to-do/TASK-841-nubox-ops-worker-config-freshness-hardening.md` |
