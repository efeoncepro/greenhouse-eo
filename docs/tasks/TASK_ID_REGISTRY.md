...
| `TASK-782` | `in-progress` | Payroll Period Report + Excel Honorarios Disaggregation (follow-up operador-facing de TASK-758: corrige `PeriodReportDocument` y `generate-payroll-excel.ts` para separar honorarios de Chile dependiente bajo subtotales propios — hoy mezclan retención SII con descuentos previsionales bajo "Total descuentos Chile"; reusa `resolveReceiptRegime` exportado por TASK-758 — single source of truth de clasificación de régimen) | `docs/tasks/in-progress/TASK-782-payroll-period-report-excel-honorarios-disaggregation.md` |
