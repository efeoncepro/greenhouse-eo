...
| `TASK-758` | `to-do` | Payroll Receipt Render Contract Hardening (4 regímenes: chile_dependent, honorarios, international_deel, international_internal — helper canónico `resolveReceiptRegime` + `buildReceiptPresentation` puro server-safe con exhaustiveness `never`-check; cierra bug raíz `isChile=payRegime==='chile'` que afecta a 3/4 regímenes; F6 PeriodReport+Excel sale a TASK-782) | `docs/tasks/to-do/TASK-758-payroll-honorarios-receipt-render-contract-hardening.md` |
