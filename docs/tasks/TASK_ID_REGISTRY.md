...
| `TASK-777` | `in-progress` | Canonical Expense Distribution & Shared Cost Pools (formaliza el resolver `expense -> distribution lane` sobre `economic_category` + payroll/payment anchors; separa `member_direct`, `shared_operational_overhead`, `shared_financial_cost`, `regulatory_payment`, `provider_payroll`, `treasury_transit` y `unallocated`; corta la contaminación actual de `operational_pl`/`member_capacity_economics` donde Deel, Previred y fees financieros hoy inflan overhead; deja recomendación explícita para abril 2026 y gate de cierre para mayo) | `docs/tasks/in-progress/TASK-777-canonical-expense-distribution-and-shared-cost-pools.md` |
