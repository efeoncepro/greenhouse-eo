...
- **Job al control plane:** `artifact-worker-deploy.yml` con `workflow_call`/SHA esperado/change-gate por etiqueta `git-sha`; Job único ⇒ bucket fijo `staging` y flags default ON en `deploy.sh` (valores vivos). Commit `d9da99df8`.
- **Auditoría:** render encargado por usuario cliente quedaba `system`; migración expand `20260916201127095` + actor en run/eventos (commit `1875fdd32`).
- **Benchmark Cloud Run staging:** 1 output por tick de 2 min; detalle en Acceptance Criteria.
- **Gateway:** `efeoncepro/efeonce-mcp#14` mergeado (`da8295a`, v1.6.0), sin deploy.
...
- [x] `requestInsightRender`/retry/cancel revalidan los tres planos + audiencia; reutilización de outputs por `(org, edición, target, audiencia)` (UNIQUE + live test). **Verificado en runtime staging 2026-09-16:** edición `internal` creada por superadmin → `agent-client` pide render `404` y lee `404`, **0 outputs creados**; el actor humano queda auditado (`client_user`/`member`) en run, encolado, retry y cancelación (migración `20260916201127095`, live test 4/4). — Autogestión cliente y gestión interna de arquitectura §7.1 revalidan autoridad al ejecutar; reutilización/idempotencia de outputs conserva audiencia y proyección. Un job cliente nunca reutiliza bytes de un draft interno aunque coincidan org y período (integración EPIC-046).
