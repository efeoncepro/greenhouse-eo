...
- **⚠️ Premisa corregida (confirmada por el doc oficial de Anthropic, 2026-06-17):** la verificación empírica + `code.claude.com/docs/sub-agents` confirman que **el Explore SKIP-ea CLAUDE.md** (*"Explore and Plan skip your CLAUDE.md files"*). El Explore que falló "live 2026-06-16" nunca cargó CLAUDE.md → este refactor **no podía** arreglar ese spawn. El valor real es el costo per-turn del main loop + subagentes general-purpose/custom + adherencia + gobernanza.
- **Mecanismos nativos adoptados (2026-06-17):** (1) **path-scoped rules** `.claude/rules/*.md` con `paths:` (8 dominios) — auto-cargan el pointer al companion al tocar `src/lib/<dominio>/**` (recomendación explícita del doc para "CLAUDE.md too large"; no cuentan al budget); (2) **`.claude/agents/explore-lite.md`** (`tools: Read, Grep, Glob, Bash`) — subagente sin MCP que spawnea siempre (el fix real del spawn, repo-level; requiere restart de sesión). Híbrido: los companions `docs/` siguen siendo el canónico portable (paridad Codex).
- **Follow-up (opcional, infra/harness):** desconectar MCP servers no usados en claude.ai (Adobe/Higgsfield/Semrush/Spotify…) si se quiere que también el Explore built-in spawnee. Fuera de scope. Ver §0.1 de `docs/operations/CLAUDE_MD_REFACTOR_MAP_2026-06-16.md`.
