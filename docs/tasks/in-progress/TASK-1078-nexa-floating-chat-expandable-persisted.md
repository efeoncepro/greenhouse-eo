...
- **Cutover (Slice 4 restante)**: flip de `NEXT_PUBLIC_NEXA_FLOATING_EXPANDABLE_ENABLED` a `true` tras validación + sign-off; `baseline.surfaceId` + `fe:capture:diff --promote` (paridad mockup↔runtime); GVC **mobile** del runtime; verificación Home sin regresión documentada.
...
- **`NexaSenderMark` primitive** + **Concepto C (sidecar) + preferencia de modo** + **Concepto A (dock)**: follow-ups committeados (ver arriba).
