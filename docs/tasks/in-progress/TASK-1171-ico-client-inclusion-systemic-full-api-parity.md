...
- **Verificación:** migración aplicada a dev DB; **command ejecutado E2E contra DB real sobre Berel → ok, alreadyEnabled=true (idempotente)**; BQ MERGE SQL dry-run válido; 303 tests focales verdes (incl. grant coverage + command denial/idempotencia/BQ-degrade) + tsc + lint limpios. **Pendiente runtime:** confirmar el endpoint live en staging (POST autenticado sobre Berel → 200) — bloqueado solo por el build Vercel (lento/encolado al cierre de sesión), NO por código; el endpoint envuelve el command ya verificado.

**Pendiente:** Slice 4 (preflight/verify-ICO profundo "configurado ≠ fluyendo" + lifecycle a ICO), Slice 5 (UI affordance). (Staleness: RESUELTA Slice 1b. Agency report: RESUELTO Slice 2. Enable-sync gobernado: CODE-COMPLETE Slice 3.)
