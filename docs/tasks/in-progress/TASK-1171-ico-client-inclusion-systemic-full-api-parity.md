...
**⚠️ Hallazgo relacionado (follow-up — NO es Berel):** efeonce/sky en el rollup junio están **STALE** (otd 1.5%/2.4%, `materialized_at=2026-06-01`, completed_tasks 1-2 — claramente desactualizado). Es la OTRA cara del incremental-delta: una entidad **ya-materializada** sin ediciones que crucen el cutoff que avanza **nunca se refresca** → su dashboard de cliente muestra data congelada. El coverage-gap fix inserta entidades AUSENTES pero NO refresca entidades STALE existentes. Requiere su propia solución (freshness/periodic-full-refresh del rollup, blast radius en los 5 rollups) — crear task derivada.

**Pendiente:** Slice 2 (agency report data-driven), Slice 3 (capability+endpoint gobernado Nexa-operable), Slice 4 (preflight/lifecycle a ICO), Slice 5 (UI) + follow-up staleness del rollup.
