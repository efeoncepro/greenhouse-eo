...
## Delta 2026-06-13 (b) — GAP A: primitive `NexaMomentComposition` construida + consolidación de rutas

Decisión del operador: **NO proliferar rutas de mockup** — toda la iteración de GAP A vive en `/knowledge/mockup/nexa-answers` (un solo espacio). Se **borró** la ruta `/knowledge/mockup/nexa-composition` (route + view + scenario standalone) que se había creado; los fixtures se reubicaron al dir de `nexa-answers`.

**Construido (UI-platform, mío):** primitive `NexaMomentComposition` (`src/components/greenhouse/primitives/nexa-moment-composition/`, P+V+K: types + controller + component + barrel + 5 tests). Patrón **propio** de Greenhouse (Google AI Overviews/AI Mode como norte, NO copia): layout primitive (hermana de `AdaptiveSidecarLayout`) que **compone** un Momento Nexa con una superficie operativa (host) **sin reemplazarla**, **reusando `NexaAnswersCanvas` por slot** (el canvas no cambia). 3 diferenciadores propios vs Google: (1) **anclaje cita↔ítem del host** (`data-nexa-anchor` → al pasar por una fuente se resalta el documento/fila real, la evidencia ES el contenido operativo), (2) **next-step gobernado** (action boundary slot), (3) el **host queda vivo** debajo + puente "Seguir con Nexa" a la lente. Variants `leadOverlay`/`anchoredAside`/`inlineExpand`; kinds `knowledgeOverview`/`financeMetricExplain`/`agencyAccountBrief`/`listAssist`. a11y/reduced-motion/tokens AXIS horneados.

**Demo:** dentro de `/knowledge/mockup/nexa-answers` (sección `NexaMomentCompositionSection`, toggle Host↔Con Nexa). GVC desktop+mobile mirado (scenario `knowledge-nexa-moment-composition`, clip de la sección): respuesta protagonista + fuentes ancladas resaltando el doc real + next-step gobernado + host condensado vivo. tsc 0 · lint 0 · design:lint 0 · 5 tests del controller.

**Es incremental, NO reemplaza:** canvas + átomos + lente (GAP B) + multi-turno se reusan; lo único nuevo es la primitive + (pendiente) un placement aditivo `composed`. Si se descarta, se borra la primitive y todo lo demás sigue igual.

**Pendiente (joint con Codex / Moment Fabric TASK-1095/1096):** placement `composed` en el contrato `surfaceContext`; Lab `/design-system/nexa-moment-composition` al promover la primitive al registry; consumer de dominio REAL (finance/agency) con eligibility + context adapter + action boundary + señales `nexa.moment.*` (eso es de Codex). El boundary: yo = la primitive de composición (presentación); Codex = cuándo aparece el Momento + la acción gobernada + el grounding de dominio.
