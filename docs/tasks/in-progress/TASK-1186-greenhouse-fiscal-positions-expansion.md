...
- **Prioritarias primero (B Retenciones + A PPM)** → se crearon las 2 children mensuales/operativas:
  ...
  - **`TASK-1189`** — PPM monthly position (child A, P2). ✅ **complete (code-complete + shadow) 2026-06-20:** tablas `ppm_rate_config` (SSOT tasa) + `ppm_monthly_positions`, materializador (base ventas netas × tasa), endpoint, signal `finance.ppm.position_drift`, flag `PPM_POSITION_ENABLED` (OFF). Shadow 19 períodos. **Con esto las 3 líneas mensuales del F29 (IVA + retenciones + PPM) están materializadas** — falta solo la vista consolidada (child E). Rollout pendiente: tasa real con contador + validación + flip.
  - **`TASK-1195`** — F29 mensual consolidada (child E, P2). ✅ **complete 2026-06-20:** reader compositor `getF29ConsolidatedMonthlyPosition` + endpoint `GET /api/finance/f29/monthly-position` que une las 3 líneas (IVA/retención/PPM) por entidad legal en un solo contrato gobernado, propagando `enabledByLine` (oficial vs shadow). **Composición pura — sin schema/migración/signal nuevos.** Smoke runtime PG 2026-06 OK. Gates: full suite 7467 + build verde. **Con esto el alcance MENSUAL del F29 está cerrado (backend).** UI visible = follow-up `ui-ux` separado → TASK-1197.
  - **`TASK-1197`** — UI F29 consolidado (follow-up `ui-ux` de child E, P3). ✅ **complete 2026-06-20:** card "Posición F29 del mes" en el Finance Dashboard (cliente puro del contrato TASK-1195, backend impact none) con las 3 líneas + badge oficial/shadow + degradación honesta. GVC desktop+mobile mirado, full suite 7474 + build verde. **Con esto la capa de visibilidad humana del alcance mensual del F29 quedó cerrada.** Rollout: deploy staging pendiente.
- **C (Renta Anual F22 → `TASK-1196`) y D (multi-country/multi-entity) quedan como children siguientes** (D no creada aún; se levantan cuando se prioricen).
