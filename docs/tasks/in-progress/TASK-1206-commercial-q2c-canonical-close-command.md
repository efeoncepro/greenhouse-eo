...
- [x] **Runtime evidence (smoke local PASS 2026-06-22):** un cierre `simple_invoice` sobre fixture manual (`scripts/commercial/task-1206-q2c-close-smoke.ts`) produjo income `INC-be2ec127` + contrato + audit Q2C `completed` + `converted_to_income_id` enlazado + outbox (`quote_to_cash.started`→`contract.created`→`quotation.converted`→`quote_to_cash.completed`→`invoice_emitted`). **Replay con el mismo idempotencyKey devolvió el MISMO incomeId (anti doble-AR confirmado).** Signals críticos en steady (converted_without_income=0, duplicate_income=0). **Pendiente staging+prod:** cutover requiere push + flip de flag + sign-off (no se usó ninguna de las 12 Nubox imports → no se duplicó AR real).
