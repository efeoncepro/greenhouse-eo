...
## Delta 2026-06-29 (cont.) — VERIFICADO E2E en staging + fix grant secretAccessor (Claude)

**Smoke humano completado ✅.** Grupo Berel conectado por el operador (`jreyes@efeoncepro.com`) → desplegable mostró todas las propiedades → eligió `sc-domain:berel.com` → conexión `active`. El reader `readSearchConsoleAnalytics` devolvió **datos reales**: consulta "berel" 2.807 clics / 21.045 impresiones, "pinturas berel" 1.654 clics, etc. Fila DB: `status=active`, `site_url=sc-domain:berel.com`, `token_secret_ref=search-console-token-operator-user-efeonce-admin-julio-reyes`.

**Fix aplicado durante el smoke — grant `secretAccessor` (gotcha load-bearing):** el primer intento del desplegable falló con "No pudimos obtener tus propiedades" (`query_failed`). Causa raíz: `greenhouse-portal@` tenía `secretVersionAdder` (escribir `search-console-token-*`) pero NO `secretAccessor` (leer). `secretVersionAdder` no incluye `versions.access` → `resolveSecretByRef` lanzaba PERMISSION_DENIED en staging (local funcionaba con ADC humano). Se agregó binding `roles/secretmanager.secretAccessor` condicionado al prefijo `search-console-token-*` (title `search-console-tokens-read`). Documentado en la memoria `reference_runtime_sa_secret_write_grant` y en el manual. **Regla:** todo secret per-cliente/operador que el runtime CREE *y* LEA necesita los dos grants condicionados.

**Documentación triple:** funcional `docs/documentation/growth/conexion-search-console.md` + manual `docs/manual-de-uso/growth/conectar-search-console.md` + técnica (este Delta + `GREENHOUSE_SOURCE_SYNC_PIPELINES_V1.md` §Delta 2026-06-29). **Estado:** operativamente completo en **staging**; **prod pendiente** (release control plane + opcional verificación Google). NO mover a `complete/` hasta el rollout de prod.
