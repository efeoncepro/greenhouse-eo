...
- **Modelo de dos planos (decisión operador 2026-06-29 — "industrias hay muchas"):** NO se enumera cada vertical como nodo. La **taxonomía gobernada es bounded: Macro (`industry`, ~24 nodos) + Mid (`sector`, curado SÓLO donde el buyer journey difiere del macro)**. La **cola larga fina** (p. ej. "pinturas y recubrimientos", "aerolínea low-cost") vive como **DATO** en el snapshot grounded (`fine_category` / `what_the_brand_does`), **NUNCA como nodo nuevo**. El nodo (macro/mid) elige el arquetipo de prompt; el descriptor fino le da especificidad (TASK-1290 consume ambos). El resolver apunta a macro de forma confiable y a mid sólo si hay un nodo curado con confianza. Ejemplo: Grupo Berel → macro `industry:manufacturing` (o `consumer_goods`) + `fine_category="fabricante de pinturas y recubrimientos"`, no un nodo `paints_coatings`.
