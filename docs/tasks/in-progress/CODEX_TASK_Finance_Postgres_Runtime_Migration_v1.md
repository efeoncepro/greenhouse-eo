...
- [x] Migrar PUT income y PUT expenses a Postgres-first
- [x] Migrar reconciliation runtime (match/unmatch/exclude/auto-match)
- [x] Verificar TypeScript: `npx tsc --noEmit` (limpio)

### Commit

- `8375edb` en `fix/codex-operational-finance` — pushed to origin

## Delta 2026-03-21 Slice 3 — PUT updates + Reconciliation runtime completo

Tercer corte ejecutado. Cubre el write path de updates (PUT) y el runtime completo de reconciliación.

### PUT income y expenses migrados

- `src/app/api/finance/income/[id]/route.ts` — PUT wired a `updateFinanceIncomeInPostgres()`
- `src/app/api/finance/expenses/[id]/route.ts` — PUT wired a `updateFinanceExpenseInPostgres()`
- Ambos usan patrón Postgres-first con BigQuery fallback via `colMap`
- Client context resolution (`resolveFinanceClientContext`) y member context resolution (`resolveFinanceMemberContext`) se ejecutan antes del Postgres path

### Reconciliation runtime migrado

Nuevo store: `src/lib/finance/postgres-reconciliation.ts` (~650 líneas)

Funciones principales:
- `listReconciliationPeriodsFromPostgres()` — lista períodos con filtros
- `createReconciliationPeriodInPostgres()` — crea período con validación de duplicado
- `getReconciliationPeriodDetailFromPostgres()` — período + statement rows
- `updateReconciliationPeriodInPostgres()` — update dinámico con fieldMap
- `getReconciliationPeriodContextFromPostgres()` — context ligero para assertMutable
- `assertReconciliationPeriodIsMutableFromPostgres()` — validación de estado
- `validateReconciledTransitionFromPostgres()` — validación pre-reconciled
- `importBankStatementsToPostgres()` — import batch transaccional
- `getStatementRowFromPostgres()` — lookup de statement row
- `updateStatementRowMatchInPostgres()` — update match status
- `clearStatementRowMatchInPostgres()` — clear match
- `excludeStatementRowInPostgres()` — marcar como excluido
- `listUnmatchedStatementRowsFromPostgres()` — rows sin match
- `listReconciliationCandidatesFromPostgres()` — candidates income + expenses
- `resolveReconciliationTargetFromPostgres()` — resolver target para match
- `setReconciliationLinkInPostgres()` — marcar income/expense como conciliado
- `clearReconciliationLinkInPostgres()` — desmarcar conciliación
- `summarizeAndUpdateIncomeReconciliation()` — resumir estado de reconciliación de income

### Diferencia arquitectónica clave: income reconciliation

En BigQuery, la reconciliación de income manipula un JSON array `payments_received` dentro de `fin_income`. En Postgres, la reconciliación opera directamente sobre la tabla `income_payments`:
- `setReconciliationLinkInPostgres` → UPDATE `income_payments SET is_reconciled = TRUE`
- `clearReconciliationLinkInPostgres` → UPDATE `income_payments SET is_reconciled = FALSE`
- `summarizeAndUpdateIncomeReconciliation` → query agregado sobre `income_payments` para determinar si toda la invoice está reconciliada

Esto elimina la manipulación de JSON y usa relaciones normalizadas.

### Rutas wired a Postgres-first

| Ruta | Método | Postgres-first | BigQuery fallback |
|------|--------|----------------|-------------------|
| `/api/finance/income/[id]` | PUT | `updateFinanceIncomeInPostgres()` | BigQuery UPDATE |
| `/api/finance/expenses/[id]` | PUT | `updateFinanceExpenseInPostgres()` | BigQuery UPDATE |
| `/api/finance/reconciliation` | GET | `listReconciliationPeriodsFromPostgres()` | BigQuery SELECT |
| `/api/finance/reconciliation` | POST | `createReconciliationPeriodInPostgres()` | BigQuery INSERT |
| `/api/finance/reconciliation/[id]` | GET | `getReconciliationPeriodDetailFromPostgres()` | BigQuery SELECT |
| `/api/finance/reconciliation/[id]` | PUT | `updateReconciliationPeriodInPostgres()` | BigQuery UPDATE |
| `/api/finance/reconciliation/[id]/statements` | POST | `importBankStatementsToPostgres()` | BigQuery INSERT loop |
| `/api/finance/reconciliation/[id]/candidates` | GET | `listReconciliationCandidatesFromPostgres()` | BQ `listReconciliationCandidates()` |
| `/api/finance/reconciliation/[id]/match` | POST | Postgres store functions | BQ `reconciliation.ts` |
| `/api/finance/reconciliation/[id]/unmatch` | POST | Postgres store functions | BQ `reconciliation.ts` |
| `/api/finance/reconciliation/[id]/exclude` | POST | Postgres store functions | BQ `reconciliation.ts` |
| `/api/finance/reconciliation/[id]/auto-match` | POST | Postgres store functions | BQ `reconciliation.ts` |

### Auto-match refactorizado

El algoritmo de auto-match se extrajo a `runAutoMatchAlgorithm()` compartido entre Postgres y BigQuery paths, eliminando duplicación de la lógica de matching (reference-first, 3 tiers de confianza).
...
- `src/app/api/finance/income/[id]/route.ts` — PUT wired
- `src/app/api/finance/expenses/[id]/route.ts` — PUT wired
- `src/app/api/finance/reconciliation/route.ts` — GET + POST wired
- `src/app/api/finance/reconciliation/[id]/route.ts` — GET + PUT wired
- `src/app/api/finance/reconciliation/[id]/statements/route.ts` — POST wired
- `src/app/api/finance/reconciliation/[id]/candidates/route.ts` — GET wired
- `src/app/api/finance/reconciliation/[id]/match/route.ts` — POST wired
- `src/app/api/finance/reconciliation/[id]/unmatch/route.ts` — POST wired
- `src/app/api/finance/reconciliation/[id]/exclude/route.ts` — POST wired
- `src/app/api/finance/reconciliation/[id]/auto-match/route.ts` — POST wired

### TypeScript

- `npx tsc --noEmit` — limpio

### Pendientes post Slice 3

- [ ] Ejecutar backfill (`pnpm exec tsx scripts/backfill-postgres-finance-slice2.ts`)
- [ ] Smoke test: crear período → importar extracto → auto-match → manual match → unmatch → exclude
- [ ] Verificar que fallback a BigQuery funciona cuando Postgres no está disponible
