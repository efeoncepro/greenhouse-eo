...
## 🔴 Discovery 2026-07-25 — lo que la skill `greenhouse-globe` encontró y esta task no sabía

El operador señaló que estaba construyendo sin discovery formal. Tenía razón, y el barrido encontró **cuatro
hechos que cambian el trabajo**. Los dos primeros habrían producido trabajo tirado.

### 1. 🔴 El selector de modelo YA EXISTE, y su diseño YA fue rechazado una vez

`TASK-1555` implementó una **galería de láminas** para elegir modelo. **El operador la rechazó al verla.** Se
reemplazó por un **desplegable compacto con el isotipo real** de cada modelo (`a45954f`), que lista **toda la
flota de la modalidad activa** (`0258534`).

Vive todavía en el payload legacy (`producer-ui.ts` + `producer-controller.ts`) y **porta con ESTA task**
(Slice 3 de ADR-014) — **no** con `TASK-1552`, que es el rediseño de jerarquía, otro trabajo.

**Qué habría pasado sin este discovery:** mi wireframe dice `[Ruta ▾]` genérico. Habría construido un selector
sin saber que (a) la galería ya se rechazó, (b) la forma aprobada es un desplegable compacto **con isotipo real**,
(c) lista la flota completa de la modalidad activa. Muy probablemente habría reconstruido la galería, porque el
prototipo aprobado la muestra así.

⚠️ **Regla dura heredada: NO llamarla "galería". Está muerta.**

### 2. 🔴 La flota completa en el selector está BLOQUEADA por un seam que es CÓDIGO, no dato

Exponer **dos modelos del mismo proveedor** (GPT Image 2 y 1.5; Nano Banana Pro y 2) **no es data**: hoy los
adapters resuelven el modelo **por capability** (`OPENAI_ROUTING[capability]`, `VERTEX_ROUTING[capability]`), y el
composite rutea imagen a **un** proveedor por capability. El compiler ancla a `estimate.model`
(`production-route-compiler.ts:154-171`), así que un binding a un 2.º modelo del mismo proveedor devuelve
`route_binding_missing` / `route_identity_mismatch` → **denegado**.

Hace falta **resolución de modelo por-ruta en los adapters**, y eso es **`TASK-1553`** (backend-data foundation).

**Consecuencia para esta task:** el selector puede **listar** la flota que `fleet.list` reporta, pero **elegir un
2.º modelo del mismo proveedor no ejecuta**. Se declara así en el `Blocked by`, y el selector muestra esas
opciones **deshabilitadas con su razón** en vez de ofrecer algo que falla al gastar.

### 3. La disponibilidad de modelos tiene DOS fuentes, y una manda

`globe.producer.fleet.list` es el **SoT LIVE** (`TASK-1554`); el ledger `GLOBE_MODEL_FLEET_STATUS.md` es el **SoT
humano**. **Si divergen, manda el reader** — y la divergencia significa que el ledger quedó viejo, no al revés.

Mi task decía "readiness de la flota visible" sin declarar cuál fuente es autoritativa.

### 4. ✅ Las 18 capabilities YA están en el grant humano — no hace falta tocar el broker

Verificado contra `PRODUCER_HUMAN_CAPABILITY_SCOPES` (`apps/studio-web/src/app.ts:241`):

| Capability del composer | Scope que la cubre | ¿Grantéada? |
|---|---|---|
| `catalog.list` · `fleet.list` | `globe.producer.catalog.read` | ✅ |
| `style.list` · `style.materialize` | `globe.lab.experiment.run` | ✅ |
| `prompt.enhance` · `prompt.history` | `globe.lab.recipe.author` | ✅ |
| `experiment.{estimate,prepare,execute,cancel,relaunch,variate,get,status}` | `globe.lab.experiment.run` | ✅ |
| `voice.preset.list` | `globe.voice.preset.manage` | ✅ |
| `credits.{balance,usage,forecast,budget}` | `globe.credits.read` + `globe.credits.estimate` | ✅ |
| `output.get` · `favorite` · `copyAsReference` | `globe.producer.assets.operate` | ✅ |

**Por qué esto importa tanto:** agregar un capability scope al grant es un **rollout de 3 pasos cero-downtime
across DOS repos** (broker `allowedScopes` → cliente Globe pide el scope → broker mueve a
`capabilityScopes`+`requiredScopes`). El broker impone `capabilityScopes ⊆ requiredScopes`, así que un scope
otorgado es **requerido**, y agregarlo de un solo movimiento **tiró abajo TODO el login de Globe** una vez
(ADR-010 / TASK-1535). Descubrir a mitad del Slice 5 que faltaba un scope habría significado parar y coordinar
dos repos. **No hace falta.**

### Lección de proceso

Los dos errores más grandes de esta sesión —medir 12 capabilities en vez de 38, y afirmar que "Serie" no tenía
contrato— fueron **fallas de discovery**, no de implementación. Un barrido sistemático al inicio los atrapaba a
los dos. La skill del dominio existe y no la cargué; el hook `/implement-task` existe y no lo corrí.
...
### 🔴 El riel de créditos — decidido con evidencia de producto, no por intuición

Investigado el 2026-07-25 contra los contratos de herramientas de **Higgsfield** y **Freepik/Magnific** (MCP
conectados; los saldos en vivo no respondieron, los contratos sí), más las quejas públicas de facturación de
productos generativos.

#### Lo que hace la industria

| Producto | Patrón observado |
|---|---|
| **Higgsfield** | el costo exacto va **en el botón Generate**, no en una tabla de precios: depende de modelo + resolución + duración juntos. Saldo e historial son **llamadas separadas**. Recuperación de sin-crédito prioriza **auto-refill** antes que packs |
| **Freepik/Magnific** | `account_balance` devuelve `credits.{available, totalPlan, spent, hasExtraCredits}` y `plan.{isUnlimitedMode, unlimitedAppliesHere}`. Su `simulate_cost` toma **los mismos argumentos que la herramienta destino**, es read-only y "nunca cobra" |
| **Adobe / Photoroom / PixAI** | cobran al click y **devuelven** si falla |

#### La queja #1 de la industria, y por qué Globe ya la tiene resuelta

El reclamo dominante no es el precio: es que **se cobra al apretar y la devolución no es instantánea**, así
que el usuario ve el descuento y no ve el retorno. Adobe tuvo que explicarlo en su propio foro; Photoroom,
PixAI y AniFlow publicaron políticas de reembolso automático.

**Globe no cobra y devuelve: RETIENE.** `CreditReservationStatusV1` es `held | settled | released | expired`,
con `settledAt`/`releasedAt`. Eso significa que el riel puede decir la verdad —«24 retenidos por esta
corrida»— en vez de mostrar un saldo que bajó y esperar que el usuario confíe en que vuelve.

⚠️ **Por eso el `Disp. / Reserv. / Gastado` del prototipo NO es decoración: es el mecanismo anti-sorpresa**, y
es mejor que lo que hace la industria. Colapsarlo en un solo número «disponible» tiraría la ventaja.

#### Dónde vive el riel — la respuesta es LAS DOS, y era una falsa dicotomía

Mi pregunta abierta era «header global o dentro del composer». La evidencia dice que son dos trabajos
distintos:

- **el riel en el header** responde *«¿cuánto me queda?»* → conciencia, siempre visible;
- **el costo en el botón** responde *«¿cuánto cuesta ESTO?»* → decisión, en el punto de acción.

Higgsfield pone el número en el botón. El prototipo pone el riel en el header. **No compiten.** Esta task
implementa el costo en el botón dentro del composer, y el riel del header queda declarado como su propia
superficie (ver Follow-ups) para no acoplar el composer al layout global.

#### Y `unlimitedAppliesHere` es una idea que hay que copiar

Freepik no modela «ilimitado» como un booleano global sino como **si aplica a esta operación**. Un plan
ilimitado que no cubre video, mostrado como «ilimitado» a secas, es una promesa falsa en el momento exacto en
que importa. Si el contrato de Globe no tiene el equivalente, el riel dice el saldo y **no** dice «ilimitado».

### 🔴 La vigencia del estimado tiene TRES ejes, y el servidor los declara

**Corrección de mi diseño anterior.** Había planteado la vigencia como contabilidad del cliente: «¿cambió
algún campo desde el último estimado?». Leyendo `CreditEstimateV1` eso resultó ser insuficiente y además
innecesario — el contrato ya trae la verdad:

```ts
CreditEstimateV1 = { estimateId, outputShapeKey, credits, rateId,
                     rateCatalogVersion, withinDayCap, createdAt, expiresAt }
```

| Eje | Cómo se detecta | Qué me faltaba |
|---|---|---|
| **La forma cambió** | `outputShapeKey` del estimado ≠ el de la recipe en pantalla | tenía un heurístico de "cambió un campo"; el contrato da la **clave** — se compara, no se rastrea |
| **Pasó el tiempo** | `expiresAt` | ❌ **no lo tenía.** Un operador que deja la pestaña 10 minutos tiene un estimado vencido sin haber tocado nada |
| **Cambió la tarifa** | `rateCatalogVersion` del estimado ≠ el vigente | ❌ **no lo tenía.** El catálogo de tarifas puede publicarse de nuevo entre estimar y ejecutar |

Y `CreditReservationV1` trae **`estimateId`**: la reserva está ligada al estimado del que se cotizó. O sea la
compuerta G3 del flow no hay que inventarla — el contrato ya la modela, y la UI sólo tiene que no romperla.

`withinDayCap` es el contrato detrás del «Cerca del límite del proyecto» del prototipo. No es un cálculo del
cliente.
...
1. la vigencia se evalúa por los **tres ejes** de arriba, no por un heurístico de "cambió un campo";
...
4. el costo va **en el botón** además del riel (patrón Higgsfield): es donde ocurre la decisión.
...
- **Riel de créditos del header** como superficie propia: saldo + uso del mes + proyección + presupuestos.
  Separado del composer para no acoplarlo al layout global, y porque responde otra pregunta.
...
- **RESUELTA — el riel de créditos.** Era una falsa dicotomía: el riel del header responde «¿cuánto me queda?»
  y el costo en el botón responde «¿cuánto cuesta esto?». Esta task hace el segundo; el riel del header es su
  propia superficie (Follow-ups).
- **¿El contrato de Globe tiene equivalente de `unlimitedAppliesHere`?** Si no, el riel dice el saldo y NO
  dice «ilimitado» — un ilimitado que no cubre la operación en curso es una promesa falsa.
