...
2. **Activar la `emailPolicy` por form** (`validation_schema_json.emailPolicy`) — **decisión de negocio del operador** (qué forms, `warn` vs `block_field`). NO la seteé en el form comercial "Lead Gen - Web" unilateralmente: gatear ese form a corporativo podría descartar leads legítimos (freelancers/pymes con gmail). Recomendación: arrancar en `warn` (etiqueta `suspect`, no rechaza) en los forms con intención B2B, observar `growth.forms.email_rejection_rate`, subir a `block_field` con sign-off. La capability ya está live en staging; activarla por form es config.
