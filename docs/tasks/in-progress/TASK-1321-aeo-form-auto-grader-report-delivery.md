...
**Palanca de activación REAL ≠ el flag:** con el flag ON pero el form sin `brandName`, cada submit cae en `missing_brand_name → skip`. El grader `/aeo-2/` no corre de verdad hasta: (1) ~~`--apply` del activation script~~ **HECHO 2026-07-02 (autorización explícita del operador):** v9 `fver-7196526d-fec6-4ee9-80f9-2d83bab22763` publicada, v8 deprecada, live verificado + screenshot (`overflowX=0`, campo nativo al diseño premium). El form ya captura `brandName` en prod; el grader aún NO corre (código sin deploy) → hoy `/aeo-2/` = lead comercial + campo nuevo. → (2) deploy del código a ops-worker vía release control plane → (3) smoke E2E. Flags aguas abajo ya ON en prod (06-30). **⚠️ Esto invalida el mitigante del ledger "`/aeo-2/` no corre el grader" (Delta 2026-07-01).**
