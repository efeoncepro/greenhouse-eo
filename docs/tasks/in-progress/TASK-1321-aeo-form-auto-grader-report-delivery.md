...
- **🔴 REGRESIÓN live + fix (v9→v10):** el primer `--apply` (v9) **perdió `form_version.style_variant='diagnostic_premium'`** porque `authorDraftForm` no lo recibía (es columna aparte de la versión, NO viaja en `field_schema`) → los selects premium se cayeron a nativos en el form live. Mi preview lo ocultó (interceptaba el contrato v8, que sí lo tenía) — **debí diffear el contrato PUBLICADO, no el interceptado.** Fix de causa raíz: el script pasa `styleVariant` con fallback a `diagnostic_premium` + `alreadyApplied` exige el variant premium (republica si se perdió). Republicado **v10 `fver-fc552f37-…`**; verificado live abriendo el dropdown (custom premium, `customTriggers=2`/`nativeSelects=0`). **Los otros `activate-aeo-*.ts` comparten el bug latente** (también omiten styleVariant) → follow-up.
...
**Palanca de activación REAL ≠ el flag:** con el flag ON pero el form sin `brandName`, cada submit cae en `missing_brand_name → skip`. El grader `/aeo-2/` no corre de verdad hasta: (1) ~~`--apply` del activation script~~ **HECHO 2026-07-02 (autorización explícita del operador):** v10 `fver-fc552f37-15a6-4d70-abfb-59929a409a62` publicada, v8 deprecada, live verificado + screenshot (`overflowX=0`, campo nativo al diseño premium). El form ya captura `brandName` en prod; el grader aún NO corre (código sin deploy) → hoy `/aeo-2/` = lead comercial + campo nuevo. → (2) deploy del código a ops-worker vía release control plane → (3) smoke E2E. Flags aguas abajo ya ON en prod (06-30). **⚠️ Esto invalida el mitigante del ledger "`/aeo-2/` no corre el grader" (Delta 2026-07-01).**
