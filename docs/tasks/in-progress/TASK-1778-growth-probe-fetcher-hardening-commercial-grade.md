...
- ~~¿Aparece algún caso real de redirect legítimo a otro subdominio del mismo dominio registrable?~~ **Apareció y se resolvió SIN PSL (2026-08-27, rollout):** `www.bancochile.cl` → 301 → `sitiospublicos.bancochile.cl` (perfil vivo de la cartera). La regla de saltos acepta ahora DESCENDIENTES del host del sujeto (sufijo anclado al host completo: `berel.com.mx → berel.com` sigue bloqueado, `evilexample.com`/`example.com.evil.test` también), y cada descendiente pasa igual por la guarda DNS. La PSL sigue descartada: el ancla al host propio no la necesita.
