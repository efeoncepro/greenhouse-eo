...
- Pendiente para `complete`: ensayo `migrate:down` (bloqueado al agente por el clasificador de permisos en tres intentos; para Codex/operador: proxy + `pnpm migrate:down` + readback + `pnpm migrate:up` + readback) y, como verificación de cliente, un `tools/list` por sesión MCP con token humano.
