...
**Slices 2–3 (FLAGS + DEPLOY) — EN CURSO.** Orden correcto: deploy del código (Slice 1 derivación + writer fix) ANTES de prender flags. Flags a prender (CEO autorizó staging+prod): MXN `FINANCE_CORE_MXN_ENABLED` + `NUBOX_EXPORT_FOREIGN_CURRENCY_ENABLED` + `FINANCE_MXN_PAYMENT_ORDERS_ENABLED` + `FINANCE_MULTI_CURRENCY_REPORTING_ENABLED`; CLF `FINANCE_CORE_CLF_INDEXED_ENABLED` + `FINANCE_CLF_INCOME_PROJECTION_ENABLED` + `FINANCE_CLF_OBLIGATIONS_ENABLED` + `FINANCE_CLF_REPORTING_ENABLED` (NO los gates de backfill `*_BACKFILL_APPLY_ENABLED`). Worker Cloud Run (Nubox sync) es deploy separado/compartido. Producción = release control plane a main + redeploy worker (gated).
