...
- [ ] Autosave repetido (mismo assessment+question) = 1 fila; el AVG del finalize no cambia con replays.
- [ ] Un evaluador con scorecard abierto NO recibe respuestas de otros evaluadores vía `listResponses`.
- [ ] Token vencido / time-limit excedido → instancia `expired` y writes rechazados con código.
- [ ] `buildPublicQuestion` testeado: output sin `answerKey`/`rubric`.
- [ ] Template con instancias es inmutable (trigger verificado vivo); `version`/`supersedes_template_id` existen.
- [ ] `decideHiringApplication` persiste `prerequisitesSnapshot.assessment` derivado del server (score/matchScore/scoredInstances/capturedAt).
- [ ] Propose IA repetido con mismo `input_digest` retorna la proposal pendiente (no duplica).
- [ ] SME gate registra actor.
...
- [ ] Los 8 criterios binarios del Backend/Data Contract (arriba) verdes.
- [ ] Migración aplicada + verificada (`information_schema`: 3 UNIQUEs parciales, 4 columnas, 1 trigger).
- [ ] `pnpm test` full verde; live tests del dominio assessment verdes contra PG.
- [ ] Docstrings ya no prometen lo que el código no hace (saveResponse/listResponses).
- [ ] Deltas cruzados dejados en 1363/1364/1365 + checkboxes 1360/1361 saneados.
...
- [ ] `GREENHOUSE_HIRING_ATS_ARCHITECTURE_V1.md` delta (hardening + invariante de versionado).
- [ ] Deltas a TASK-1363/1364/1365; checkboxes 1360/1361.
