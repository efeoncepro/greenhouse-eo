...
- [ ] El card muestra "Total F29 a pagar" = suma de las 3 líneas del VM (test del cálculo).
- [ ] El total es "Provisional (en validación)" si alguna línea está en shadow; "Oficial" solo si las 3 oficiales (test).
- [ ] Líneas `null` se excluyen de la suma con nota de incompleto, sin romper.
- [ ] Selector de período cambia `year/month` y refetch; default = período vigente.
- [ ] Etiqueta distingue mes en curso (proyección) vs cerrado (a declarar).
- [ ] Copy es-CL tokenizado en `GH_F29_CONSOLIDATED`.
- [ ] GVC desktop + mobile mirado; `scrollWidth == clientWidth`.
- [ ] `Backend impact: none` (no se tocó endpoint/reader).
...
- [ ] doc funcional `libro-iva-posicion-mensual.md` actualizado (total a pagar + selector)
- [ ] chequeo de impacto cruzado: nota en TASK-1197 (card extendido)
