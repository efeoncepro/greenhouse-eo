...
- Ejecución iniciada en `develop` local-first (sin push). Diseño de referencia: proyecto Claude Design
  "Task 1430 moderno y elegante" (`Cockpit de CTAs.dc.html`); donde el mock diverge del contrato
  (action kinds inventados, kill switch per-CTA, placements 4/6, breakpoints 680/448) **manda el contrato**.
- **Instrucción del operador (2026-07-18): el detalle DEBE mostrar CTR y métricas de marketing** como en
  el diseño; si no existen readers canónicos, se resuelven server-side. Consecuencia: `Backend impact`
  pasa de `none` a `reader` — se agrega `getCtaMarketingMetrics` (ledger Tier A + rollup Tier B, ventana
  actual vs previa, rates computados server-side; la UI jamás deriva rates) wired a `CtaDetailVm`, y el
  command `authorDraftCta` acepta `suppressionPolicy` opcional validado con `ctaSuppressionPolicySchema`
...
El grueso de la task es `ui-ux` (workbench sobre contratos existentes). El componente backend nace
del Delta (4): dos extensiones **aditivas, sin migración, reversibles con revert** dentro del mismo
primitive (`getCtaMarketingMetrics` reader + `suppressionPolicy` opcional en `authorDraftCta`). No
ameritan task `backend-data` separada: son <150 líneas, cero DDL, cero flag, y la UI es su único
consumer inicial (parity vía endpoints existentes). Orden interno de ejecución obligatorio:
**Slice 0 backend primero** (SQL ejercitado contra PG real + tests del dominio verdes) → recién
después la UI los consume (Slices 1-3). Si el reader creciera (nuevas agregaciones, BQ, ventanas
configurables), se extrae a task backend-data propia.
...
- Impacto principal: `reader` (+extensión aditiva de `command`)
- Source of truth afectado: `src/lib/growth/ctas/{readers,store,exposure,commands}.ts` (primitive `growth.cta`)
- Consumidores afectados: UI cockpit + `GET /api/admin/growth/ctas/[ctaId]` (parity) + Nexa/MCP futuros
- Runtime target: `local|staging|production` (Vercel; sin workers/crons)

### Contract surface

- Contrato existente a respetar: `CtaDetailVm` aditivo; `AuthorCtaInput` backward-compatible
- Contrato nuevo o modificado: `CtaMarketingMetricsVm` en `CtaDetailVm.metrics` (reader
  `getCtaMarketingMetrics`: ventana actual vs previa sobre `cta_conversion_event` accepted +
  `cta_exposure_rollup` viewed; rates server-side) · `authorDraftCta` acepta `suppressionPolicy`
  opcional validado con `ctaSuppressionPolicySchema`
- Backward compatibility: `compatible` (campos aditivos y opcionales)
- Full API parity: la UI consume el reader vía page server + GET detail existente; cero endpoint nuevo

### Data model and invariants

- Entidades/tablas/views afectadas: `greenhouse_growth.cta_conversion_event` (SELECT),
  `greenhouse_growth.cta_exposure_rollup` (SELECT) — cero DDL
- Invariantes que no se pueden romper:
  - Solo `server_confirmed` cuenta como conversión; `browser_reported` se etiqueta como tal
  - Los eventos `error` server_confirmed (breadcrumbs) NUNCA cuentan como conversión
  - La UI jamás computa rates/deltas: llegan resueltos del reader
- Tenant/space boundary: admin interno (capability `growth.cta.read`); sin datos por-tenant
- Idempotency/concurrency: read-only; el command extendido conserva la tx existente del store
- Audit/outbox/history: sin eventos nuevos

### Migration, backfill and rollout

- Migration posture: `none`
- Default state: `enabled with rationale` (read-only sobre data existente; sin flag nuevo)
- Backfill plan: `n/a`
- Rollback path: revert del commit (aditivo puro)
- External coordination: ninguna

### Security and access

- Auth/access gate: capabilities `growth.cta.*` existentes en la capa API/page
- Sensitive data posture: agregados sin PII (jamás hashes/keys del ledger)
- Error contract: degradación honesta (`metrics: null` + región parcial); `captureWithDomain('growth')`
