...
People Ops cierra el pipeline: ve la cola de contrataciones aprobadas (handoffs `internal_hire` de 356, expuestos por 770) → abre el detalle → revisa el journey y el readiness → crea el colaborador (member no-activo, `pending_intake`) → abre onboarding → resuelve blockers → **activa solo con readiness OK**. Todo vía commands de 770 (propose→confirm); la UI no reimplementa activación.

## Surfaces Involved

| Surface | Ruta | Nodo |
|---|---|---|
| Activation lane (cola) | `/hr/workforce/activation` | N11 |
| Activation detail | `/hr/workforce/activation/[id]` (o sidecar) | N11 |
| People 360 (journey derivado) | `/people/[id]` (card) | N11-derivado |

## Flow Map

```
Cola (listHiringActivationQueue) ──click──▶ Detalle (getHiringActivationDetail)
...
- **Abrir detalle** (N11): click en fila de la cola.
...
- **Resolver blocker**: drawer/dialog (forms-ux) por ítem faltante (contrato, legal entity, manager, template…).
...
- `src/app/(dashboard)/hr/workforce/activation/**` (NUNCA `[lang]`). Extiende la surface HR existente; el detalle abre como ruta hija `[id]` o sidecar sobre la lane.
...
- `hiring-activation-lane` (loaded/empty/blocked/error) · `hiring-activation-detail` (journey + readiness + Activar disabled-con-motivo) · `hiring-activation-resolve-blocker` · `people-360-hiring-journey`.
- Checks: `scrollWidth==clientWidth` (1440+390), consola limpia, reduced-motion, a11y (tabs/dialogs/Activar no-mudo), foco. Datos reales vía 770.

## Design Decision Log

- Extiende surface existente (mockup 763); cliente delgado de 770; Activar gated por readiness (no-mudo); member nace no-activo; People 360 derivado; bilingüe; PII masked/reveal.

## Acceptance Checklist

- [ ] N11 implementado con sus estados; cola consume el contrato de 356 vía 770.
- [ ] Crear/activar vía commands de 770 (idempotente, gobernado); Activar solo con readiness OK.
- [ ] Readiness reusa `resolveWorkforceActivationReadiness`/`assessPersonLegalReadiness` (no reimplementa).
- [ ] Ruta alcanzable (reachability) + viewCode con seed mismo PR si aplica.
- [ ] a11y (tabs, dialogs, Activar no-mudo) + GVC desktop+mobile.
- [ ] `## Delta` al master flow (N11) si cambia un nodo/regla.
