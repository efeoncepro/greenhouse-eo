...
| Hero cursor route | Después de la entrada del hero + idle | El cursor extraído entra desde fuera del panel, barre controles en continuidad, sobrepasa el objetivo, corrige brevemente, muestra un halo de hover en su punta, marca un clic teal y sale invisible. Es una capa SVG decorativa; no sigue al mouse ni intercepta acciones. | SVG inline (`animateTransform`/`animate`) + PNG transparente | yes (desktop motion) |
...
| Hero cursor route | arte estático sin cursor | cursor visible → barrido → sobrepaso/corrección → hover → clic → salida | loop de 5,8s, inicio diferido tras la entrada | el recorrido visible dura ~4s y usa sólo translate + opacity. El halo se ancla a la punta real del PNG, el clic dura <1s y la vuelta ocurre ya invisible; no hay puntos de espera salvo la pausa de confirmación. | SVG oculto; se conserva el mismo arte sin cursor estático |
