...
| Hero cursor route | arte estático sin cursor | cursor visible → aproximación → lectura → microcorrección → clic → salida | loop de 11,5s, inicio diferido tras la entrada | translate + opacity dentro del SVG; las curvas mantienen inercia entre puntos de lectura y desaceleran sólo ante el clic. El anillo teal es breve y la vuelta ocurre ya invisible para no producir un salto perceptible. | SVG oculto; se conserva el mismo arte sin cursor estático |
