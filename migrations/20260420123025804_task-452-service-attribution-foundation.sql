...
COMMENT ON TABLE greenhouse_serving.service_attribution_facts IS
  'TASK-452: canonical service-level attribution facts by service_id and period for revenue, direct cost, labor cost and overhead cost.';

COMMENT ON TABLE greenhouse_serving.service_attribution_unresolved IS
  'TASK-452: unresolved or ambiguous service attribution attempts persisted for remediation and auditability.';

-- Down Migration

DROP INDEX IF EXISTS greenhouse_serving.idx_service_attr_unresolved_source;
DROP INDEX IF EXISTS greenhouse_serving.idx_service_attr_unresolved_org_period;
DROP INDEX IF EXISTS greenhouse_serving.idx_service_attr_unresolved_space_period;
DROP TABLE IF EXISTS greenhouse_serving.service_attribution_unresolved;

DROP INDEX IF EXISTS greenhouse_serving.idx_service_attr_facts_source;
DROP INDEX IF EXISTS greenhouse_serving.idx_service_attr_facts_service_period;
DROP INDEX IF EXISTS greenhouse_serving.idx_service_attr_facts_space_period;
DROP TABLE IF EXISTS greenhouse_serving.service_attribution_facts;
