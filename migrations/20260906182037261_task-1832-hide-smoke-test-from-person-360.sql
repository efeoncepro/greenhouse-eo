-- Up Migration
-- TASK-1832 — `smoke_test` permanece en la raíz canónica para auth/retención/audit, pero nunca se
-- proyecta en Person 360. Misma lista de columnas y joins de la view vigente; sólo cambia el filtro.
...
SELECT
  ...
  COALESCE(m.phone, first_contact.phone) AS resolved_phone,
  COALESCE(ip.job_title, first_contact.job_title) AS resolved_job_title,
  m.member_id,
...
LEFT JOIN greenhouse_core.members AS m ON m.identity_profile_id = ip.profile_id
LEFT JOIN greenhouse_core.departments AS d ON d.department_id = m.department_id
LEFT JOIN LATERAL (
  SELECT cu.* FROM greenhouse_core.client_users cu
  WHERE cu.identity_profile_id = ip.profile_id
  ORDER BY cu.active DESC, cu.created_at ASC LIMIT 1
) AS first_user ON TRUE
LEFT JOIN greenhouse_core.clients AS c ON c.client_id = first_user.client_id
LEFT JOIN LATERAL (
  SELECT ct.* FROM greenhouse_crm.contacts ct
  WHERE ct.linked_identity_profile_id = ip.profile_id
  ORDER BY ct.created_at ASC LIMIT 1
) AS first_contact ON TRUE
LEFT JOIN LATERAL (
  SELECT COUNT(*)::int AS user_count FROM greenhouse_core.client_users cu2
...
LEFT JOIN LATERAL (
  ...
  FROM greenhouse_core.identity_profile_source_links sl WHERE sl.profile_id = ip.profile_id
) AS link_agg ON TRUE
LEFT JOIN LATERAL (
  SELECT ARRAY_AGG(DISTINCT ura.role_code) FILTER (WHERE ura.active AND ura.role_code IS NOT NULL)
    AS active_role_codes
  FROM greenhouse_core.user_role_assignments ura WHERE ura.user_id = first_user.user_id
) AS role_agg ON TRUE
WHERE ip.status <> 'merged' AND ip.data_origin IS DISTINCT FROM 'smoke_test';
...
DO $$ BEGIN
  IF pg_get_viewdef('greenhouse_serving.person_360'::regclass, TRUE) NOT LIKE '%data_origin%smoke_test%' THEN
    RAISE EXCEPTION 'TASK-1832: person_360 smoke_test exclusion missing';
  END IF;
END $$;

-- Down Migration
-- Forward-only visibility hardening. A correction must replace the view explicitly; rollback is not
-- allowed to re-expose synthetic authentication fixtures in Person 360.
DO $$ BEGIN RAISE EXCEPTION 'TASK-1832 person_360 exclusion is forward-only'; END $$;
