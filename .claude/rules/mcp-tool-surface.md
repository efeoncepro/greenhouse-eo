...
🔴 **Los manuales de uso agent-facing también son un manifiesto (`TASK-1804`).** `src/mcp/greenhouse/skill-manifest.ts` + `docs/mcp/skills/<name>/SKILL.md` (frontmatter Agent Skills) son la fuente; el runtime consume el artefacto generado `skill-catalog.generated.json` (`pnpm mcp:skills:generate` / `pnpm mcp:skills:check`, en `local:check` y CI). **NUNCA** `node:fs` en un módulo alcanzable por una ruta (Turbopack incluyó el proyecto entero: función de 397 MB, tres builds de staging fallidos) — la lectura de filesystem vive sólo en `skill-catalog-fs.ts` (generador y tests). **NUNCA** publicar contenido interno (el test de fuga rompe el build) ni servir `.claude/skills/**` por MCP. Un manual `internal` no existe para un binding que no sea `internal` (404, nunca 403). Toda tool que gaste entra al `appliesTo` de `seo-spend-discipline` en el mismo PR. Canon: `MCP_TOOL_SURFACE_INVARIANTS.md` §8.
