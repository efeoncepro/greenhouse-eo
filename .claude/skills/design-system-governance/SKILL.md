...
- **Follow-ups:** [abierto] rol semántico para peso **500** (énfasis medio, requiere aprobación visual); [abierto] adapter PDF Geist 600/800 — `register-fonts.ts` registra por **nombre de familia** (400/500/700), faltan las familias SemiBold/ExtraBold → el PDF aproxima esos pesos con Bold/Medium (refinamiento de paridad, NO "cae a Helvetica"; el web no tiene gap); [abierto] adapter charts desde el SoT (sweep); [✓] lint rule `greenhouse/no-fontsize-inline-typography` (scopeada a `<Typography>`, warn) + rule tests en CI (`pnpm test:lint-rules`). Spec: `docs/tasks/in-progress/TASK-1038-typography-scale-redesign.md`.
