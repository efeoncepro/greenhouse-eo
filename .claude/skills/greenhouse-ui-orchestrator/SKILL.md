...
For `TASK-1376+`, validate `## Modular Placement Contract` before selecting placement. Reusable primitives may be future `ui-package` candidates, but remain in their current home until an approved extraction task/ADR; document browser/build coupling per `docs/operations/MODULAR_MIGRATION_NEW_WORK_OPERATING_MODEL_V1.md`.
