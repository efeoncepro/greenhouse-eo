...
## Contractor compensation and episode recovery

Use the sibling reference `contractor-compensation-reentry.md` in this skill bundle. Runtime owners: `src/lib/contractor-engagements/store.ts` (terms/PK lookup), `work-submissions/store.ts` (period amount), `payables/store.ts` (snapshot/creation), `payables/readiness.ts` (invoice and agreed-amount gates). Relative code paths after the first share `src/lib/contractor-engagements/`. Availability recovery belongs to `src/lib/workforce/offboarding/lifecycle-recovery.ts`; it must not mutate the financial aggregates.
