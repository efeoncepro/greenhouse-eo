...
- `references/contractor-compensation-reentry.md`: recurring gross terms, authorized partial periods, episode separation, payable preservation and recovery readbacks.
...
Load `references/contractor-compensation-reentry.md` before changing a contractor agreement, correcting a partial month, diagnosing an engagement lookup or reconciling a returning person's pay. Confirm the longitudinal person, episode, period and existing submission/payable first. A saved recurring rate does not update prior payables; fixed-rate dates do not currently prorate automatically. A missing invoice remains a real readiness blocker. Recovery of availability must preserve legal episodes and financial records and must be checked again after reactive consumers run.
