...
For `TASK-1376+`, read and validate `## Modular Placement Contract` before choosing view, primitive, package, or client boundary. Shared UI may be marked as a future `ui-package` candidate, but remains in its current home until an approved extraction task/ADR moves it. Keep browser-only dependencies at leaf boundaries and document build impact under `docs/operations/MODULAR_MIGRATION_NEW_WORK_OPERATING_MODEL_V1.md`.
