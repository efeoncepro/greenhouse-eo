...
- Proof row polish matters: it should feel like evidence, not a CTA. Current treatment is low-shadow pill, color logo discs for Berel/Sky/Bresler plus a frosted-gray `+90` count disc tucked behind Bresler, rendered by a controlled `::after` count in DM Sans so it stays legible inside the visible part of the disc, and a compact navy country list `Chile · Colombia · México · Perú` with a flat inline globe. Do not restore visible `marcas`, `4 países`, a teal circular icon, a navy count fill, oversized count text, normal DOM text sizing for the hidden span, or a dashed mono box.
...
- Proof: `Marcas que ya confían en nosotros`, separado del panel navy; widget/primitiva publica `greenhouse_logo_marquee` (`whylogom`) con 7 logos únicos en 3 sets idénticos (`sky`, `anam`, `gobierno-santiago`, `berel`, `carozzi`, `bresler`, `marca-chile`) y proof row `BrandProofAvatarGroup` canonizado en `docs/architecture/public-site/PRIMITIVES.md`: discos solapados en color de Berel, Sky y Bresler, más un cuarto disco claro/frosted `+90` detrás de Bresler con count slate renderizado por `::after` en DM Sans. El copy visible posterior debe ser solo `Chile · Colombia · México · Perú` con ícono flat de mundo en navy compacto; no reintroducir la palabra visible `marcas`, `4 países`, fondo navy para el count, ni texto count grande/DOM normal. La pill debe sentirse como proof caption secundario: sombra muy baja, borde sutil, sin círculo teal ni mono/dashed box genérico. Validar visualmente por fases; no usar assets que ocupan ancho pero quedan invisibles o abren huecos perceptibles.
