...
- Editorial post tool MVP: `pnpm public-website:content-factory:post-tool -- edit-pullquote` is the fast lane for small Gutenberg editorial fixes. It uses remote WP-CLI, resolves a post by URL/id, selects a `core/pullquote` by nearby heading context, clones the source post to `private|draft`, replaces only that pullquote in the clone, preserves non-destructive context and never mutates the published source. Smoke on post `248398` created private draft `250136` and verified block `path=58` with the Gartner replacement. Use this for guided quote/highlight tests while the heavier bridge HMAC draft-clone endpoint remains undeployed.
...
pnpm public-website:content-factory:post-tool -- edit-pullquote --post-url https://efeoncepro.com/loop-marketing/que-es-loop-marketing/ --near-heading "El funnel = linealidad muerta" --replacement "..." --apply --write
