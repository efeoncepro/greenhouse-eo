...
> Cura una **paleta sonora** por proyecto igual que una paleta de color: 2-3 familias de SFX
...
3. **Hit points / mickey-mousing controlado.** Identifica los golpes musicales (downbeats, el drop,
   el break) y alinea los momentos clave visuales a ellos: reveal del logo en el drop, corte duro en
...
4. **Arco emocional / sube-baja.** Un motion de 30s no es energía plana: build → break → drop →
   resolución. Mapea el arco de la música al arco de la historia (`modules/04`). El bajón antes del
...
6. **Licencia y disclosure.** Verifica licencia (comercial vs personal, broadcast vs web). Música IA
...
- **Offset perceptual:** el oído detecta desync de audio adelantado ~20ms y de audio atrasado ~40ms.
  Para acentos, alinea el *transient* del SFX (no el inicio del archivo) al frame del evento.
...
- **Jerarquía de mezcla:** decide qué manda en cada momento. Diálogo/VO > música > SFX > ambience,
...
> plataforma baja y suena débil. Anota el target en `templates/motion-delivery-spec.md`.
...
- **Síntesis de voz (ElevenLabs vía Higgsfield):** genera VO en múltiples voces/idiomas; útil para
  scratch tracks, versionado multi-idioma y personajes. Verifica licencia de uso comercial y
...
- **Voice consistency:** para un personaje recurrente, fija la **voz** con la misma disciplina que
  fijas el rostro (Soul ID). Modelos con *Voice Binding* (Kling) mantienen la voz consistente entre
  cortes/idiomas — ver `modules/09`. La voz que deriva rompe la ilusión igual que el rostro que deriva.
- **Prosodia > TTS plano:** dirige la voz IA como a un actor — marca pausas, énfasis, ritmo. Una VO
...
> **Handoff:** completa `templates/sound-design-brief.md` con paleta sonora, referencia musical
