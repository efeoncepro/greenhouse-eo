...
- Customer access needs entitlements that issue and revoke access per tenant and capability — and those EXIST
  ...
  The gateway enforces **six** scope classes: base `efeonce.mcp.read`, Globe reader
  ...
  `efeonce.mcp.identity.write` (TASK-1837), and the flag-gated Hiring reader `efeonce.mcp.hiring.read`.
...
- 🔴 **A write scope is NEVER wired into the shared public PKCE client.** The three write scopes
  (`efeonce.mcp.globe.credits.funding.ensure`, `efeonce.mcp.seo.write`, `efeonce.mcp.identity.write`) exist in
  their owning issuer/resource contract. The Entra resource contains Globe/SEO; the native issuer owns identity.
