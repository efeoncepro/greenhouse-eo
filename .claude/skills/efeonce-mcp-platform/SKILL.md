...
- The live runtime still has no federated Hiring provider. `TASK-1726` implements, behind default-OFF flags, only
  `hiring.talent_pool.search` and `hiring.talent_pool.profile.get` through the `greenhouse-hiring` adapter. They use
  delegated internal identity, exact `efeonce.mcp.hiring.read`, downstream `hiring.talent_pool.read`, purpose/audit
  headers and strict allowlisted DTOs. Keep them unavailable until the production allow/deny/redaction multi-host
  canary passes. Application 360/CV, assessment tokens and every Hiring write remain outside this provider and keep
...
| Hiring/ATS, Talent Pool, candidate review, assessment assignment or selection journey | `greenhouse-talent-people-operator` + identity/integrations owners | `TASK-1726` Talent Pool readers are code-ready but default OFF; they never expose CV/contact or write. `TASK-1718`–`TASK-1722` and `TASK-1631` retain the other read/write/external gates. |
