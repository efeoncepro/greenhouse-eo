...
**Paso 7 del protocolo: mover `version` del gateway.** El protocolo versiona el SERVIDOR, nunca la tool, así que `package.json` de `efeonce-mcp` es el único lugar donde se declara que la superficie cambió — y `test/version.test.ts` lo exige. ⚠️ Ese gate midió un tiempo sólo el hash del manifiesto FEDERADO: dos tools que el gateway define por su cuenta llevaron el servidor de 37 a 39 en verde y con `version` congelada (2026-09-06, `TASK-1837`). Cerrado en el mismo repo con `src/surface.ts`, que mide la superficie del **servidor construido** (federadas + propias, nombre + descripción) y comparte el cálculo con `pnpm surface:baseline`. Reglas: **NUNCA** midas superficie leyendo archivos con regex ni dependiendo de env vars (el mismo commit daría hashes distintos según quién corra el gate); **SIEMPRE** incluye las descripciones, porque editar una es rompiente para el agente e invalida su caché de prompt; **SIEMPRE** ve el detector ENCENDERSE (agregar una tool de mentira y editar una descripción) antes de confiar en su verde. Al federar: agregar la tool y correr `pnpm surface:baseline` DESPUÉS de decidir la clase del bump (agregar = minor; renombrar, quitar o reescribir una descripción = major). Canon: `docs/architecture/agent-invariants/MCP_TOOL_SURFACE_INVARIANTS.md` §9.
...
3. For any Efeonce ID relying-party entry, client class or consent decision, read
...
4. The provider's canonical architecture, task and live runtime handoff.
5. The smallest reference below that matches the work.
...
The relying-party boundary is fixed by
[`EFEONCE_ID_RELYING_PARTY_ENTRY_AND_CONSENT_DECISION_V1.md`](../../../docs/architecture/EFEONCE_ID_RELYING_PARTY_ENTRY_AND_CONSENT_DECISION_V1.md):

- A registered Efeonce first-party product may enter through a contextual server redirect and omit delegated
  consent only when the issuer derives that class from the validated client and authorization transaction.
- MCP and third-party clients retain consent by client and scope, any required step-up, and their own audience
  and token. An Efeonce ID session can avoid repeating authentication; it never grants product or MCP access.
- Never model this convergence as an iframe, a fifth provider or a click-only vestibule.
...
- Since TASK-1813, the supported client bootstrap is Efeonce ID only when native auth is enabled. Both
  protected-resource metadata routes announce one authorization server (`MCP_NATIVE_ISSUER`) and one bootstrap
  scope (`efeonce.mcp.read`). Domain and write scopes are disclosed only by the tool-specific
  `403 insufficient_scope` challenge. Never add them to `scopes_supported` to make an old client connect.
- The gateway is not an authorization server. It does not expose `/.well-known/oauth-authorization-server` or
  `POST /register`; the TASK-1654 Entra shim and `OAUTH_PUBLIC_CLIENT_ID` are retired from code and deploy.
  DCR/CIMD, PKCE, callbacks, consent and refresh belong to `auth.efeonce.org`. Never mirror another issuer under
  the gateway origin.
- Keep Entra as a pinned **legacy token-validation lane**, not as a second bootstrap while native auth is enabled.
  `OAUTH_ISSUER`, `OAUTH_JWKS_URI` and `OAUTH_AUDIENCE` remain required for that verifier lane. With native auth
  OFF, direct Entra discovery may expose the qualified base scope, but it has no DCR and is not the supported path
  for new clients.
- The old shared Entra public client may still exist for governed legacy sessions; the gateway no longer
  distributes it. Never expand it, mutate redirects or delete it as a side effect of gateway work. Any Entra
  mutation needs explicit task/operator authority and readback.
- Client compatibility is versioned evidence, not a generic claim. TASK-1832 verified Codex `0.153.4`, Claude
...
- Customer access needs entitlements that issue and revoke access per tenant and capability — and those EXIST
  ...
  yet, so `create_insight_edition` and the three render writes still answer `insufficient_scope` until a governed
  grant/consent exists. **Update 2026-09-16 (`TASK-1846`):** gateway `v1.6.0` (PR #14 `da8295a`) deployed by run
  `35156353046` (exposure `public-oauth`) as revision `efeonce-mcp-gateway-00054-n78` at 100% traffic (digest
  `sha256:6c7aa768eb91…`, `/health` ok); gateway surface **51 tools**; the provider canary with `--render-run`
  ...
  **Update 2026-09-26 (`TASK-1888`):** gateway `v1.9.0` (PR #18 merged as `2cf78af91`, deploy run `36226550358`)
  serving `efeonce-mcp-gateway-00062-ct5` at 100 %. Two Insights tools added: `get_insight_cover_preference` (base
  scope) and `set_insight_cover_preference` (`efeonce.mcp.insights.write`; the ecosystem write only works for internal
  bindings). The production ecosystem lane `cover-preference` answered 200 in its canary.
...
- 🔴 **A write scope is NEVER wired into the shared public PKCE client.** The four write scopes
  ...
  that carries it (`gv` claim minted by the native issuer, verified by the multi-issuer gateway) is live. The
  TASK-1832 synthetic matrix remains base-only; a real client grant/consent and pilot belong to TASK-1841. Until
  that gate passes, write tools stay federated and **fail-closed** for customer access. Verbatim rationale in the gateway ADR,
...
- **Operating manuals travel through the protocol (Greenhouse `TASK-1804`).** The internal MCP and the gateway both
  ...
  `EXPECTED_GREENHOUSE_PLATFORM_TOOLS` (+ `computeFederatedNonSeoToolFindings`) — today `get_greenhouse_skill`,
  the three `*_client_service_enablement` tools (TASK-1852) and, since 2026-09-15, the four Insights tools
  `get_insights_catalog` / `list_insight_editions` / `get_insight_edition` / `create_insight_edition` (TASK-1845,
  domain `insights`; manifest 51 tools then) and, since 2026-09-16, the four render tools `request_insight_render` /
  `get_insight_render_run` / `retry_insight_render` / `cancel_insight_render` (TASK-1846; Greenhouse manifest 55
  tools; lane errors `503 render_disabled` ⇒ `policy_blocked`, `422 render_rejected` ⇒ `invalid_request`, `404`
  anti-oracle) and, since 2026-09-18, the seven sharing/delivery/schedule tools `create_insight_share` /
  `list_insight_shares` / `revoke_insight_share` / `list_insight_deliveries` / `get_insight_delivery` /
  `list_insight_schedules` / `get_insight_schedule` (TASK-1848; Greenhouse manifest 62 tools; `503 sharing_disabled|
  delivery_disabled|schedules_disabled` ⇒ `policy_blocked`, `429 quota_exceeded` ⇒ `rate_limited`). **Live example of the federation protocol for a provider that rides
  another's config:** `greenhouse-insights` (`src/providers/greenhouse-insights.ts`) takes `GreenhouseSeoConfig`
  (same ecosystem lane, same service identity, same `GREENHOUSE_SEO_PROVIDER_ENABLED`) exactly like
...
- The live runtime federates the internal read-only Hiring provider from `TASK-1726` and `TASK-1718`:
  ...
  write gates plus an eligible per-organization/per-person grant, consent and Hiring sign-off for B2B access. The
  native issuer and multi-issuer verifier are live; TASK-1718 remains open for formal sign-offs and
  revocation/rollback evidence.
...
- **Client service enablement is federated with delegated HUMAN authority (`TASK-1852`, live 2026-09-10).** Provider
  `greenhouse-client-services` (`contractVersion: 'task-1852-v1'`) federates `preview_client_service_enablement`,
  `apply_client_service_enablement` and `rollback_client_service_enablement` — the first gateway writes whose actor is
  the PERSON, not the machine. Mechanics (same as Hiring, different verb): the person's Entra token is exchanged
  (RFC 8693) through the confidential client `efeonce-mcp-client-services` (`mcp-token-exchange.ts`: input scope
  `efeonce.mcp.client_services.write` → Greenhouse scope `client_services.enablement.write`,
  `resourceFamily: 'client_services'`; `authorizeClientServices` = internal tenant + admin route group +
  `client_portal.module.enable`), and with that bearer the gateway calls ONLY the App lane
  `/api/platform/app/client-services/enablement/{preview,apply,rollback}`, where `resolveServiceEnablementAuthority`
  re-reads the human's rights on every call (`read_assignment` / `enable` / `pause`) and stamps the receipt with
  `authority: { kind: 'delegated_oauth', clientId, accessTokenId, correlationId }` (`app_session` is the portal's own
  kind). No scope → `403 scope_not_allowed`; a tenant diagnostic agent may preview but never write; the ecosystem lane
  keeps answering `403 invalid_delegated_context` to writes **by design** (machine binding, no per-human capability —
  the same reasoning as the write-scope rule above). Gateway policy: Entra issuer only; `unsupported` for the native
  issuer (`nativeUnsupportedReason: provider_delegation_required` — internal v2 stays base-only per D9); the scope is
  required by **all three tools, preview included** (the administrative inventory is part of the same act).
  Greenhouse gates: `CLIENT_SERVICE_ENABLEMENT_WRITES_ENABLED=true` in Vercel Production (release `f69b9d326a6f`,
  PR #232; the flag is read only in Vercel) and the exchange allowlist
  `GREENHOUSE_SISTER_PLATFORM_OAUTH_ALLOWED_CONSUMERS=efeonce-mcp-hiring,efeonce-mcp-hiring-review,efeonce-mcp-client-services`
  (baked by redeploy `greenhouse-naxc5guq3`); client seeded by migration `20260910005222927`
  (`spoauth-client-efeonce-mcp-client-services`, applied). Gateway: `efeonce-mcp` **`1.4.0`**, PR #9 (`e457c31b`),
  surface **43 tools** (`surface-baseline.json`); the three tools live in Greenhouse's manifest (domain `platform`, 47
  entries; `apply`/`rollback` carry `writes: true`) and in `EXPECTED_GREENHOUSE_PLATFORM_TOOLS`. PR #10 (`80ea8d74`)
  fixed `efeonce.gateway.status`, which did NOT list the new provider although it was deployed — **SIEMPRE** make the
  status tool report a new provider in the same PR, tested through the HTTP door (invariants §11). Serving revision
  at closure: `efeonce-mcp-gateway-00052-slt` (100 %), status listing `greenhouse-client-services` `enabled`. Env:
  `GREENHOUSE_CLIENT_SERVICES_PROVIDER_ENABLED|_API_URL|_TOKEN_EXCHANGE_URL|_VERCEL_BYPASS_SECRET`. Canary
  `pnpm client-services:canary` exercises the PREVIEW only and needs a HUMAN Entra bearer carrying the scope
  (interactive PKCE; never unattended). Honest state: federated and verified at the surface; **no write canary yet**
  and no `apply` executed through the channel — that needs a person. Greenhouse `scopes.ts` declares the class in
  parity (`EFEONCE_MCP_WRITE_SCOPES`, now four; `scopes.test.ts`); it ships with the next release and the live channel
  does not depend on it. Owner docs: `docs/operations/CLIENT_SERVICE_ENABLEMENT_RUNBOOK_V1.md`,
  `GREENHOUSE_CLIENT_SERVICE_EXPERIENCE_DECISION_V1.md` §Delta 2026-09-10.
...
- External invitation delivery & delegated authority (`TASK-1837`; migration and release applied 2026-09-06):
  the complete real-email, accept, magic-link, session, forced-bounce, resend, reveal and delegated-lane matrix
  was verified in staging. Production TASK-1832 separately verified synthetic invitation/login plus fail-closed
  delegated negatives; it did not execute bounce/resend/reveal or a designated-admin write. Audit:
  ...
  `efeonce-mcp` (**MERGED 2026-09-06 11:32Z**, followed by #4 and #5). Resolve the current `main`, package version,
  Ready revision and `GATEWAY_BUILD_SHA` live rather than copying a mutable snapshot: provider
  ...
  never present in bootstrap `scopes_supported`); policies native issuer only, population
...
Historical evidence inside the long domain rows preserves the state measured on its dated rollout. Their old DCR
shim, EPIC-044-pending and external-token-gate phrases are historical, not current instructions; the hard rules
above supersede them. TASK-1813 verified `1.2.0` on the Ready `00047-8b5` rollout, while customer grants/pilot and
internal multi-organization authority remain separate gates. Every later runtime claim requires fresh live readback.
