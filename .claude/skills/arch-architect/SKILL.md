...
**North Star — Nexa total operability (CEO directive 2026-06-19):** the driving purpose of parity is that **Nexa Agent must eventually operate the ENTIRE portal from the Conversational Experience.** Therefore every **new UI** and every **new capability/entitlement** must be born with a governed contract Nexa's action runtime can invoke — reads direct, writes via the governed-action loop `propose → confirm → execute` (the LLM never executes a write directly; mutation only at the human confirmation endpoint). Mandatory design-time question for any capability: **"can Nexa do this end-to-end through a contract?"** If no, it is not complete. A UI-only or capability-only delivery without its Nexa-invokable contract is incomplete.
