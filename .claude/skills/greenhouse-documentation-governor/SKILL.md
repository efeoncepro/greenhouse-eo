...
argument-hint: '[implemented change, task id, docs/closure concern]'
---
...
evidence for consumer adapters, visual/accessibility behavior, and rollback. `TASK-1591` may move to
`complete` when the opt-in adapter slice and its evidence are verified; that status does not imply
production promotion. Publishing AXIS packages is foundation/distribution evidence only. CI/Cloud Build
secret wiring, a dedicated browser canary, persisted visual diffs, and external promotion remain separate
rollout gates when they are not yet implemented.
...
- `AGENTS.md`: standing rules for Codex/generic agents. Update only for
  ...
  guardrails.
- `CLAUDE.md`: **a router, not a spec-store (TASK-1160).** Standing CROSS-CUTTING
  rules for Claude Code + a domain router table mapping each domain to its skill
  and its invariants doc. Domain-specific invariants live load-on-demand in their
  spec/`agent-invariants/*` companion; `CLAUDE.md` carries only a 1-2 line pointer
...
- `changelog.md`: recent behavior, structure, workflow, rollout, or operating
  capability deltas. One concise entry is usually enough. It is a bounded window: history lives under
  `docs/changelog/internal/`; rotate with `pnpm docs:context-rotate --apply`, never delete or paste it back.
