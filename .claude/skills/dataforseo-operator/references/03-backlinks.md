...
- **Conversión a una escala tipo DR 0–100 cuando el conector NO expone `rank_scale`** (as-of 2026-09-11; fórmula tomada de la skill `seo-visibility-report` **del proveedor**, NO de la doc oficial): `dr = round(sin(rank / 636.62) * 100, 1)`. `636.62 ≈ 2000/π`, o sea `sin(rank·π/2000)`: un mapeo **cóncavo** que satura cerca de `rank` 1000 → DR 100. La skill lo justifica porque su MCP no expone `rank_scale`, y prohíbe mostrar el `rank` crudo al cliente. ⚠️ **Contraste con el gotcha 9 de §7:** dividir por 10 (lineal) y esta fórmula (cóncava) NO dan el mismo número — `rank` 500 da **50** lineal y **70,7** con la sinusoidal. Si el endpoint acepta `rank_scale: one_hundred`, esa es la vía canónica; la fórmula es un sustituto para conectores que no la exponen. Sea cual sea, **la escala elegida se declara en el dato** — mezclarlas silenciosamente reordena cualquier ranking de autoridad.
...
- **Filtro canónico de tóxicos** (as-of 2026-09-11, verbatim del template n8n 13538 *"Detect toxic backlinks + disavow file"* **publicado por DataForSEO**, no de la doc de referencia): `filters: ["backlink_spam_score", ">", 50]` + `include_indirect_links: false` (sólo enlaces directos), paginando `offset = página × 1000`. ⚠️ El corte **50 es el default editable del template**, no una banda del proveedor: las bandas oficiales del spam score son 0–30 / 31–60 / 61–100 (§3), y por eso los ejemplos de este dossier usan `> 60`. Los dos cortes conviven; **declarar cuál se usó**, porque un mismo perfil pasa de "limpio" a "riesgoso" entre ambos.
...
12. 🔴 **`domain_intersection` con N targets devuelve la INTERSECCIÓN (AND), no la unión.** (as-of 2026-09-11; regla crítica literal de la skill `competitor-backlink-gap` **del proveedor**: *"call once per competitor — do NOT pass all competitors in one `targets` array"*.) Para un link gap real hay que llamarlo **una vez por competidor** (`targets: ["comp1.com"]`, `exclude_targets: ["cliente.com"]`, `limit: 1000`, `offset += 1000` hasta que una página devuelva <1 000 filas, cacheando cada página) y **hacer la unión en el cliente**. Pasar los 3–5 competidores juntos devuelve sólo los dominios que enlazan a **todos a la vez**: la lista de prospectos colapsa y el run "funciona" igual, sin error ni warning. ⚠️ **Nota de contraste:** la doc oficial (§2) documenta `intersection_mode: all|partial` para este endpoint — `all` (default) es justamente el AND que describe la skill, y `partial` debería relajarlo. **No verificamos `partial` contra una respuesta real**: hasta medirlo, asumir AND y unir en cliente.
13. **`rank_scale: "one_hundred"` es obligatorio en `bulk_ranks` para cualquier blend que asuma 0–100.** El default es 0–1000 (gotcha 9); sin el parámetro, una fórmula tipo `0.30 × authority_score` queda **~10× fuera de escala**, no lanza ningún error y produce rankings plausibles pero incorrectos. La skill `competitor-backlink-gap` lo marca como **required** en su body de `bulk_ranks` por esa razón exacta (as-of 2026-09-11).
14. 🔴 **`info.target_spam_score` (spam del PROPIO dominio) ≠ `backlinks_spam_score` (spam agregado de los enlaces ENTRANTES).** En `summary/live` el score del target vive **anidado bajo `info{}`** (`items[0].info.target_spam_score` en el shape plano V3); `items[0].backlinks_spam_score` es el promedio de los backlinks que llegan al target (§3). Confundirlos **invierte la lectura de riesgo**: un sitio limpio con enlaces sucios y un sitio sucio con enlaces limpios quedan intercambiados, y ambos números son creíbles. (as-of 2026-09-11, field map de la skill `seo-portfolio-audit` del proveedor, consistente con la semántica de la doc oficial citada en §3.)
15. **El "lost-link spike" NO viene como campo: se deriva de la serie.** `timeseries_summary/live` no devuelve `lost_referring_domains`/`new_referring_domains` precalculados en V3, así que la caída se calcula como `items[0].referring_domains − items[último].referring_domains` (la serie llega en orden cronológico **ascendente**), con `items[0].referring_domains` como baseline del umbral (la skill del proveedor usa **>20% en 30 días** como HIGH RISK). Confiar en un campo "que debería estar" produce un check que **siempre pasa en silencio**. (as-of 2026-09-11, skill `seo-portfolio-audit`; en V1, si el item trae `lost_referring_domains`, usarlo y caer al delta sólo como fallback.)
...
1. **Auditoría de toxicidad a escala** — `bulk_spam_score` (1 000 dominios referentes por request, sacados antes de `referring_domains/live`) + `backlinks/live` con filtro `["backlink_spam_score",">",60]` para aislar los links tóxicos concretos. Bandas oficiales 0–30/31–60/61–100 dan el semáforo directo para un reporte de disavow o de riesgo de perfil.
   > **El disavow como ENTREGABLE, no como dato** (as-of 2026-09-11, template n8n 13538 del proveedor): el archivo tiene dos guardas duras de Google que se chequean **antes** de entregar — **<100.000 líneas** y **<2 MB** — y el template las implementa, pero emite **URLs crudas sin prefijo `domain:` y sin deduplicar**. Al replicar el patrón hay que corregir ambos defectos: un audit real desautoriza a nivel dominio, y sin dedupe el archivo infla contra el tope de 2 MB con repetidos.
