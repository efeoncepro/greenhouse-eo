...
4. **El catálogo no cubre AEO, y su silencio se lee como aprobación.** Un sitio puede puntuar 95/100 y estar bloqueando a todos los answer engines. **OnPage sigue sin verlo y no va a verlo** — es un passthrough de su propio catálogo. Lo que cambió es quién lo resuelve: desde `TASK-1670` Greenhouse tiene su **capa propia de hallazgos de SITIO** (ver el bloque de estado más abajo). Cuatro cosas que OnPage **no** te dice:
   - **Acceso de crawlers IA.** El check sitewide `robots_txt` es booleano de *existencia*; no hay check de `OAI-SearchBot` / `PerplexityBot` / `ClaudeBot` bloqueados. Es el hallazgo más caro y el único que no degrada ninguna métrica del reporte. Y ojo: **el bloqueo más común no está en `robots.txt`, está en el borde/CDN/WAF** — 403 al rastreador con el archivo impecable, 2 de cada 3 casos con problema en una muestra propia de 12 dominios LatAm/CL (2026-08-15). Leer sólo `robots.txt` (o `raw_html` del crawl) le diría "acceso correcto" a esos sitios.
   ...
   - **Salud del sitemap.** `domain_info.checks.sitemap` es presencia, no calidad: no dice si trae URLs `noindex`, 404s, no-canónicas o `lastmod` mentido. Cruzarlo con `non_indexable` + `pages.checks.from_sitemap`. Y un 404 en `/sitemap.xml` **no es defecto** si el índice está declarado en la directiva `Sitemap:` del `robots.txt` (3 de esos 12 dominios están así, y están bien).
   ...
   🔴 **Estado en Greenhouse (as-of 2026-09-01) — el hueco tiene dueño, pero todavía NO está tapado.** `TASK-1670` agregó al site audit una capa de hallazgos de **sitio** (propiedades del dominio, no de una página) evaluada con el **fetcher propio** (`@/lib/growth/site-substrate`, casa canónica desde TASK-1697; el juicio se escribe en `growth/seo`, el sustrato sólo obtiene la evidencia) y **cero gasto de proveedor**: `ai_retrieval_crawlers_blocked` (`critical`) ⊥ `ai_training_crawlers_blocked` (`notice` — bloquear entrenamiento es postura de derechos, **nunca** crítico), `ai_crawler_edge_access_denied` (`critical`, tipo propio porque la remediación es WAF/CDN), `structured_data_missing`, `sitemap_missing` / `sitemap_declared_broken`, y `site_check_unverified` para lo que no se pudo medir. El chequeo de borde varía **nuestro** token (`GreenhouseAEOGrader-EdgeCheck/1.0`): suplantar `GPTBot`/`OAI-SearchBot` está prohibido — los WAF lo validan por DNS inverso y el costo reputacional lo paga el dominio auditado. El fetcher además **obedece** el `robots.txt` matcheando su propio token (`GreenhouseAEOGrader`, fallback `*`, TASK-1778 desplegada 2026-08-27), así que una ruta que el sitio nos prohíba vuelve `blocked_robots` / "no verificado" — **jamás** "roto" (caso `reuters.com`: nos prohíbe la ruta del sitemap que él mismo declara, y reportarlo como sitemap roto le inventaba un defecto al cliente). **Pero el flag `GROWTH_SEO_SITE_FINDINGS_ENABLED` está OFF** (runtime ops-worker) hasta que `TASK-1671` despliegue la superficie de hallazgos: `code complete, rollout pendiente`. **Hasta ese flip, el punto ciego sigue abierto** — el audit todavía declara sano un sitio invisible para los motores de IA, y estas verificaciones se hacen a mano. Fuera de alcance a propósito: `core_web_vitals` (Lighthouse es laboratorio, igual que lo que OnPage ya da; el campo viene de GSC) y `llms.txt` (ROI marginal, Google no lo usa).
