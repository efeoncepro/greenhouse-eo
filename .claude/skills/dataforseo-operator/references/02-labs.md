...
8. **`keyword_difficulty` es una métrica PURA de backlinks con un piso duro — y en SERPs es-LATAM colapsa a 0 masivamente.** Fórmula oficial (help center, verificada 2026-08-13 reproduciendo los valores del API con `avg_backlinks_info`): por cada top-10, `(domain_rank×0.1 + page_rank×0.9)/500`; KD = `(max(mediana, promedio) − 0.2)/0.8 × 100`, **clampeada a 0**. El page rank de la URL ESPECÍFICA pesa 90%, y en LATAM el top-10 suele ser páginas de categoría/producto sin backlinks a nivel URL → score bajo el umbral 0.2 → **KD = 0 exacto aunque el volumen sea 135.000/mes** (`pintura` MX, medido). Es country-specific (doc: *"keyword_difficulty is a country-specific metric"* — `berel` da `null` en CL y `8` en MX), pero el país correcto NO lo rescata: la métrica no distingue "fácil" de "competido que se gana con autoridad de dominio + on-page". **Para dificultad real en estos mercados usar `avg_backlinks_info` (viene GRATIS en la misma respuesta: page_rank/domain_rank/referring domains promedio del top-10) como señal cruda, no la KD.** Fuente: https://dataforseo.com/help-center/what-is-keyword-difficulty-and-how-is-it-calculated
