...
12. **`historical_serps` es asimétrico entre conectores y NO es una serie mensual continua.** (as-of 2026-09-11; forma de respuesta verificada contra corridas reales por la skill `keyword-cannibalization-detector` **del proveedor**, no por la doc.) Cuatro cosas que muerden:
    - 🔴 **V3 devuelve items completos con `url` + `rank_group`; V1 los RECORTA** y entrega sólo `type`, `title`, `domain`, `rank_absolute` — **sin `url`**. Cualquier reconstrucción de rotación de URLs (canibalización, cambio de página ganadora) es **inservible por V1**: el extractor no encuentra URLs, no lanza nada y devuelve vacío, que se lee como "no hay rotación".
    - **Ventana ≤ ~12 meses**: un `date_to − date_from` más ancho devuelve `Invalid Field: 'date_from'` (**error 40501**).
    - **Cobertura irregular**: sólo las keywords tracked tienen historia (el resto devuelve `[]`) y los snapshots llegan **cada 1–2 meses**, no mensuales. No prometer series continuas ni leer un hueco como caída.
    - Para posición usar **`rank_group`** (posición orgánica); `rank_absolute` cuenta todos los elementos del SERP y no es comparable entre snapshots con features distintas.
13. **Paginar con `offset` dirigido por `total_count`, o estás llamando "inventario" a una ventana.** El techo práctico es **1 000 filas por página** (§4), así que un dominio con más ranked keywords que eso devuelve por default una fracción — y el agregado se ve completo igual. El idiom canónico del proveedor, visible en dos de sus propios templates n8n (5 y 7, as-of 2026-09-11), es `offset = página × 1000` con bucle mientras `página < total_count / 1000 − 1`: **`total_count` se usa para paginar, no sólo para reportar el universo**. 🔴 Al adoptarlo cruza directo con el bug class de **paginación por keyset que saltea filas en silencio** (`CLAUDE.md` → §SQL embebido en TS): el orden va por **BYTES** y la comparación del cursor se escribe expandida, y se valida **paginando una corrida real de punta a punta** comparando cuenta Y secuencia contra lo persistido.
...
7. **AI Overviews**: `item_types` acepta `ai_overview_reference` en ranked_keywords → medir presencia en AI Overviews de Google, directamente relevante para la práctica AEO de Efeonce. Es la lente **target-first** (*"¿en qué keywords estoy citado YO?"*): barata, paginable y sin pagar un SERP por keyword. Su complemento **SERP-first** (*"para ESTA keyword, ¿a quién cita Google hoy?"*, que sí muestra competidores y autoridades) vive en `01-serp.md` §4.3. **No son intercambiables** — elegir cuál responde la pregunta del contrato, y nombrarlo ahí (as-of 2026-09-11, contraste derivado de los templates n8n 7539/7540 vs 13431 del proveedor).
