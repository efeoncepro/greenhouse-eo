...
- `DATAFORSEO_API_LOGIN` — env var plana (login Basic auth). En Vercel staging+production; en ops-worker viene del **GitHub Actions secret** `DATAFORSEO_API_LOGIN` inyectado por `.github/workflows/ops-worker-deploy.yml`; `services/ops-worker/deploy.sh` exige su presencia antes de build cuando SEO o AIO están ON.
- `DATAFORSEO_API_PASSWORD` / `DATAFORSEO_API_PASSWORD_SECRET_REF` — resuelto con `resolveSecret({ envVarName: 'DATAFORSEO_API_PASSWORD' })` (`src/lib/ai/dataforseo.ts`), o sea el patrón `*_SECRET_REF` server-side. Secret GCP: **`greenhouse-dataforseo-api-password`** (default y binding en `services/ops-worker/deploy.sh`; SA `greenhouse-portal@` con `secretAccessor`). `.env.example:14-15` declara ambos vacíos. NUNCA poner el password en `.env.local` (manual `docs/manual-de-uso/growth/ai-visibility-grader-smoke.md:77`).
...
- Drift recuperado 2026-09-19: Vercel env NO basta — el grader async corre en Cloud Run `ops-worker`. ISSUE-175 restauró login en la revisión viva 00698 y el canary discovery confirmó transporte y gasto. TASK-1341 añadió preflight antes de build y readback de revisiones con tráfico en develop; rollout y smoke AIO siguen pendientes. Sin login en la revisión viva, `google_ai_overview` degrada `skipped:missing_secret`.
